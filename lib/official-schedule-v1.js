...
const slateInflight = globalThis.__MLB_OFFICIAL_TAIPEI_SLATE_INFLIGHT__ || new Map();
const fetchIdentity = globalThis.__MLB_OFFICIAL_FETCH_IDENTITIES__ || new WeakMap();
globalThis.__MLB_OFFICIAL_TAIPEI_SLATE_INFLIGHT__ = slateInflight;
globalThis.__MLB_OFFICIAL_FETCH_IDENTITIES__ = fetchIdentity;
let fetchIdentitySequence = globalThis.__MLB_OFFICIAL_FETCH_ID_SEQUENCE__ || 0;

function transportKey(fetchImpl) {
  if (!fetchIdentity.has(fetchImpl)) {
    fetchIdentitySequence += 1;
    globalThis.__MLB_OFFICIAL_FETCH_ID_SEQUENCE__ = fetchIdentitySequence;
    fetchIdentity.set(fetchImpl, fetchIdentitySequence);
  }
  return fetchIdentity.get(fetchImpl);
}
...
async function fetchOfficialDate(date, { fetchImpl, timeoutMs }) {
  ...
  try {
    ...
    const response = await fetchImpl(url, {
      cache: 'no-store',
      headers: { Accept: 'application/json', 'User-Agent': 'Baseball-Positive-EV/9.6.2' },
...
export async function fetchOfficialTaipeiSlate(boardDate, {
  ...
  const key = `${boardDate}|${timeoutMs}|${transportKey(fetchImpl)}`;
  if (slateInflight.has(key)) return slateInflight.get(key);
  ...
  slateInflight.set(key, request);
  try {
    return await request;
  } finally {
    if (slateInflight.get(key) === request) slateInflight.delete(key);
  }
}
...
export function assertGameHasNotStarted(game, now = Date.now()) {
  ...
  if (Number(now) >= start
    || /in progress|game over|final|completed|live|postponed|cancelled/.test(status)
    || ['I', 'F', 'O', 'D', 'C'].includes(String(game?.statusCode || '').toUpperCase())) {
    throw httpError('比賽已達官方預定開打時間、已開始、延期或取消｜賽前模型停止評分', 409, 'GAME_ALREADY_STARTED');
  }
}
