...
// A heartbeat cannot repair a failed server freshness verdict. Route these
// saved results back through signed server repricing, even if prices match.
export function readerAnalysisNeedsRevalidation(item) {
  const analysis = item?.customData?.analysis;
  const rows = [...(analysis?.results || []), ...(analysis?.directionSlots || []), ...(analysis?.slots || [])];
  return rows.some(row => row?.sourceType === 'ACTUAL_TW_CREDIT'
    && String(row?.provider || '').toUpperCase() === 'TAI888_READER_AUTO'
    && row?.evCalibration?.actualReaderEligible === false
    && (row?.evCalibration?.qualified === true || (row?.evCalibration?.reasons || [])
      .some(reason => String(reason).startsWith('Tai888 Reader 實際盤已過期或尚未完成最新版本驗證'))));
}
...
export function readerTaskGameRevisionIsStale(taskEvidenceHash, currentEvidenceHash) {
  const task = clean(taskEvidenceHash);
  const current = clean(currentEvidenceHash);
...
export function bindVerifiedReaderContractsForItem(item, rows) {
  const provenance = item?.readerProvenance || {};
  const gameMarketHash = clean(provenance?.readerGameMarketHash);
  const currentGameHashes = [...new Set((Array.isArray(item?.customMarkets) ? item.customMarkets : [])
    .map(market => clean(market?.readerGameMarketHash))
    .filter(Boolean))];
  // A payload hash covers the whole league board. It can advance when a
  // different game moves even though this game's signed contract is unchanged.
  // Keep the per-game market-content hash as the stable client authority; the
  // bet API still re-verifies the exact latest Reader contract server-side.
  const sameBoardRevision = Boolean(item?.readerPayloadHash)
    && clean(provenance?.payloadHash) === clean(item.readerPayloadHash);
  const sameGameRevision = Boolean(gameMarketHash)
    && currentGameHashes.length === 1
    && currentGameHashes[0] === gameMarketHash;
  const verified = item?.actualSource?.provider === 'TAI888_READER_AUTO'
    && provenance?.provider === 'TAI888_READER_AUTO'
    && (sameBoardRevision || sameGameRevision);
...
export function shouldAcceptReaderStatus(current, next) {
  ...
  const nextTime = readerStatusTime(next);
  // An unavailable status without source evidence must still surface. A
  // delayed response carrying an older capture, however, must not replace a
  // newer accepted status just because that old capture is now marked stale.
  if (next?.fresh !== true && !Number.isFinite(nextTime)) return true;
  return nextTime >= readerStatusTime(current);
}
...
export function readerCoverageCounts(value = {}) {
  ...
  const identityBlocked = nonnegativeInteger(value?.identityBlockedGameCount);
  const locked = identityBlocked ? nonnegativeInteger(value?.lockedGameCount) : Math.max(0, captured - open);
  const notRendered = identityBlocked ? nonnegativeInteger(value?.notRenderedGameCount) : Math.max(0, total - captured);
  return { total, captured, open, waiting, locked, notRendered, ...(identityBlocked ? { identityBlocked } : {}) };
}
