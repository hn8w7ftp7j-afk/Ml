...
function touchAnalysisData(data, pageActivityAt) {
  if (!data?.analysis || !Array.isArray(data.analysis.results)) return data;
  return {
    ...data,
    analysis: {
      ...data.analysis,
      results: data.analysis.results.map(row => (
        row?.sourceType === 'ACTUAL_TW_CREDIT'
          && String(row?.provider || '').toUpperCase() === 'TAI888_READER_AUTO'
          ? { ...row, lineAsOf: pageActivityAt }
          : row
      )),
    },
  };
...
export function touchReaderHeartbeat(item, payloadHash, pageActivityAt) {
  ...
  return {
    ...
    customMarkets: (item.customMarkets || []).map(row => ({ ...row, lineAsOf: timestamp })),
...
export function advanceUnchangedReaderGame(item, markets, payloadHash, pageActivityAt, now = Date.now()) {
