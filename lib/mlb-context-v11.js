...
export async function buildGameContextV11(game, options = {}) {
  ...
  const awayStarter = normalizePitching(personPitchingStatForTeamV11(awayStarterRes.data, game.awayTeamId), league, game.awayProbableId ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.PROJECTED);
  const homeStarter = normalizePitching(personPitchingStatForTeamV11(homeStarterRes.data, game.homeTeamId), league, game.homeProbableId ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.PROJECTED);
