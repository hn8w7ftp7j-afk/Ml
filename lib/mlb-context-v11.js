...
export const MLB_CONTEXT_V11_VERSION = 'MLB-STANDALONE-POINT-IN-TIME-CONTEXT-2026-09-v11.0.2';
...
const finite = (value, fallback = null) => !['number', 'string'].includes(typeof value) || (typeof value === 'string' && value.trim() === '') ? fallback : Number.isFinite(Number(value)) ? Number(value) : fallback;
...
// Request-scoped responses may omit identity metadata; explicit conflicts and
// ambiguous duplicate splits must never be accepted as that request's entity.
export function scopedStatSplitsV11(payload, { group, teamId, playerId, season, sitCode } = {}) {
  return (Array.isArray(payload?.stats) ? payload.stats : [])
    .filter(block => !block?.group?.displayName || block.group.displayName === group)
    .flatMap(block => Array.isArray(block?.splits) ? block.splits : [])
    .filter(row => row && typeof row === 'object'
      && (!group || !row.group?.displayName || row.group.displayName === group)
      && (teamId == null || (row.team?.id == null && row.teamId == null) || Number(row.team?.id ?? row.teamId) === Number(teamId))
      && (playerId == null || (row.player?.id == null && row.playerId == null) || Number(row.player?.id ?? row.playerId) === Number(playerId))
      && (!season || !row.season || String(row.season) === String(season))
      && (!sitCode || !row.split?.code || row.split.code === sitCode)
      && (row.sport?.id == null || Number(row.sport.id) === 1));
}

function teamStat(payload, teamId, group, asOf) {
  const rows = scopedStatSplitsV11(payload, { teamId, group, season: asOf.slice(0, 4) });
  return rows.length === 1 ? rows[0].stat : null;
}

export function personPitchingStatForTeamV11(payload, teamId) {
  const id = Number(teamId);
  if (!Number.isSafeInteger(id) || id <= 0) return null;
  const matches = statSplits(payload).filter(split => Number(split?.team?.id ?? split?.teamId) === id);
  if (!matches.length) return null;
  return [...matches]
    .sort((left, right) => parseBaseballInningsV11(right?.stat?.inningsPitched) - parseBaseballInningsV11(left?.stat?.inningsPitched))[0]?.stat || null;
}
...
export function normalizeHittingV11(stat, status = FEATURE_STATUS.CONFIRMED) {
  ...
  return {
    available: games > 0 && (runs != null || ops != null),
    status: games > 0 && (runs != null || ops != null) ? (runs != null && ops != null ? status : FEATURE_STATUS.PROJECTED) : FEATURE_STATUS.MISSING,
    games,
    plateAppearances,
    runs,
    runsPerGame: games > 0 && runs != null ? runs / games : null,
    avg: finite(stat.avg), obp, slg, ops,
    homeRuns: finite(stat.homeRuns), baseOnBalls: finite(stat.baseOnBalls), strikeOuts: finite(stat.strikeOuts),
  };
}
...
export function normalizePitchingV11(stat, league = {}, status = FEATURE_STATUS.CONFIRMED) {
  ...
  const earnedRuns = finite(stat.earnedRuns);
  const hits = finite(stat.hits);
  const walks = finite(stat.baseOnBalls);
  const strikeouts = finite(stat.strikeOuts);
  const homeRuns = finite(stat.homeRuns);
  const kPer9 = rate(strikeouts, inningsPitched, 9, finite(stat.strikeoutsPer9Inn ?? stat.kPer9));
  const bbPer9 = rate(walks, inningsPitched, 9, finite(stat.walksPer9Inn ?? stat.bbPer9));
  const hrPer9 = rate(homeRuns, inningsPitched, 9, finite(stat.homeRunsPer9 ?? stat.hrPer9));
  const era = rate(earnedRuns, inningsPitched, 9, finite(stat.era));
  const whip = rate(hits != null && walks != null ? hits + walks : null, inningsPitched, 1, finite(stat.whip));
  const available = inningsPitched > 0 && [era, whip, kPer9, bbPer9, hrPer9].some(value => value != null);
  const componentCount = [kPer9, bbPer9, hrPer9].filter(value => value != null).length;
  const complete = [era, whip, kPer9, bbPer9, hrPer9].every(value => value != null);
  return {
    available,
    status: available ? (complete ? status : FEATURE_STATUS.PROJECTED) : FEATURE_STATUS.MISSING,
    inningsPitched,
    gamesPitched: finite(stat.gamesPitched), gamesStarted: finite(stat.gamesStarted),
    earnedRuns, hits, baseOnBalls: walks, strikeOuts: strikeouts, homeRuns,
    era, whip, kPer9, bbPer9, hrPer9,
    metricCoverage: [era, whip, kPer9, bbPer9, hrPer9].filter(value => value != null).length / 5,
    metricSources: {
      era: earnedRuns != null && inningsPitched > 0 ? 'OBSERVED_COUNTS' : era != null ? 'OFFICIAL_RATE' : 'MISSING',
      whip: hits != null && walks != null && inningsPitched > 0 ? 'OBSERVED_COUNTS' : whip != null ? 'OFFICIAL_RATE' : 'MISSING',
    },
    fip: componentCount > 0 ? componentFip({ kPer9, bbPer9, hrPer9 }, league) : null,
    fipStatus: componentCount > 0 ? FEATURE_STATUS.PROJECTED : FEATURE_STATUS.MISSING,
    fipSource: componentCount === 3 ? 'K_BB_HR_COMPONENT_ESTIMATE_NOT_ERA_COPY' : componentCount > 0 ? 'PARTIAL_COMPONENT_ESTIMATE_MISSING_COMPONENTS_NEUTRAL' : 'MISSING_COMPONENTS_NEUTRAL',
  };
}

function neutralMissingStarter(probableId) {
  return {
    available: false,
    status: probableId ? FEATURE_STATUS.PROJECTED : FEATURE_STATUS.MISSING,
...
// Carry the transport's original clock through normalization and cache hits.
// A requested statistical cutoff is deliberately separate from acquisition.
export function mlbSourceReceiptV11(response, source, extra = {}) {
  if (!response?.sourceRecord) return null;
  return {
    source,
...
async function fetchPersonPitching(personId, teamId, startDate, endDate, options) {
...
async function fetchInjuries(teamId, asOf, options) {
  ...
  const verified = verifiedInjuredRoster(roster);
  if (!verified.available) return { ...response, ok: false, data: null, error: verified.reason };
  return { ...response, data: { roster: verified.roster } };
}

export async function fetchWeatherV11(game, venue, options = {}) {
  if (!venue?.available) return { available: false, status: FEATURE_STATUS.MISSING, meanRunFactor: 1, error: '場館座標缺失' };
  const target = new Date(game?.gameDate || NaN);
  if (!Number.isFinite(target.getTime())) return { available: false, status: FEATURE_STATUS.MISSING, meanRunFactor: 1, error: '開賽時間缺失' };
  ...
  url.searchParams.set('hourly', ['temperature_2m', 'relative_humidity_2m', ...(historical ? [] : ['precipitation_probability']), 'surface_pressure', 'wind_speed_10m', 'wind_direction_10m'].join(','));
  ...
  const response = await requestJson(url, { ...options, ttlMs: 10 * 60 * 1000 });
  return normalizeWeatherV11(response, target.toISOString(), venue, { historical });
}

export function normalizeWeatherV11(response, gameStart, venue, { historical = false } = {}) {
  const target = new Date(gameStart);
  const provenance = {
    source: historical ? 'OPEN_METEO_ARCHIVE_REANALYSIS' : 'OPEN_METEO_FORECAST',
    targetTime: gameStart, fetchedAt: response.fetchedAt, rawPayloadHash: response.rawPayloadHash, sourceRecord: response.sourceRecord,
    temporalContract: historical ? 'POST_EVENT_REANALYSIS_NOT_PREGAME_FORECAST' : 'FORECAST_AS_FETCHED_NOT_HISTORICAL_ARCHIVE',
    historicalReconstruction: false, pitVerified: false,
  };
  if (!response.ok) return { ...provenance, available: false, status: FEATURE_STATUS.MISSING, meanRunFactor: 1, error: response.error };
  ...
  if (best < 0 || difference > 3600000) return { ...provenance, available: false, status: FEATURE_STATUS.MISSING, meanRunFactor: 1, error: '找不到開賽前後一小時內的天氣' };
  const temperature = finite(hourly.temperature_2m?.[best]);
  const humidity = finite(hourly.relative_humidity_2m?.[best]);
  const pressure = finite(hourly.surface_pressure?.[best]);
  const modelInputs = { temperature: temperature ?? 21, relativeHumidity: humidity ?? 50, surfacePressure: pressure ?? 1013 };
  const substitutions = Object.entries({ temperature, relativeHumidity: humidity, surfacePressure: pressure })
    .filter(([, value]) => value == null).map(([key]) => ({ key, value: modelInputs[key], reason: 'MISSING_WEATHER_NEUTRAL_COMPONENT' }));
  const temperatureFactor = clamp(1 + (modelInputs.temperature - 21) * 0.0024, 0.94, 1.06);
  const pressureFactor = clamp(1 + (1013 - modelInputs.surfacePressure) * 0.00018, 0.97, 1.03);
  const humidityFactor = clamp(1 + (modelInputs.relativeHumidity - 50) * 0.00012, 0.99, 1.01);
  ...
  return {
    ...provenance,
    available: substitutions.length < 3,
    status: substitutions.length === 3 ? FEATURE_STATUS.MISSING : historical && substitutions.length === 0 ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.PROJECTED,
    validAt: `${times[best]}${String(times[best]).endsWith('Z') ? '' : 'Z'}`,
    modelInputs, substitutions, observationStatus: substitutions.length === 3 ? FEATURE_STATUS.MISSING : FEATURE_STATUS.CONFIRMED,
    ...
    roofStatus: venue.roof === 'dome' || venue.roof === 'open' ? 'VENUE_TYPE' : 'PROJECTED',
    roofReason: venue.roof === 'dome' || venue.roof === 'open' ? 'FIXED_VENUE_ROOF_TYPE' : 'RETRACTABLE_ROOF_STATE_UNCONFIRMED_EXISTING_0_35_PRIOR',
  };
}
...
export async function buildGameContextV11(game, options = {}) {
  ...
  const rosterDate = isoDate(game?.officialDate || game?.gameDate);
  ...
  const [leagueHitRes, leaguePitchRes, awayHitRes, homeHitRes, awayPitchRes, homePitchRes, awayRecentHitRes, homeRecentHitRes, awayRecentPitchRes, homeRecentPitchRes, awayStarterRes, homeStarterRes, awayScheduleRes, homeScheduleRes, venueRes, awayInjuryRes, homeInjuryRes, parkFactor] = await Promise.all([
    ...
    fetchPersonPitching(game.awayProbableId, game.awayTeamId, startDate, asOf, options), fetchPersonPitching(game.homeProbableId, game.homeTeamId, startDate, asOf, options),
    fetchTeamSchedule(game.awayTeamId, teamStart, asOf, options), fetchTeamSchedule(game.homeTeamId, teamStart, asOf, options),
    fetchVenue(game.venueId, options), fetchInjuries(game.awayTeamId, rosterDate, options), fetchInjuries(game.homeTeamId, rosterDate, options), fetchSavantParkFactor(game.venueId, asOf, options),
  ]);
  ...
  const weather = await fetchWeatherV11(game, venue, options);
  const awayInjuries = parseInjuries(awayInjuryRes.data);
  const homeInjuries = parseInjuries(homeInjuryRes.data);
  const awayHitting = normalizeHittingV11(teamStat(awayHitRes.data, game.awayTeamId, 'hitting', asOf));
  const homeHitting = normalizeHittingV11(teamStat(homeHitRes.data, game.homeTeamId, 'hitting', asOf));
  const awayRecentHitting = normalizeHittingV11(teamStat(awayRecentHitRes.data, game.awayTeamId, 'hitting', asOf), FEATURE_STATUS.PROJECTED);
  const homeRecentHitting = normalizeHittingV11(teamStat(homeRecentHitRes.data, game.homeTeamId, 'hitting', asOf), FEATURE_STATUS.PROJECTED);
  const awayPitching = normalizePitchingV11(teamStat(awayPitchRes.data, game.awayTeamId, 'pitching', asOf), league);
  const homePitching = normalizePitchingV11(teamStat(homePitchRes.data, game.homeTeamId, 'pitching', asOf), league);
  const awayRecentPitching = normalizePitchingV11(teamStat(awayRecentPitchRes.data, game.awayTeamId, 'pitching', asOf), league, FEATURE_STATUS.PROJECTED);
  const homeRecentPitching = normalizePitchingV11(teamStat(homeRecentPitchRes.data, game.homeTeamId, 'pitching', asOf), league, FEATURE_STATUS.PROJECTED);
  for (const [block, response, teamId, group, from] of [
    [awayHitting, awayHitRes, game.awayTeamId, 'hitting', startDate], [homeHitting, homeHitRes, game.homeTeamId, 'hitting', startDate],
    [awayPitching, awayPitchRes, game.awayTeamId, 'pitching', startDate], [homePitching, homePitchRes, game.homeTeamId, 'pitching', startDate],
    [awayRecentHitting, awayRecentHitRes, game.awayTeamId, 'hitting', recentStart], [homeRecentHitting, homeRecentHitRes, game.homeTeamId, 'hitting', recentStart],
    [awayRecentPitching, awayRecentPitchRes, game.awayTeamId, 'pitching', recentStart], [homeRecentPitching, homeRecentPitchRes, game.homeTeamId, 'pitching', recentStart],
  ]) {
    Object.assign(block, { teamId, group, startDate: from, endDate: asOf, asOf,
      observationStatus: response.ok && block.available ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.MISSING,
      modelStatus: block.status, source: 'MLB_STATS_API_TEAM_BY_DATE_RANGE',
      sourceReceipts: [mlbSourceReceiptV11(response, 'MLB_STATS_API_TEAM_BY_DATE_RANGE', { asOf, purpose: `${group.toUpperCase()}_${from}_${asOf}`, temporalContract: 'OFFICIAL_STATS_THROUGH_PREVIOUS_GAME_DAY' })].filter(Boolean),
    });
  }
  const awayStarterStats = normalizePitchingV11(personPitchingStatForTeamV11(awayStarterRes.data, game.awayTeamId), league, game.awayProbableId ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.PROJECTED);
  const homeStarterStats = normalizePitchingV11(personPitchingStatForTeamV11(homeStarterRes.data, game.homeTeamId), league, game.homeProbableId ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.PROJECTED);
  const awayStarter = awayStarterStats.available
    ? { ...awayStarterStats, individualPitcherStatsAvailable: true, teamPitchingProxyRejected: false, projectedFromTeamPitching: false }
    : neutralMissingStarter(game.awayProbableId);
  const homeStarter = homeStarterStats.available
    ? { ...homeStarterStats, individualPitcherStatsAvailable: true, teamPitchingProxyRejected: false, projectedFromTeamPitching: false }
    : neutralMissingStarter(game.homeProbableId);
  awayStarter.sourceReceipts = [mlbSourceReceiptV11(awayStarterRes, 'MLB_STATS_API_PERSON_BY_DATE_RANGE', { asOf, temporalContract: 'OFFICIAL_STATS_THROUGH_PREVIOUS_GAME_DAY' })].filter(Boolean);
  homeStarter.sourceReceipts = [mlbSourceReceiptV11(homeStarterRes, 'MLB_STATS_API_PERSON_BY_DATE_RANGE', { asOf, temporalContract: 'OFFICIAL_STATS_THROUGH_PREVIOUS_GAME_DAY' })].filter(Boolean);
  const expectedStarterInnings = starter => {
    ...
    const games = Math.max(starts, finite(starter?.gamesPitched, starts));
    const innings = Math.max(0, finite(starter?.inningsPitched, 0));
    if (starts >= 2) return clamp(innings / starts, 1.0, 7.2);
    if (starts === 1) return clamp(innings, 1.0, 6.2);
    return games > 0 ? 3.0 : 4.8;
  };
  ...
  for (const [team, parsed, response] of [[away, awayInjuries, awayInjuryRes], [home, homeInjuries, homeInjuryRes]]) {
    team.injuryReport = { ...parsed, asOf: rosterDate, source: 'MLB_STATS_API_GAME_DAY_INJURED_ROSTER',
      sourceReceipts: [mlbSourceReceiptV11(response, 'MLB_STATS_API_GAME_DAY_INJURED_ROSTER', { asOf: rosterDate, purpose: 'BASE_MODEL_INJURY_UNCERTAINTY', temporalContract: 'GAME_DAY_ROSTER_AS_FETCHED' })].filter(Boolean) };
  }
  ...
  const featureResponses = {
    leagueBaseline: [leagueHitRes, leaguePitchRes],
    awayTeamStats: [awayHitRes, awayPitchRes, awayRecentHitRes, awayRecentPitchRes],
    homeTeamStats: [homeHitRes, homePitchRes, homeRecentHitRes, homeRecentPitchRes],
    awayStarter: [awayStarterRes], homeStarter: [homeStarterRes], venueRegistry: [venueRes],
    parkFactor: [parkFactor], weather: [weather], awayInjuries: [awayInjuryRes], homeInjuries: [homeInjuryRes],
  };
  const featureProvenance = [
    ...
    feature(gamePk, 'awayStarter', awayStarter, awayStarter.status, awayStarter.individualPitcherStatsAvailable ? 'MLB_STATS_API_PERSON_BY_DATE_RANGE' : 'TEAM_PITCHING_PROXY_REJECTED_NEUTRAL', { entityId: game.awayProbableId || null, asOf, fallbackUsed: !awayStarter.individualPitcherStatsAvailable }),
    feature(gamePk, 'homeStarter', homeStarter, homeStarter.status, homeStarter.individualPitcherStatsAvailable ? 'MLB_STATS_API_PERSON_BY_DATE_RANGE' : 'TEAM_PITCHING_PROXY_REJECTED_NEUTRAL', { entityId: game.homeProbableId || null, asOf, fallbackUsed: !homeStarter.individualPitcherStatsAvailable }),
    feature(gamePk, 'venueRegistry', venue, venue.status, 'MLB_STATS_API_VENUE', { entityId: game.venueId }),
    feature(gamePk, 'parkFactor', parkFactor.available ? parkFactor : null, parkFactor.status, 'BASEBALL_SAVANT_STATCAST_PARK_FACTORS_INDEX_RUNS', { entityId: game.venueId, asOf, sampleSize: parkFactor.sampleSize || 0, fetchedAt: parkFactor.fetchedAt, rawPayloadHash: parkFactor.rawPayloadHash, sourceRecord: parkFactor.sourceRecord, fallbackUsed: !parkFactor.available }),
    feature(gamePk, 'weather', weather.available ? weather : null, weather.status, 'OPEN_METEO_POINT_IN_TIME', { entityId: game.venueId, validAt: weather.validAt || game.gameDate, fetchedAt: weather.fetchedAt, rawPayloadHash: weather.rawPayloadHash, fallbackUsed: !weather.available, qualityFlags: ['FIELD_ORIENTATION_NOT_AVAILABLE_WIND_DIRECTION_NOT_APPLIED'] }),
    feature(gamePk, 'awayInjuries', awayInjuries.rows, awayInjuries.status, 'MLB_STATS_API_ROSTER', { entityId: game.awayTeamId, asOf: rosterDate, fetchedAt: awayInjuryRes.fetchedAt, sourceRecord: awayInjuryRes.sourceRecord, rawPayloadHash: awayInjuryRes.rawPayloadHash, temporalContract: 'GAME_DAY_ROSTER_AS_FETCHED', fallbackUsed: !awayInjuries.available }),
    feature(gamePk, 'homeInjuries', homeInjuries.rows, homeInjuries.status, 'MLB_STATS_API_ROSTER', { entityId: game.homeTeamId, asOf: rosterDate, fetchedAt: homeInjuryRes.fetchedAt, sourceRecord: homeInjuryRes.sourceRecord, rawPayloadHash: homeInjuryRes.rawPayloadHash, temporalContract: 'GAME_DAY_ROSTER_AS_FETCHED', fallbackUsed: !homeInjuries.available }),
  ...
  const warnings = [
    ...
    '個別先發能力資料缺失時採中性先發品質與擴大情境，不得以整隊投球 ERA／FIP／WHIP 冒充先發。',
    '牛棚尚無可靠relief-only point-in-time切分時，中央牛棚品質固定中性並擴大情境；整隊投球只留稽核，不進平均得分。',
