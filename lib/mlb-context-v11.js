...
export const MLB_CONTEXT_V11_VERSION = 'MLB-STANDALONE-POINT-IN-TIME-CONTEXT-2026-08-v10.1.0';
...
function parseScheduleScoring(payload, teamId, venueId = null) {
  ...
  const homeAverage = mean(homeTotals);
...
export async function buildGameContextV11(game, options = {}) {
  ...
  const [leagueHitRes, leaguePitchRes, awayHitRes, homeHitRes, awayPitchRes, homePitchRes, awayRecentHitRes, homeRecentHitRes, awayRecentPitchRes, homeRecentPitchRes, awayStarterRes, homeStarterRes, awayScheduleRes, homeScheduleRes, venueRes, awayInjuryRes, homeInjuryRes] = await Promise.all([
    ...
    fetchVenue(game.venueId, options), fetchInjuries(game.awayTeamId, asOf, options), fetchInjuries(game.homeTeamId, asOf, options),
  ]);
  ...
  const park = { id: venue.id, name: venue.name || game.venue, lat: venue.latitude, lon: venue.longitude, roof: venue.roof, runFactor: homeSchedule.park.runFactor, factorStatus: homeSchedule.park.status, registryStatus: venue.status };
  const rows = [
  ...
  const featureProvenance = [
    ...
    feature(gamePk, 'parkFactor', homeSchedule.park, homeSchedule.park.status, 'MLB_STATS_API_SCHEDULE_DERIVED', { entityId: game.venueId, asOf, sampleSize: homeSchedule.park.homeGames + homeSchedule.park.roadGames, fallbackUsed: !homeSchedule.park.available }),
  ...
  const warnings = [
    'V10.1比分核心已完全切離Legacy context與Legacy distribution。',
    '正式打線未公布時使用球隊point-in-time進攻投影，不虛構個別打序。',
    '牛棚使用近期與球季球隊投球表現的收縮代理，標記PROJECTED。',
