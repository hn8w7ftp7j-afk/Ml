...
export const MLB_CONTEXT_V11_VERSION = 'MLB-STANDALONE-POINT-IN-TIME-CONTEXT-2026-09-v11.0.1';
...
export async function buildGameContextV11(game, options = {}) {
  ...
  const awayPitching = normalizePitchingV11(firstStat(awayPitchRes.data), league);
  const homePitching = normalizePitchingV11(firstStat(homePitchRes.data), league);
  const awayRecentPitching = normalizePitchingV11(firstStat(awayRecentPitchRes.data), league, FEATURE_STATUS.PROJECTED);
  const homeRecentPitching = normalizePitchingV11(firstStat(homeRecentPitchRes.data), league, FEATURE_STATUS.PROJECTED);
