...
export const MODEL_VERSION = 'BASEBALL-STATE-AWARE-LINKED-SCORE-DISTRIBUTION-2026-08-v10.6.0';
export const RULES_VERSION = 'BASEBALL-STATE-AWARE-MODEL-EV-OPTIONAL-INDEPENDENT-AUDIT-2026-08-v10.6.0';
export const ANALYSIS_V11_VERSION = 'BASEBALL-STATE-AWARE-CORRELATED-JOINT-DISTRIBUTION-EV-2026-08-v10.6.0';
...
export const PROVISIONAL_MARKET_BASELINE_START_GAP = 0.05;
export const PROVISIONAL_MARKET_BASELINE_GAP = 0.10;

const finite = (value, fallback = 0) => Number.isFinite(Number(value)) ? Number(value) : fallback;
const clamp = (value, minimum, maximum) => Math.max(minimum, Math.min(maximum, value));
const clean = value => String(value || '').trim();

export function continuousMarketBaselineWeight(probabilityGap) {
  const gap = Math.abs(finite(probabilityGap, 0));
  if (gap <= PROVISIONAL_MARKET_BASELINE_START_GAP) return 0;
  if (gap >= PROVISIONAL_MARKET_BASELINE_GAP) return 1;
  return (gap - PROVISIONAL_MARKET_BASELINE_START_GAP)
    / (PROVISIONAL_MARKET_BASELINE_GAP - PROVISIONAL_MARKET_BASELINE_START_GAP);
}
...
function prepareScenarioContracts({ scenarios, first5, pick, context }) {
  const parsed = parseTaiwanLine(pick);
  return scenarios.map(scenario => {
    const distribution = scoreDistributionForScenario(scenario, first5);
    const cells = distribution.cells.map(cell => {
      const settlement = outcomeSettlementForScore(parsed, cell.awayRuns, cell.homeRuns, context?.game?.away, context?.game?.home);
      return { ...cell, settlement, tiltSignal: finite(settlement?.winFraction, 0) - finite(settlement?.lossFraction, 0) };
    });
    return { scenario, cells };
  });
}

function tiltedScenarioRows(prepared, theta, pick, water, context, rebateRate) {
  return prepared.map(({ scenario, cells }) => {
    const weighted = cells.map(cell => ({ ...cell, probability: cell.probability * Math.exp(theta * cell.tiltSignal) }));
    const normalization = weighted.reduce((sum, cell) => sum + cell.probability, 0) || 1;
    for (const cell of weighted) cell.probability /= normalization;
    const summary = summaryForContract({ cells: weighted, pick, water, context, rebateRate });
    return { id: scenario.id, weight: scenario.weight, value: summary.ev, summary: { ...summary, scoreCoverage: 1 }, shocks: scenario.shocks };
  });
}

function provisionalMarketBaseline({ scenarios, first5, pick, water, context, rebateRate, targetProbability }) {
  const prepared = prepareScenarioContracts({ scenarios, first5, pick, context });
  const probabilityAt = theta => {
    let equivalentWin = 0;
    let equivalentLoss = 0;
    for (const { scenario, cells } of prepared) {
      let normalization = 0;
      let scenarioWin = 0;
      let scenarioLoss = 0;
      for (const cell of cells) {
        const probability = cell.probability * Math.exp(theta * cell.tiltSignal);
        normalization += probability;
        scenarioWin += probability * finite(cell.settlement?.winFraction, 0);
        scenarioLoss += probability * finite(cell.settlement?.lossFraction, 0);
      }
      const divisor = normalization || 1;
      equivalentWin += scenario.weight * scenarioWin / divisor;
      equivalentLoss += scenario.weight * scenarioLoss / divisor;
    }
    const resolved = equivalentWin + equivalentLoss;
    return resolved > 0 ? equivalentWin / resolved : 0.5;
  };
  const evaluate = theta => {
    const rows = tiltedScenarioRows(prepared, theta, pick, water, context, rebateRate);
    return { rows, weighted: aggregateScenarioSummaries(rows, rebateRate) };
  };
  let low = -12;
  let high = 12;
  for (let index = 0; index < 36; index += 1) {
    ...
    const probability = probabilityAt(middle);
    if (probability < targetProbability) low = middle;
    else high = middle;
  }
  const theta = (low + high) / 2;
  return { ...evaluate(theta), theta, targetProbability };
}
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  const contract = assertAnalysisModeContract(context);
  if (!distributionSnapshot || distributionSnapshot.modelVersion !== contract.modelVersion || distributionSnapshot.rulesVersion !== contract.rulesVersion || distributionSnapshot.legacyDistributionUsed !== false || distributionSnapshot.stateAwareBottomNinth !== true) throw new Error('V10.6狀態感知比分分布版本不相容，必須完整重算');
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      const marketBaselineWeight = anchor.source === 'Tai888雙邊去水診斷'
        ? continuousMarketBaselineWeight(rawModelGap)
        : 0;
      const marketBaselineApplied = marketBaselineWeight > 0;
      const marketBaselineTargetProbability = marketBaselineApplied
        ? rawEvaluation.weighted.modelProbability
          + marketBaselineWeight * (anchor.probability - rawEvaluation.weighted.modelProbability)
        : null;
      const evaluation = marketBaselineApplied
        ? provisionalMarketBaseline({ scenarios, first5, pick: row.pick, water, context, rebateRate, targetProbability: marketBaselineTargetProbability })
        : rawEvaluation;
      ...
      if (previous && clean(previous.pick) === clean(row.pick) && hasActualWater(previous.water)) {
        ...
        movement = { available: true, lineChanged: false, previousPick: previous.pick, previousWater: Number(previous.water), currentPick: row.pick, currentWater: water, previousWeightedEV: old.ev, deltaEV: rawWeightedEV - old.ev, method: '同一V10.6狀態感知凍結比分分布重算原始診斷；保留既有W/R資格與市場基準安全規則' };
      ...
      results.push({
        ...
        marketCalibrationWeight: marketBaselineWeight, marketCalibrationApplied: marketBaselineApplied, targetPriceCalibratesDistribution: marketBaselineApplied,
        marketBaselineApplied,
        marketBaselineVersion: marketBaselineApplied ? 'TAI888-DOUBLE-SIDED-NO-VIG-CONTINUOUS-v1.1.0' : null,
        marketBaselineTilt: marketBaselineApplied ? evaluation.theta : null,
        marketBaselineTargetProbability,
        rawModelWeightedEV,
        ...
        outcomeProbabilitiesSource: evCalibration.qualified
          ? marketBaselineApplied
            ? '市場基準暫行：比分分布以Tai888雙邊去水中心作指數傾斜；原始模型只留後台稽核'
            : 'V10.6打線／左右投／預計局數／純牛棚與九局終止狀態聯合比分分布；Tai888只作成交payoff；獨立市場僅為可選外部稽核'
        ...
        primaryRisks: [...(gate.projected || []).map(name => `預估資料：${name}`), ...(gate.missing || []).map(name => `缺失資料：${name}`), ...(marketBaselineApplied ? [`原始模型與Tai888去水市場差距 ${(rawModelGap*100).toFixed(1)}%，連續合理性校準權重 ${(marketBaselineWeight*100).toFixed(0)}%；不是已驗證長期優勢`] : []), ...(evCalibration.auditWarnings || []).map(reason => `模型／外部稽核：${reason}`), ...(evCalibration.qualified ? [] : evCalibration.reasons.map(reason => `模型評分阻擋：${reason}`))],
  ...
  const provisionalBaselineApplied = results.some(row => row.marketBaselineApplied);
  const expectedRuns = {
  ...
  const analysis = {
    leagueId: contract.leagueId, analysisMode: contract.analysisMode, modelVersion: contract.modelVersion, rulesVersion: contract.rulesVersion, modelConfig: { ...contract.modelConfig, targetMarketCalibration: provisionalBaselineApplied },
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: gate.passedForShadowScore ? 'V10.6狀態感知聯合比分分布模型EV｜既有市場基準與QA安全規則保留' : '資料Gate BLOCK', dataQuality: gate.quality, dataQualificationQuality: gate.qualificationQuality ?? gate.quality, dataGateV10: gate,
    expectedRuns, modelInputs: distributionSnapshot.profile?.components || {}, sourceStatuses: context?.sourceStatuses || {},
    scenarioSummary: { count: scenarios.length, simulationsPerScenario: 0, conservativeQuantile: .10, sharedDistribution: true, linkedSegmentPath: distributionSnapshot.linkedSegmentPath === true, linkedPathAudit: distributionSnapshot.linkedPathAudit || null, jointPortfolioDistribution: false, portfolioStatus: 'NOT_INTEGRATED', exactDistribution: true, gameStateAudit, quadratureVersion: distributionSnapshot.quadratureVersion, runProfileVersion: distributionSnapshot.runProfileVersion, evCalibrationVersion: EV_CALIBRATION_V103_VERSION, persistedForReprice: true, targetPriceCalibratesDistribution: provisionalBaselineApplied, marketProbabilityCalibrationApplied: provisionalBaselineApplied, legacyDistributionUsed: false },
    alignmentAudit: { instructionVersion: 'V10.6 PIT打線／左右投／純牛棚→預計局數交接→F5至全場連結路徑→狀態感知九局／再見／延長賽→Tai888逐腿payoff→既有W/R與QA安全規則→驗證中S分數', targetMarketCalibration: provisionalBaselineApplied ? 'PROVISIONAL_EXTREME_GAP_MARKET_BASELINE' : 'DISABLED_EXECUTION_PRICE_ONLY', independentMarketRole: 'OPTIONAL_EXTERNAL_AUDIT_ONLY', legacyContext: context?.legacyContextUsed === false ? 'DISABLED' : 'BLOCK', legacyDistribution: 'DISABLED', gptNumericScoring: 'DISABLED', formalRecommendation: 'DISABLED_UNTIL_LOCKED_OOS_FORWARD_VALIDATION' },
...
export function repriceMarkets({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  if (!distributionSnapshot) throw new Error('缺少V10.6狀態感知凍結比分分布');
  return evaluateMarketsFromDistribution({ context, markets, previousMarkets, settings, distributionSnapshot });
}
