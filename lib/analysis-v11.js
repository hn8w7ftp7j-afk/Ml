...
export const RULES_VERSION = 'BASEBALL-TW-EXACT-EV-NO-TARGET-CALIBRATION-2026-08-v10.3.0';
export const ANALYSIS_V11_VERSION = 'BASEBALL-EXACT-WEIGHTED-ROBUST-EV-2026-08-v10.3.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  const contract = assertAnalysisModeContract(context);
  if (!distributionSnapshot || distributionSnapshot.modelVersion !== contract.modelVersion || distributionSnapshot.rulesVersion !== contract.rulesVersion || distributionSnapshot.legacyDistributionUsed !== false) throw new Error('V10.3獨立比分分布版本不相容，必須完整重算');
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: 'V10.3去重複計權point-in-time棒球資料＋精確NB分段聯合比分分布；Tai888只作成交payoff', fairWater: weighted.fairWater,
  ...
  const analysis = {
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: gate.passedForShadowScore ? 'V10.3去重複計權精確比分模型｜EV Calibration Safety' : '資料Gate BLOCK', dataQuality: gate.quality, dataGateV10: gate,
...
export function repriceMarkets({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  if (!distributionSnapshot) throw new Error('缺少V10.3凍結比分分布');
  return evaluateMarketsFromDistribution({ context, markets, previousMarkets, settings, distributionSnapshot });
}
