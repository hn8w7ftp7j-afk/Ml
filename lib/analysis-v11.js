...
export const RULES_VERSION = 'BASEBALL-TW-EXACT-EV-EXTREME-OOS-HOLD-2026-08-v10.3.1';
export const ANALYSIS_V11_VERSION = 'BASEBALL-EXACT-WEIGHTED-ROBUST-EV-2026-08-v10.3.1';
export const ROBUST_EV_VERSION = 'GH27-Q10-DATA-MARGIN-INDEPENDENT-PRIOR-v1.2.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  const contract = assertAnalysisModeContract(context);
  if (!distributionSnapshot || distributionSnapshot.modelVersion !== contract.modelVersion || distributionSnapshot.rulesVersion !== contract.rulesVersion || distributionSnapshot.legacyDistributionUsed !== false) throw new Error('V10.3.1獨立比分分布版本不相容，必須完整重算');
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      if (previous && clean(previous.pick) === clean(row.pick) && hasActualWater(previous.water)) {
        ...
        movement = { available: true, lineChanged: false, previousPick: previous.pick, previousWater: Number(previous.water), currentPick: row.pick, currentWater: water, previousWeightedEV: old.ev, deltaEV: rawWeightedEV - old.ev, method: '同一V10.3.1獨立凍結比分分布直接重算成交價格' };
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: 'V10.3.1去重複計權point-in-time棒球資料＋精確NB分段聯合比分分布；Tai888只作成交payoff', fairWater: weighted.fairWater,
  ...
  const analysis = {
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: gate.passedForShadowScore ? 'V10.3.1去重複計權精確比分模型｜Extreme EV OOS Hold' : '資料Gate BLOCK', dataQuality: gate.quality, dataGateV10: gate,
...
export function repriceMarkets({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  if (!distributionSnapshot) throw new Error('缺少V10.3.1凍結比分分布');
  return evaluateMarketsFromDistribution({ context, markets, previousMarkets, settings, distributionSnapshot });
}
