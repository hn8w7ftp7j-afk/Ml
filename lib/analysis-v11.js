import { explainMlbRunProfile } from './mlb-analysis-explanation-v1.js';
import { auditScenarioDistributions } from './distribution-integrity-v1.js';
import Decimal from 'decimal.js';
import { buildAnalysisDataAudit } from './analysis-data-audit-v1.js';
import {
  MARKET_ORDER,
...
import { buildAsianJointScoreSnapshotV1 } from './asian-joint-score-v1.js';
import {
...
export const MODEL_VERSION = 'BASEBALL-STATE-AWARE-LINKED-SCORE-DISTRIBUTION-2026-09-v11.0.3';
export const RULES_VERSION = 'BASEBALL-W-FIRST-EIGHT-DIRECTION-EXECUTION-PRICE-ONLY-2026-08-v11.1.0';
export const ANALYSIS_V11_VERSION = 'BASEBALL-W-FIRST-SHARED-JOINT-DISTRIBUTION-EV-2026-09-v11.1.1';
export const ROBUST_EV_VERSION = 'MODEL-SCENARIO-Q10-CONTINUOUS-PLAUSIBILITY-v3.1.0';
export const MODEL_EV_FORMULA_VERSION = 'SCORE-PMF-X-TAIWAN-LEG-PAYOFF-v1.0.0';
...
function assertReleasedDistributionEngine(context = {}) {
  ...
  if (['NPB', 'KBO', 'CPBL'].includes(leagueId)
    && context?.analysisReadiness?.distributionEngineReady === true
    && context?.analysisReadiness?.coreInputsReady === true) return leagueId;
  const error = new Error(`${leagueId}逐場PIT核心資料Gate未通過｜禁止回退analysis-v10或MLB參數｜不評分`);
...
export function enforceShadowAnalysisSafety(value, context = {}) {
  ...
  const repriceSnapshot = value.repriceSnapshot && typeof value.repriceSnapshot === 'object'
    ? {
      ...
      directionSlots: Array.isArray(value.repriceSnapshot.directionSlots) ? value.repriceSnapshot.directionSlots.map(lockResult) : value.repriceSnapshot.directionSlots,
    }
    : value.repriceSnapshot;
  return {
    ...value,
    ...
    directionSlots: Array.isArray(value.directionSlots) ? value.directionSlots.map(lockResult) : value.directionSlots,
  };
}
...
function summaryForContract({ cells, pick, water, context, rebateRate }) {
  ...
  for (const cell of cells || []) {
    ...
    const bucket = buckets.get(signature) || { probability: new Decimal(0), profit,
      settlementWeights: { win: win.toNumber(), loss: loss.toNumber(), push: push.toNumber() },
      legs: (settlement.legs || []).map(({ exactLine, ...leg }) => leg),
      calculation: { ...calculation, legs: (calculation.legs || []).map(({ exactLine, ...leg }) => leg) } };
  ...
  return {
    settlementEvents: [...buckets.entries()].map(([signature, bucket]) => ({ signature, scope: 'PAYOFF_EQUIVALENCE_CLASS',
      modelEventProbability: bucket.probability.toNumber(), settlementWeights: bucket.settlementWeights,
      legs: bucket.legs, calculation: bucket.calculation })),
...
function aggregateScenarioSummaries(rows, rebateRate) {
  ...
  for (const row of rows) for (const event of row.summary?.settlementEvents || []) {
    const current = events.get(event.signature) || { ...event, modelEventProbability: 0 };
    current.modelEventProbability += row.weight * event.modelEventProbability;
    events.set(event.signature, current);
  }
  output.settlementEvents = [...events.values()];
  return output;
}
...
function minimumWater(weighted, scenarioRows, rebateRate, crossMarketVerified) {
  ...
  return { currentLineOnly: true, score7_2: required(0,0), score7_5: required(.02,.008), score8_0: required(.04,.02), score8_5: { ...required(.07,.04), crossMarketVerified, marketQualificationRequired: false } };
}
...
export function independentMinimumWater(evCalibration, rebateRate, crossMarketVerified) {
  ...
  if (evCalibration?.referencePriorType === 'PAYOFF_VECTOR' && payoffVectors.length >= 3) {
    ...
    return {
      ...
      score8_5: { ...required(.07, .04), crossMarketVerified, marketQualificationRequired: false },
    };
  }
...
function weightedGameStateAudit(scenarios) {
  ...
  return { ...output, version: 'GH27-WEIGHTED-OFFICIAL-LEAGUE-GAME-STATE-AUDIT-v2.0.0', noUnverifiedWalkoffExtraMargin: true };
}

export function buildDistributionSnapshot({ context }) {
  const leagueId = assertReleasedDistributionEngine(context);
  const contract = assertAnalysisModeContract(context);
  return leagueId === 'MLB'
    ? buildJointScoreSnapshotV13({ context, modelVersion: contract.modelVersion, rulesVersion: contract.rulesVersion })
    : buildAsianJointScoreSnapshotV1({ context, modelVersion: contract.modelVersion, rulesVersion: contract.rulesVersion });
}
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  const contract = assertAnalysisModeContract(context);
  if (!distributionSnapshot || distributionSnapshot.modelVersion !== contract.modelVersion || distributionSnapshot.rulesVersion !== contract.rulesVersion || distributionSnapshot.legacyDistributionUsed !== false || distributionSnapshot.stateAwareBottomNinth !== true) throw new Error('狀態感知比分分布版本不相容，必須完整重算');
  ...
  const distributionQA = contract.leagueId === 'MLB' ? auditScenarioDistributions(scenarios, scoreDistributionForScenario) : null;
  const results = [];
  for (const marketName of MARKET_ORDER) {
    const rows = (Array.isArray(markets) ? markets : []).filter(row => row?.market === marketName && clean(row?.pick));
    if (rows.length > 2) throw new Error(`${marketName} coverage duplicate：同市場${rows.length}筆方向，禁止靜默截斷`);
    if (!rows.length) continue;
    const first5 = marketName.includes('上半');
    for (const row of rows) {
      const period = first5 ? 'FIRST5' : 'FULL';
      const periodErrors = distributionQA?.periods?.[period]?.errors || [];
      const parsed = parseTaiwanLine(row.pick);
      const inputErrors = !parsed.valid ? ['INVALID_DIRECTION_CONTRACT'] : !hasActualWater(row.water) ? ['MISSING_DIRECTION_WATER'] : [];
      if (periodErrors.length || inputErrors.length) {
        const errors = [...periodErrors, ...inputErrors];
        results.push({ ...row, weightedEV: null, robustEV: null, score: null, betEligible: false,
          mathematicalIntegrityPassed: false, mathematicalIntegrityFailures: errors,
          integrityWarning: true, integrityMessage: errors.join('；'),
          numericalQA: { passed: false }, dataGateV10: gate,
          qaScope: { level: distributionQA?.matchErrors?.length ? 'MATCH' : periodErrors.length ? 'PERIOD' : 'DIRECTION', period, market: marketName, errors },
      ...
      const evDoubleCheckError = Math.abs(rawWeightedEV - scenarioWeightedEV);
      const mathematicalIntegrityFailures = [];
      if (Math.abs(weighted.coverage - 1) > 1e-9) mathematicalIntegrityFailures.push('聯合比分分布總和不等於1');
      if (settlementAudit.bucketCoverageError > 1e-9) mathematicalIntegrityFailures.push('結算機率桶合計與分布覆蓋率不一致');
      if (settlementAudit.equivalentCoverageError > 1e-9) mathematicalIntegrityFailures.push('等效輸贏走水機率與分布覆蓋率不一致');
      if (settlementAudit.probabilityIdentityError > 1e-12) mathematicalIntegrityFailures.push('模型等效條件勝率與等效輸贏機率不一致');
      if (settlementAudit.evIdentityError > 1e-9) mathematicalIntegrityFailures.push('模型W與逐腿等效輸贏機率不一致');
      if (evDoubleCheckError > 1e-9) mathematicalIntegrityFailures.push('模型W與不確定性情境加總不一致');
      if (!Number.isFinite(rawWeightedEV) || !Number.isFinite(rawRobust.robustEV)) mathematicalIntegrityFailures.push('原始EV不是有限數值');
      if (rawRobust.robustEV > rawWeightedEV + 1e-12) mathematicalIntegrityFailures.push('原始Robust EV高於原始Weighted EV');
      const integrityFailures = [
        ...mathematicalIntegrityFailures,
        ...(!gate.passedForShadowScore ? [`資料Gate未通過：${(gate.blocking || []).join('、')}`] : []),
      ];
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: 'V11.0 PIT打線／左右投／預計局數／純牛棚與九局終止狀態聯合比分分布；同場八方向共用同一凍結分布；Tai888只作成交payoff與合理性QA；獨立市場僅為可選外部稽核', fairWater: weighted.fairWater,
        ...
        settlementEvidenceVersion: 'SETTLEMENT-EVENT-TRACE-v1', settlementEvents: weighted.settlementEvents,
        robustEvidence: { ...rawRobust, scenarioEVs: scenarioRows.map(scenario => ({ id: scenario.id, weight: scenario.weight, ev: scenario.value })), note: '現行引擎計算紀錄，不是獨立重播或歷史成效驗收。' },
        distributionCoverage: weighted.coverage, rawWeightedEV, rawRobustEV: rawRobust.robustEV,
        modelEV: rawWeightedEV, weightedEV: rawWeightedEV, robustEV: rawRobust.robustEV,
        conservativeEV: rawRobust.robustEV, cev: rawRobust.robustEV, rawEV: rawWeightedEV, ev: rawWeightedEV,
        modelEVFormulaVersion: MODEL_EV_FORMULA_VERSION, evCalibration, calibrationQualified: evCalibration.qualified,
        evFlipProbabilityDiagnostic: rawRobust.flipWeight, evFlipStatus: 'Gauss-Hermite 27情境參數不確定性權重；尚未以locked OOS校準覆蓋率，不當成歷史頻率',
        worstVariant: [...rawRobust.variants].sort((a, b) => a.value - b.value)[0]?.id || '',
        robustVariants: [...rawRobust.variants].sort((a, b) => a.value - b.value),
        externalAuditVariants: evCalibration.externalAuditVariants || [],
        robustEVVersion: ROBUST_EV_VERSION, numericStandardError: null, numericLower95: robustEV, modelErrorMarginEV: null,
        mathematicalIntegrityPassed: mathematicalIntegrityFailures.length === 0,
        mathematicalIntegrityFailures,
        ...
        movement, distributionId: distributionSnapshot.distributionId, distributionHash: distributionSnapshot.distributionHash, sourceStatuses: context?.sourceStatuses || {}, dataGateV10: gate,
        numericalQA: { passed: mathematicalIntegrityFailures.length === 0 && Number.isFinite(rawWeightedEV) && Number.isFinite(rawRobust.robustEV), exactDistribution: true, standardError: null, lower95: rawRobust.robustEV, signStable: rawWeightedEV <= 0 || rawRobust.robustEV > 0, simulationsPerScenario: 0 },
        evDoubleCheck: {
          passed: evDoubleCheckError <= 1e-9,
          directEV: rawWeightedEV,
          scenarioWeightedEV,
          combinedDistributionEV: rawWeightedEV,
          absoluteError: evDoubleCheckError,
          tolerance: 1e-9,
          methods: ['每情境精確比分PMF逐比分逐腿損益', '27情境Gauss-Hermite權重精確加總'],
        },
  ...
  const analysis = {
    ...
    calculationSettings: { rebateRate },
    distributionQA,
    runExplanation: contract.leagueId === 'MLB' ? explainMlbRunProfile(context, distributionSnapshot.profile) : null,
    expectedRuns, modelInputs: distributionSnapshot.profile?.components || {}, sourceStatuses: context?.sourceStatuses || {},
    dataAudit: buildAnalysisDataAudit(context, distributionSnapshot.profile),
