...
export const RULES_VERSION = 'BASEBALL-MODEL-EV-INDEPENDENT-AUDIT-2026-08-v10.5.0';
export const ANALYSIS_V11_VERSION = 'BASEBALL-JOINT-DISTRIBUTION-WEIGHTED-ROBUST-EV-2026-08-v10.5.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
    ...
    for (const row of rows) {
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: evCalibration.qualified
          ? '同一份棒球聯合比分分布；Tai888只作成交payoff；獨立市場只作外部稽核'
          : '模型、核心資料或獨立市場稽核未通過時不得產生W/R或S分數', fairWater: weighted.fairWater,
  ...
  const analysis = {
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: 'V10.5聯合比分分布模型EV｜Tai888只作成交價｜獨立市場只作外部稽核', dataQuality: gate.quality, dataQualificationQuality: gate.qualificationQuality ?? gate.quality, dataGateV10: gate,
