...
export const RULES_VERSION = 'BASEBALL-TW-INDEPENDENT-CONSENSUS-EV-GATE-2026-08-v10.4.0';
export const ANALYSIS_V11_VERSION = 'BASEBALL-INDEPENDENT-CONSENSUS-WEIGHTED-ROBUST-EV-2026-08-v10.4.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  const contract = assertAnalysisModeContract(context);
  if (!distributionSnapshot || distributionSnapshot.modelVersion !== contract.modelVersion || distributionSnapshot.rulesVersion !== contract.rulesVersion || distributionSnapshot.legacyDistributionUsed !== false) throw new Error('V10.4.0獨立比分分布版本不相容，必須完整重算');
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      if (previous && clean(previous.pick) === clean(row.pick) && hasActualWater(previous.water)) {
        ...
        movement = { available: true, lineChanged: false, previousPick: previous.pick, previousWater: Number(previous.water), currentPick: row.pick, currentWater: water, previousWeightedEV: old.ev, deltaEV: rawWeightedEV - old.ev, method: '同一V10.4.0凍結比分分布重算原始診斷；有效EV另由獨立同合約共識價格決定' };
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: evCalibration.qualified
          ...
          : 'V10.4.0原始棒球比分模型只供伺服器稽核；未取得獨立同合約共識時不得產生有效EV', fairWater: weighted.fairWater,
  ...
  const analysis = {
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: gate.passedForShadowScore ? 'V10.4.0獨立同合約市場共識EV｜原始模型只作一致性稽核' : '資料Gate BLOCK', dataQuality: gate.quality, dataGateV10: gate,
    ...
    alignmentAudit: { instructionVersion: 'V10.4.0獨立同合約共識去水機率→Tai888逐腿payoff→跨莊保守EV→資料模型一致性阻擋→固定雙EV影子分數', targetMarketCalibration: 'INDEPENDENT_EXACT_CONTRACT_ONLY', legacyContext: context?.legacyContextUsed === false ? 'DISABLED' : 'BLOCK', legacyDistribution: 'DISABLED', gptNumericScoring: 'DISABLED', formalRecommendation: 'DISABLED_UNTIL_LOCKED_OOS_FORWARD_VALIDATION' },
...
export function repriceMarkets({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  if (!distributionSnapshot) throw new Error('缺少V10.4.0凍結比分分布');
  return evaluateMarketsFromDistribution({ context, markets, previousMarkets, settings, distributionSnapshot });
}
