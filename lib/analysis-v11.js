...
export const MODEL_VERSION = 'BASEBALL-EXACT-JOINT-SCORE-DISTRIBUTION-2026-08-v10.2.0';
export const RULES_VERSION = 'BASEBALL-TW-EXACT-EV-NO-TARGET-CALIBRATION-2026-08-v10.2.0';
export const ANALYSIS_V11_VERSION = 'BASEBALL-EXACT-WEIGHTED-ROBUST-EV-2026-08-v10.2.0';
export const ROBUST_EV_VERSION = 'GH27-Q10-DATA-MARGIN-DIAGNOSTIC-UNCALIBRATED-v1.1.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  const contract = assertAnalysisModeContract(context);
  if (!distributionSnapshot || distributionSnapshot.modelVersion !== contract.modelVersion || distributionSnapshot.rulesVersion !== contract.rulesVersion || distributionSnapshot.legacyDistributionUsed !== false) throw new Error('V10.2獨立比分分布版本不相容，必須完整重算');
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: 'V10.2獨立point-in-time棒球資料＋精確NB分段聯合比分分布；Tai888只作成交payoff', fairWater: weighted.fairWater,
  ...
  const analysis = {
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: gate.passedForShadowScore ? 'V10.2獨立精確比分模型｜Shadow Diagnostic' : '資料Gate BLOCK', dataQuality: gate.quality, dataGateV10: gate,
