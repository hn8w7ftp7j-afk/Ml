...
export const ROBUST_EV_VERSION = 'MODEL-SCENARIO-Q10-DATA-LOWER-v3.0.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      const marketBaselineApplied = anchor.source === 'Tai888雙邊去水診斷'
        && rawModelGap != null
        && rawModelGap > PROVISIONAL_MARKET_BASELINE_GAP;
      const evaluation = marketBaselineApplied
        ? provisionalMarketBaseline({ scenarios, first5, pick: row.pick, water, context, rebateRate, targetProbability: anchor.probability })
        : rawEvaluation;
      ...
      results.push({
        ...
        marketCalibrationWeight: marketBaselineApplied ? 1 : 0, marketCalibrationApplied: marketBaselineApplied, targetPriceCalibratesDistribution: marketBaselineApplied,
        marketBaselineApplied,
        marketBaselineVersion: marketBaselineApplied ? 'TAI888-DOUBLE-SIDED-NO-VIG-PROVISIONAL-v1.0.0' : null,
