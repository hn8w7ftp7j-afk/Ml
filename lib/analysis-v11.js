  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      results.push({
        ...
        // This raw gap is the direct model-vs-Tai888 double-sided no-vig
        // diagnostic used by the distribution-plausibility QA gate.  Keep the
        // optional independent reference gap in its own field; conflating the
        // two made a missing external reference hide extreme target-market
        // disagreement.
        ...
        primaryRisks: [...(gate.projected || []).map(name => `預估資料：${name}`), ...(gate.missing || []).map(name => `缺失資料：${name}`), ...(rawGap != null && rawGap >= .08 ? [`模型與Tai888去水市場差距 ${(rawGap*100).toFixed(1)}%，只作診斷，不拿下注盤反校準`] : []), ...(evCalibration.auditWarnings || []).map(reason => `模型／外部稽核：${reason}`), ...(evCalibration.qualified ? [] : evCalibration.reasons.map(reason => `模型評分阻擋：${reason}`))],
