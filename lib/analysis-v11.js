...
export const MODEL_VERSION = 'BASEBALL-STATE-AWARE-LINKED-SCORE-DISTRIBUTION-2026-08-v10.6.1';
export const RULES_VERSION = 'BASEBALL-SHARED-DISTRIBUTION-EXECUTION-PRICE-ONLY-2026-08-v10.6.1';
export const ANALYSIS_V11_VERSION = 'BASEBALL-STATE-AWARE-SHARED-JOINT-DISTRIBUTION-EV-2026-08-v10.6.1';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
    ...
    for (const row of rows) {
      ...
      results.push({
        ...
        outcomeProbabilitiesSource: evCalibration.qualified
          ? 'V10.6.1打線／左右投／預計局數／純牛棚與九局終止狀態聯合比分分布；同市場正反方向共用同一凍結分布；Tai888只作成交payoff與合理性QA；獨立市場僅為可選外部稽核'
  ...
  const analysis = {
    ...
    distributionSnapshot, createdAt: new Date().toISOString(), analysisStatus: gate.passedForShadowScore ? 'V10.6.1狀態感知共用聯合比分分布模型EV｜Tai888僅作成交payoff與合理性QA' : '資料Gate BLOCK', dataQuality: gate.quality, dataQualificationQuality: gate.qualificationQuality ?? gate.quality, dataGateV10: gate,
