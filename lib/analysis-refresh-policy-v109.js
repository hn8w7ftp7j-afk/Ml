import { ALLOWED_FUTURE_SKEW_MS } from './market-freshness-v1.js';
...
export function assessCoreSnapshotFreshnessV109(context = {}, now = Date.now()) {
  ...
  // Keep the signed age: clamping future source timestamps to zero made even
  // tomorrow's data look freshly fetched. Reuse the existing clock-skew
  // allowance used for Reader evidence without changing any refresh TTL.
  const ageMs = fetchedAt == null ? null : now - fetchedAt;
  const quality = contextQuality(context);
  const reasons = [];
  if (fetchedAt == null) reasons.push('CORE_FETCH_TIME_MISSING');
  if (ageMs != null && ageMs < -ALLOWED_FUTURE_SKEW_MS) reasons.push('CORE_FETCH_TIME_IN_FUTURE');
