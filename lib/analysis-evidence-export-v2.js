...
export const ANALYSIS_EVIDENCE_EXPORT_VERSION = 'analysis-evidence-export-v3';
export function replayEvidenceView(evidence) {
  if (!evidence) return { status: 'HISTORICAL_EVIDENCE_NOT_RECORDED' };
  const { sourceArtifact, ...rest } = evidence;
  return { ...rest, sourceArtifact: sourceArtifact ? { ...sourceArtifact, content: undefined } : null };
}
...
export function settlementEvidence(analysis = {}) {
  ...
  return {
    ...
    directions: (analysis.results || []).map(row => ({
      ...
      externalVerificationEvidence: row.marketVerification ?? null,
