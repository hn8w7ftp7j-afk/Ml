import { selectPitReplayEngine, CURRENT_REPLAY_ENGINE } from './pit-replay-engines.js';
...
export function auditSavedPitModel(bundle) {
  ...
  const savedRebate = saved.calculationSettings?.rebateRate;
  const settingsRecorded = Number.isFinite(savedRebate) && savedRebate >= 0 && savedRebate <= 0.1;
  const replayRebateRate = settingsRecorded ? savedRebate : 0.015;
  const engine = selectPitReplayEngine(bundle.versions?.modelVersion);
  ...
  }, { engine: engine || CURRENT_REPLAY_ENGINE });
  const persisted = Date.parse(bundle.databasePersistedAt || '');
  const distributionPersisted = Date.parse(bundle.distributionDatabasePersistedAt || '');
  const gameStart = Date.parse(bundle.gameIdentity?.gameDate || '');
  const captureTiming = {
    databasePersistedAt: bundle.databasePersistedAt || null,
    distributionDatabasePersistedAt: bundle.distributionDatabasePersistedAt || null,
    savedBeforeGame: [persisted, distributionPersisted, gameStart].every(Number.isFinite)
      && persisted < gameStart && distributionPersisted < gameStart,
    scope: 'DATABASE_INSERTION_PROOF_NOT_PER_FEATURE_FRESHNESS',
  };
  const report = { captureTiming,
    ...
    verificationLayers: { saved: 'CONFIRMED', integrity: 'PASS', distributionReplay: 'PENDING', evReplay: 'PENDING', scoreReplay: 'NOT_EXECUTED',
      sourceAudit: 'NOT_INDEPENDENTLY_VERIFIED', modelVersionAcceptance: saved.replayEnvironment?.modelValidation || { status: 'HISTORICAL_EVIDENCE_NOT_RECORDED', scope: 'MODEL_VERSION_COHORT_NOT_SINGLE_GAME' } },
    replayEngineRevision: engine?.revision || null,
    archivedEngineUsed: engine?.archived === true,
    ...
    limitations: ['重播一致不代表勝率已校準或資料內容正確。',
      settingsRecorded ? '依保存的退水設定重播；一致性不代表預測準確。' : 'EV比較使用1.5%退水假設；舊快照未保存設定，不能宣稱完整設定精確重播。'],
  };
  if (!engine || !validation.compatibility.compatible) return { ...report, status: 'ORIGINAL_ENGINE_UNAVAILABLE' };
  ...
  const rebuilt = engine.build({ context });
  const replay = engine.evaluate({ context, markets,
    previousMarkets: structuredClone(saved.previousMarkets || []),
    settings: { rebateRate: replayRebateRate }, distributionSnapshot: rebuilt });
  ...
  report.verificationLayers.distributionReplay = distributionMatches ? 'MATCH' : 'MISMATCH';
  report.verificationLayers.evReplay = rows.length > 0 && rows.every(r => r.deltaW !== null && r.deltaR !== null && Math.abs(r.deltaW) <= 1e-9 && Math.abs(r.deltaR) <= 1e-9) ? 'MATCH_UNDER_STATED_SETTINGS' : 'MISMATCH_OR_MISSING';
  return { ...report, status: distributionMatches ? 'DISTRIBUTION_REPRODUCED' : 'DISTRIBUTION_MISMATCH',
    ...
    replayRebateRate, settingsRecorded, rows,
