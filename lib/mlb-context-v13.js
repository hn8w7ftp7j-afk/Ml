import { buildGameContextV11, FEATURE_STATUS, parseBaseballInningsV11, normalizePitchingV11 } from './mlb-context-v11.js';
...
export const MLB_CONTEXT_V13_VERSION = 'MLB-PIT-LINEUP-PLATOON-RELIEF-CONTEXT-2026-09-v11.0.4';
...
async function fetchInjuredRoster(teamId, date, options) {
  ...
  return { ...response, available: response?.ok === true && Array.isArray(response?.data?.roster), roster: response?.data?.roster || [] };
}
...
export async function hydrateLineupBattingV13(lineup, teamId, teamOps, asOf, options = {}) {
  ...
  const rows = lineupRows(lineup).map(row => {
    ...
    const splits = response.ok ? statSplits(response.data).filter(split => !split?.player?.id || Number(split.player.id) === Number(row.id)) : [];
    const aggregate = splits.find(split => !split?.team?.id && !split?.teamId);
    const split = aggregate || splits.find(item => Number(item?.team?.id ?? item?.teamId) === Number(teamId)) || (splits.length === 1 ? splits[0] : null);
    ...
    return {
      ...
      metricProvenance: {
        ...
        teamScope: aggregate ? 'ALL_TEAMS' : Number(split?.team?.id ?? split?.teamId) || null,
...
export function buildBullpenV13({ roster = [], recentFeeds = [], teamId, gameDate, probableStarterId, league = {}, rosterComplete = false }) {
  ...
  for (const feed of recentFeeds || []) {
    ...
    for (const used of usage) {
      ...
      // Membership and current stats always win over an older game's metadata.
      for (const [key, value] of Object.entries(used)) {
        if (row[key] == null && value != null) row[key] = value;
      }
      row.appearances += 1;
...
export async function buildGameContextV13(game, options = {}) {
  ...
  const awayRoster = awayRosterState.roster;
  const homeRoster = homeRosterState.roster;
  ...
  const awayStarterInput = awayStarterOnly.available ? { ...base?.away?.starter, ...Object.fromEntries(Object.entries(awayStarterOnly).filter(([, value]) => value != null)) } : base?.away?.starter;
  const homeStarterInput = homeStarterOnly.available ? { ...base?.home?.starter, ...Object.fromEntries(Object.entries(homeStarterOnly).filter(([, value]) => value != null)) } : base?.home?.starter;
  ...
  const umpire = homePlateUmpire(currentFeed);
  ...
  const [awayInjuredResponse, homeInjuredResponse] = await Promise.all([
    fetchInjuredRoster(game.awayTeamId, asOf, options),
    fetchInjuredRoster(game.homeTeamId, asOf, options),
  ]);
