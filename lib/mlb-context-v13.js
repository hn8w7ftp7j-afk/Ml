...
export const MLB_CONTEXT_V13_VERSION = 'MLB-PIT-LINEUP-PLATOON-RELIEF-CONTEXT-2026-08-v11.0.0';
...
export function expectedStarterInningsV13(starter, { probableId = null, scheduledInnings = 9 } = {}) {
  ...
  const fallback = games > 0 && starts === 0 ? 3.0 : probableId ? 4.8 : 4.5;
  ...
  return {
    ...
    role: starts === 0 && games > 0
      ? 'OPENER_OR_BULK_RISK'
      : starts > 0 && (expectedInnings < 3.5 || starts / Math.max(1, games) < 0.45) ? 'OPENER_OR_BULK_RISK' : 'STARTER',
...
export async function buildGameContextV13(game, options = {}) {
  ...
  let away = {
    ...
    vsRight: awayVsRight,
    bullpen: awayBullpen,
    starter: starterWithExpectedInnings(base.away.starter, { id: game.awayProbableId || null, name: game.awayProbable || '', throws: awayThrows, throwsStatus: awayThrows ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.MISSING }, awayExpected),
  };
  let home = {
    ...base.home,
    lineup: homeLineup,
    ...
    vsRight: homeVsRight,
    bullpen: homeBullpen,
    starter: starterWithExpectedInnings(base.home.starter, { id: game.homeProbableId || null, name: game.homeProbable || '', throws: homeThrows, throwsStatus: homeThrows ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.MISSING }, homeExpected),
  };
  ...
  const addedProvenance = [
    ...
    feature(gamePk, 'starterExpectedInnings', { away: awayExpected, home: homeExpected }, rows.find(row => row.name === 'starterExpectedInnings').status, 'MLB_PERSON_PIT_OR_TEAM_PROXY_REJECTED_NEUTRAL', { asOf }),
