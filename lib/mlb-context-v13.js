...
export const MLB_CONTEXT_V13_VERSION = 'MLB-PIT-LINEUP-PLATOON-RELIEF-CONTEXT-2026-08-v11.0.3';
...
export function starterOnlyGameLogV13(payload, { teamId = null, endDate = '' } = {}) {
  ...
  const sum = key => starts.reduce((total, split) => total + Math.max(0, finite(split?.stat?.[key], 0) || 0), 0);
  ...
  const rate9 = value => inningsPitched > 0 ? value * 9 / inningsPitched : null;
  return {
    available: inningsPitched > 0,
    status: inningsPitched > 0 ? FEATURE_STATUS.CONFIRMED : FEATURE_STATUS.MISSING,
    inningsPitched,
    ...
    whip: inningsPitched > 0 ? (hits + baseOnBalls) / inningsPitched : null,
...
export async function buildGameContextV13(game, options = {}) {
  ...
  const awayStarterInput = awayStarterOnly.available ? { ...base?.away?.starter, ...awayStarterOnly } : base?.away?.starter;
  const homeStarterInput = homeStarterOnly.available ? { ...base?.home?.starter, ...homeStarterOnly } : base?.home?.starter;
