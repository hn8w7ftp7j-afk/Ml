import { verifiedInjuredRoster } from './mlb-roster-status-v1.js';
import { evaluateMlbLineupEligibilityV1 } from './mlb-lineup-eligibility-v1.js';
import { withFeatureSourceTime, nestedSourceReceipts } from './feature-source-time.js';
import { reliefOnlyGameLog, reliefMetricBlock, bullpenUsageCoverage } from './mlb-bullpen-evidence-v1.js';
...
export const MLB_CONTEXT_V13_VERSION = 'MLB-PIT-LINEUP-PLATOON-RELIEF-CONTEXT-2026-09-v11.0.9';
...
const finite = (value, fallback = null) => (typeof value === 'number' || typeof value === 'string' && value.trim() !== '') && Number.isFinite(Number(value)) ? Number(value) : fallback;
...
export async function recoverRecentPitchCountsV13(response, options = {}) {
  const feed = response?.data;
  if (!response?.ok || !feed?.gamePk) return response;
  const gaps = ['away', 'home'].flatMap(side => {
    const team = feed.liveData?.boxscore?.teams?.[side];
    return (team?.pitchers || []).filter(id => {
      const value = finite(team.players?.[`ID${id}`]?.stats?.pitching?.numberOfPitches);
      return !Number.isSafeInteger(value) || value < 0;
    }).map(id => ({ side, id }));
  });
  if (!gaps.length) return response;
  const backup = await requestJson(`${MLB_API}/game/${feed.gamePk}/boxscore`, { ...options, ttlMs: 15 * 60 * 1000 });
  const copy = structuredClone(feed);
  let recovered = 0;
  for (const { side, id } of gaps) {
    const team = backup.data?.teams?.[side];
    const original = feed.liveData.boxscore.teams[side];
    const player = team?.players?.[`ID${id}`];
    const pitches = finite(player?.stats?.pitching?.numberOfPitches);
    if (!backup.ok || Number(team?.team?.id) !== Number(feed.gameData?.teams?.[side]?.id)
      || JSON.stringify(team?.pitchers) !== JSON.stringify(original.pitchers)
      || Number(player?.person?.id) !== Number(id) || !Number.isSafeInteger(pitches) || pitches < 0) continue;
    const target = copy.liveData.boxscore.teams[side].players?.[`ID${id}`];
    if (!target || Number(target.person?.id) !== Number(id)) continue;
    target.stats = { ...(target.stats || {}), pitching: { ...(target.stats?.pitching || {}), numberOfPitches: pitches } };
    recovered++;
  }
  return { ...response, data: copy, sourceReceipts: [...(response.sourceReceipts || []), {
    ...mlbSourceReceiptV11(backup, 'MLB_GAME_BOXSCORE', { sourceGameId: feed.gamePk, sourceGameDate: feed.gameData?.datetime?.officialDate,
      purpose: 'MISSING_RECENT_RELIEF_PITCH_COUNTS', temporalContract: 'OFFICIAL_BOXSCORE_AS_FETCHED' }), recoveredPitchCounts: recovered,
  }] };
}
...
async function fetchInjuredRoster(teamId, date, options) {
  ...
  const verified = verifiedInjuredRoster(response?.ok ? response?.data?.roster : null);
  return { ...response, rosterDate: date, ...verified, rosterScope: 'OFFICIAL_40MAN_RESPONSE_STATUS_FILTERED',
...
export function parseHomePlateUmpireV13(feed) {
  const officials = Array.isArray(feed?.liveData?.boxscore?.officials) ? feed.liveData.boxscore.officials : [];
  const rows = officials.filter(item => clean(item?.officialType) === 'Home Plate');
  const missing = reason => ({ id: null, name: '', status: FEATURE_STATUS.MISSING, identityStatus: FEATURE_STATUS.MISSING, reason });
  if (!rows.length) return missing('HOME_PLATE_IDENTITY_NOT_IN_FEED');
  const identities = rows.map(row => ({
    id: finite(row?.official?.id),
    name: typeof row?.official?.fullName === 'string' ? row.official.fullName.trim() : '',
  }));
  if (identities.some(row => !Number.isSafeInteger(row.id) || row.id <= 0 || !row.name)) {
    return missing('INVALID_HOME_PLATE_IDENTITY');
  }
  const unique = new Map(identities.map(row => [`${row.id}:${row.name}`, row]));
  if (unique.size !== 1) return missing('CONFLICTING_HOME_PLATE_IDENTITIES');
  return { ...unique.values().next().value, status: FEATURE_STATUS.CONFIRMED, identityStatus: FEATURE_STATUS.CONFIRMED, reason: null };
}
...
function startingLineupRows(rows) {
  const unique = new Map();
  const slotIdentities = new Map();
  const identitySlots = new Map();
  for (const row of rows || []) {
    const order = Number(row?.battingOrder || 0);
    // MLB encodes starters as 100..900 and their replacements as 101, 102, etc.
    if (!Number.isInteger(order) || order < 100 || order > 900 || order % 100 !== 0) continue;
    const id = Number(row?.person?.id || row?.id || 0) || null;
    const key = id || clean(row?.person?.fullName || row?.name).toLowerCase();
    if (!key) continue;
    if (!unique.has(key)) unique.set(key, row);
    if (!slotIdentities.has(order)) slotIdentities.set(order, new Set());
    if (!identitySlots.has(key)) identitySlots.set(key, new Set());
    slotIdentities.get(order).add(key);
    identitySlots.get(key).add(order);
  }
  // A conflicting slot/identity is unknown; insertion order cannot decide which
  // of two players was an official starter.
  return [...unique.entries()]
    .filter(([key, row]) => identitySlots.get(key).size === 1 && slotIdentities.get(Number(row.battingOrder)).size === 1)
    .map(([, row]) => row)
    .sort((left, right) => Number(left.battingOrder) - Number(right.battingOrder));
}

function assignLineupSlots(rows, slots = Array.from({ length: 9 }, (_, index) => (index + 1) * 100)) {
  const candidates = rows.slice(0, slots.length);
  const memo = new Map();
  const solve = (index, used) => {
    if (index === candidates.length) return { cost: 0, orders: [] };
    const key = `${index}:${used}`;
    if (memo.has(key)) return memo.get(key);
    let best = null;
    for (let slotIndex = 0; slotIndex < slots.length; slotIndex += 1) {
      if (used & (1 << slotIndex)) continue;
      const rest = solve(index + 1, used | (1 << slotIndex));
      const cost = (slots[slotIndex] - Number(candidates[index].battingOrder)) ** 2 + rest.cost;
      if (!best || cost < best.cost) best = { cost, orders: [slots[slotIndex], ...rest.orders] };
    }
    memo.set(key, best);
    return best;
  };
  // The mean minimizes squared slot distance; solve the same objective jointly
  // so two hitters cannot both occupy its independently rounded slot.
  const assignment = solve(0, 0).orders;
  return candidates.map((row, index) => ({ ...row, battingOrder: assignment[index] }));
}

function lineupFromRows(rows, teamOps, { official = false, projected = false, source = '', sampleGames = 0 } = {}) {
  const ordered = startingLineupRows(rows);
  ...
  const players = ordered.map(row => {
    ...
    const weight = slotWeights[Number(row.battingOrder) / 100 - 1] * (modelMetricUsed ? reliability : 1);
    totalWeight += weight;
    ...
    return {
      ...
      // Keep the unrounded historical mean separate from the assigned slot.
      // Both use MLB's 100-based batting-order units; official rows have no mean.
      ...(finite(row.historicalAverageBattingOrder) != null
        ? { historicalAverageBattingOrder: Number(row.historicalAverageBattingOrder) } : {}),
      ...stats,
      reliability,
      modelMetricUsed,
    };
  });
  const occupiedSlots = new Set(players.map(row => row.battingOrder));
  for (let index = 0; index < 9; index += 1) {
    if (!occupiedSlots.has((index + 1) * 100)) totalWeight += slotWeights[index];
  }
  ...
  return {
    ...
    offensiveIndexBasis: 'TEAM_SEASON_OPS',
    offensiveIndexBaselineOps: teamBaseline,
...
export function parseOfficialLineupV13(feed, teamId, teamOps = null) {
  const rows = startingLineupRows(teamPlayers(feed, teamId));
...
export function projectLineupV13(feeds, teamId, teamOps = null, eligibilityEvidence = null) {
  const appearances = new Map();
  const latestBattingRows = new Map();
  const weights = [1, 0.86, 0.72, 0.58, 0.45, 0.34];
  (feeds || []).slice(0, 6).forEach((feed, index) => {
    const recencyWeight = weights[index] || 0.25;
    const players = teamPlayers(feed, teamId);
    for (const player of players) {
      const id = Number(player?.person?.id || 0) || null;
      // A newer pinch-hit appearance can carry the latest season batting block,
      // but it must not count as a start or contribute to the opening-slot mean.
      if (id && Number(player?.battingOrder || 0) > 0 && !latestBattingRows.has(id)) latestBattingRows.set(id, player);
    }
    for (const player of startingLineupRows(players)) {
      const id = Number(player?.person?.id || 0) || null;
      if (!id) continue;
  ...
  const rosterEligibility = eligibilityEvidence ? evaluateMlbLineupEligibilityV1({
    ...eligibilityEvidence, teamId, candidateIds: [...appearances.keys()],
  }) : null;
  const eligibleIds = rosterEligibility ? new Set(rosterEligibility.eligiblePlayerIds) : null;
  const candidates = [...appearances.values()]
    .filter(row => !eligibleIds || eligibleIds.has(Number(row.player.person.id)))
    .sort((left, right) => right.appearanceWeight - left.appearanceWeight || Number(left.player.person.id) - Number(right.player.person.id))
    .slice(0, 9)
    .map(row => {
      const battingRow = latestBattingRows.get(Number(row.player.person.id)) || row.player;
      return {
        ...row.player,
        // Retain a single latest batting block without mixing older PA or OPS.
        seasonStats: battingRow.seasonStats,
        pregameBattingStats: battingRow.pregameBattingStats,
        metricSource: battingRow.metricSource,
        metricProvenance: battingRow.metricProvenance,
        battingOrder: row.orderWeight / row.orderTotal,
        historicalAverageBattingOrder: row.orderWeight / row.orderTotal,
        position: row.catcherWeight > row.appearanceWeight * 0.45
          ? { ...(row.player.position || {}), abbreviation: 'C' }
          : row.player.position,
      };
    });
  const rows = assignLineupSlots(candidates);
  return { ...lineupFromRows(rows, teamOps, {
    projected: true,
...
export function mergePartialOfficialV13(projected, official, teamOps) {
  if (official?.official || !official?.players?.length) return official?.official ? official : projected;
  const officialRows = startingLineupRows(lineupRows(official));
  const officialIds = new Set(officialRows.map(row => row.id).filter(Boolean));
  const occupiedSlots = new Set(officialRows.map(row => Number(row.battingOrder)));
  const availableSlots = Array.from({ length: 9 }, (_, index) => (index + 1) * 100).filter(order => !occupiedSlots.has(order));
  const projectedRows = startingLineupRows(lineupRows(projected)).filter(row => !officialIds.has(row.id));
  const rows = [
    ...officialRows,
    ...assignLineupSlots(projectedRows, availableSlots),
  ];
  return { ...lineupFromRows(rows, teamOps, {
    projected: true,
    source: 'MLB_PARTIAL_OFFICIAL_PLUS_RECENT_PROJECTION',
    sampleGames: projected?.sampleGames || 0,
  }), ...(projected?.rosterEligibility ? { rosterEligibility: {
    ...projected.rosterEligibility,
    officialOverridePlayerIds: projected.rosterEligibility.excluded.filter(row => officialIds.has(row.id)).map(row => row.id),
  } } : {}) };
}
...
export async function hydrateLineupBattingV13(lineup, teamId, teamOps, asOf, options = {}) {
  ...
  return { ...lineupFromRows(rows, teamOps, { official: lineup.official, projected: lineup.projected, source: lineup.source, sampleGames: lineup.sampleGames }),
    ...(lineup.rosterEligibility ? { rosterEligibility: lineup.rosterEligibility } : {}) };
}
...
export function expectedStarterInningsV13(starter, { probableId = null, scheduledInnings = 9 } = {}) {
  ...
  if (teamProxyRejected) {
    return {
      ...
      expectedInningsCalculation: { version: 'STARTER_INNINGS_TRACE-v1', estimateStatus: 'PROJECTED', branch: 'TEAM_PROXY_REJECTED_FALLBACK', raw: null, fallback: probableId ? 4.8 : 4.5, bounds: null },
  ...
  return {
    ...
    expectedInningsCalculation: { version: 'STARTER_INNINGS_TRACE-v1', estimateStatus: 'PROJECTED',
      branch: raw != null ? 'HISTORICAL_IP_PER_START' : 'ROLE_OR_MISSING_FALLBACK',
      starterOnly, mixedRoleAggregate, innings, starts, games, raw, fallback: raw == null ? fallback : null,
      bounds: [1, maximum], bounded: raw != null && expectedInnings !== raw,
      formula: 'clamp(raw ?? fallback, 1, maximum)',
      limitations: ['歷史平均不等於當場投球局數已確認', '開局投手不會只因 gamesStarted 大於零而被排除', '此分支未加入對手打線或當場球數限制的額外局數調整'] },
...
export async function hydrateBullpenReliefV13(roster, recentFeeds, teamId, asOf, options = {}) {
  const reliefIds = new Set(recentFeeds.flatMap(feed => recentPitchingUsage(feed, teamId, true).map(row => row.id)));
  const candidates = roster.filter(row => !(finite(row.gamesStarted) === 0 && finite(row.gamesPitched, 0) > 0)
    && (reliefIds.has(row.id) || row.position === 'RP' || row.position === 'P' && (finite(row.gamesPitched, 0) === 0 || 1 - finite(row.gamesStarted, 0) / row.gamesPitched >= 0.65)));
  const results = new Map();
  for (let offset = 0; offset < candidates.length; offset += 4) {
    await Promise.all(candidates.slice(offset, offset + 4).map(async row => {
      const response = await fetchStarterGameLog(row.id, asOf.slice(0, 4), options);
      const reliefMetrics = reliefOnlyGameLog(response.ok ? response.data : null, { playerId: row.id, endDate: asOf });
      const receipt = mlbSourceReceiptV11(response, 'MLB_PERSON_GAME_LOG_RELIEF_ONLY', { asOf, purpose: 'RELIEF_ONLY_QUALITY', temporalContract: 'OFFICIAL_STATS_THROUGH_PREVIOUS_GAME_DAY' });
      results.set(row.id, { reliefMetrics, reliefMetricProvenance: { ...receipt, accepted: reliefMetrics.available, error: response.error || reliefMetrics.reason || null },
        sourceReceipts: [...(row.sourceReceipts || []), ...(receipt ? [receipt] : [])] });
    }));
  }
  return roster.map(row => ({ ...row, ...(results.get(row.id) || {}) }));
}
...
function recentPitchingUsage(feed, teamId, reliefOnly = false) {
  const side = sideForTeam(feed, teamId);
  ...
  return (reliefOnly ? pitcherIds.slice(1) : pitcherIds).map(id => {
    const player = players[`ID${id}`] || {};
    const gameStats = player?.stats?.pitching || {};
    return {
      id,
      starterId,
      isRelief: id !== starterId,
      name: clean(player?.person?.fullName),
      position: clean(player?.position?.abbreviation),
      pitches: Number.isSafeInteger(finite(gameStats.numberOfPitches)) && finite(gameStats.numberOfPitches) >= 0 ? finite(gameStats.numberOfPitches) : null,
...
export function buildBullpenV13({ roster = [], recentFeeds = [], teamId, gameDate, probableStarterId, league = {}, rosterComplete = false, expectedRecentGames }) {
  const usageCoverage = bullpenUsageCoverage({ recentFeeds, expectedRecentGames, teamId, gameDate, playerIds: roster.map(row => Number(row.id)).filter(id => id !== Number(probableStarterId)) });
  const target = new Date(gameDate || Date.now()).getTime();
  const byId = new Map();
  for (const row of roster || []) {
    if (!row?.id || Number(row.id) === Number(probableStarterId)) continue;
    byId.set(Number(row.id), { ...row, appearances: 0, weightedPitches: 0, pitchesLast1: 0, pitchesLast2: 0, usageGames: [], daysUsed: new Set() });
  }
  const historicalOnly = new Map();
  const seenUsageGames = new Set();
  let feedCount = 0;
  for (const feed of recentFeeds || []) {
    if (feed?.gamePk && seenUsageGames.has(String(feed.gamePk))) continue;
    if (feed?.gamePk) seenUsageGames.add(String(feed.gamePk));
    ...
    const usage = recentPitchingUsage(feed, teamId);
    ...
    for (const used of usage) {
      ...
      if (!row) {
        ...
        past.appearances += 1;
        past.pitches = past.pitches == null || used.pitches == null ? null : past.pitches + used.pitches;
      ...
      row.usageGames.push({ gamePk: feed.gamePk || null, date: feed?.gameData?.datetime?.officialDate || null,
        startedAt: feed?.gameData?.datetime?.dateTime || null, role: used.isRelief ? 'RELIEF' : 'START',
        pitches: used.pitches, daysAgo, fatigueWeight: weight,
        weightedPitches: used.pitches == null ? null : used.pitches * weight });
      if (used.isRelief) row.appearances += 1;
      row.weightedPitches += (used.pitches ?? 0) * weight;
      if (used.pitches == null) row.missingPitchCount = true;
      if (daysAgo <= 1) row.pitchesLast1 += used.pitches ?? 0;
      if (daysAgo <= 2) row.pitchesLast2 += used.pitches ?? 0;
  ...
  const relievers = [...byId.values()].filter(row => {
    ...
    const row = { ...seasonRow, ...reliefMetricBlock(seasonRow),
      metricProvenance: seasonRow.reliefMetricProvenance || seasonRow.metricProvenance };
    ...
    return {
      ...
      availabilityStatus: usageCoverage.complete ? FEATURE_STATUS.PROJECTED : FEATURE_STATUS.MISSING,
      seasonGamesStarted: seasonRow.gamesStarted ?? null,
      seasonGamesPitched: seasonRow.gamesPitched ?? null,
      consecutiveUse,
      availabilityCalculation: { version: 'PITCH_COUNT_AVAILABILITY_TRACE-v1',
        lastDayPitches: usageCoverage.complete ? row.pitchesLast1 : null,
        baseAvailability: usageCoverage.complete ? (row.pitchesLast1 >= 35 ? 0.25 : row.pitchesLast1 >= 25 ? 0.50 : row.pitchesLast1 >= 15 ? 0.78 : 1) : null,
        consecutiveMultiplier: consecutiveUse ? 0.78 : 1, bounds: [0.20, 1],
        status: usageCoverage.complete ? 'PROJECTED' : 'MISSING' },
      usageComplete: usageCoverage.complete && !row.missingPitchCount,
      observedWeightedPitches: row.missingPitchCount ? null : usageCoverage.complete ? row.weightedPitches : null,
      knownUsage: { weightedPitches: row.weightedPitches, pitchesLast1: row.pitchesLast1, pitchesLast2: row.pitchesLast2 },
      weightedPitches: usageCoverage.complete ? row.weightedPitches : null,
      pitchesLast1: usageCoverage.complete ? row.pitchesLast1 : null,
      pitchesLast2: usageCoverage.complete ? row.pitchesLast2 : null,
      availability: usageCoverage.complete ? clamp(availability, 0.20, 1) : null,
      modelAvailability: usageCoverage.complete ? clamp(availability, 0.20, 1) : 1,
  ...
  const weightedQualityRows = relievers.map(row => ({
    ...row,
    weight: Math.max(3, finite(row.inningsPitched, 0)) * row.modelAvailability,
  }));
  ...
  const observedFatigueIndex = clamp(totalWeightedPitches / Math.max(220, relievers.length * 30) + consecutiveHeavy * 0.07 + lastDayHeavy * 0.06, 0, 1);
  ...
  const highLeverageAvailability = leverageWeight > 0
    ? leverageRows.reduce((sum, row) => sum + row.modelAvailability * row.leverageScore, 0) / leverageWeight
    : relievers.length ? relievers.reduce((sum, row) => sum + row.modelAvailability, 0) / relievers.length : 0.75;
  const fatigueIndex = usageCoverage.complete ? observedFatigueIndex : null;
  const qualityFactor = relievers.length ? clamp(baseQuality * (1 + (fatigueIndex ?? 0) * 0.05 + (usageCoverage.complete ? 1 - highLeverageAvailability : 0) * 0.04), 0.78, 1.30) : 1;
  const qualityCoverage = relievers.length
    ...
    : 0;
  const status = rosterComplete && relievers.length >= 6 && usageCoverage.complete && qualityCoverage >= 0.65
    ? FEATURE_STATUS.CONFIRMED
  ...
  return {
    ...
    usageAvailable: usageCoverage.complete,
    usageCoverage,
    incompleteReasons: usageCoverage.complete ? [] : ['RECENT_USAGE_INCOMPLETE_NEUTRAL_MODEL_INPUTS_NOT_OBSERVED_REST'],
    modelUsageInputs: { fatigueIndex: fatigueIndex ?? 0, highLeverageAvailability: usageCoverage.complete ? highLeverageAvailability : 1 },
    recentFeedCount: feedCount,
    rosterCount: relievers.length,
    qualityCoverage,
    fatigueIndex,
    highLeverageAvailability: usageCoverage.complete ? clamp(highLeverageAvailability, 0.25, 1) : null,
    qualityFactor,
    qualityFactorIncludesUsage: usageCoverage.complete && relievers.length > 0,
    qualityFactorUsageSource: usageCoverage.complete && relievers.length > 0 ? 'MLB_RECENT_GAME_RELIEF_USAGE' : 'MISSING_NEUTRAL',
    relievers,
...
export async function buildGameContextV13(game, options = {}) {
  ...
  for (let offset = 0; offset < recentResponses.length; offset += 4) {
    const recovered = await Promise.all(recentResponses.slice(offset, offset + 4).map(response => recoverRecentPitchCountsV13(response, options)));
    recentResponses.splice(offset, recovered.length, ...recovered);
  }
  ...
  const recentReceipts = (side, purpose) => (base?.[side]?.scoring?.recentGames || [])
    ...
    .filter(Boolean)
    .flatMap(response => [feedReceipt(response, purpose), ...(response.sourceReceipts || [])]).filter(Boolean);

  const rosterDate = clean(game?.officialDate || currentFeed?.gameData?.datetime?.officialDate) || isoDate(game.gameDate);
  const [awayRosterResponse, homeRosterResponse, awayInjuredResponse, homeInjuredResponse] = await Promise.all([
  ...
  const projectionCutoffAt = new Date().toISOString();
  const eligibilityScope = { officialDate: rosterDate, firstPitch: game.gameDate, cutoffAt: projectionCutoffAt };
  ...
  const awayProjected = projectLineupV13(awayRecentFeeds, game.awayTeamId, base?.away?.hitting?.ops, {
    ...eligibilityScope, activeRosterResponse: awayRosterResponse, injuredRosterResponse: awayInjuredResponse,
  });
  const homeProjected = projectLineupV13(homeRecentFeeds, game.homeTeamId, base?.home?.hitting?.ops, {
    ...eligibilityScope, activeRosterResponse: homeRosterResponse, injuredRosterResponse: homeInjuredResponse,
  });
  const battingAsOf = shiftDate(rosterDate, -1);
  const [awayLineup, homeLineup] = await Promise.all([
    hydrateLineupBattingV13(mergePartialOfficialV13(awayProjected, awayOfficial, base?.away?.hitting?.ops), game.awayTeamId, base?.away?.hitting?.ops, battingAsOf, options),
    hydrateLineupBattingV13(mergePartialOfficialV13(homeProjected, homeOfficial, base?.home?.hitting?.ops), game.homeTeamId, base?.home?.hitting?.ops, battingAsOf, options),
  ]);
  for (const [side, lineup] of [['away', awayLineup], ['home', homeLineup]]) {
    lineup.sourceReceipts = [
      ...
      ...(lineup.official ? [] : lineup.rosterEligibility?.sourceReceipts || []),
    ].filter(Boolean);
  }
  ...
  const rosterFor = (response, teamId, starterId) => {
    ...
    const current = activePitchersFromFeed(currentFeed, teamId, starterId).map(row => ({ ...row, metricProvenance: { ...feedReceipt(currentFeedResponse, 'CURRENT_ROSTER_PITCHING_METRICS'), playerId: row.id, accepted: completePitchingMetrics(row), temporalContract: 'LIVE_FEED_AS_FETCHED_NOT_HISTORICAL_ARCHIVE' } }));
  ...
  const [awaySeasonRoster, homeSeasonRoster] = await Promise.all([
  ...
  const [awayRoster, homeRoster] = await Promise.all([
    hydrateBullpenReliefV13(awaySeasonRoster, awayRecentFeeds, game.awayTeamId, asOf, options),
    hydrateBullpenReliefV13(homeSeasonRoster, homeRecentFeeds, game.homeTeamId, asOf, options),
  ]);
  const awayRosterComplete = awayRosterState.complete;
  const homeRosterComplete = homeRosterState.complete;

  const awayBullpen = buildBullpenV13({ roster: awayRoster, recentFeeds: awayRecentFeeds, teamId: game.awayTeamId, gameDate: game.gameDate, probableStarterId: game.awayProbableId, league: base.league, rosterComplete: awayRosterComplete, expectedRecentGames: base.away?.scoring?.recentGames || [] });
  const homeBullpen = buildBullpenV13({ roster: homeRoster, recentFeeds: homeRecentFeeds, teamId: game.homeTeamId, gameDate: game.gameDate, probableStarterId: game.homeProbableId, league: base.league, rosterComplete: homeRosterComplete, expectedRecentGames: base.home?.scoring?.recentGames || [] });
  ...
  const umpire = { ...parseHomePlateUmpireV13(currentFeed),
    source: 'MLB_CURRENT_GAME_LIVE_FEED', publishedAt: null,
    effectStatus: 'NEUTRAL_UNVALIDATED',
    sourceReceipts: [feedReceipt(currentFeedResponse, 'CURRENT_HOME_PLATE_UMPIRE_IDENTITY')].filter(Boolean) };
  ...
  // The legacy `umpire` gate is an effect-validation placeholder. Keep its
  // numerical quality/error contribution unchanged; identity evidence is
  // reported separately and cannot promote an unvalidated umpire run effect.
  ...
  const advancedReceipts = advancedSnapshot.sourceReceipts || [];
  const lineupResponses = (side, lineup) => [
    currentFeedResponse,
    ...(lineup.official ? [] : (base?.[side]?.scoring?.recentGames || [])
      .map(row => recentResponseMap.get(Number(row.gamePk))).filter(Boolean)),
    ...(lineup.official ? [] : (lineup.rosterEligibility?.sourceReceipts || [])
      .filter(receipt => receipt.accepted === true && Number(receipt.teamId) === Number(game[`${side}TeamId`]))
      .map(receipt => ({ ...receipt, ok: true }))),
  ];
  const featureResponses = {
    awayLineup: lineupResponses('away', awayLineup),
    homeLineup: lineupResponses('home', homeLineup),
    starterExpectedInnings: [awayStarterLogResponse, homeStarterLogResponse],
    starterHandedness: [currentFeedResponse],
    platoonSplits: [awayVsLResponse, awayVsRResponse, homeVsLResponse, homeVsRResponse],
    reliefOnlyBullpen: [awayRosterResponse, homeRosterResponse, currentFeedResponse, ...recentResponses],
    advancedSavantSnapshot: advancedReceipts,
    injuryRunValue: [awayInjuredResponse, homeInjuredResponse],
    umpireIdentity: [currentFeedResponse],
    umpireZone: [currentFeedResponse, ...advancedReceipts],
    parkWindOrientation: [base.weather, ...advancedReceipts],
  };
  const addedProvenance = [
    feature(gamePk, 'awayLineup', awayLineup, awayLineup.status, awayLineup.source, { asOf, rawPayloadHash: sha256(featureResponses.awayLineup.map(row => row?.rawPayloadHash)) }),
    feature(gamePk, 'homeLineup', homeLineup, homeLineup.status, homeLineup.source, { asOf, rawPayloadHash: sha256(featureResponses.homeLineup.map(row => row?.rawPayloadHash)) }),
    ...
    feature(gamePk, 'umpireIdentity', umpire, umpire.status, 'MLB_CURRENT_GAME_LIVE_FEED', { asOf: null, publishedAt: null, rawPayloadHash: currentFeedResponse?.rawPayloadHash, qualityFlags: ['IDENTITY_ONLY_NO_VALIDATED_RUN_EFFECT', 'FETCH_TIME_IS_NOT_PUBLICATION_TIME'] }),
  ...
  ].map(row => withFeatureSourceTime(row, [
    ...(featureResponses[row.featureName] || []).map(response => ({
      sourceRecord: response?.sourceRecord || row.sourceProvider,
      fetchedAt: response?.fetchedAt || null, rawPayloadHash: response?.rawPayloadHash || null,
      fetchStatus: response?.ok === true || response?.available === true ? 'FETCHED' : 'FAILED',
    })), ...nestedSourceReceipts(row.value),
  ]));
  const warnings = [
    ...
    awayLineup.official && homeLineup.official
      ...
      : '至少一隊未取得完整已確認的當場打線，使用近六場打序投影並提高模型誤差；不代表官方尚未公布。',
  ...
  return {
    ...
    sourceStatuses: { ...Object.fromEntries(rows.map(row => [row.name, row.status])),
      umpire: umpire.status, umpireIdentity: umpire.status, umpireEffect: 'NEUTRAL_UNVALIDATED' },
