...
export const JOINT_SCORE_V13_VERSION = 'BASEBALL-STATE-AWARE-LINKED-SEGMENT-SCORE-2026-08-v10.6.0';
export const MLB_STATE_RUN_MODEL_V13_VERSION = 'MLB-CORRELATED-STATE-FEATURE-RUN-PROFILE-2026-08-v10.6.0';
...
function bullpenFactor(team, fallbackFactor) {
  ...
  return clamp(finite(fallbackFactor, 1), 0.95, 1.05);
}
...
export function estimateRunProfileV13(context) {
  const base = estimateRunProfileV103(context);
  const advanced = buildMlbAdvancedAdjustmentV2(context);
  ...
  const awayOffense = clamp(awayBaseOffense * awayLineup * awayPlatoon * advanced.awayRunFactor, 0.80, 1.22);
  const homeOffense = clamp(homeBaseOffense * homeLineup * homePlatoon * advanced.homeRunFactor, 0.80, 1.22);
  const awayStarter = finite(base?.components?.awayStarter, 1);
  const homeStarter = finite(base?.components?.homeStarter, 1);
  const awayBullpen = bullpenFactor(away, base?.components?.awayBullpen);
  const homeBullpen = bullpenFactor(home, base?.components?.homeBullpen);
  ...
  return {
    ...
    components: {
      ...
      bullpenProxy: away?.bullpen?.pureRelief === true && home?.bullpen?.pureRelief === true ? 'RELIEF_ONLY_PIT_ROSTERS' : base.components.bullpenProxy,
...
export function extraInningsKernelV13(scenario, maximumInnings = 12) {
...
function fullDistribution(scenario) {
  ...
  return {
    ...
    coverage: 1,
    gameStateAudit: {
      version: GAME_STATE_V13_VERSION,
...
export function buildJointScoreSnapshotV13({ context, modelVersion, rulesVersion }) {
