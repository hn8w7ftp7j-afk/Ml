...
export const JOINT_SCORE_V13_VERSION = 'BASEBALL-STATE-AWARE-LINKED-SEGMENT-SCORE-2026-08-v11.0.1';
export const MLB_STATE_RUN_MODEL_V13_VERSION = 'MLB-CORRELATED-STATE-FEATURE-RUN-PROFILE-2026-08-v11.0.1';
...
// Retained for model compatibility. The original 5% gate rationale is obsolete;
// removing this calibration choice requires a separate historical comparison.
...
function bullpenFactor(team) {
  ...
  if (bullpen?.pureRelief === true && finite(bullpen?.qualityFactor, 0) > 0 && bullpen?.status !== 'MISSING') {
...
function mlbDataUsage(context, advanced) {
  const rows = [];
  ...
  for (const side of ['away', 'home']) {
    ...
    const opponent = context?.[side === 'away' ? 'home' : 'away'] || {};
    add(`${side}.hitting`, true, true, 'SEASON_RUN_RATE_AND_OPS_RESIDUAL_WITH_SAMPLE_SHRINKAGE');
    add(`${side}.recentHitting`, true, true, 'RECENT_RUN_RATE_DELTA_WITH_EXISTING_SMALL_SAMPLE_SHRINKAGE');
    add(`${side}.pitching`, false, false, 'TEAM_PITCHING_DIAGNOSTIC_ONLY_NOT_INDIVIDUAL_STARTER_OR_RELIEF');
    add(`${side}.recentPitching`, false, false, 'TEAM_RECENT_PITCHING_DIAGNOSTIC_ONLY_NO_DUPLICATE_RELIEF_EFFECT');
    add(`${side}.starter`, true, true, 'INDIVIDUAL_PITCHING_AND_EXPECTED_INNINGS_HANDOFF;EXISTING_SAMPLE_UNCERTAINTY');
    add(`${side}.starter.throws`, ['L', 'R'].includes(team.starter?.throws), true, 'OPPONENT_SPLIT_SELECTION;EXISTING_HANDEDNESS_RESIDUAL');
    add(`${side}.lineup.offensiveIndex`, finite(team?.lineup?.offensiveIndex, null) != null, true, 'LINEUP_INDEX;SOURCE_STATUS_UNCERTAINTY;MISSING_SLOT_METRICS_REMAIN_NEUTRAL');
    add(`${side}.bullpen.qualityFactor`, team.bullpen?.pureRelief === true && finite(team.bullpen?.qualityFactor, 0) > 0 && team.bullpen?.status !== 'MISSING', true, 'PURE_RELIEF_QUALITY_AND_EXISTING_STATUS_UNCERTAINTY');
    for (const key of ['fatigueIndex', 'highLeverageAvailability']) {
      const indirect = team.bullpen?.pureRelief === true && finite(team.bullpen?.qualityFactor, 0) > 0
        && team.bullpen?.status !== 'MISSING' && team.bullpen?.qualityFactorIncludesUsage === true
        && finite(team.bullpen?.[key], null) != null;
      add(`${side}.bullpen.${key}`, indirect, false, indirect
        ? 'INDIRECT_VIA_BULLPEN_QUALITY_FACTOR;NO_SECOND_RUN_ADJUSTMENT'
        : 'DIAGNOSTIC_ONLY_NO_VERIFIED_USAGE_FOLDING;MISSING_USAGE_REMAINS_NEUTRAL');
    }
    for (const [key, hand] of [['vsLeft', 'L'], ['vsRight', 'R']]) {
      const selected = opponent.starter?.throws === hand;
      add(`${side}.${key}`, selected && team[key]?.available === true, selected, selected ? 'SELECTED_OPPONENT_HAND_SPLIT_AND_EXISTING_STATUS_RESIDUAL' : 'NOT_SELECTED_OPPONENT_HAND');
    }
    add(`${side}.injuries`, false, true, 'BASE_MODEL_UNCERTAINTY_ONLY;ADVANCED_REPLACEMENT_RUN_EFFECT_REQUIRES_PROMOTION');
  }
  add('weather.meanRunFactor', true, true, 'TEMPERATURE_PRESSURE_HUMIDITY_AND_EXISTING_ROOF_PRIOR;SOURCE_STATUS_UNCERTAINTY');
  add('park.runFactor', true, true, 'PARK_RUN_FACTOR_AND_SOURCE_STATUS_UNCERTAINTY');
  add('umpire', false, false, 'IDENTITY_ONLY_NO_VALIDATED_CATCHER_NEUTRAL_RESIDUAL');
  for (const [key, component] of Object.entries(advanced?.promotion?.families || {})) {
    const active = component?.appliedToAwayRunMean === true || component?.appliedToHomeRunMean === true;
    add(`advanced.${key}`, active, false, active ? 'PROMOTED_BY_SERVER_POLICY' : `DIAGNOSTIC_ONLY_${component?.runtimeStatus || 'NOT_PROMOTED'}`);
  }
  return rows;
}
...
export function estimateRunProfileV13(context, { serverOwnedPromotionPolicy = null } = {}) {
  ...
  const awaySplit = home?.starter?.throws === 'L' ? away?.vsLeft : home?.starter?.throws === 'R' ? away?.vsRight : null;
  const homeSplit = away?.starter?.throws === 'L' ? home?.vsLeft : away?.starter?.throws === 'R' ? home?.vsRight : null;
  ...
  return {
    ...
    uncertainty: {
      // These residual terms were already calculated for the state model but
      // never reached its scenarios. Combine variances without changing their
      // existing calibration constants or moving either predicted mean.
      away: Math.hypot(base.uncertainty.away, awayFeatureResidual),
      home: Math.hypot(base.uncertainty.home, homeFeatureResidual),
    ...
    dataUsage: mlbDataUsage(context, advanced),
    diagnostics: {
      ...base.diagnostics,
      stateFeatureResidual: { away: awayFeatureResidual, home: homeFeatureResidual, usedInUncertainty: true, method: 'SUM_EXISTING_BASE_AND_RESIDUAL_VARIANCES' },
      scenarioSigmaScale: { value: STATE_AWARE_SCENARIO_SIGMA_SCALE, status: 'RETAINED_COMPATIBILITY_CALIBRATION;REMOVAL_PENDING_VALIDATION' },
    },
  };
}
