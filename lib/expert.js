export const EXPERT_VERSION = 'GPT-MLB-RESEARCH-LAYER-2026-08-v1';
...
function mergeAudit(base, supplied) {
  ...
  return {
    confirmed: unique([...(base.confirmed || []), ...(source.confirmed || [])]).slice(0, 18),
    estimated: unique([...(base.estimated || []), ...(source.estimated || [])]).slice(0, 18),
    unknown: unique([...(base.unknown || []), ...(source.unknown || [])]).slice(0, 18),
    blocking: unique([...(base.blocking || []), ...(source.blocking || [])]).slice(0, 10),
    unmodeled: unique([...(base.unmodeled || []), ...(source.unmodeled || [])]).slice(0, 14),
  };
}
...
function compactTeam(team) {
  return {
    seasonHitting: team?.seasonHitting,
    recentHitting: team?.recentHitting,
    vsLeft: team?.vsLeft,
    vsRight: team?.vsRight,
    starter: team?.starter ? {
      ...
      season: team.starter.season,
      recent: team.starter.recent,
    ...
    bullpen: team?.bullpen ? {
      ...
      relievers: (team.bullpen.relievers || []).slice(0, 8),
      daily: (team.bullpen.daily || []).slice(0, 4),
    } : null,
    injuries: (team?.injuries || []).slice(0, 12),
...
async function gatewayAssessment(key, model, content, timeoutMs) {
  ...
  if (!response.ok) throw new Error(`GPT 研究層服務失敗（${response.status}）`);
  const payload = JSON.parse(raw);
  return cleanGatewayJSON(payload?.choices?.[0]?.message?.content || '');
}

export async function buildExpertAssessment({ context, markets, mode = 'auto', timeoutMs = 12000 }) {
  if (mode === 'off') return fallbackExpertAssessment(context, '設定為純統計模式');
  const key = process.env.AI_GATEWAY_API_KEY;
  if (!key) return fallbackExpertAssessment(context, 'AI_GATEWAY_API_KEY 未設定');

  const keyValue = cacheKey(context, markets);
  const hit = cache.get(keyValue);
  if (hit && hit.expires > Date.now()) return hit.value;

  const model = process.env.AI_ANALYSIS_MODEL || process.env.AI_MODEL || 'google/gemini-2.5-flash';
  try {
    const payload = compactPayload(context, markets);
    const parsed = await gatewayAssessment(key, model, promptFor(payload), timeoutMs);
    const result = sanitizeExpertAssessment(parsed, context, model);
    cache.set(keyValue, { value: result, expires: Date.now() + 300000 });
    return result;
  } catch (error) {
    if (mode === 'required') throw new Error(`GPT 研究層無法完成：${String(error?.message || error)}`);
    return fallbackExpertAssessment(context, `GPT 研究層失敗，已切換統計備援：${String(error?.message || error)}`);
  }
}
