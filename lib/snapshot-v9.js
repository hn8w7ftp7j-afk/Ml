...
export const DATA_VERSION = 'BASEBALL-POINT-IN-TIME-DATA-SNAPSHOT-2026-09-v11.0.3';
...
export function buildSnapshotFingerprints({
  ...
  const corePayload = {
    ...
    ...(['NPB', 'KBO', 'CPBL'].includes(league) && context?.sourceEvidence ? {
      sourceEvidenceIdentity: { inputCutoffAt: context.inputCutoffAt || null,
        events: (context.sourceEvidence.events || []).map(row => [row.id, row.contentHash, row.fetchedAt, row.success]) },
    } : {}),
