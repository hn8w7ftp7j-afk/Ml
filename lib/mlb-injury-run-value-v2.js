...
export function buildInjuryRunValueV2({ injuredRoster, lineup, teamOps = 0.72, leagueOps = 0.72, observedAt = '' } = {}) {
  ...
  if (!injuredRoster?.available) {
    return { status: 'MISSING', observationStatus: 'MISSING', validationStatus: 'PENDING', observedAt, sourceReceipts, source: 'MLB_INJURED_LIST', reason: 'INJURED_LIST_UNAVAILABLE' };
  }
  ...
  return {
    ...
    sourceReceipts,
    rosterPlayers: rows.map(row => ({ id: Number(row?.person?.id || row?.id || 0) || null, name: String(row?.person?.fullName || row?.name || ''), position: row?.position?.abbreviation || null, hasBattingMetrics: playerValue(row, leagueOps) != null })),
    coverage: { listedPlayers: rows.length, playersWithBattingMetrics: rows.filter(row => playerValue(row, leagueOps) != null).length, diagnosticAbsentPlayers: absent.length },
