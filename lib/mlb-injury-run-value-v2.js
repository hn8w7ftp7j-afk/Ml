...
function playerValue(row, leagueOps) {
  ...
  const plateAppearances = Math.max(0, finite(stat.plateAppearances, 0));
...
export function buildInjuryRunValueV2({ injuredRoster, lineup, teamOps = 0.72, leagueOps = 0.72, observedAt = '' } = {}) {
  ...
  const sourceReceipts = injuredRoster?.sourceReceipts || [];
  const lineupIds = new Set((lineup?.players || []).map(row => Number(row.id)).filter(Boolean));
  const verifiedRows = rows.filter(row => injuryMembership(row) === true);
  const conflictingIds = verifiedRows.filter(row => lineupIds.has(Number(row?.person?.id))).map(row => Number(row.person.id));
  const absent = verifiedRows
    ...
    .slice(0, 8);
  if (!injuredRoster?.available || rows.some(row => injuryMembership(row) === null) || conflictingIds.length) {
    return { status: 'MISSING', observationStatus: 'MISSING', validationStatus: 'PENDING', observedAt, sourceReceipts, source: 'MLB_INJURED_LIST', conflictingIds, reason: conflictingIds.length ? 'INJURED_LINEUP_IDENTITY_CONFLICT' : 'INJURED_LIST_UNVERIFIED' };
  }
  ...
  return {
    ...
    modelStatus: 'DIAGNOSTIC_ONLY_VALIDATION_PENDING',
    rosterDate: injuredRoster.rosterDate || null,
    rosterScope: injuredRoster.rosterScope || 'UNSPECIFIED',
    completeInjuryCensusVerified: false,
    sourceReceipts,
    rosterPlayers: verifiedRows.map(row => ({ id: Number(row?.person?.id || row?.id || 0) || null, name: String(row?.person?.fullName || row?.name || ''), position: row?.position?.abbreviation || null, hasBattingMetrics: playerValue(row, leagueOps) != null })),
    coverage: { listedPlayers: verifiedRows.length, playersWithBattingMetrics: verifiedRows.filter(row => playerValue(row, leagueOps) != null).length, diagnosticAbsentPlayers: absent.length },
