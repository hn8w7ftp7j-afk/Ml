...
async function diagnoseDirectionParentBinding(record) {
  ...
  return {
    ...
    parentMismatchFields: directionParentBindingMismatches(parent),
...
export async function persistAnalysisDirectionHistory(bundle) {
  ...
  if (rows.length !== 8) {
    let diagnostic = {
      parentFound: null,
      parentMismatchFields: ['DIAGNOSTIC_UNAVAILABLE'],
      preGame: null,
    };
    try { diagnostic = await diagnoseDirectionParentBinding(validated.records[0]); }
    catch {
      // The persistence result remains fail-closed even when its read-only
      // diagnostic query is unavailable.
    }
    ...
    console.error('[ANALYSIS_DIRECTION_HISTORY_WRITE_NOT_CONFIRMED]', {
      snapshotId: validated.snapshotId,
