...
async function diagnoseDirectionParentBinding(expected) {
  const rows = await sql()`
    SELECT snapshot_id, league_id, external_game_id, game_number, game_start, analysis_type,
      input_hash, price_fingerprint, distribution_id, distribution_hash,
      analysis_as_of, data_as_of, line_as_of, parent_snapshot_id,
      game_identity, versions, game_start > NOW() AS game_start_future
    FROM baseball_analysis_pit_snapshots
    WHERE snapshot_id = ${expected.snapshot_id}
    LIMIT 1
  `;
  if (!rows.length) return ['PARENT_SNAPSHOT_MISSING'];
  const parent = rows[0];
  const storedJson = value => {
    if (value == null || typeof value === 'object') return value;
    try { return JSON.parse(value); } catch { return null; }
  };
  const gameIdentity = storedJson(parent.game_identity) || {};
  const versions = storedJson(parent.versions) || {};
  const mismatches = [];
  const same = (field, actual, wanted) => {
    if (String(actual ?? '') !== String(wanted ?? '')) mismatches.push(field);
  };
  const sameInstant = (field, actual, wanted) => {
    if (Date.parse(actual || '') !== Date.parse(wanted || '')) mismatches.push(field);
  };
  same('league_id', parent.league_id, expected.league_id);
  same('external_game_id', parent.external_game_id, expected.external_game_id);
  same('game_number', parent.game_number, expected.game_number);
  sameInstant('game_start', parent.game_start, expected.game_start);
  same('analysis_type', parent.analysis_type, expected.analysis_type);
  same('input_hash', parent.input_hash, expected.input_hash);
  same('price_fingerprint', parent.price_fingerprint, expected.price_fingerprint);
  same('distribution_id', parent.distribution_id, expected.distribution_id);
  same('distribution_hash', parent.distribution_hash, expected.distribution_hash);
  sameInstant('analysis_as_of', parent.analysis_as_of, expected.analysis_as_of);
  sameInstant('data_as_of', parent.data_as_of, expected.data_as_of);
  sameInstant('line_as_of', parent.line_as_of, expected.line_as_of);
  same('parent_snapshot_id', parent.parent_snapshot_id, expected.parent_snapshot_id);
  same('away_team_id', gameIdentity.awayTeamId, expected.away_team_id);
  same('home_team_id', gameIdentity.homeTeamId, expected.home_team_id);
  same('model_ev_formula_version', versions.modelEvFormulaVersion, expected.model_ev_formula_version);
  same('robust_ev_version', versions.robustEvVersion, expected.robust_ev_version);
  same('direction_slot_contract_version', versions.directionSlotContractVersion, expected.direction_slot_contract_version);
  if (parent.game_start_future !== true) mismatches.push('GAME_START_NOT_FUTURE');
  return mismatches.length ? mismatches : ['UNKNOWN_BOUND_FILTER'];
}
...
export async function persistAnalysisDirectionHistory(bundle) {
  ...
  `;
  if (rows.length !== 8) {
    const bindingMismatches = await diagnoseDirectionParentBinding(payload[0]);
    const incomingRows = await sql()`
      SELECT COUNT(*)::integer AS incoming_count
      FROM jsonb_to_recordset(${JSON.stringify(payload)}::jsonb) AS row(snapshot_id TEXT)
    `;
    const uniqueIndexes = await sql()`
      SELECT indexname FROM pg_indexes
      WHERE schemaname = current_schema()
        AND tablename = 'baseball_analysis_direction_results'
        AND indexdef ILIKE '%UNIQUE%'
      ORDER BY indexname
    `;
    const incomingCount = Number(incomingRows[0]?.incoming_count || 0);
    const uniqueIndexNames = uniqueIndexes.map(row => clean(row?.indexname, 180)).filter(Boolean);
