...
import { teamNameZh } from './i18n.js';
import {
  parseTaiwanContract,
...
function officialIdentityFailures(record, result) {
  ...
  // Stored MLB schedules use canonical Chinese names; live-feed results use
  // official English names. Keep ID checks above and normalize known aliases
  // only at this result boundary, without changing frozen records or picks.
  const name = value => record.leagueId === 'MLB' ? teamNameZh(value) : value;
  if (!resultAway || !sameTeam(name(record.away), name(resultAway))) failures.push('正式賽果客隊名稱不一致');
  if (!resultHome || !sameTeam(name(record.home), name(resultHome))) failures.push('正式賽果主隊名稱不一致');
...
export async function ensureAnalysisDirectionHistorySchema() {
  ...
  if (!schemaReady) schemaReady = (async () => {
    // Cold serverless workers share PostgreSQL catalogs, not this promise.
    // Serialize and commit all DDL together so concurrent function/trigger
    // replacement cannot fail with 'tuple concurrently updated'.
    const statements = [sql()`SELECT pg_advisory_xact_lock(hashtext('baseball_analysis_direction_schema_v1'))`];
    const schemaQuery = (strings, ...values) => { statements.push(sql()(strings, ...values)); };
    schemaQuery`
    ...
    `;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_game ON baseball_analysis_direction_results(league_id, external_game_id, analysis_as_of DESC)`;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_snapshot ON baseball_analysis_direction_results(snapshot_id, slot_index)`;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_market_w ON baseball_analysis_direction_results(league_id, market, model_ev DESC) WHERE status = 'CALCULATED'`;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_dimensions ON baseball_analysis_direction_results(league_id, market, qa_status, line_type, lead_minutes) WHERE status = 'CALCULATED'`;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_distribution ON baseball_analysis_direction_results(league_id, distribution_hash, snapshot_id)`;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_settlement_latest ON baseball_analysis_direction_settlements(direction_result_id, settled_at DESC, created_at DESC)`;
    schemaQuery`CREATE INDEX IF NOT EXISTS idx_analysis_direction_settlement_outcome ON baseball_analysis_direction_settlements(status, outcome, settled_at DESC)`;
    schemaQuery`CREATE UNIQUE INDEX IF NOT EXISTS idx_analysis_direction_settlement_root ON baseball_analysis_direction_settlements(direction_result_id) WHERE supersedes_settlement_id IS NULL`;
    schemaQuery`CREATE UNIQUE INDEX IF NOT EXISTS idx_analysis_direction_settlement_child ON baseball_analysis_direction_settlements(supersedes_settlement_id) WHERE supersedes_settlement_id IS NOT NULL`;
    schemaQuery`
...
const DIRECTION_PARENT_BINDING_FIELDS = Object.freeze([
  'leagueId', 'gamePk', 'gameNumber', 'gameStart', 'analysisType',
  'inputHash', 'priceFingerprint', 'distributionId', 'distributionHash',
  'analysisAsOf', 'dataAsOf', 'lineAsOf', 'parentSnapshotId',
  'awayTeamId', 'homeTeamId', 'modelEvFormulaVersion', 'robustEvVersion',
  'directionSlotContractVersion',
]);

function directionParentBindingMismatches(row) {
  if (!row) return ['snapshotId'];
  return DIRECTION_PARENT_BINDING_FIELDS.filter(field => row[field] !== true);
}

async function diagnoseDirectionParentBinding(record) {
  const rows = await sql()`
    SELECT
      league_id IS NOT DISTINCT FROM ${record.leagueId} AS "leagueId",
      external_game_id IS NOT DISTINCT FROM ${record.gamePk} AS "gamePk",
      game_number IS NOT DISTINCT FROM ${record.gameNumber} AS "gameNumber",
      game_start IS NOT DISTINCT FROM ${record.gameStart}::timestamptz AS "gameStart",
      analysis_type IS NOT DISTINCT FROM ${record.analysisType} AS "analysisType",
      input_hash IS NOT DISTINCT FROM ${record.inputHash} AS "inputHash",
      price_fingerprint IS NOT DISTINCT FROM ${record.priceFingerprint} AS "priceFingerprint",
      distribution_id IS NOT DISTINCT FROM ${record.distributionId} AS "distributionId",
      distribution_hash IS NOT DISTINCT FROM ${record.distributionHash} AS "distributionHash",
      analysis_as_of IS NOT DISTINCT FROM ${record.analysisAsOf}::timestamptz AS "analysisAsOf",
      data_as_of IS NOT DISTINCT FROM ${record.dataAsOf}::timestamptz AS "dataAsOf",
      line_as_of IS NOT DISTINCT FROM ${record.lineAsOf}::timestamptz AS "lineAsOf",
      parent_snapshot_id IS NOT DISTINCT FROM ${record.parentSnapshotId} AS "parentSnapshotId",
      NULLIF(game_identity->>'awayTeamId', '')::bigint IS NOT DISTINCT FROM ${record.awayTeamId} AS "awayTeamId",
      NULLIF(game_identity->>'homeTeamId', '')::bigint IS NOT DISTINCT FROM ${record.homeTeamId} AS "homeTeamId",
      versions->>'modelEvFormulaVersion' IS NOT DISTINCT FROM ${record.modelEvFormulaVersion} AS "modelEvFormulaVersion",
      versions->>'robustEvVersion' IS NOT DISTINCT FROM ${record.robustEvVersion} AS "robustEvVersion",
      versions->>'directionSlotContractVersion' IS NOT DISTINCT FROM ${record.directionSlotContractVersion} AS "directionSlotContractVersion",
      game_start > NOW() AS "preGame"
    FROM baseball_analysis_pit_snapshots
    WHERE snapshot_id = ${record.snapshotId}
    LIMIT 1
  `;
  const parent = rows[0] || null;
  const bindingMismatches = directionParentBindingMismatches(parent);
  return {
    parentFound: Boolean(parent),
    bindingMismatches,
    preGame: parent ? parent.preGame === true : null,
  };
}

export async function persistAnalysisDirectionHistory(bundle) {
  const initialValidated = validateAnalysisDirectionHistory(bundle);
  if (!analysisDirectionHistoryDatabaseConfigured()) return { stored: false, reason: 'DATABASE_NOT_CONFIGURED', snapshotId: initialValidated.snapshotId };
  await ensureAnalysisDirectionHistorySchema();
  // A retry may resolve to an already-frozen parent whose PIT clocks differ
  // from the new request. Preserve PostgreSQL microseconds as text, then hash
  // and store every child direction against those canonical parent instants.
  const parentTimeRows = await sql()`
    SELECT
      to_char(analysis_as_of AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS "analysisAsOf",
      to_char(data_as_of AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS "dataAsOf",
      to_char(line_as_of AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.US"Z"') AS "lineAsOf"
    FROM baseball_analysis_pit_snapshots
    WHERE snapshot_id = ${initialValidated.snapshotId}
    LIMIT 1
  `;
  let validated = initialValidated;
  if (parentTimeRows.length) {
    const parentTimes = parentTimeRows[0];
    const records = initialValidated.records.map(record => {
      const canonicalRecord = {
        ...record,
        analysisAsOf: parentTimes.analysisAsOf,
        dataAsOf: parentTimes.dataAsOf,
        lineAsOf: parentTimes.lineAsOf,
        leadMinutes: Math.max(0, (Date.parse(record.gameStart) - Date.parse(parentTimes.analysisAsOf)) / 60_000),
      };
      canonicalRecord.recordHash = sha256(recordHashPayload(canonicalRecord));
      return validateAnalysisDirectionRecord(canonicalRecord);
    });
    validated = validateAnalysisDirectionHistory({
      ...initialValidated,
      records,
  ...
  const inserted = await sql()`
    WITH incoming AS (
      ...
      SELECT i.*,
        p.analysis_as_of AS parent_analysis_as_of,
        p.data_as_of AS parent_data_as_of,
        p.line_as_of AS parent_line_as_of
      FROM incoming i
    ...
    INSERT INTO baseball_analysis_direction_results (
      ...
      robust_status, robust_scenario_source, robust_scenario_hash,
      reprice_version, distribution_id, distribution_hash, input_hash, price_fingerprint,
      parent_analysis_as_of AS analysis_as_of, parent_data_as_of AS data_as_of,
      parent_line_as_of AS line_as_of, lead_minutes, result_payload, record_payload
  ...
  if (rows.length !== 8) {
    let diagnostic = {
      parentFound: null,
      bindingMismatches: ['DIAGNOSTIC_UNAVAILABLE'],
      preGame: null,
    };
    try { diagnostic = await diagnoseDirectionParentBinding(validated.records[0]); }
    catch {
      // Persistence remains fail-closed if this read-only diagnostic is unavailable.
    }
    const storedSlotIds = rows.map(row => row.slot_id);
    const missingSlotIds = validated.records
      .map(record => record.slotId)
      .filter(slotId => !storedSlotIds.includes(slotId));
    console.error('[ANALYSIS_DIRECTION_HISTORY_BOUND_MISMATCH]', {
      snapshotId: validated.snapshotId,
    ...
    return {
      stored: false,
      reason: 'EIGHT_SLOT_WRITE_NOT_CONFIRMED',
      snapshotId: validated.snapshotId,
      insertedCount: inserted.length,
      storedCount: rows.length,
      missingSlotIds,
...
export function buildChangedAnalysisDirectionSettlements(rows, result) {
  ...
  const changed = candidates.filter(({ row, event }) => (
    ...
    (row?.latest_status ?? row?.latestStatus) === 'MANUAL_REVIEW' && event.status === 'SETTLED'
  ));
  return {
    candidates,
    changed,
...
export async function settlePendingAnalysisDirections({
  ...
  const pendingGames = await sql()`
    SELECT d.league_id, d.external_game_id, MIN(d.official_date) AS official_date,
      MIN(d.record_payload->>'away') AS expected_away,
      MIN(d.record_payload->>'home') AS expected_home,
      MIN(d.record_payload->>'gameNumber') AS expected_game_number,
      BOOL_OR(EXISTS (
  ...
  const batch = await runAnalysisDirectionSettlementTasks(pendingGames, async game => {
      ...
      const result = await resolver(game.league_id, Number(game.external_game_id), {
        date: game.official_date || undefined,
        expectedAway: game.expected_away || '',
        expectedHome: game.expected_home || '',
        expectedGameNumber: Number(game.expected_game_number || 1),
      });
      ...
      const rows = await sql()`
        ...
        FROM baseball_analysis_direction_results d
        LEFT JOIN LATERAL (
          SELECT s.settlement_id, s.status, s.official_result_hash, s.settlement_rule_version
      ...
      for (const { event } of correction.candidates) {
        if (event.status !== 'MANUAL_REVIEW') continue;
        const reason = clean(event.settlementError, 500);
        summary.manualReviewReasons[reason] = (summary.manualReviewReasons[reason] || 0) + 1;
      }
  ...
  console.info('[ANALYSIS_DIRECTION_SETTLEMENT_RUN]', {
    ...summary,
    failureCount: summary.failures.length,
    failures: summary.failures.slice(0, 12),
  });
  return summary;
}
