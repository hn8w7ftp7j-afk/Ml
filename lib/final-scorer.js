...
async function gatewayScore(key, models, prompt, timeoutMs) {
...
export function auditFinalScoreShape(assessment, payload) {
...
export async function buildFinalScoreAssessment({ context, analysis, settings = {}, timeoutMs = 50000 }) {
  ...
  const basePrompt = promptFor(payload);
...
function distributionAudit(results) {
  ...
  const failures = [];
  if (rows.length >= 6 && maximumBucket >= Math.ceil(rows.length * 0.50) && evRange > 0.04) {
