...
import { wrGapExceedsReference as exceedsWrGap, wrGapWarning, WR_GAP_REFERENCE } from './wr-gap-warning.js';
...
export const MAX_RAW_SCENARIO_EV_SPREAD = WR_GAP_REFERENCE;
...
export function qualifyEvV103({ row, rawWeightedEV, rawRobustEV, modelProbability, rebateRate = 0.015, gate = {} }) {
  ...
  // R includes both scenario downside and a data-quality margin. This is a
  // W/R gap, not proof that scenarios themselves are unstable. Ignore only
  // arithmetic noise; genuine sub-display-precision excess must still warn.
  const wrGapExceedsReference = exceedsWrGap(rawScenarioSpread);
  ...
  if (quality < MINIMUM_DATA_QUALITY) {
    auditWarnings.push(`核心棒球資料品質${quality.toFixed(2)}低於${MINIMUM_DATA_QUALITY.toFixed(2)}參考線；保留評分與排名，並標示預估資料風險`);
  }
  ...
  if (!referenceEvidenceEligible) {
    ...
    auditWarnings.push(`${externalReason}；外部稽核無資料；不影響模型W/R、S分數與排名`);
  }
  if (extreme) {
    auditWarnings.push(`極高模型EV（W +${(rawW * 100).toFixed(1)}%），建議複核；W、R、S與排名資格照實保留`);
  }

  if (wrGapExceedsReference) {
    auditWarnings.push(wrGapWarning(rawScenarioSpread));
  }
  ...
  return {
    ...
    marketEdgeExtreme,
    rawScenarioSpread,
    wrGapExceedsReference,
    // Compatibility alias for existing consumers; not a scenario-only test.
    scenarioStable: rawScenarioSpread != null && !wrGapExceedsReference,
    weightedRobustGap,
