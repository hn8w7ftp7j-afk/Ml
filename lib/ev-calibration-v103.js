...
export const EV_CALIBRATION_V103_VERSION = 'MODEL-EV-OPTIONAL-INDEPENDENT-MARKET-AUDIT-2026-08-v10.5.1';
...
export function qualifyEvV103({ row, rawWeightedEV, rawRobustEV, modelProbability, rebateRate = 0.015, gate = {} }) {
  ...
  if (!actualReaderSource) {
    reasons.push('只允許 Tai888 Reader 自動同步的實際信用盤建立模型 W/R；手動盤只可永久記錄，不得評分或列排名');
  ...
  if (extreme) {
    auditWarnings.push(`未校準模型W達+${(rawW * 100).toFixed(1)}%，列為待複核；數值照實顯示但不進排名`);
  }
