...
export const EV_CALIBRATION_V103_VERSION = 'INDEPENDENT-CONSENSUS-PAYOFF-EV-GATE-2026-08-v10.4.3';
...
export function qualifyEvV103({ row, rawWeightedEV, rawRobustEV, modelProbability, rebateRate = 0.015, gate = {} }) {
  ...
  if (rawW == null || rawR == null || probability == null) auditWarnings.push('原始模型EV或機率不是有限數值；不參與市場價差資格判定');
  if (gate?.passedForShadowScore !== true) auditWarnings.push('原始棒球資料Gate未通過；只影響模型稽核，不否決三莊市場價差');
  if (quality < MINIMUM_DATA_QUALITY) auditWarnings.push(`原始棒球核心資料品質${quality.toFixed(2)}低於${MINIMUM_DATA_QUALITY.toFixed(2)}；只供稽核`);
  ...
  // The raw baseball model is not locked-OOS/forward calibrated, so it cannot
  // veto an independently established market-price W/R. Keep every raw-model
  // anomaly visible for audit, but never mix it into qualification or score.
  if (extreme) {
    auditWarnings.push(`原始模型EV達${(Math.abs(rawW) * 100).toFixed(1)}%，超過15%極端值線；僅供稽核，不否決三莊市場價差`);
  }

  if (rawScenarioSpread != null && rawScenarioSpread > MAX_RAW_SCENARIO_EV_SPREAD) {
    auditWarnings.push(`原始模型中央與壓力情境EV差距${(rawScenarioSpread * 100).toFixed(1)}個百分點，超過${(MAX_RAW_SCENARIO_EV_SPREAD * 100).toFixed(0)}個百分點；僅供稽核`);
  }

  if (priorEligible) {
    if (probabilityGap > probabilityTolerance) {
      auditWarnings.push(`原始模型與獨立市場機率差距${(probabilityGap * 100).toFixed(1)}個百分點；原始模型不參與資格與分數`);
    }
    if (sign(rawW) !== 0 && sign(referenceEV) !== 0 && sign(rawW) !== sign(referenceEV)) {
      auditWarnings.push('原始模型EV與獨立市場價格EV方向相反；以三莊市場價格為影子W/R，原始模型只供稽核');
    }
  ...
  // model remains server-side diagnostic evidence and cannot qualify, veto,
  // or alter the independent market-price result.
  ...
  const status = qualified
    ...
    : marketEdgeExtreme ? 'EXTREME_MARKET_EDGE_HELD_FOR_REVIEW' : 'CALIBRATION_BLOCK';
