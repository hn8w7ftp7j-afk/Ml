...
export const BET_CLOSING_LINE_VERSION = 'BASEBALL-BET-CLOSING-LINE-v1.0.1';
...
function serverVerifiedCapturedReader(bet) {
  return clean(bet?.readerEvidenceStatus).toUpperCase() === 'SERVER_VERIFIED_CAPTURED_READER'
    && Boolean(clean(bet?.serverEvidenceVersion))
    && Boolean(iso(bet?.serverEvidenceVerifiedAt));
}
...
function baseSnapshot(bet, row, evidence = {}) {
  ...
  const outsidePlacementWindow = lineTime < Date.parse(placedAt) - 10 * 60_000;
  if ((outsidePlacementWindow && !serverVerifiedCapturedReader(bet)) || lineTime >= Date.parse(gameStart)) return null;
