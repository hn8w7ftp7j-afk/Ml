...
export function analysisStarterDisplay(item, side) {
  ...
  if (starter?.name && sameTeam && starter.identityMismatch !== true) {
    ...
    const rosterIdentityChecked = /(?:^|_)ROSTER_VALIDATED(?:_|$)|(?:^|_)VALIDATED_BY_[A-Z0-9_]*ROSTER(?:_|$)/.test(source);
    ...
    const label = projected ? '輪值推估' : assignmentLabel
      || (source.includes('TAI888') ? rosterIdentityChecked ? 'Reader人選／名冊核對' : 'Reader人選／未核對' : '當場未核對');
    return `${starter.name}（${label}）`;
  }
  return game[`${side}Probable`] ? `${game[`${side}Probable`]}（賽程人選／未核對）` : '賽程未提供先發';
}
