...
function normalizedHash(value, fallback) {
  const candidate = clean(value).toLowerCase();
  return /^[a-f0-9]{64}$/.test(candidate) ? candidate : sha256(fallback);
}
...
export function buildPitPredictionFromBetV109(bet = {}) {
  ...
  const sourcePayloadHash = normalizedHash(bet.readerPayloadHash || bet.rawBoardHash, [
    bet.readerRevision,
    lineAsOf,
    bet.placedContractSnapshot,
  ]);
  const modelInputHash = normalizedHash(bet.inputHash || bet.snapshotId, [
    bet.gamePk,
    bet.modelVersion,
    bet.analysisAsOf,
    bet.weightedEV,
...
export function settledBetToPitObservationV109(bet = {}) {
  const prediction = bet.pitPrediction || buildPitPredictionFromBetV109(bet).prediction;
