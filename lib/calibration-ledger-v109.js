...
import {
  ...
  CONTINUOUS_CALIBRATION_V109_VERSION,
...
export const CALIBRATION_LEDGER_V109_VERSION = 'BASEBALL-IMMUTABLE-FORWARD-CALIBRATION-LEDGER-v10.9.1';
...
// A legacy alias is only valid when the raw field did not exist. An explicit
// missing raw result must not be replaced with a potentially different value.
const rawOrLegacy = (bet, rawKey, legacyKey) => Object.hasOwn(bet, rawKey) ? bet[rawKey] : bet[legacyKey];
...
export function featureObservedAtsFromContextV109(context = {}) {
  ...
  const fallback = asian ? null : clean(context?.fetchedAt) || null;
  const output = {};
  for (const row of context?.featureProvenance || []) {
    const name = clean(row?.featureName || row?.feature);
    const observedAt = clean(asian ? row?.fetchedAt : row?.observedAt || row?.asOf || fallback);
...
export function buildPitPredictionFromBetV109(bet = {}) {
  ...
  const prediction = {
    ...
    rawWeightedEv: finite(rawOrLegacy(bet, 'rawModelWeightedEV', 'weightedEV')),
    rawRobustEv: finite(rawOrLegacy(bet, 'rawModelRobustEV', 'robustEV')),
...
export function settledBetToPitObservationV109(bet = {}) {
  ...
  const checked = validatePitObservationV109({ ...prediction, settledAt, realizedNetReturn });
  return checked.ok ? checked.value : null;
}

export function buildCalibrationStatusFromBetsV109(bets = [], options = {}) {
  const signature = sha256({ ledgerVersion: CALIBRATION_LEDGER_V109_VERSION, calibrationVersion: CONTINUOUS_CALIBRATION_V109_VERSION, options, bets: (bets || []).map(bet => [bet?.id, bet?.status, bet?.settlement?.settledAt, bet?.settlement?.roi, bet?.pitPredictionStatus, bet?.pitEvidenceVerified, bet?.calibrationEligibility, bet?.pitPrediction]) });
