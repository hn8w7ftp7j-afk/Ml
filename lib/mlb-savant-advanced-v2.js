...
function buildFielding(lineup, fielders, framing, observedAt) {
  ...
  const catcherFramingRuns = rawCatcherFramingRuns * catcherReliability;
  ...
  return {
    ...
    rawValue: { fieldingRunValue: rawFieldingRunValue, catcherFramingRuns: rawCatcherFramingRuns, players },
    regressedValue: { fieldingRunValue: regressedFieldingRunValue, catcherFramingRuns },
...
function buildPitchMatchup(lineup, opposingStarter, batterPitches, pitcherPitches, observedAt) {
  ...
  (lineup?.players || []).slice(0, 9).forEach((player, index) => {
    ...
    for (const [pitchType, pitcherRow] of mix) {
      ...
      batterRv += finite(batterRow.run_value_per_100, 0) * usage;
