...
async function fetchCsv(url, { fetchImpl = fetch, timeoutMs = 15000, ttlMs = 30 * 60 * 1000 } = {}) {
  ...
  try {
    ...
    const rawPayloadHash = sha256(text);
    const value = response.ok
      ? { ok: true, rows: parseCsvV2(text), observedAt, fetchedAt, rawPayloadHash, sourceRecord: key, error: '' }
      : { ok: false, rows: [], observedAt, fetchedAt, rawPayloadHash, sourceRecord: key, error: `HTTP ${response.status}` };
    cache.set(key, { value, expiresAt: Date.now() + (value.ok ? ttlMs : 60000) });
    return value;
  } catch (error) {
    return { ok: false, rows: [], observedAt, fetchedAt: new Date().toISOString(), rawPayloadHash: null, sourceRecord: key, error: error?.name === 'AbortError' ? '資料取得逾時' : String(error?.message || error) };
  } finally {
    clearTimeout(timer);
  }
}
...
export async function buildSavantAdvancedSnapshotV2(context, options = {}) {
  ...
  return {
    ...
    home,
    sources: urls,
    sourceReceipts: [fieldingResponse, framingResponse, batterResponse, pitcherResponse].map(r => ({
      sourceRecord: r.sourceRecord, fetchedAt: r.fetchedAt, rawPayloadHash: r.rawPayloadHash,
      ok: r.ok, fetchStatus: r.ok ? 'FETCHED' : 'FAILED',
    })),
    sourceStatus: {
