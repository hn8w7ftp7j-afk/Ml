...
export const ANALYSIS_CACHE_VERSION = 'BASEBALL-ANALYSIS-CACHE-W-FIRST-EIGHT-DIRECTION-v5.0.0';
...
const SHADOW_LEAGUES = new Set(['MLB', 'NPB', 'KBO', 'CPBL']);
...
export function analysisContractSignature(leagueValue, game, markets) {
  ...
  return sha256({
    domain: 'baseball-positive-ev/analysis-contract/v5',
    league,
...
export function analysisCachePayloadMatches(entry, { league: leagueValue, game, fingerprints, signature }) {
  ...
  const payloadGameLeague = String(payload?.game?.league || payload?.game?.leagueId || '').trim().toUpperCase();
  const contextGameLeague = String(payload?.context?.game?.league || payload?.context?.game?.leagueId || payload?.context?.leagueId || '').trim().toUpperCase();
  return gameLeague === league
    && payload?.league === league
    && payloadGameLeague === league
    && contextGameLeague === league
