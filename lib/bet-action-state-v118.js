...
export const BET_ACTION_STATE_VERSION = '11.8.40';

function retainedGameIsInactive(item) {
  return String(item?.statusLabel || '').includes('目前已不在官方賽前清單');
}

export function capturedReaderContractReady(item, row, now = Date.now()) {
...
export function evaluateBetAction({
  ...
  const readerReady = capturedReaderContractReady(item, row, now);
  ...
  if (!readerReady) return {
    kind: 'none', text: '等待Reader', title: '等待Reader最新盤口驗證；完成後會自動開放記錄', disabled: true,
