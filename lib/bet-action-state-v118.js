...
export function evaluateBetAction({
  ...
  if (queued && ['queued', 'saving', 'uncertain'].includes(queued.status) && !latest) return {
    kind: 'none', disabled: true, recordable: false, readerReady: false,
    text: queued.status === 'queued' ? '排隊中…' : queued.status === 'saving' ? '儲存確認中…' : '結果待確認',
    title: queued.message || '依點選順序寫入並回讀帳本；確認前不會標記已記錄',
    reasonCode: 'RECORD_QUEUE',
  };
  ...
  if (queued?.status === 'failed' && queued.requiresRecheck
    && (!item?.customData?.analysis?.pitSnapshotId
      || item.customData.analysis.pitSnapshotId === queued.rejectedPitSnapshotId)) return {
    kind: 'none', text: '記錄未成功｜需重新複核',
    title: `${queued.message || '盤口證據未通過驗證'}。尚未入單；請重新複核此場，確認盤口後再記錄。`,
    disabled: true, recordable: false, readerReady,
    reasonCode: 'RECORD_RECHECK_REQUIRED',
    canRecheck: betsEnabled && prestart && item?.status !== 'running',
  };
  if (recordable) return {
    kind: 'record',
    text: queued?.status === 'failed' && !queued.requiresRecheck ? '記錄失敗｜重試' : cancelled ? '重新紀錄下注' : '紀錄實際下注',
    ...
    failureMessage: queued?.status === 'failed' && !queued.requiresRecheck ? queued.message : '',
  };
