...
function injuryAdjustment(block, baselineRuns, gameStart, promotionPolicy) {
  ...
  const replacementDelta = finite(block?.replacementRunDeltaPerGame, null);
  ...
  return {
    ...
    runsLost,
    state,
    audit: { absentShare, replacementDelta, coverage, weight },
  };
}
...
function directionalWindAdjustment(block, baselineRuns, gameStart, promotionPolicy) {
  ...
  const runDelta = coefficient * windSpeed * alignment * roofOpenProbability * statusWeight(state.status);
  return {
    factor: logFactorFromRuns(runDelta, baselineRuns, 1),
    runDelta,
    state,
    audit: { coefficient, fieldBearing, windFrom, windTo, windSpeed, alignment, roofOpenProbability },
  };
}
