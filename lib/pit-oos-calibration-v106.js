...
export const OOS_CALIBRATION_VERSION = 'pit-oos-isotonic-block-bootstrap-v2-result-cutoff';
...
export function buildOosCalibration(inputs, { minimumTrainRows = 200, minimumValidationRows = 50, robustQuantile = 0.10 } = {}) {
  ...
  for (const validationSeason of seasons) {
    ...
    const trainingCutoff = validation.reduce((cutoff, row) => row.modelAsOf < cutoff ? row.modelAsOf : cutoff, `${validationSeason}-01-01T00:00:00.000Z`);
    const train = rows.filter(row => seasonOf(row.gameStart) < validationSeason && row.settledAt < trainingCutoff);
    ...
    const knownBlocks = blockResiduals(knownResiduals);
    const foldRobustAdjustment = knownBlocks.length >= 3 ? quantile(knownBlocks.map(row => row.residual), robustQuantile) : null;
    const foldPredictions = validation.map(row => {
      const calibratedW = predictIsotonic(groupModels.get(groupKey(row)) || globalModel, row.rawWeightedEv);
      return {
        ...row, calibratedW, validationSeason, trainingCutoff,
    ...
    folds.push({ validationSeason, trainRows: train.length, validationRows: validation.length, trainingCutoff, trainedThrough: latest(train, 'settledAt'), trainingGameThrough: latest(train, 'gameStart'), robustResidualRows: knownResiduals.length, robustTrainedThrough: latest(knownResiduals, 'settledAt') });
  }
  ...
  // Keep historical R chronological; the final artifact adjustment is a fit
  // using all now-known residuals, not a past prediction.
  ...
  const artifactCore = {
    ...
    trainedThrough: latest(rows, 'settledAt'), trainingGameThrough: latest(rows, 'gameStart'), trainedThroughBasis: 'LATEST_RESULT_SETTLED_AT', sampleSize: rows.length, oosSampleSize: withRobust.length,
    ...
    diagnostics: {
      ...
      oosMeanW: mean(withRobust.map(row => row.calibratedW)), oosMeanR: mean(withRobust.map(row => row.robustR).filter(Number.isFinite)),
...
export function applyOosCalibration(artifact, rawWeightedEv) {
  ...
  const calibratedW = predictIsotonic(artifact.isotonicModel, finite(rawWeightedEv));
