...
export const TAI888_READER_PARSER_VERSION = 'TAI888-READER-PARSER-v2.0.0';
...
const TEAM_IDS = Object.freeze({
  ...
  WSH: 120, WAS: 120, NYM: 121, OAK: 133, ATH: 133, PIT: 134, SD: 135,
...
function matchReaderGame(raw, schedule) {
  ...
  let candidates = (schedule || []).filter(game => Number(game.awayTeamId) === awayId && Number(game.homeTeamId) === homeId);
  if (!candidates.length) return null;
  if (candidates.length === 1) return candidates[0];
  const boardMinutes = minutes(raw.boardTime);
  if (boardMinutes == null) return candidates[0];
  candidates = candidates.map(game => {
    const local = taipeiDateTime(game.gameDate);
    const candidateMinutes = minutes(local.time);
    let distance = candidateMinutes == null ? 99999 : Math.abs(candidateMinutes - boardMinutes);
    if (distance > 720) distance = 1440 - distance;
    if (raw.boardDate && local.date !== raw.boardDate) distance += 1440;
...
export function normalizeTai888ReaderPayload(payload, schedule, { deviceId = '', receivedAt = new Date().toISOString() } = {}) {
  ...
  for (const raw of rawGames) {
    ...
    if (!TEAM_IDS[awayCode] || !TEAM_IDS[homeCode]) {
      unmatched.push(`${awayCode || '?'}@${homeCode || '?'}`);
      continue;
    }
  ...
  const hashSource = canonical({
    version: clean(payload.version, 80), sourceHost, boardDate,
    games: games.map(game => ({ gamePk: game.gamePk, markets: game.markets.map(row => ({ market: row.market, pick: row.pick, water: row.water })) })),
  });
