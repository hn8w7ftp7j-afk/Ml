...
import { officialPrestartSlate } from './official-schedule-v1.js';

export const TAI888_READER_PARSER_VERSION = 'TAI888-READER-PARSER-v2.2.1';
...
function normalizedRawGame(raw) {
  const allowedState = value => ['AVAILABLE', 'UNAVAILABLE', 'BLOCKED'].includes(value) ? value : 'UNAVAILABLE';
  const normalized = {
  ...
  if (raw?.marketStates && typeof raw.marketStates === 'object') {
    normalized.marketStates = Object.fromEntries(['FULL_HANDICAP', 'FULL_TOTAL', 'FIRST_HALF_HANDICAP', 'FIRST_HALF_TOTAL']
      .map(name => [name, allowedState(raw.marketStates[name])]));
  }
  return normalized;
}
...
function canonicalReaderPayloadForClientHash(payload, league) {
  const games = (Array.isArray(payload?.games) ? payload.games : [])
    .map(game => {
      const normalized = normalizedRawGame(game);
      // The claimed hash is calculated by the Reader before the server maps
      // Tai888 aliases (ATH -> OAK, WAS -> WSH, etc.) to official team IDs.
      // Hash the exact wire codes here; canonical alias normalization belongs
      // to the server-owned rawBoardHash and schedule validation below.
      normalized.awayCode = clean(game?.awayCode, 12);
      normalized.homeCode = clean(game?.homeCode, 12);
      if (clean(payload?.version, 100) === 'TAI888-READER-DOM-v2.1.0') delete normalized.marketStates;
      return normalized;
    })
    .sort((left, right) => {
...
function validateClaimedPayloadHash(payload, league, readerVersion) {
  ...
  if (!['TAI888-READER-DOM-v2.1.0', 'TAI888-READER-DOM-v2.2.0'].includes(clean(payload?.version, 100))) {
    throw readerError('Reader DOM payload 版本不受支援', 426);
  }
  ...
  const expected = createHash('sha256')
    ...
    .digest('hex');
  // This is an unkeyed client-side digest, not an authentication boundary.
  // Browser/runtime serialization differences must never discard an otherwise
  // valid board. The server authenticates the paired device, validates every
  // game against the official slate and computes its own rawBoardHash below.
  // Keep the claimed digest format gate for diagnostics, but never trust it as
  // the stored board identity or reject a board solely because it differs.
  return { claimed, expected, matches: claimed === expected };
}
...
export function validateTai888ReaderEnvelope(payload, {
  ...
  if (payload.parseIssues != null && (!Array.isArray(payload.parseIssues) || payload.parseIssues.length)) {
    throw readerError('Reader 盤口解析衝突，已拒絕接收；請重新整理原始盤面', 409);
  }
  const league = normalizedLeague(payload.league, { allowLegacyMlb: true });
...
function matchCompleteBoard(rawGames, schedule, boardDate, league, identityIssues = null) {
  ...
  for (const [key, rawRows] of rawGroups) {
    ...
    if (!assignment) {
      ...
      if (identityIssues && rawRows.length <= officialRows.length) {
        identityIssues.push({
          matchup: label,
          boardTimes: rawRows.map(row => row.boardTime),
          officialGames: officialRows.map(game => ({ gamePk: Number(game.gamePk), gameDate: game.gameDate })),
          rawIndices: rawRows.map(row => row.index),
        });
        continue;
      }
  ...
  const isolatedCount = (identityIssues || []).reduce((sum, issue) => sum + issue.rawIndices.length, 0);
  if (matches.size + isolatedCount !== rawGames.length
    || new Set([...matches.values()].map(game => Number(game.gamePk))).size !== matches.size) {
...
export function readerMarketsAvailable(markets) {
  if (!Array.isArray(markets) || markets.length < 2 || markets.length > 8 || markets.length % 2 !== 0) return false;
  const names = [...new Set(markets.map(row => row?.market).filter(Boolean))];
  if (names.length * 2 !== markets.length || names.some(market => !MARKET_ORDER.includes(market))) return false;
  const unique = new Set();
  return names.every(market => {
    const pair = markets.filter(row => row?.market === market);
    if (pair.length !== 2 || validateMarketPair(market, pair).length) return false;
...
export function readerSnapshotIsComplete(snapshot) {
  ...
  return Boolean(league)
    && snapshot?.version === TAI888_READER_PARSER_VERSION
    ...
    && games.every(game => (
      ...
      && readerMarketsAvailable(game.markets)
...
export function normalizeTai888ReaderPayload(payload, schedule, options = {}) {
  ...
  const captureTime = Date.parse(envelope.pageActivityAt || '');
  if (!Number.isFinite(captureTime)
    || officialPrestartSlate(officialSchedule, captureTime).length !== officialSchedule.length) {
    throw readerError('Reader 賽程包含擷取當下已開賽、延期或取消場次，本次未寫入', 409);
  }
  ...
  const identityIssues = options.isolateIdentityFailures ? [] : null;
  const matches = matchCompleteBoard(rawGames, officialSchedule, envelope.boardDate, league, identityIssues);
  const identityBlockedPks = new Set((identityIssues || []).flatMap(issue => issue.officialGames.map(game => game.gamePk)));
  const normalizedRows = rawGames.map((raw, index) => {
    const game = matches.get(index);
    if (!game) return null; // Never parse or attach prices from an unresolved matchup.
    ...
    if (!locked && !readerMarketsAvailable(markets)) {
      throw readerError(`${awayCode}@${homeCode}：已開市場必須各自完整提供兩個方向`);
    }
    const stateByMarket = {
      '全場讓分': raw?.marketStates?.FULL_HANDICAP,
      '全場大小': raw?.marketStates?.FULL_TOTAL,
      '上半讓分': raw?.marketStates?.FIRST_HALF_HANDICAP,
      '上半大小': raw?.marketStates?.FIRST_HALF_TOTAL,
    };
    const availableMarkets = MARKET_ORDER.filter(name => markets.some(row => row.market === name));
    const blockedMarkets = MARKET_ORDER.filter(name => !availableMarkets.includes(name) && stateByMarket[name] === 'BLOCKED');
    const unavailableMarkets = MARKET_ORDER.filter(name => !availableMarkets.includes(name) && !blockedMarkets.includes(name));
    return {
      ...
      markets,
      marketCoverage: {
        openMarkets: availableMarkets.length,
        totalMarkets: MARKET_ORDER.length,
        directionCount: markets.length,
        availableMarkets,
        unavailableMarkets,
        blockedMarkets,
        missingMarkets: [...unavailableMarkets, ...blockedMarkets],
      },
      readerMeta: {
        awayCode,
        homeCode,
  ...
  }).filter(Boolean).sort((left, right) => Number(left.gamePk) - Number(right.gamePk));
  ...
  const inferredUnopenedGames = officialSchedule
    ...
    .map(game => ({
      ...
      source: {
        league,
        provider: 'TAI888_READER_AUTO',
        label: identityBlockedPks.has(Number(game.gamePk)) ? 'Reader 場次待核對' : 'Tai888 Reader 未呈現盤口',
        sourceType: 'ACTUAL_TW_CREDIT',
        observedAt: envelope.observedAt,
        receivedAt: envelope.receivedAt,
        pageActivityAt: envelope.pageActivityAt,
      ...
      marketStatus: 'locked',
      unavailableReason: identityBlockedPks.has(Number(game.gamePk)) ? 'reader-identity-unresolved' : 'not-rendered-by-reader',
      markets: [],
      marketCoverage: {
        openMarkets: 0,
        totalMarkets: MARKET_ORDER.length,
        directionCount: 0,
        availableMarkets: [],
        unavailableMarkets: identityBlockedPks.has(Number(game.gamePk)) ? [] : [...MARKET_ORDER],
        blockedMarkets: identityBlockedPks.has(Number(game.gamePk)) ? [...MARKET_ORDER] : [],
  ...
  const payloadHash = sha256({
    domain: 'baseball-positive-ev/reader-normalized-snapshot/v3',
    league,
    sourceHost: envelope.sourceHost,
    boardDate: envelope.boardDate,
    games: [...games, ...unopenedGames]
      .sort((left, right) => Number(left.gamePk) - Number(right.gamePk))
      .map(row => ({
        gamePk: row.gamePk,
        ...
        marketCoverage: {
          availableMarkets: row.marketCoverage?.availableMarkets || [],
          unavailableMarkets: row.marketCoverage?.unavailableMarkets || [],
          blockedMarkets: row.marketCoverage?.blockedMarkets || [],
        },
      })),
  });
  const normalized = {
    ...
    receivedAt: envelope.receivedAt,
    pageActivityAt: envelope.pageActivityAt,
    activityAt: envelope.pageActivityAt,
    ...
    marketCount: games.reduce((count, game) => count + game.markets.length / 2, 0),
    directionCount: games.reduce((count, game) => count + game.markets.length, 0),
    partialGameCount: games.filter(game => game.markets.length < 8).length,
    unopenedGameCount: unopenedGames.length,
    scheduleGameCount: officialSchedule.length,
    unmatched: [],
    identityIssues: (identityIssues || []).map(({ rawIndices, ...issue }) => issue),
    identityBlockedGameCount: identityBlockedPks.size,
    games,
    unopenedGames,
  };
