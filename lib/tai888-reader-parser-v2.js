...
export function normalizeTai888ReaderPayload(payload, schedule, options = {}) {
  ...
  if (envelope.expectedGameCount > officialSchedule.length) {
    const fullOfficialSchedule = officialTaipeiBoardSchedule(options.fullSchedule, envelope.boardDate);
    ...
    const visibleMatches = matchCompleteBoard(visibleRawGames, fullOfficialSchedule, envelope.boardDate);
