...
function rowReceipt(context, profile, side, category, label, aliases) {
  ...
  const data = category === 'injuries' ? object(team.advanced?.injuryRunValue || team.injuryReport)
  ...
  const substitutions = unique([
    ...
    ...(numeric(data.excludedHistoricalCount) > 0 ? [`EXCLUDED_${numeric(data.excludedHistoricalCount)}_HISTORICAL_RELIEVERS_NOT_ON_CURRENT_ROSTER`] : []),
  ...
  } : category === 'bullpen' ? { rosterCount: numeric(data.rosterCount) ?? playerIds.length, rosterComplete: data.rosterComplete === true, qualityCoverage: numeric(data.qualityCoverage), usageAvailable: data.usageAvailable === true } : null;
  ...
  return {
    ...
    asOf: text(data.asOf) || uniqueSources.find(item => item.asOf)?.asOf || null,
