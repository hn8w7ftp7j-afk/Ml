...
function rowReceipt(context, profile, side, category, label, aliases) {
  ...
  } : category === 'bullpen' ? { rosterCount: numeric(data.rosterCount) ?? playerIds.length, rosterComplete: data.rosterComplete === true, qualityCoverage: numeric(data.qualityCoverage), qualityCount: numeric(data.qualityCoverage) == null ? null : Math.round(data.qualityCoverage * (numeric(data.rosterCount) ?? players.length)), usageAvailable: data.usageAvailable === true } : null;
  const metrics = category === 'starter' ? { ...metric({ ...object(data.season), ...data }, ['era', 'fip', 'whip', 'woba', 'leagueWoba', 'inningsPitched', 'battersFaced', 'estimatedInningsPitched', 'expectedInnings', 'qualityFactor']), performanceMetric: text(data.performanceMetric) || null }
    : category === 'lineup' ? metric(data, ['offensiveIndex', 'observedAtBats', 'observedHits', 'observedBattingAverage'])
  ...
  return {
    ...
    ...(category === 'bullpen' ? { players: players.map(player => ({ id: text(player.id), name: text(player.name), metrics: metric(player, ['inningsPitched', 'era', 'whip', 'strikeOuts', 'baseOnBalls', 'homeRuns']), metricSource: sourceName(player), metricAsOf: text(player.metricAsOf || player.metricProvenance?.asOf) || null, metricProvenance: player.metricProvenance || null, qualityComplete: numeric(player.inningsPitched) > 0 && ['era', 'whip', 'strikeOuts', 'baseOnBalls', 'homeRuns'].every(key => numeric(player[key]) != null) })), exclusions: list(data.historicalOnlyRelievers) } : {}),
...
function supportingReceipts(context, profile) {
  ...
  const rows = ['away', 'home'].flatMap(side => [
    ...
    return { key: `${side}.${key}`, label: `${side === 'away' ? '客隊' : '主隊'}${label}`,
      ...
      metrics: metric(data, key.toLowerCase().includes('hitting') ? ['games', 'plateAppearances', 'runs', 'runsPerGame', 'ops'] : ['gamesPitched', 'inningsPitched', 'era', 'whip', 'fip']),
      sources: list(data.sourceReceipts).map(sourceReceipt), usage: usageFor(`${side}.${key}`),
    };
  }));
