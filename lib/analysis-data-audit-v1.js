...
function rowReceipt(context, profile, side, category, label, aliases) {
  ...
  const substitutions = unique([
    ...list(data.incompleteReasons), text(data.projectionMode), text(data.offensiveIndexMethod), ...(data.offensiveIndexBasis === 'TEAM_SEASON_OPS' ? ['打線指數比較基準：本隊本季OPS（非聯盟平均）'] : []), text(data.uncertaintyMethod), text(data.modelUsage), text(data.rejectedReason),
