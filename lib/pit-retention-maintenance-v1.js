...
export async function compactExpiredUnprotectedRepricePayloads() {
  if (!pitRetentionDatabaseConfigured()) throw new Error('PIT retention compaction requires DATABASE_URL');
  const sql = neon(durableDatabaseUrl());
  const before = await planPitRetentionMaintenance();
  await sql`
    DO $pit_retention_compaction$
    BEGIN
      PERFORM pg_advisory_xact_lock(hashtext('pit_retention_compaction_v1'));
      EXECUTE 'ALTER TABLE baseball_analysis_pit_snapshots DISABLE TRIGGER baseball_analysis_pit_immutable';
      WITH RECURSIVE bet_roots AS (
        SELECT DISTINCT COALESCE(
          NULLIF(payload->>'pitSnapshotId', ''),
          NULLIF(payload#>>'{analysis,pitSnapshotId}', '')
      ...
      UPDATE baseball_analysis_pit_snapshots pit
      SET
        frozen_context_payload = jsonb_build_object(
          'version', COALESCE(pit.frozen_context_payload->>'version', 'BASEBALL-PIT-JSON-PAYLOAD-v1.0.0'),
          'encoding', 'OMITTED_HASH_ONLY',
          'rawBytes', COALESCE((pit.frozen_context_payload->>'rawBytes')::bigint, 0),
          'compressedBytes', NULL,
          'base64Bytes', NULL,
          'payloadHash', pit.frozen_context_payload->>'payloadHash',
          'reason', 'RETENTION_COMPACTED_UNREFERENCED_EXPIRED_REPRICE'
        ),
        market_analysis_payload = jsonb_build_object(
          'version', COALESCE(pit.market_analysis_payload->>'version', 'BASEBALL-PIT-JSON-PAYLOAD-v1.0.0'),
          'encoding', 'OMITTED_HASH_ONLY',
          'rawBytes', COALESCE((pit.market_analysis_payload->>'rawBytes')::bigint, 0),
          'compressedBytes', NULL,
          'base64Bytes', NULL,
          'payloadHash', pit.market_analysis_payload->>'payloadHash',
          'reason', 'RETENTION_COMPACTED_UNREFERENCED_EXPIRED_REPRICE'
        )
      WHERE pit.analysis_type = 'PRICE_ONLY_REPRICE'
        AND pit.game_start < NOW()
        AND NOT EXISTS (
          SELECT 1 FROM protected_snapshots protected
          WHERE protected.snapshot_id = pit.snapshot_id
        )
        AND (
          pit.frozen_context_payload->>'encoding' IS DISTINCT FROM 'OMITTED_HASH_ONLY'
          OR pit.market_analysis_payload->>'encoding' IS DISTINCT FROM 'OMITTED_HASH_ONLY'
        );
      EXECUTE 'ALTER TABLE baseball_analysis_pit_snapshots ENABLE TRIGGER baseball_analysis_pit_immutable';
    EXCEPTION WHEN OTHERS THEN
      RAISE;
    END
    $pit_retention_compaction$
  `;
  const after = await planPitRetentionMaintenance();
  if (after.snapshots.protected !== before.snapshots.protected) {
    throw new Error('PIT retention protected-set count changed unexpectedly');
  }
  return {
    executedAt: new Date().toISOString(),
    compactedSnapshots: before.snapshots.eligible - after.snapshots.eligible,
    eligibleBytesBeforeCompaction: before.payloads.eligibleBytesBeforeCompaction,
    protectedSnapshots: after.snapshots.protected,
    before,
    after,
  };
}
