...
async function schema() {
  if (!ready) ready = sql().transaction([
    ...
    sql()`CREATE TABLE IF NOT EXISTS analysis_push_results (run_id TEXT PRIMARY KEY, encrypted TEXT NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`,
  ]).catch(error => { ready = null; throw error; });
  await ready;
}
// Publish the immutable result before delivering its completion notification.
// The workflow itself may still be waiting for push delivery/retries at this point.
export async function saveNotificationResult(runId, result) {
  await schema();
  const encrypted = seal(result);
  await sql()`INSERT INTO analysis_push_results (run_id, encrypted) VALUES (${runId}, ${encrypted}) ON CONFLICT (run_id) DO NOTHING`;
}
export async function getNotificationResult(runId) {
  if (!durableDatabaseConfigured()) return null;
  await schema();
  const rows = await sql()`SELECT encrypted FROM analysis_push_results WHERE run_id=${runId} LIMIT 1`;
  return rows[0] ? unseal(rows[0].encrypted) : null;
}
async function keys() {
  await schema();
