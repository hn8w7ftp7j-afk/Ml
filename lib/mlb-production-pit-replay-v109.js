...
function replayCompatibility(input, context) {
  ...
  else if (modelVersions.some(version => version !== MODEL_VERSION)) reasons.push('ORIGINAL_MODEL_ENGINE_UNAVAILABLE');
  if (!dataVersions.length) reasons.push('ORIGINAL_DATA_VERSION_MISSING');
  else if (dataVersions.some(version => version !== DATA_VERSION)) reasons.push('ORIGINAL_DATA_VERSION_UNAVAILABLE');
  if (ruleVersions.some(version => version !== RULES_VERSION)) reasons.push('ORIGINAL_RULES_VERSION_UNAVAILABLE');
  if (distributionEngine && distributionEngine !== DEFAULT_MODEL_CONFIG.engine) reasons.push('ORIGINAL_DISTRIBUTION_ENGINE_UNAVAILABLE');
  return {
    ...
    currentModelVersion: MODEL_VERSION, currentDataVersion: DATA_VERSION,
    currentRulesVersion: RULES_VERSION, currentDistributionEngine: DEFAULT_MODEL_CONFIG.engine,
    archivedEngineAvailable: false, immutableDistributionReplaySupported: false,
...
export function validateProductionPitSnapshotV109(input = {}) {
  ...
  const compatibility = replayCompatibility(input, context);
