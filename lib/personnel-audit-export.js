...
const LEGACY_COMPRESSED_LIMIT_BYTES = 2_000_000;
...
// Audit-only compatibility with the exact-byte JSON_BASE64/GZIP_BASE64 contract
...
export function decodePersonnelAuditPayload(envelope, currentDecode) {
  ...
  if (!['JSON_BASE64', 'GZIP_BASE64'].includes(envelope.encoding)) throw new Error('PIT快照內容編碼不支援');
  ...
  const compressed = envelope.encoding === 'GZIP_BASE64';
  if (compressed && (!Number.isSafeInteger(envelope.compressedBytes) || envelope.compressedBytes <= 0
    || envelope.compressedBytes > LEGACY_COMPRESSED_LIMIT_BYTES)) throw new Error('PIT快照壓縮大小宣告無效');
  ...
  if (encoded.length !== (compressed ? envelope.compressedBytes : envelope.rawBytes) || encoded.toString('base64') !== data) {
    throw new Error(compressed ? 'PIT快照Base64／gzip內容不一致' : 'PIT快照Base64／原始JSON內容不一致');
  }
  const raw = compressed ? gunzipSync(encoded, { maxOutputLength: LEGACY_RAW_LIMIT_BYTES + 1 }) : encoded;
  if (raw.length !== envelope.rawBytes) throw new Error('PIT快照內容雜湊或大小不一致');
...
export function personnelEnvelopeFailureDiagnostic(envelope, error, stage) {
  ...
  return { reason, stage: ['ROW_JSON', 'CONTEXT_ENVELOPE_JSON', 'CONTEXT_DECODE', 'CONTEXT_PROJECTION', 'ANALYSIS_ENVELOPE_JSON', 'ANALYSIS_DECODE', 'METADATA'].includes(stage) ? stage : 'UNKNOWN',
...
export function exportPersonnelAuditRow(row, decode) {
  ...
  try {
    ...
    let analysisEvidence = null, analysisEvidenceIntegrity = 'NOT_AVAILABLE', analysisEvidenceDiagnostic = null;
    if (row.market_analysis_payload != null) {
      let analysisEnvelope = null, analysisFailureStage = 'ANALYSIS_ENVELOPE_JSON';
      try {
        analysisEnvelope = jsonColumn(row.market_analysis_payload);
        analysisFailureStage = 'ANALYSIS_DECODE';
        ...
        analysisEvidenceIntegrity = 'FAILED_OR_UNAVAILABLE';
        analysisEvidenceDiagnostic = personnelEnvelopeFailureDiagnostic(analysisEnvelope, error, analysisFailureStage);
      }
    }
    return { snapshotId, ok: true, leagueId: row.league_id, externalGameId: String(row.external_game_id),
      ...
      omitted: projected.omitted, analysisEvidence, analysisEvidenceIntegrity, analysisEvidenceDiagnostic,
      analysisUsageStatus: analysisEvidence?.dataAudit || analysisEvidence?.dataUsage ? 'SAVED_DECLARATIONS_ONLY'
        : analysisEvidenceIntegrity === 'FAILED_OR_UNAVAILABLE' ? 'ANALYSIS_EVIDENCE_UNREADABLE' : 'NOT_SAVED',
