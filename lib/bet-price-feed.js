...
function sanitizePrice(row, fallback = {}) {
  ...
  return {
    ...
    metricStatus: String(row?.metricStatus || '').slice(0, 40) || null,
    formulaDiagnosticScore: optionalFinite(row?.formulaDiagnosticScore),
    shadowDiagnosticScore: optionalFinite(row?.shadowDiagnosticScore),
    weightedEV: optionalFinite(row?.weightedEV),
    robustEV: optionalFinite(row?.robustEV),
    scoreStatus: String(row?.scoreStatus || '').slice(0, 60) || null,
    modelVersion: String(row?.modelVersion || '').slice(0, 100) || null,
    scoreFormulaVersion: String(row?.scoreFormulaVersion || '').slice(0, 100) || null,
  };
}
...
export function verifiedClosingPriceForBet(bet, { now = Date.now() } = {}) {
  ...
  if (!Number.isFinite(lineAsOf) || !Number.isFinite(gameStart)
    || lineAsOf >= gameStart || Number(now) < gameStart) return null;
