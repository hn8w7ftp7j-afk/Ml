...
export function analyzeNbaShadow(games, { teamId, seasonType = 'regular', includeHistoricalDiagnostics = false } = {}) {
  ...
  try {
    for (const row of rows) {
      ...
      report.folds.push({ gameId: row.id, date: row.date, trainingRows: training.length, trainingThrough: training.at(-1).date, lambda: selected.lambda, selection: selected.basis,
        ...(includeHistoricalDiagnostics ? { historicalDiagnostics: { predictedOwn: p.own, predictedOpponent: p.opponent, actualOwn: row.own, actualOpponent: row.opponent, baselineOwn: row.baselineOwn, baselineOpponent: row.baselineOpponent } } : {}) });
    }
