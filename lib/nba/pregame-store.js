...
export async function saveNbaPregame(payload) {
  ...
  const rows = await sql()`INSERT INTO sports_nba_pregame_v1 (game_id, revision, captured_at, payload)
    ...
    ON CONFLICT DO NOTHING RETURNING revision, payload`;
  const saved = rows.length ? rows : await sql()`SELECT revision, payload FROM sports_nba_pregame_v1 WHERE game_id=${payload.gameId} AND revision=${revision}`;
  assertNba(saved.length === 1 && saved[0].payload.gameId === payload.gameId && hash({ ...saved[0].payload, capturedAt: undefined }) === revision, 'PREGAME_INVALID', '無法核對永久保存回條');
  validateNbaPregame(saved[0].payload);
  return { persisted: true, inserted: rows.length === 1, revision, capturedAt: saved[0].payload.capturedAt };
}
