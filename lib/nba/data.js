...
import { deriveNbaOnOff } from './on-off.js';
import { archivedNbaInjuryReport } from './injury-archive.js';
...
function validateSummary(raw, id) {
  ...
  const players = summaryPlayers(raw, game);
  ...
  for (const block of raw.boxscore?.teams || []) {
    ...
    assertNba(!teamIds.has(team.id), 'TEAM_IDENTITY_MISMATCH', '比賽統計球隊重複');
    teamIds.add(team.id);
    game[game.home.id === team.id ? 'home' : 'away'].statistics = normalizeStatistics(block.statistics);
  }
  const features = deriveNbaBoxscore(game, players);
  assertNba(features.status !== 'blocked', 'SCORE_INVALID', features.qa.issues.find(row => row.severity === 'BLOCK')?.message || 'Box score 完整性錯誤');
}
// An independently validated completed-game team box is sufficient for the
// existing team-only Shadow model. It does not validate or return player data.
function historicalTeamBox(raw, id) {
  assertNba(raw?.header?.id === id, 'GAME_IDENTITY_MISMATCH', '歷史球隊統計賽事 ID 不一致');
  assertNbaLeague(raw.header.league);
  const game = normalizeGame(raw.header);
  assertNba(game.completed && game.status === 'final', 'SCORE_INVALID', '僅接受已完賽球隊統計');
  const blocks = raw.boxscore?.teams;
  assertNba(Array.isArray(blocks) && blocks.length === 2, 'SCHEMA_INVALID', '需要兩隊完整統計');
  const seen = new Set();
  for (const block of blocks) {
    const team = normalizeTeam(block.team);
    assertNba([game.home.id, game.away.id].includes(team.id) && !seen.has(team.id), 'TEAM_IDENTITY_MISMATCH', '歷史統計球隊衝突');
    seen.add(team.id); game[game.home.id === team.id ? 'home' : 'away'].statistics = normalizeStatistics(block.statistics);
  }
  const box = deriveNbaBoxscore(game);
  assertNba(box.status === 'ready' && box.pace !== null, 'SCORE_INVALID', '歷史球隊比分、回合數或比賽長度無法驗證');
  return game;
}
...
export async function loadNbaData({ view = 'schedule', date, id, season, seasonType = 'regular' } = {}, options = {}) {
  ...
  try {
    assertNba(['schedule', 'teams', 'game', 'historical-team-box', 'team', 'player', 'injuries', 'history'].includes(view), 'VIEW_INVALID', '不支援的 NBA 資料頁籤');
    if (view === 'schedule') {
      ...
      let boards;
      try { boards = [await get(path, validateScoreboard)]; }
      catch (error) {
        if (error.code !== 'UPSTREAM_HTTP_400') throw error;
        // Some historical boards reject date ranges. Two UTC dates cover a
        // Taiwan calendar day; each response keeps its own URL/hash and QA.
        boards = await Promise.all([previous, selected].map(day => get(`scoreboard?dates=${day.replaceAll('-', '')}&limit=1000`, validateScoreboard)));
        const rejected = sources.find(s => s.url.endsWith(path) && s.errorCode === 'UPSTREAM_HTTP_400');
        if (rejected) rejected.recoveredBy = boards.map(b => b.source.url);
      }
      data.games = uniqueGames(boards.flatMap(({value,source}) => value.events.map(event => ({ ...normalizeGame(event, value.leagues[0].season), sourceUrl: source.url, retrievedAt: source.fetchedAt })))).filter(game => game.taipeiDate === selected);
    ...
    } else if (view === 'historical-team-box') {
      const gameId = sourceId(id, 'game');
      const {value,source} = await get(`summary?event=${gameId}`, raw => historicalTeamBox(raw, gameId));
      data.game = {...historicalTeamBox(value,gameId), sourceUrl:source.url,retrievedAt:source.fetchedAt,venue:value.gameInfo?.venue?.fullName||null};
      data.basketball = deriveNbaBoxscore(data.game);
      data.availability = {teamBoxscore:'independently_verified',players:'not_validated_or_used',lineups:'not_validated_or_used',pointInTimeFeatures:'unavailable'};
      issues.push(issue('TEAM_ONLY_RESEARCH_SCOPE','僅驗證已完賽球隊統計；球員、先發與賽前證據不納入。'));
    } else if (view === 'game') {
      const gameId = sourceId(id, 'game');
      ...
      data.injuryArchive = archivedNbaInjuryReport(data.game, data.players);
      ...
      data.basketball = deriveNbaBoxscore(data.game, data.players);
      // Optional descriptive reconstruction has its own fail-closed QA. A bad
      // event feed must not erase independently validated scores/box scores.
      data.onOff = deriveNbaOnOff(data.game, data.players, value.plays);
      data.onOff.source = { url: source.url, fetchedAt: source.fetchedAt, publishedAt: source.publishedAt, hash: source.hash };
      issues.push(...data.basketball.qa.issues.map(row => issue(row.code, row.message)));
      ...
      data.availability.basketballFeatures = data.basketball.status;
      data.availability.playerOnOff = data.onOff.status === 'ready' ? 'postgame_descriptive_reconstruction_not_official' : data.onOff.status;
      if (data.onOff.status !== 'ready') issues.push(issue('ON_OFF_RECONSTRUCTION_PENDING', 'On/Off 逐事件重建尚未通過獨立 QA；不影響已核對比分，亦未送入 Shadow 模型'));
      data.availability.usage = data.basketball.players.some(row => row.usageEstimate !== null) ? 'boxscore_estimate_not_official' : 'unavailable';
  ...
  const failedSource = sources.some(source => source.status !== 'ready' && !source.recoveredBy);
