...
async function json(url, fallback = null, ttl = 300000) {
  ...
  if (hit) return hit;
  try {
    const response = await fetch(url, { cache: 'no-store', headers: { 'User-Agent': 'MLB-Positive-EV/6.1' } });
...
function battingStatsForPlayer(player) {
...
async function bullpenUsage(teamId, gameDate) {
  ...
  if (!rows.length) return { usageAvailable: false, fatigueIndex: 0.2, highLeverageAvailability: 0.75, qualityFactor: 1, daily: [] };
  ...
  feeds.forEach((feed, index) => {
    ...
    for (const reliever of usage.relievers) {
      const previous = pitcherUse.get(reliever.id) || { name: reliever.name, weightedPitches: 0, appearances: 0, lastDayPitches: 0, qualityWeighted: 0, qualityWeight: 0, saves: 0, holds: 0 };
  ...
  if (!daily.length) return { usageAvailable: false, fatigueIndex: 0.2, highLeverageAvailability: 0.75, qualityFactor: 1, daily: [] };
  ...
  const qualityFactor = qualityRows.length
    ...
    : 1;
  return { usageAvailable: true, fatigueIndex, highLeverageAvailability, qualityFactor, daily, relievers };
}
...
export async function buildGameContext(game) {
  ...
  const [league, awayTeam, homeTeam, awayStarter, homeStarter, feed, weather, awayRest, homeRest, awayBullpen, homeBullpen, awayProjection, homeProjection] = await Promise.all([
  ...
  const awayOfficialLineup = parseLineup(feed, 'away');
  ...
  const home = { ...homeTeam, starter: homeStarter, lineup: homeLineup, rest: homeRest, bullpen: homeBullpen, injuryImpact: injuryImpactForLineup(homeTeam.injuries, homeLineup) };
  ...
  const featureProvenance = [
    ...
    feature('正式／預估打線', awayLineup.official && homeLineup.official ? '已確認' : '預估', awayLineup.official && homeLineup.official ? 'MLB live feed' : 'MLB recent game feeds weighted projection'),
...
function finalResultError(message, code = 'OFFICIAL_FINAL_RESULT_INVALID', status = 502) {
...
function officialIdentity(value) {
  const id = Number(value);
  return Number.isSafeInteger(id) && id > 0 ? id : null;
}

function officialIsoDate(value) {
...
export function normalizeMlbFinalResult(feed, requestedGamePk) {
  const requested = officialIdentity(requestedGamePk);
  if (!requested) throw finalResultError('缺少或無效的 MLB gamePk', 'INVALID_GAME_PK', 400);
  if (!feed || typeof feed !== 'object') throw finalResultError('MLB 官方 live feed 無法讀取', 'OFFICIAL_RESULT_UNAVAILABLE', 503);

  const officialGamePk = officialIdentity(feed?.gamePk || feed?.gameData?.game?.pk);
  if (!officialGamePk || officialGamePk !== requested) {
    throw finalResultError('MLB 官方賽果場次識別不一致', 'OFFICIAL_IDENTITY_MISMATCH', 409);
  }
  const officialDate = officialIsoDate(feed?.gameData?.datetime?.officialDate);
  const awayTeam = feed?.gameData?.teams?.away || {};
  const homeTeam = feed?.gameData?.teams?.home || {};
  const awayTeamId = officialIdentity(awayTeam?.id);
  const homeTeamId = officialIdentity(homeTeam?.id);
  const awayName = String(awayTeam?.name || '').trim();
  const homeName = String(homeTeam?.name || '').trim();
  const gameNumber = officialIdentity(feed?.gameData?.game?.gameNumber || 1);
  if (!officialDate || !awayTeamId || !homeTeamId || awayTeamId === homeTeamId || !awayName || !homeName || !gameNumber) {
    throw finalResultError('MLB 官方賽果缺少日期、雙重賽或球隊身分');
  }

  const state = String(feed?.gameData?.status?.abstractGameState || '').trim();
  const statusEnglish = String(feed?.gameData?.status?.detailedState || state).trim();
  const finalState = state.toUpperCase() === 'FINAL';
  const awayScore = officialScore(feed?.liveData?.linescore?.teams?.away?.runs);
  const homeScore = officialScore(feed?.liveData?.linescore?.teams?.home?.runs);
  if (finalState && (awayScore == null || homeScore == null)) {
    throw finalResultError('MLB 官方狀態為 Final 但終場比分不完整');
  }

  const inningRows = Array.isArray(feed?.liveData?.linescore?.innings) ? feed.liveData.linescore.innings : [];
  const scheduledInnings = officialIdentity(feed?.gameData?.game?.scheduledInnings || feed?.gameData?.scheduledInnings || 9) || 9;
  if (finalState && inningRows.length < scheduledInnings) {
    throw finalResultError(`MLB 異常完賽局數 ${inningRows.length}/${scheduledInnings}，禁止自動結算`);
  }
  if (finalState && awayScore === homeScore) throw finalResultError('MLB 正式終場不得為和局，禁止自動結算');

  const first5Rows = inningRows.slice(0, 5);
  const first5Complete = finalState
    && first5Rows.length === 5
    && first5Rows.every(inning => officialScore(inning?.away?.runs) != null && officialScore(inning?.home?.runs) != null);
  const awayFirst5 = first5Complete
    ? first5Rows.reduce((sum, inning) => sum + officialScore(inning.away.runs), 0)
    : null;
  const homeFirst5 = first5Complete
    ? first5Rows.reduce((sum, inning) => sum + officialScore(inning.home.runs), 0)
    : null;
  ...
  return {
    ...
    awayFirst5,
    homeFirst5,
    innings: inningRows.length || null,
    scheduledInnings,
    first5Complete,
    status: statusNameZh(statusEnglish),
    statusEnglish,
    provider: 'MLB_STATS_API_LIVE_FEED',
    providerRevision: String(feed?.metaData?.timeStamp || feed?.metaData?.wait || '').trim() || null,
    sourceRecord: `statsapi.mlb.com/api/v1.1/game/${officialGamePk}/feed/live`,
  };
}

export async function fetchFinalResult(gamePk) {
  return normalizeMlbFinalResult(await fetchFeed(gamePk), gamePk);
}
