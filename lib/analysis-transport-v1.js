...
export function compactAnalysisContext(context) {
  ...
  const side = value => ({
    starter: value?.starter ? {
      ...
      identityReady: value?.upstreamReadiness?.starterIdentity ?? value.starter.identityReady ?? null,
      identityConfirmed: value.starter.identityConfirmed ?? null,
      identityMismatch: value.starter.identityMismatch ?? null,
      confirmed: value.starter.confirmed ?? null,
      projected: value.starter.projected ?? null,
      status: value.starter.status || null,
      projectionMode: value.starter.projectionMode || null,
    } : null,
  ...
  return {
    ...
    game: {
      ...
      awayTeamId: context?.game?.awayTeamId ?? null,
      homeTeamId: context?.game?.homeTeamId ?? null,
    },
