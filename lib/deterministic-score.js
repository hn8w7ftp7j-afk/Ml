...
export const SCORE_FORMULA_VERSION = 'DUAL-EV-BOTTLENECK-2026-08-v1.9.0';
export const SCORE_POLICY_VERSION = 'BASEBALL-FIXED-DUAL-EV-SCORE-2026-08';
...
export function deterministicScore(input = {}) {
  ...
  const caps = [];
  const common = { scoreType: actualWater ? 'FORMAL' : 'REFERENCE_SCREENING', formulaVersion: SCORE_FORMULA_VERSION, policyVersion: SCORE_POLICY_VERSION, weightedEV: toNumber(weightedEV), robustEV: toNumber(robustEV), rawMarketProbabilityGap: toNumber(rawMarketProbabilityGap) };

  if (!qaPassed) return { ...common, score: null, rawScore: null, displayedScore: null, band: 'BLOCKED', label: '⛔ QA未通過｜不評分｜不下注', eligible: false, caps: ['QA_BLOCK'], progress: null, highScoreAnomaly: false };

  // Locked policy: a non-positive central EV is PASS 6.6.  Do not manufacture
  // a second 1.x-6.x scale from the magnitude of negative EV.
  if (weightedEV.lte(0)) return { ...common, score: 6.6, rawScore: 6.6, displayedScore: 6.6, band: 'PASS_6.6', label: 'PASS', eligible: false, caps: ['WEIGHTED_EV_NON_POSITIVE'], progress: null, highScoreAnomaly: false };

  // Locked policy: W>0/R<=0 is observation 7.1 and can never rank.
  if (robustEV.lte(0)) return { ...common, score: 7.1, rawScore: 7.1, displayedScore: 7.1, band: 'OBSERVATION_7.1', label: '觀察', eligible: false, caps: ['ROBUST_EV_NON_POSITIVE'], progress: null, highScoreAnomaly: false };
  ...
  return {
    ...common, score, rawScore: toNumber(rawScore), displayedScore: score, band: band.id,
    label: score >= 8.5 ? '最強主推' : score >= 8.0 ? '主推' : score >= 7.5 ? '正常下注' : '小注候選',
...
export function scoreBoundaryAudit(result, input = {}) {
  ...
  if (weighted.lte(0) && score !== 6.6) errors.push('W≤0必須固定PASS 6.6');
