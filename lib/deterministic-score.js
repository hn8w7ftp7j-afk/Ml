...
export const SCORE_POLICY_VERSION = 'FOUR-LEAGUE-CONTINUOUS-DUAL-EV-SCORE-2026-08-v11';
...
export function deterministicScore(input = {}) {
  ...
  if (!qaPassed) return { score: null, rawScore: null, displayedScore: null, band: 'BLOCKED', label: '⛔ QA未通過｜不評分｜不下注', eligible: false, formulaEligible: false, formalEligible: false, scoreType: actualWater ? 'SHADOW_DIAGNOSTIC' : 'REFERENCE_SCREENING', formulaVersion: SCORE_FORMULA_VERSION, policyVersion: SCORE_POLICY_VERSION, weightedEV: toNumber(weightedEV), robustEV: toNumber(robustEV), rawMarketProbabilityGap: toNumber(rawMarketProbabilityGap), caps: ['QA_BLOCK'], progress: null, highScoreAnomaly: false };

  if (weightedEV.lte(0)) {
    const score = toNumber(nonPositiveScore(weightedEV, robustEV));
    return { score, rawScore: score, displayedScore: score, band: 'PASS_1.0-6.6', label: 'PASS', eligible: false, formulaEligible: false, formalEligible: false, scoreType: actualWater ? 'SHADOW_DIAGNOSTIC' : 'REFERENCE_SCREENING', formulaVersion: SCORE_FORMULA_VERSION, policyVersion: SCORE_POLICY_VERSION, weightedEV: toNumber(weightedEV), robustEV: toNumber(robustEV), rawMarketProbabilityGap: toNumber(rawMarketProbabilityGap), caps: ['WEIGHTED_EV_NON_POSITIVE'], progress: null, highScoreAnomaly: false };
  }

  if (robustEV.lte(0)) {
    const score = toNumber(observationScore(weightedEV, robustEV));
    return { score, rawScore: score, displayedScore: score, band: 'OBSERVATION_6.7-7.1', label: '觀察', eligible: false, formulaEligible: false, formalEligible: false, scoreType: actualWater ? 'SHADOW_DIAGNOSTIC' : 'REFERENCE_SCREENING', formulaVersion: SCORE_FORMULA_VERSION, policyVersion: SCORE_POLICY_VERSION, weightedEV: toNumber(weightedEV), robustEV: toNumber(robustEV), rawMarketProbabilityGap: toNumber(rawMarketProbabilityGap), caps: ['ROBUST_EV_NON_POSITIVE'], progress: null, highScoreAnomaly: false };
  }
  ...
  // The 8.5 band is reserved for a model edge that also survives two
  // independent quotes of the same contract.  The audit can cap a score, but
  // it never rewrites the upstream baseball probability or either EV value.
  if (published.gte('8.5') && input.crossMarketVerified !== true) {
  ...
  const formulaEligible = actualWater && executable && score >= 7.2;
  ...
  return {
    ...
    label: score >= 8.5 ? '8.5級影子候選' : score >= 8.0 ? '8.0級影子候選' : score >= 7.5 ? '7.5級影子候選' : score >= 7.2 ? '7.2級影子候選' : score >= 6.7 ? '觀察' : 'PASS',
    eligible: false, formulaEligible, formalEligible: false, scoreType: actualWater ? 'SHADOW_DIAGNOSTIC' : 'REFERENCE_SCREENING', formulaVersion: SCORE_FORMULA_VERSION, policyVersion: SCORE_POLICY_VERSION,
