export const MLB_RUN_MODEL_V103_VERSION = 'MLB-CORRELATED-RISK-RUN-PROFILE-2026-08-v11.0.4';
...
export function standardFipEvidence(block = {}, league = {}) {
  ...
  const raw = (13 * hr9 + 3 * bb9 - 2 * k9) / 9 + constant;
  return { version: 'MODEL-FIP-TRACE-v1',
    formula: 'clamp((13 * hr9 + 3 * bb9 - 2 * k9) / 9 + constant, 2, 7.5)',
    constantFormula: 'leagueEra - (13 * leagueHR9 + 3 * leagueBB9 - 2 * leagueK9) / 9',
    hbpUsed: false, actualRates: { k9, bb9, hr9 },
    leagueInputs: { leagueEra, leagueK9, leagueBB9, leagueHR9 }, constant,
    suppliedRates: { kPer9: block.kPer9 ?? null, bbPer9: block.bbPer9 ?? null, hrPer9: block.hrPer9 ?? null },
    rateBounds: { k9: [3, 16], bb9: [0.5, 8], hr9: [0.1, 4] },
    raw, bounds: [2, 7.5], value: clamp(raw, 2.0, 7.5),
    reportedFip: block.fip ?? null, reportedFipSource: block.fipSource ?? null,
    startDate: block.startDate ?? null, endDate: block.endDate ?? block.asOf ?? null,
    starterOnly: block.starterOnly ?? null,
    note: '實際模型率值含原有預設與截斷；人員回報 FIP 是不同欄位，不宣稱為官方原值。',
  };
}
...
function pitchingProfile(block = {}, league = {}, { starter = false } = {}) {
  ...
  const rejectedTeamStarterProxy = starter && (
    block?.projectedFromTeamPitching === true
    || block?.teamPitchingProxyRejected === true
    || block?.individualPitcherStatsAvailable === false
  );
  if (rejectedTeamStarterProxy) {
    return {
      factor: 1,
      uncertainty: 0.165,
      reliability: 0,
      ...
      inputs: { era: null, whip: null, innings: 0 },
  ...
  const fip = standardFipEvidence(block, league).value;
...
function bullpenProfile(team = {}, league = {}) {
  ...
  const relief = team?.bullpen || {};
  const pureReliefAvailable = relief?.pureRelief === true
    && finite(relief?.qualityFactor, 0) > 0
    && String(relief?.status || '').toUpperCase() !== 'MISSING';
  const reliefReliability = String(relief?.status || '').toUpperCase() === 'CONFIRMED' ? 0.60 : 0.35;
  const factor = pureReliefAvailable
    ? clamp(logShrinkFactor(relief.qualityFactor, reliefReliability, 0.085), 0.918, 1.089)
    : 1;
  const uncertainty = pureReliefAvailable
    ? statusUncertainty(relief.status, 0.055, 0.085, 0.15)
    : 0.15;
  return {
    factor,
    uncertainty,
    status: pureReliefAvailable ? relief.status : 'MISSING',
    proxy: pureReliefAvailable ? 'RELIEF_ONLY_PIT' : 'TEAM_PITCHING_AUDIT_ONLY_NEUTRAL',
    season,
    recent,
  };
}
...
function expectedStarterInnings(block = {}) {
  if (block?.projectedFromTeamPitching === true || block?.teamPitchingProxyRejected === true || block?.individualPitcherStatsAvailable === false) {
    return clamp(finite(block?.expectedInnings, 4.8), 4.5, 5.0);
  }
  const supplied = finite(block?.expectedInnings, null);
  if (supplied != null) return clamp(supplied, 1.0, 7.2);
  const innings = Math.max(0, finite(block?.inningsPitched, 0));
...
export function estimateRunProfileV103(context = {}) {
  ...
  return {
    ...
    components: {
      ...
      homeAdvantage,
      bullpenProxy: awayBullpen.proxy === 'RELIEF_ONLY_PIT' && homeBullpen.proxy === 'RELIEF_ONLY_PIT'
        ? 'RELIEF_ONLY_PIT'
        : 'TEAM_PITCHING_AUDIT_ONLY_NEUTRAL',
      injuryMeanEffect: 1,
    },
