export const MLB_RUN_MODEL_V103_VERSION = 'MLB-CONSERVATIVE-DECORELATED-RUN-PROFILE-2026-08-v10.3.0';
...
function environmentProfile(context = {}) {
  ...
  return {
    ...
    uncertainty: clamp(
      Math.sqrt(
        statusUncertainty(context?.park?.factorStatus, 0.018, 0.035, 0.060) ** 2
          + statusUncertainty(context?.weather?.status, 0.018, 0.042, 0.070) ** 2,
      ),
      0.025,
      0.105,
    ),
...
export function estimateRunProfileV103(context = {}) {
  ...
  const awayDataSigma = Math.sqrt(
    awayOffense.uncertainty ** 2
      + homeStarter.uncertainty ** 2
      + (homeBullpen.uncertainty * 0.55) ** 2
      + lineupSigma ** 2
      + injuryUncertainty(away) ** 2,
  );
  const homeDataSigma = Math.sqrt(
    homeOffense.uncertainty ** 2
      + awayStarter.uncertainty ** 2
      + (awayBullpen.uncertainty * 0.55) ** 2
      + lineupSigma ** 2
      + injuryUncertainty(home) ** 2,
  );
  ...
  return {
    ...
    uncertainty: {
      away: clamp(awayDataSigma, 0.060, 0.180),
      home: clamp(homeDataSigma, 0.060, 0.180),
