...
export const MARKET_VERIFICATION_V2_VERSION = 'INDEPENDENT-PAYOFF-VECTOR-CONSENSUS-2026-08-v2.3.0';
...
function buildPayoffEvidence(actual, references, toleranceMs, verifiedAtMs) {
  ...
  const centreIndex = [...vectors.keys()].sort((a, b) => vectors[a].effectiveWinProbability - vectors[b].effectiveWinProbability)[Math.floor(vectors.length / 2)];
  const centre = vectors[centreIndex];
  return {
    attempted: true,
    eligible: true,
    candidates,
    ...
    vector: {
      ...
      effectiveWinProbability: centre.effectiveWinProbability,
      settlementRate: centre.settlementRate,
    },
