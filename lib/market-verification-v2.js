...
function buildPayoffEvidence(actual, references, toleranceMs, verifiedAtMs) {
  ...
  if (!candidates.length) {
    return { attempted: true, eligible: false, reasonCode: 'NO_ELIGIBLE_PERIOD_PRICE', reason: '缺少5分鐘內獨立國際市場同賽事同期間價格', candidates: [] };
  }

  const contract = requiredReferencesForContract(actual, candidates);
  if (!contract.ok) {
    return { attempted: true, eligible: false, reasonCode: 'REQUIRED_CONTRACT_MISSING', reason: contract.reason, candidates, required: contract.required || [] };
  }
  const eventIds = [...new Set(contract.required.map(row => clean(row?.providerEventId)).filter(Boolean))];
  if (eventIds.length !== 1) {
    return { attempted: true, eligible: false, reasonCode: 'EVENT_ID_MISMATCH', reason: '相鄰盤不是同一場賽事，禁止合併機率', candidates, required: contract.required };
  }
  if (contract.required.some(row => row?.referenceEvidenceEligible !== true)) {
    return { attempted: true, eligible: false, reasonCode: 'ADJACENT_PRICE_INELIGIBLE', reason: '相鄰半分盤未通過三莊鮮度或分散安全門檻', candidates, required: contract.required };
  }
  ...
  if (keys.length < MINIMUM_CONSENSUS_BOOKS) {
    return {
      ...
      reasonCode: 'INSUFFICIENT_COMMON_BOOKS',
      candidates,
  ...
  if (monotonicFailure || vectors.length < MINIMUM_CONSENSUS_BOOKS) {
    return {
      ...
      reasonCode: monotonicFailure ? 'NON_MONOTONIC_PROBABILITY' : 'INSUFFICIENT_VALID_BOOKS',
      candidates,
      required: contract.required,
      keys,
    };
  ...
  if (freshnessMaxMs < -MAX_REFERENCE_QUOTE_FUTURE_SKEW_MS
    ...
    return { attempted: true, eligible: false,
      reasonCode: freshnessMaxMs < -MAX_REFERENCE_QUOTE_FUTURE_SKEW_MS || futureSkewMs > MAX_REFERENCE_QUOTE_FUTURE_SKEW_MS
        ? 'FUTURE_QUOTE' : freshnessMaxMs > MAX_REFERENCE_QUOTE_AGE_MS ? 'STALE_OVER_300S' : 'QUOTE_SPAN_EXCEEDED',
      reason: '獨立市場報價超過5分鐘、來自未來，或完整相鄰盤時間差超過3分鐘', candidates, required: contract.required, keys };
  }
  ...
  if (spread > MAX_REFERENCE_PROBABILITY_SPREAD || mad > MAX_REFERENCE_PROBABILITY_MAD) {
    return { attempted: true, eligible: false, reasonCode: 'DISPERSION_EXCEEDED', reason: '逐莊勝／走／負推導分散超過安全門檻', candidates, required: contract.required, keys };
  }
...
function payoffVerification(row, actualSource, contractKey, evidence) {
  ...
  return {
    ...
    rejectionCode: eligible ? null : evidence.reasonCode || 'PAYOFF_VECTOR_INELIGIBLE',
    verified: eligible,
                  ...
                  : !dispersionValid ? '獨立市場去水機率分散超過安全門檻'
                    : !pairMarginValid ? '獨立市場正反價格水位結構異常'
                      : probabilityComplementError == null || probabilityComplementError > 1e-6 ? '獨立市場正反方向中心機率未互補'
                        : '獨立市場先驗不可用';
    const rejectionCode = priorEligible ? null : !isBinaryNoPush(row) ? 'NON_BINARY_CONTRACT'
      : !match || providerGroup(match) !== 'THE_ODDS_API' ? 'NO_ELIGIBLE_CONTRACT'
        : !opposite ? 'OPPOSITE_MISSING'
          : consensusBookCount < MINIMUM_CONSENSUS_BOOKS || oppositeBookCount < MINIMUM_CONSENSUS_BOOKS ? 'INSUFFICIENT_BOOKS'
            : !sameBookSnapshot || !sameSnapshot ? 'CONSENSUS_SNAPSHOT_MISMATCH'
              : !timestampEvidenceValid ? 'QUOTE_TIME_INVALID' : !dispersionValid ? 'DISPERSION_EXCEEDED'
                : !pairMarginValid ? 'PAIR_MARGIN_INVALID'
                  : probabilityComplementError == null || probabilityComplementError > 1e-6 ? 'PROBABILITY_NOT_COMPLEMENTARY' : 'CONSENSUS_INELIGIBLE';
