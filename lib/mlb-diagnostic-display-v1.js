...
export function lineupCoverageDisplay(row = {}) {
  ...
  const expected = number(coverage.expectedCount);
  const modelCoverage = number(coverage.modelMetricCoverage);
  const model = modelCoverage == null || expected == null ? '未提供' : `${Math.round(modelCoverage * expected)}/${expected}`;
  const reported = (row.reportedBattingRates || []).filter(player => player.statistics != null).length;
  return `名單 ${coverage.identityCount ?? '未知'}/${expected ?? '未知'} 人；系統回報模型用指標覆蓋 ${model}；${reported ? `逐人打擊率明細已列 ${reported}/${expected ?? '未知'} 人` : '本份明細未提供逐人打擊率'}。覆蓋判定與來源仍須核對。`;
}

export function sourceStatusLabel(key, value, audit, explanation) {
  if (key === 'catcherFraming' && explanation?.calculation?.components?.advanced) {
    const advanced = explanation.calculation.components.advanced;
    return `捕手Framing摘要原值：${value}｜${['away', 'home'].map(side => {
      const framing = advanced[side]?.components?.framing;
      return `${side === 'away' ? '客' : '主'}計算輸入：${framing?.state?.status ?? 'UNKNOWN'}／可用：${framing?.state?.usable === true ? '是' : framing?.state?.usable === false ? '否' : '未知'}`;
    }).join('｜')}（各層語意與來源須分別核對）`;
  }
  if (key === 'starterExpectedInnings' && audit?.rows?.some(row => row.category === 'starter' && row.inningsEstimate)) {
    return ['away', 'home'].map(side => {
      const input = audit.rows.find(row => row.key === `${side}.starter`)?.inningsEstimate;
      return `${side === 'away' ? '客' : '主'}局數歷史輸入：${input?.historicalInputStatus ?? 'UNKNOWN'}／預估局數：${input?.estimateStatus ?? 'UNKNOWN'}`;
    }).join('｜');
  }
...
export function starterInningsDisplay(starter = {}) {
  ...
  return {
    ...
    source: starter.expectedInningsSource || starter.source || null,
    expectedInningsSource: starter.expectedInningsSource || null,
    evidence: starter.expectedInningsEvidence || null,
    sampleGames: number(starter.expectedInningsSampleGames),
...
export function externalVerificationExplanation(verification = {}, fresh = false) {
  ...
  if (verification.acquisition) {
    const acquisition = verification.acquisition;
    const attempts = acquisition.attempts || [];
    const causes = attempts.map(row => `${row.provider}／${row.stage}：${row.reasonCode}`).join('；');
    return `取得證據：${acquisition.status}｜${causes || acquisition.reasonCode || 'UNKNOWN'}。配對判定：${verification.rejectionCode || '未保存代碼'}／${verification.priorIneligibleReason || verification.policyStatus || '未保存'}。取得、配對與報價時效分開；不改 W/R。`;
  }
