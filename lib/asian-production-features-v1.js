import { parseCpblBatterRates, matchCpblBatterRates, cpblRatesEligible } from './cpbl-batter-rates-v1.js';
...
import { parseKboOfficialTbf, parseKboOfficialStarts, kboTbfAcquisitionEligible, kboInningsOuts } from './kbo-official-tbf.js';
import { parseKboOfficialLineup } from './kbo-official-lineup.js';

export const ASIAN_PRODUCTION_FEATURES_V1_VERSION = 'ASIAN-OFFICIAL-PIT-PLAYER-PIPELINES-2026-09-v1.0.9';
...
const NPB_TEAM_OFFICIAL_NAME = Object.freeze({
  YOM: '読売ジャイアンツ', HAN: '阪神タイガース', YDB: '横浜DeNAベイスターズ', HIR: '広島東洋カープ',
  YAK: '東京ヤクルトスワローズ', CHU: '中日ドラゴンズ', SOF: '福岡ソフトバンクホークス',
  NIP: '北海道日本ハムファイターズ', LOM: '千葉ロッテマリーンズ', RAK: '東北楽天ゴールデンイーグルス',
  ORI: 'オリックス・バファローズ', SEI: '埼玉西武ライオンズ',
});
...
function npbPlayerSourceScope(html, event, game, side, kind) {
  const code = game?.[`${side}Code`], year = clean(game?.officialDate).slice(0, 4);
  const suffix = NPB_TEAM_SUFFIX[code], team = NPB_TEAM_OFFICIAL_NAME[code];
  if (!suffix || !team || !/^\d{4}$/.test(year) || !event?.success) return false;
  const url = kind === 'pitching' ? `https://npb.jp/bis/${year}/stats/idp1_${suffix}.html`
    : `https://npb.jp/bis/teams/rst_${suffix}.html`;
  if (event.url !== url) return false;
  const $ = cheerio.load(String(html || ''));
  const title = npbHeaderKey($('title').text());
  const expected = kind === 'pitching' ? `${year}年度 ${team} 個人投手成績` : `${team} ${year}年度 選手一覧`;
  return title.startsWith(npbHeaderKey(expected));
}

export function resolveNpbStarterPlayerId({ game, side, identity, statsHtml, statsEvent, rosterHtml, rosterEvent }) {
  if (clean(identity?.id) || !identity?.name
    || !npbPlayerSourceScope(statsHtml, statsEvent, game, side, 'pitching')) return null;
  const stats = matchNpbStarterStats(parseNpbPitchingStatsHtml(statsHtml), identity);
  if (!stats) return null;
  // Match the full individual name resolved uniquely from the team statistics.
  // A schedule surname is never matched directly to the entire league roster.
  let id = /^(?=.*[1-9])\d+$/.test(clean(stats.id)) ? clean(stats.id) : null;
  let idEvent = statsEvent, sourcePath = 'table tr:matched-player a[href*="/players/"]';
  if (!id) {
    if (!npbPlayerSourceScope(rosterHtml, rosterEvent, game, side, 'roster')) return null;
    const $ = cheerio.load(String(rosterHtml || ''));
    const matches = $('tr.rosterPlayer td.rosterRegister a[href*="/players/"]').toArray()
      .filter(link => npbNameKey($(link).text()) === npbNameKey(stats.name));
    if (matches.length !== 1) return null;
    id = clean($(matches[0]).attr('href')).match(/^\/bis\/players\/(\d+)\.html$/)?.[1] || null;
    idEvent = rosterEvent; sourcePath = 'tr.rosterPlayer td.rosterRegister a';
  }
  if (!id || !/^(?=.*[1-9])\d+$/.test(id)) return null;
  return { ...identity, id, playerIdentityVerified: true,
    identityResolutionEvidence: { method: 'UNIQUE_TEAM_SEASON_PITCHER_FULL_NAME_AND_OFFICIAL_PLAYER_LINK',
      teamCode: game[`${side}Code`], season: Number(clean(game.officialDate).slice(0, 4)),
      fullName: stats.name, sourceEventId: idEvent.id, contentHash: idEvent.contentHash,
      fetchedAt: idEvent.fetchedAt, sourcePath, statsSourceEventId: statsEvent.id,
      statsContentHash: statsEvent.contentHash, statsFetchedAt: statsEvent.fetchedAt, publishedAt: null },
  };
}
...
function npbBattingTable($, table) {
  ...
  return $(table).find('tbody tr, tr.gmstats').map((_, row) => {
    ...
    // gmbatter is an original slot; gmnxtbatter is a substitute. A pitcher
    // batting eighth/ninth remains part of the nine starting slots.
    order += 1;
    if (!name || seen.has(compactName(name))) return null;
    seen.add(compactName(name));
    return { id: compactName(name), name, order, position: clean(text.split(',').slice(1).join(',')) };
...
export function parseNpbGameDetailHtml(html) {
  ...
  const teamCodes = $('#gmdivtbl td.flagteam2 img').map((_, img) => {
    const suffix = clean($(img).attr('src')).match(/flag\d+_([a-z]+)_/i)?.[1]?.toLowerCase();
    return Object.entries(NPB_TEAM_SUFFIX).find(([, value]) => value === suffix)?.[0] || '';
  }).get();
  return {
    venue,
    away: { teamCode: teamCodes[0] || null, lineup: batting[0] ? npbBattingTable($, batting[0]) : [], pitchers: pitching[0] ? npbPitchingTable($, pitching[0]) : [] },
    home: { teamCode: teamCodes[1] || null, lineup: batting[1] ? npbBattingTable($, batting[1]) : [], pitchers: pitching[1] ? npbPitchingTable($, pitching[1]) : [] },
  };
}
...
export function parseKboGameListPayload(payload, providerGameId = '', awayCode = '', homeCode = '') {
  ...
  const rows = Array.isArray(payload?.game) ? payload.game : [];
  const teamsMatch = row => (!expectedAway || clean(row?.AWAY_ID) === expectedAway)
    && (!expectedHome || clean(row?.HOME_ID) === expectedHome);
  if (clean(providerGameId)) {
    const exact = rows.filter(row => clean(row?.G_ID) === clean(providerGameId));
    return exact.length === 1 && teamsMatch(exact[0]) ? exact[0] : null;
  }
  const matches = rows.filter(row => expectedAway && expectedHome && teamsMatch(row));
...
export function parseKboStarterAnalysisPayload(payload) {
  return (Array.isArray(payload?.rows) ? payload.rows : []).map((row, index) => {
    ...
    return {
      ...
      gamesStarted: null,
      appearances: null,
      reportedGameCount: appearances,
      gamesStartedAcquisitionStatus: 'UNVERIFIED_GAMECENTER_COUNT_IS_NOT_GS',
...
export function parseKboBoxScorePayload(payload) {
  ...
  sides.forEach((side, index) => {
    ...
    const headers = pitcherTable?.headers?.[0]?.row?.map(cell => clean(cell?.Text)) || [];
    output[side].pitchers = (Array.isArray(pitcherTable?.rows) ? pitcherTable.rows : []).map(row => {
      const values = (Array.isArray(row?.row) ? row.row : []).map(cell => clean(cell?.Text));
      const outs = kboInningsOuts(values[6]?.replace(/^(\d+)\.([12])$/, '$1 $2/3'));
      const starter = values[1] === '선발';
      const relief = /^[1-9]\d*\.[1-9]$/.test(values[1]);
      const pitches = headers[8] === '투구수' && /^\d+$/.test(values[8]) ? Number(values[8]) : null;
      return {
        name: values[0], starter: starter ? true : relief ? false : null, roleVerified: starter || relief,
        inningsPitched: outs === null ? null : outs / 3, inningsOuts: outs,
        pitchCount: Number.isSafeInteger(pitches) ? pitches : null,
        battersFaced: number(values[7]),
        hits: number(values[10]), walks: null, walksAndHitByPitch: number(values[12]), strikeouts: number(values[13]),
...
export function parseCpblGameDetailPayload(payload) {
  ...
  const parseSide = block => {
    const lineup = (Array.isArray(block?.Hitters) ? block.Hitters : [])
      .filter(row => number(row?.Lineup) >= 1 && number(row?.Lineup) <= 9
        && !/^[（(]/.test(clean(row?.DefendStation)))
    ...
    const pitchers = (Array.isArray(block?.Pitchers) ? block.Pitchers : []).map(row => ({
      ...
      pitchCount: number(row?.PitchCnt),
  ...
  return {
    providerGameId: clean(root?.GameId), status: clean(root?.GameStatus),
    officialStart: clean(root?.PreExeDate),
...
export function currentPersonnelAcquisitionEligible(game, observedAt) {
  const observed = Date.parse(observedAt || '');
  const firstPitch = Date.parse(game?.gameDate || '');
  return Number.isFinite(observed) && Number.isFinite(firstPitch) && observed < firstPitch
    && !/^(F|I|LIVE|FINAL|FINISHED|START|POSTPONED|CANCELLED)$/i.test(clean(game?.statusCode));
}

function validStartingNine(players) {
  return players.length === 9 && players.every(row => clean(row.name) && Number.isInteger(row.order)
    && row.order >= 1 && row.order <= 9)
    && new Set(players.map(row => row.order)).size === 9
    && new Set(players.map(row => row.officialPlayerId || compactName(row.name))).size === 9;
}

function currentLineupSnapshot(players, game, side, league, evidence, battingStats = [], battingStatsObservedAt = null) {
  if (!validStartingNine(players)) return null;
  const snapshot = projectedLineup([{ game, detail: { [side]: { lineup: players } } }],
    game[`${side}TeamId`], league, battingStats, battingStatsObservedAt);
  return { ...snapshot, official: true, projected: false, rosterConfirmedToday: true,
    battingOrderStatus: 'OFFICIAL_CURRENT_GAME_ORDER',
    assignmentStatus: 'OFFICIAL_CURRENT_GAME_LINEUP',
    source: `${league}_OFFICIAL_CURRENT_GAME_LINEUP_PIT`, sourceEvidence: evidence,
    observedAt: evidence.fetchedAt, historicalLineupGameDate: null,
    incompleteReasons: snapshot.incompleteReasons.filter(reason => reason !== 'CURRENT_GAME_STARTING_LINEUP_NOT_CONFIRMED'),
  };
}

export function parseCpblCurrentPersonnel(payload, game, event) {
  const root = payload?.Data?.Game || payload?.data?.game || payload?.Data || payload?.data;
  if (!event?.success || !currentPersonnelAcquisitionEligible(game, event.fetchedAt)
    || clean(root?.GameId) !== clean(game.providerGameId) || root?.GameStatus !== 'SCHEDULED') return null;
  const detail = parseCpblGameDetailPayload(payload);
  const result = { lineups: {}, starters: {} };
  for (const [side, key] of [['away', 'Visiting'], ['home', 'Home']]) {
    const block = root?.[key];
    if (!CPBL_TEAM_ID[game[`${side}Code`]] || block?.Team?.Code !== CPBL_TEAM_ID[game[`${side}Code`]]) return null;
    // A stale SCHEDULED flag cannot legitimize rows already carrying play data.
    if ((block.Hitters || []).some(row => number(row.PlateAppearances) > 0)
      || (block.Pitchers || []).some(row => number(row.PitchCnt) > 0 || number(row.PlateAppearances) > 0)
      || (block.InningScore || []).some(row => clean(row.Score))) return null;
    const path = payload?.Data?.Game ? '$.Data.Game' : payload?.data?.game ? '$.data.game' : payload?.Data ? '$.Data' : '$.data';
    const evidence = { sourceEventId: event.id, contentHash: event.contentHash,
      fetchedAt: event.fetchedAt, sourcePath: `${path}.${key}.Hitters`,
      identitySourcePath: `${path}.${key}.Team.Code`, flagSourcePath: `${path}.GameStatus`,
      publishedAt: null, pregameReceiptVerified: true };
    const startingRows = (block.Hitters || []).filter(row => number(row.Lineup) >= 1 && number(row.Lineup) <= 9);
    if (startingRows.length === 9 && detail[side].lineup.every(row => row.officialPlayerId)) {
      result.lineups[side] = currentLineupSnapshot(detail[side].lineup, game, side, 'CPBL', evidence);
    }
    const starters = detail[side].pitchers.filter(row => row.starter && row.id && row.name);
    if (starters.length === 1) result.starters[side] = { ...starters[0],
      source: 'CPBL_OFFICIAL_CURRENT_GAME_STARTER', officialAssignmentConfirmed: true,
      sourceEvidence: { ...evidence, sourcePath: `${path}.${key}.Pitchers` } };
  }
  return result;
}

export function parseNpbCurrentLineups(html, game, event, battingStats = {}, battingStatsObservedAt = {}) {
  if (!event?.success || !currentPersonnelAcquisitionEligible(game, event.fetchedAt)) return null;
  const $ = cheerio.load(String(html || ''));
  const suffix = `${NPB_TEAM_SUFFIX[game.homeCode]}-${NPB_TEAM_SUFFIX[game.awayCode]}`;
  const prefix = `https://npb.jp/scores/${clean(game.officialDate).slice(0, 4)}/${clean(game.officialDate).slice(5).replace('-', '')}/${suffix}-`;
  const canonical = clean($('link[rel="canonical"]').attr('href'));
  if (!canonical.startsWith(prefix) || canonical !== event.url || !/^\d+\/$/.test(canonical.slice(prefix.length))
    || !/試合(?:開始)?前|開始予定/.test(clean($('.line-score .game_info').text()))
    || /終了|中止|延期/.test(clean($('.line-score .game_info').text()))) return null;
  // Current order changes after substitutions. Never accept any inning data.
  if ($('#tablefix_ls tbody tr td').toArray().some(cell => /^[0-9xX]+$/.test(clean($(cell).text())))) return null;
  const output = {};
  for (const [side, rowClass, half] of [['away', 'top', 'half_left'], ['home', 'bottom', 'half_right']]) {
    const marker = $(`#tablefix_ls tr.${rowClass} th [class]`).first().attr('class') || '';
    if (!marker.split(/\s+/).includes(`flag_${NPB_TEAM_SUFFIX[game[`${side}Code`]]}_${clean(game.officialDate).slice(0, 4)}`)) return null;
    const players = $(`#player-order .${half} table tr`).map((_, row) => {
      const th = $(row).children('th');
      const order = number(th.eq(0).text());
      if (!(order >= 1 && order <= 9)) return null;
      const link = $(row).find('a[href*="/players/"]').first();
      const id = clean(link.attr('href')).match(/\/players\/(\d+)\.html$/)?.[1];
      if (!id) return null;
      return { id, officialPlayerId: id, name: clean(link.text()), order, position: clean(th.eq(1).text()) };
    }).get().filter(Boolean);
    const evidence = { sourceEventId: event.id, contentHash: event.contentHash, fetchedAt: event.fetchedAt,
      sourcePath: `#player-order .${half} table`, identitySourcePath: `#tablefix_ls tr.${rowClass}`,
      flagSourcePath: '.line-score .game_info', publishedAt: null, pregameReceiptVerified: true };
    output[side] = currentLineupSnapshot(players, game, side, 'NPB', evidence,
      battingStats[side], battingStatsObservedAt[side]);
  }
  return output;
}
...
export function projectedLineup(details, teamId, baseline, battingStats = [], battingStatsObservedAt = null) {
  ...
  const orderedSlots = players.map(row => number(row.order));
  const orderComplete = players.length === 9 && new Set(orderedSlots).size === 9
    && orderedSlots.every(slot => Number.isInteger(slot) && slot >= 1 && slot <= 9);
  return {
    ...
    lineupComplete: players.length === 9, battingOrderComplete: orderComplete,
    battingOrderStatus: orderComplete ? 'HISTORICAL_ORDER_PROJECTED' : 'INCOMPLETE_OR_UNVERIFIED_ORDER',
    ...
    incompleteReasons: [
      ...
      ...(players.length !== 9 ? ['NINE_STARTING_SLOTS_NOT_AVAILABLE'] : []),
      ...(!orderComplete ? ['BATTING_ORDER_NOT_FULLY_VERIFIED'] : []),
...
export function bullpenSnapshot(details, teamId, leagueId, referenceEra, firstPitch) {
  const rows = details.flatMap(item => {
    ...
    return (side?.pitchers || []).filter(pitcher => leagueId === 'KBO'
      ? pitcher.starter === false && pitcher.roleVerified === true : pitcher.starter !== true).map(pitcher => ({
      ...pitcher,
      ...
      ...(item.sourceEvidence ? { sourceEvidence: item.sourceEvidence } : {}),
    }));
  });
  ...
  return {
    ...
    ...(leagueId === 'KBO' ? { usageAppearances: rows, observedPitchCount: rows.every(row => Number.isSafeInteger(row.pitchCount))
      ? rows.reduce((sum, row) => sum + row.pitchCount, 0) : null } : {}),
    ...
    usageCoverage: { sampledGames: new Set(rows.map(row => row.gamePk)).size, startDate: rows.map(row => row.date).sort()[0] || null, endDate: rows.map(row => row.date).sort().at(-1) || null, complete: false,
      pitchCountsAvailable: leagueId === 'KBO' && rows.length > 0 && rows.every(row => Number.isSafeInteger(row.pitchCount)) },
...
export function officialStarterIdentitySnapshot({ game, side, identity }) {
  const source = clean(identity?.source);
  const rawId = clean(identity?.id);
  const officialPlayerId = /^\d+$/.test(rawId) && /[1-9]/.test(rawId) ? rawId : null;
  const assignmentConfirmed = identity?.officialAssignmentConfirmed === true
    || /^(?:NPB_OFFICIAL_(?:PROBABLE_STARTER|SCHEDULE_DETAIL_STARTER|STARTER_SOURCES)|KBO_OFFICIAL_GAMECENTER_STARTER|CPBL_OFFICIAL_CURRENT_GAME_STARTER)$/.test(source);
  if (!clean(identity?.name) || !assignmentConfirmed || identity.projected === true
    || identity.officialAssignmentConfirmed === false
    || (!officialPlayerId && !/^NPB_OFFICIAL_/.test(source))) return null;
  // Missing ability data must never reopen a known game assignment and choose
  // another rotation pitcher. Keep identity evidence separate from performance.
  return {
    id: officialPlayerId, officialPlayerId,
    name: clean(identity.name), teamId: Number(game[`${side}TeamId`]),
    identityConfirmed: true, confirmed: true, playerIdentityVerified: true,
    projected: false, assignmentStatus: 'OFFICIAL_CONFIRMED',
    identitySource: source, source,
    sourceEvidence: identity.sourceEvidence || null,
    identityResolutionEvidence: identity.identityResolutionEvidence || null,
    scheduleIdentitySourceEvidence: game.identitySourceEvidence || null,
    performanceAvailable: false, performanceStatus: 'MISSING_INDIVIDUAL_STARTER_PERFORMANCE',
    performanceMissingReason: 'OFFICIAL_STARTER_PERFORMANCE_UNAVAILABLE_OR_INSUFFICIENT',
    performanceScope: 'UNAVAILABLE', qualityFactor: null, expectedInnings: null,
    throws: identity.throws || null, officialThrows: identity.throws || null,
    projectionMode: 'OFFICIAL_ASSIGNMENT_MISSING_PERFORMANCE', candidates: [],
  };
}
...
export function starterSnapshot({ leagueId, game, side, identity, stats, referenceEra, recentStarts = [] }) {
  ...
  return {
    ...
    identityConfirmed: identity.projected !== true && identity.officialAssignmentConfirmed !== false,
    confirmed: identity.projected !== true && identity.officialAssignmentConfirmed !== false,
    playerIdentityVerified: true, projected: identity.projected === true,
    projectionBased: identity.projected === true,
    identitySource: clean(identity.source), sourceEvidence: identity.sourceEvidence || null, performanceAvailable: true,
    identityResolutionEvidence: identity.identityResolutionEvidence || null,
    ...
    season: {
      ...
      inningsPitchedSourceEvidence: stats.inningsPitchedSourceEvidence || null,
      appearancesSourceEvidence: stats.appearancesSourceEvidence || null,
      gamesStartedSourceEvidence: stats.gamesStartedSourceEvidence || null,
      gamesStartedAcquisitionStatus: stats.gamesStartedAcquisitionStatus || null,
      sampleSizeEvidence: {
    ...
    ...(identity.projected ? {
      ...
      probabilityScope: identity.probabilityScope || 'CONDITIONAL_ON_OBSERVED_CANDIDATE_SET',
      candidateSetComplete: false,
      rotationCandidates: identity.candidates,
      candidates: identity.candidates,
    } : { assignmentStatus: identity.officialAssignmentConfirmed === false
      ? 'ROSTER_VALIDATED_REPORTED_STARTER' : 'OFFICIAL_CONFIRMED' }),
  };
}
...
async function npbDetails(games, options) {
  const settled = await Promise.allSettled(games.map(async game => {
    ...
    const detail = parseNpbGameDetailHtml(html);
    if (detail.away.teamCode !== game.awayCode || detail.home.teamCode !== game.homeCode) {
      throw officialError('NPB逐場主客隊與歷史場次不符', 'ASIAN_OFFICIAL_TEAM_IDENTITY_MISMATCH');
    }
    return { game, detail };
  }));
...
async function kboDetails(games, options, cookie) {
  const settled = await Promise.allSettled(games.map(async game => {
    ...
    const payload = await kboPost('/ws/Schedule.asmx/GetBoxScoreScroll', request, options, cookie);
    const requestBodyHash = createHash('sha256').update(String(new URLSearchParams(request))).digest('hex');
    const event = options.sourceObservations?.filter(row => row.success
      && row.url.endsWith('/GetBoxScoreScroll') && row.requestBodyHash === requestBodyHash).at(-1);
    return { game, detail: parseKboBoxScorePayload(payload), sourceEvidence: event ? {
      sourceEventId: event.id, contentHash: event.contentHash, fetchedAt: event.fetchedAt,
      requestBodyHash, providerGameId: game.providerGameId, sourcePath: '$.arrPitcher[*].table.rows',
    } : null };
  }));
  return settled.filter(row => row.status === 'fulfilled').map(row => row.value);
}

async function cpblDetails(games, options, targetGame) {
  const settled = await Promise.allSettled(games.map(async game => {
    const url = `https://stats.cpbl.com.tw/api/proxy/v1/games/${encodeURIComponent(clean(game.providerGameId))}`;
    const detail = parseCpblGameDetailPayload(await officialFetch(url, {
      ...options,
      timeoutMs: Math.max(25_000, Number(options?.timeoutMs) || 0),
    }));
    const event = options.sourceObservations?.filter(row => row.success && row.url === url).at(-1);
    if (detail.providerGameId !== clean(game.providerGameId)
      || detail.away.teamCode !== CPBL_TEAM_ID[game.awayCode] || detail.home.teamCode !== CPBL_TEAM_ID[game.homeCode]) {
      throw officialError('中職逐場比賽或主客隊身分不符', 'CPBL_DETAIL_IDENTITY_NOT_VERIFIED');
    }
    if (detail.status !== 'FINISHED') throw officialError('中職投手能力僅使用完賽明細', 'CPBL_DETAIL_NOT_FINISHED');
    const rawStart = detail.officialStart;
    if (rawStart && Date.parse(/(?:Z|[+-]\d{2}:\d{2})$/.test(rawStart) ? rawStart : `${rawStart}+08:00`) !== Date.parse(game.gameDate)) {
      throw officialError('中職逐場日期與完賽紀錄不符', 'CPBL_DETAIL_GAME_DATE_MISMATCH');
    }
    if (!event || !(Date.parse(event.fetchedAt) < Date.parse(targetGame?.gameDate || ''))) {
      throw officialError('中職明細缺少目標賽前取樣證據', 'CPBL_DETAIL_AFTER_TARGET_CUTOFF');
    }
    return { game, detail, sourceEvidence: event ? { sourceEventId: event.id, contentHash: event.contentHash,
      fetchedAt: event.fetchedAt, providerGameId: game.providerGameId, sourcePath: '$.Data.Game' } : null };
  }));
  return {
    details: settled.filter(row => row.status === 'fulfilled').map(row => row.value),
    failures: settled.flatMap((row, index) => row.status === 'rejected'
      ? [{ providerGameId: games[index].providerGameId, reason: row.reason?.code || 'COMPLETED_DETAIL_FETCH_FAILED' }] : []),
  };
}

const CPBL_ROTATION_MAX_REST_DAYS = 10;

export function selectedCpblRotationGames(history, game) {
  const cutoff = Date.parse(game?.gameDate || '');
  const relevant = row => [Number(game?.awayTeamId), Number(game?.homeTeamId)].some(teamId =>
    Number(row?.awayTeamId) === teamId || Number(row?.homeTeamId) === teamId);
  const eligibleWindow = (Array.isArray(history) ? history : []).filter(row => {
    const at = Date.parse(row?.gameDate || '');
    return relevant(row) && Number.isFinite(at) && at < cutoff
      && cutoff - at <= CPBL_ROTATION_MAX_REST_DAYS * 86_400_000;
  });
  // Preserve the existing six-game performance sample and add every game from
  // the already-supported rest window, even when it is a team's seventh game.
  return [...new Map([...selectedRecentGames(history, game, 6), ...eligibleWindow]
    .map(row => [clean(row.providerGameId) || row.gamePk, row])).values()];
}

export function parseCpblReservedStarterAppearances(payload, scheduleGame, targetGame, event) {
  const reject = reason => ({ detail: null, reason });
  const root = payload?.Data?.Game || payload?.data?.game || payload?.Data || payload?.data;
  const at = Date.parse(scheduleGame?.gameDate || '');
  const cutoff = Date.parse(targetGame?.gameDate || '');
  if (!/^(RESERVED|SUSPENDED)$/.test(clean(scheduleGame?.statusEnglish).toUpperCase())) return reject('SCHEDULE_NOT_RESERVED');
  if (!Number.isFinite(at) || !Number.isFinite(cutoff) || at >= cutoff) return reject('APPEARANCE_NOT_BEFORE_TARGET');
  if (!event?.success || !clean(event.id) || !clean(event.contentHash) || !Number.isFinite(Date.parse(event.fetchedAt || ''))
    || Date.parse(event.fetchedAt) < at || Date.parse(event.fetchedAt) >= cutoff) return reject('MISSING_PRE_TARGET_SOURCE_RECEIPT');
  if (event.url !== `https://stats.cpbl.com.tw/api/proxy/v1/games/${encodeURIComponent(clean(scheduleGame.providerGameId))}`
    || clean(root?.GameId) !== clean(scheduleGame.providerGameId)) return reject('OFFICIAL_GAME_ID_MISMATCH');
  // A resumed final box may contain innings and appearances after the target.
  // It is never used to reconstruct the formerly suspended segment here.
  if (!/^(START|RESERVED|SUSPENDED)$/.test(clean(root?.GameStatus).toUpperCase())) return reject('DETAIL_NOT_UNFINISHED_SEGMENT');
  const rawStart = clean(root?.PreExeDate);
  const start = Date.parse(/(?:Z|[+-]\d{2}:\d{2})$/.test(rawStart) ? rawStart : `${rawStart}+08:00`);
  if (!Number.isFinite(start) || start !== at) return reject('ORIGINAL_GAME_DATE_NOT_VERIFIED');
  const idMatch = clean(root.GameId).match(/^(\d{4})-A-(\d+)$/);
  if (!idMatch) return reject('OFFICIAL_GAME_KIND_MISMATCH');
  const detail = { away: { lineup: [], pitchers: [] }, home: { lineup: [], pitchers: [] } };
  let count = 0;
  for (const [side, key, battingSide] of [['away', 'Visiting', '2'], ['home', 'Home', '1']]) {
    const block = root?.[key], expected = CPBL_TEAM_ID[scheduleGame[`${side}Code`]];
    if (!expected || clean(block?.Team?.Code) !== expected) return reject('OFFICIAL_TEAM_ID_MISMATCH');
    detail[side].teamCode = expected;
    const rows = Array.isArray(block.Pitchers) ? block.Pitchers : [];
    const starters = rows.filter(row => clean(row?.RoleType) === '先發');
    if (starters.length !== 1) continue;
    const row = starters[0], id = clean(row.PitcherAcnt);
    if (!/^(?=.*[1-9])\d+$/.test(id) || !clean(row.PitcherName)
      || !(number(row.PitchCnt) > 0 || number(row.PlateAppearances) > 0 || number(row.InningPitchedCnt) > 0 || number(row.InningPitchedDiv3Cnt) > 0)) continue;
    const logs = Array.isArray(root.LiveLog) ? root.LiveLog : [];
    const logIndex = logs.findIndex(log => clean(log.PitcherAcnt) === id && clean(log.Year) === idMatch[1]
      && clean(log.KindCode) === 'A' && number(log.GameSno) === Number(idMatch[2])
      && clean(log.VisitingHomeType) === battingSide && number(log.PitchCnt) > 0
      && number(log.InningSeq) === 1 && clean(log.IsChangePlayer) !== '1');
    if (logIndex < 0) continue;
    const parsed = parseCpblGameDetailPayload(payload)[side].pitchers.find(pitcher => pitcher.id === id && pitcher.starter);
    if (!parsed) continue;
    detail[side].pitchers.push({ ...parsed, inningsPitched: null,
      appearanceVerified: true, appearanceOnly: true,
      appearanceEvidence: { sourceEventId: event.id, contentHash: event.contentHash, fetchedAt: event.fetchedAt,
        sourcePath: `$.Data.Game.${key}.Pitchers[${rows.indexOf(row)}]`, playSourcePath: `$.Data.Game.LiveLog[${logIndex}]`,
        identitySourcePath: `$.Data.Game.${key}.Team.Code`, publishedAt: null,
        dateBasis: 'OFFICIAL_ORIGINAL_GAME_START_NOT_PITCH_TIMESTAMP', gameDate: scheduleGame.gameDate,
        originalStartAt: scheduleGame.gameDate, latestAppearanceAt: null, latestAppearanceVerified: false,
        observedCumulativePitchCount: parsed.pitchCount, observedCumulativeInningsPitched: parsed.inningsPitched,
        aggregateWorkloadUsed: false,
        scheduleStatus: scheduleGame.statusEnglish, detailStatus: root.GameStatus,
        usage: 'ROTATION_ORIGINAL_START_ONLY;RESUMPTION_TIME_AND_WORKLOAD_NOT_INFERRED' } });
    count += 1;
  }
  return count ? { detail, reason: null } : reject('ACTUAL_STARTER_APPEARANCE_NOT_VERIFIED');
}

async function cpblReservedDetails(appearanceHistory, targetGame, options) {
  const cutoff = Date.parse(targetGame.gameDate);
  const selected = [...new Map((Array.isArray(appearanceHistory) ? appearanceHistory : []).filter(row => {
    const at = Date.parse(row?.gameDate || '');
    return /^(RESERVED|SUSPENDED)$/.test(clean(row?.statusEnglish).toUpperCase())
      && [Number(targetGame.awayTeamId), Number(targetGame.homeTeamId)].some(id =>
        Number(row.awayTeamId) === id || Number(row.homeTeamId) === id)
      && Number.isFinite(at) && at < cutoff && cutoff - at <= CPBL_ROTATION_MAX_REST_DAYS * 86_400_000;
  }).map(row => [clean(row.providerGameId), row])).values()];
  const records = await Promise.all(selected.map(async game => {
    const url = `https://stats.cpbl.com.tw/api/proxy/v1/games/${encodeURIComponent(clean(game.providerGameId))}`;
    try {
      const payload = await officialFetch(url, { ...options, cacheMs: 30_000 });
      const event = options.sourceObservations?.filter(row => row.success && row.url === url).at(-1);
      const parsed = parseCpblReservedStarterAppearances(payload, game, targetGame, event);
      return { game, ...parsed, appearanceOnly: true };
    } catch { return { game, detail: null, reason: 'RESERVED_DETAIL_FETCH_FAILED', appearanceOnly: true }; }
  }));
  return { details: records.filter(row => row.detail), diagnostics: {
    selectedGames: selected.length, verifiedGames: records.filter(row => row.detail).length,
    selectedGameIds: selected.map(row => row.providerGameId),
    requestedGameIds: records.map(row => row.game.providerGameId),
    loadedGameIds: records.filter(row => row.detail).map(row => row.game.providerGameId),
    verifiedStarterAppearances: records.reduce((sum, row) => sum
      + (row.detail?.away?.pitchers?.length || 0) + (row.detail?.home?.pitchers?.length || 0), 0),
    rejectedGames: records.filter(row => !row.detail).map(row => ({ providerGameId: row.game.providerGameId, reason: row.reason })),
    unverifiedSides: records.filter(row => row.detail).flatMap(row => ['away', 'home']
      .filter(side => !row.detail[side]?.pitchers?.length).map(side => ({ providerGameId: row.game.providerGameId, side,
        reason: 'ACTUAL_INITIAL_STARTER_PLAY_NOT_VERIFIED' }))),
  } };
}
...
function recentStarterRows(details, teamId, identity) {
  ...
  return details.filter(item => item.appearanceOnly !== true).flatMap(item => (detailSide(item.detail, item.game, teamId)?.pitchers || [])
...
export function rotationPrediction(details, teamId, firstPitch, leagueId) {
  ...
  for (const item of details) {
    ...
    for (const pitcher of detailSide(item.detail, item.game, teamId)?.pitchers || []) {
      ...
      existing.starts.push({ ...pitcher, appearanceOnly: item.appearanceOnly === true,
        teamId: Number(teamId), officialPlayerId, gameDate: item.game.gameDate, gamePk: item.game.gamePk });
  ...
  const candidates = [...grouped.values()].map(candidate => {
    ...
    const completedStarts = candidate.starts.filter(row => row.appearanceOnly !== true);
    const inningsPitched = completedStarts.reduce((sum, row) => sum + (number(row.inningsPitched) || 0), 0);
    const earnedRuns = completedStarts.every(row => number(row.earnedRuns) != null)
      ? completedStarts.reduce((sum, row) => sum + number(row.earnedRuns), 0) : null;
    const hits = completedStarts.every(row => number(row.hits) != null)
      ? completedStarts.reduce((sum, row) => sum + number(row.hits), 0) : null;
    const walks = completedStarts.every(row => number(row.walks) != null)
      ? completedStarts.reduce((sum, row) => sum + number(row.walks), 0) : null;
    const battersFaced = completedStarts.reduce((sum, row) => sum + (number(row.battersFaced) || 0), 0)
      || inningsPitched * 4.25;
    ...
    return {
      ...
      lastStartAt,
      lastStartEvidence: candidate.starts[0]?.appearanceEvidence || null,
      lastStartDateBasis: candidate.starts[0]?.appearanceEvidence?.dateBasis || 'COMPLETED_GAME_START',
      lastKnownOriginalStart: candidate.starts[0]?.appearanceOnly ? lastStartAt : null,
      possibleResumptionTimeUnknown: candidate.starts[0]?.appearanceOnly === true,
      restDaysBasis: candidate.starts[0]?.appearanceOnly
        ? 'ORIGINAL_GAME_START_ONLY_LATEST_APPEARANCE_UNVERIFIED' : 'COMPLETED_GAME_START_INTERVAL',
      restDays,
      priorStarts: completedStarts.length,
      inningsPitched,
      battersFaced,
      ...
      expectedInnings: completedStarts.length ? inningsPitched / completedStarts.length : null,
      rawWeight: rotationFit * Math.min(1, 0.65 + completedStarts.length * 0.18),
      evidenceGamePks: completedStarts.map(row => row.gamePk).filter(Boolean),
    };
...
export function projectCpblRotationStarter(details, teamId, firstPitch) {
  ...
  const appearances = (Array.isArray(details) ? details : []).flatMap(item => {
    ...
    return (detailSide(item?.detail, item?.game, teamId)?.pitchers || [])
      ...
      .map(row => ({
        ...
        lastStartEvidence: row.appearanceEvidence || item.sourceEvidence || null,
      }));
  });
  ...
  const candidates = [...latestByPitcher.values()].map(row => {
    ...
    const rotationScore = restDays >= 3.75 && restDays <= CPBL_ROTATION_MAX_REST_DAYS
  ...
  const weighted = candidates.map(row => ({
    ...
    lastStartEvidence: row.lastStartEvidence,
    lastStartDateBasis: row.lastStartEvidence?.dateBasis || 'COMPLETED_GAME_START',
    lastKnownOriginalStart: row.lastStartEvidence?.latestAppearanceVerified === false ? row.gameDate : null,
    possibleResumptionTimeUnknown: row.lastStartEvidence?.latestAppearanceVerified === false,
    restDaysBasis: row.lastStartEvidence?.latestAppearanceVerified === false
      ? 'ORIGINAL_GAME_START_ONLY_LATEST_APPEARANCE_UNVERIFIED' : 'COMPLETED_GAME_START_INTERVAL',
    restDays: Number(row.restDays.toFixed(2)),
    weight: total > 0 ? row.rotationScore / total : 0,
  }));
  for (const candidate of weighted) {
    const starts = recentStarterRows(details, teamId, candidate)
      .filter(row => Number.isFinite(Date.parse(row.date || '')) && Date.parse(row.date) < firstPitchAt);
    const sumObserved = key => starts.length && starts.every(row => number(row[key]) != null)
      ? starts.reduce((sum, row) => sum + number(row[key]), 0) : null;
    ...
    Object.assign(candidate, { officialPlayerId: candidate.id, teamId: Number(teamId),
      probability: candidate.weight, probabilityKind: 'HEURISTIC_CANDIDATE_WEIGHT', confidenceCalibrated: false,
      priorStarts: starts.length, inningsPitched, battersFaced: sumObserved('battersFaced'),
      era: inningsPitched > 0 && earnedRuns != null ? earnedRuns * 9 / inningsPitched : null,
      whip: inningsPitched > 0 && hits != null && walks != null ? (hits + walks) / inningsPitched : null,
      expectedInnings: inningsPitched > 0 && starts.length ? inningsPitched / starts.length : null,
      performanceAvailable: inningsPitched > 0 && earnedRuns != null && sumObserved('battersFaced') > 0,
      evidenceGamePks: starts.map(row => row.gamePk).filter(Boolean),
    });
  }
...
async function npbCurrentPage(game, options) {
  if (!currentPersonnelAcquisitionEligible(game, new Date().toISOString())) return null;
  const date = clean(game.officialDate);
  const base = `https://npb.jp/scores/${date.slice(0, 4)}/${date.slice(5).replace('-', '')}/`;
  const daily = await officialFetch(base, { ...options, format: 'text', cacheMs: 30_000 });
  const $ = cheerio.load(daily);
  const prefix = `${base}${NPB_TEAM_SUFFIX[game.homeCode]}-${NPB_TEAM_SUFFIX[game.awayCode]}-`;
  const urls = [...new Set($('a[href]').map((_, link) => {
    try { return new URL($(link).attr('href'), base).href; } catch { return ''; }
  }).get().filter(url => url.startsWith(prefix) && /^\d+\/$/.test(url.slice(prefix.length))))];
  // Do not choose arbitrarily when a doubleheader or ambiguous identity exists.
  if (urls.length !== 1) return null;
  const url = urls[0];
  const html = await officialFetch(url, { ...options, format: 'text', cacheMs: 30_000 });
  const event = options.sourceObservations?.filter(row => row.success && row.url === url).at(-1);
  return event ? { html, event } : null;
}

async function buildNpb(game, history, options) {
  const year = clean(game.officialDate).slice(0, 4);
  const recentGames = selectedRecentGames(history, game, 8);
  const [details, awayStatsHtml, homeStatsHtml, awayBattingHtml, homeBattingHtml, currentPage] = await Promise.all([
    ...
    npbCurrentPage(game, options).catch(() => null),
  ]);
  ...
  await Promise.all(['away', 'home'].map(async side => {
    ...
    const statsUrl = `https://npb.jp/bis/${year}/stats/idp1_${NPB_TEAM_SUFFIX[game[`${side}Code`]]}.html`;
    const statsEvent = options.sourceObservations?.filter(item => item.success && item.url === statsUrl).at(-1);
    const statsHtml = side === 'away' ? awayStatsHtml : homeStatsHtml;
    starterStats[side] = row ? { ...row, observedAt: statsEvent?.fetchedAt || null } : null;
    if (!row || clean(identity[side].id) || !npbPlayerSourceScope(statsHtml, statsEvent, game, side, 'pitching')) return;
    const args = { game, side, identity: identity[side], statsHtml, statsEvent };
    let resolved = resolveNpbStarterPlayerId(args);
    if (!resolved && !row.id && currentPersonnelAcquisitionEligible(game, new Date().toISOString())) {
      const rosterUrl = `https://npb.jp/bis/teams/rst_${NPB_TEAM_SUFFIX[game[`${side}Code`]]}.html`;
      const rosterHtml = await officialFetch(rosterUrl, { ...options, format: 'text' }).catch(() => '');
      const rosterEvent = options.sourceObservations?.filter(item => item.success && item.url === rosterUrl).at(-1);
      resolved = resolveNpbStarterPlayerId({ ...args, rosterHtml, rosterEvent });
    }
    if (resolved) {
      identity[side] = resolved;
      gamePatch[`${side}ProbableId`] = resolved.id;
    }
  }));
  const battingStats = { away: parseNpbBattingStatsHtml(awayBattingHtml), home: parseNpbBattingStatsHtml(homeBattingHtml) };
  const battingStatsObservedAt = Object.fromEntries(['away', 'home'].map(side => [side,
    options.sourceObservations?.find(row => row.url.endsWith(`/idb1_${NPB_TEAM_SUFFIX[game[`${side}Code`]]}.html`))?.observedAt || null,
  ]));
  return {
    details, identity, starterStats, gamePatch,
    battingStats, battingStatsObservedAt,
    currentLineups: currentPage ? parseNpbCurrentLineups(currentPage.html, game, currentPage.event, battingStats, battingStatsObservedAt) : null,
  };
}
...
async function buildKbo(game, history, options) {
  ...
  if (!meta) return { details: [], identity: {}, starterStats: {}, gamePatch: {} };
  const [analysis, details, weatherPayload, lineupPayload] = await Promise.all([
    meta.T_PIT_P_ID && meta.B_PIT_P_ID ? kboPost('/ws/Schedule.asmx/GetPitcherRecordAnalysis', {
    ...
    }, options, cookie).catch(() => null) : Promise.resolve(null),
    ...
    kboTbfAcquisitionEligible(game, new Date().toISOString()) && String(meta.GAME_STATE_SC) === '1'
      ? kboPost('/ws/Schedule.asmx/GetLineUpAnalysis', { leId: '1', srId: String(meta.SR_ID ?? 0),
        seasonId: String(meta.SEASON_ID), gameId: clean(meta.G_ID) }, { ...options, cacheMs: 0 }, cookie).catch(() => null)
      : Promise.resolve(null),
  ]);
  const lineupEvent = options.sourceObservations?.filter(row => row.success && row.url.endsWith('/GetLineUpAnalysis')).at(-1);
  const currentLineups = lineupEvent && kboTbfAcquisitionEligible(game, lineupEvent.fetchedAt)
    && kboTbfAcquisitionEligible(game, new Date().toISOString())
    ? parseKboOfficialLineup(lineupPayload, game, { away: clean(meta.AWAY_ID), home: clean(meta.HOME_ID) }) : null;
  for (const lineup of Object.values(currentLineups || {})) lineup.sourceEvidence = {
    sourceEventId: lineupEvent.id, contentHash: lineupEvent.contentHash, fetchedAt: lineupEvent.fetchedAt,
    sourcePath: lineup.sourcePath, identitySourcePath: lineup.identitySourcePath, flagSourcePath: '$[0][0].LINEUP_CK',
  };
  ...
  const gamePatch = {
    providerGameId: clean(meta.G_ID),
    awayProbableId: Number.isSafeInteger(Number(meta.T_PIT_P_ID)) && Number(meta.T_PIT_P_ID) > 0 ? Number(meta.T_PIT_P_ID) : null,
    homeProbableId: Number.isSafeInteger(Number(meta.B_PIT_P_ID)) && Number(meta.B_PIT_P_ID) > 0 ? Number(meta.B_PIT_P_ID) : null,
  ...
  const identity = {
    away: { id: gamePatch.awayProbableId, name: gamePatch.awayProbable, source: gamePatch.probableSource, throws: statsRows[0]?.throws,
      officialAssignmentConfirmed: gamePatch.awayProbableId > 0 && Boolean(gamePatch.awayProbable) },
    home: { id: gamePatch.homeProbableId, name: gamePatch.homeProbable, source: gamePatch.probableSource, throws: statsRows[1]?.throws,
      officialAssignmentConfirmed: gamePatch.homeProbableId > 0 && Boolean(gamePatch.homeProbable) },
  };
  await Promise.all(['away', 'home'].map(async (side, index) => {
    ...
    try {
      ...
      if (parsed.inningsOuts !== null) {
        stats.estimatedInningsPitched = stats.inningsPitched;
        stats.inningsPitched = parsed.inningsPitched;
        stats.inningsPitchedEstimated = false;
        stats.inningsPitchedMethod = 'OFFICIAL_SEASON_IP_EXACT_OUTS';
        stats.inningsPitchedSourceEvidence = { ...stats.battersFacedSourceEvidence,
          sourceField: 'IP', rawValue: parsed.inningsRawValue, sourcePath: parsed.sourcePath.replace('td[TBF]', 'td[IP]') };
      }
      stats.appearances = parsed.appearances;
      stats.appearancesSourceEvidence = parsed.appearances === null ? null : { ...stats.battersFacedSourceEvidence,
        sourceField: 'G', rawValue: parsed.appearancesRawValue, sourcePath: parsed.sourcePath.replace('td[TBF]', 'td[G]') };
      stats.gamesStartedAcquisitionStatus = 'UNAVAILABLE_OR_DAILY_TOTAL_MISMATCH';
      const dailyUrl = url.replace('/Basic.aspx', '/Daily.aspx');
      const dailyHtml = await officialFetch(dailyUrl, { ...options, format: 'text' });
      const starts = parseKboOfficialStarts(dailyHtml, parsed, parsed, game.officialDate);
      const dailyEvent = options.sourceObservations?.filter(row => row.url === dailyUrl && row.success).at(-1);
      if (!starts || !dailyEvent || !kboTbfAcquisitionEligible(game, dailyEvent.fetchedAt)
        || !kboTbfAcquisitionEligible(game, new Date().toISOString())) return;
      stats.gamesStarted = starts.gamesStarted;
      stats.gamesStartedAcquisitionStatus = 'OFFICIAL_DAILY_RECONCILED_WITH_SEASON';
      stats.gamesStartedSourceEvidence = { ...starts, url: dailyUrl, sourceEventId: dailyEvent.id,
        contentHash: dailyEvent.contentHash, fetchedAt: dailyEvent.fetchedAt,
        seasonTotalSourceEventId: event.id, seasonTotalContentHash: event.contentHash };
  ...
  return {
    details, identity, gamePatch, currentLineups,
...
function findCpblPlayer(players, evidence, teamCode) {
  ...
  return match ? { id: clean(match.Acnt), name: clean(match.CHName).replace(/^[*#]/, ''), source: evidence.source,
    officialAssignmentConfirmed: false, throws: evidence.throws } : null;
}

async function buildCpbl(game, history, options, starterEvidence, appearanceHistory = []) {
  const selectedGames = selectedCpblRotationGames(history, game);
  const existingDetailIds = new Set(selectedRecentGames(history, game, 6).map(row => clean(row.providerGameId) || row.gamePk));
  const [completed, rosterPayload, pitcherPayload, reserved] = await Promise.all([
    cpblDetails(selectedGames, options, game),
    ...
    cpblReservedDetails(appearanceHistory, game, options),
  ]);
  const starterDetails = completed.details;
  const details = starterDetails.filter(row => existingDetailIds.has(clean(row.game.providerGameId) || row.game.gamePk));
  const rotationDetails = [...new Map([...reserved.details, ...starterDetails]
    .map(row => [clean(row.game.providerGameId) || row.game.gamePk, row])).values()];
  const fetchedIds = new Set(starterDetails.map(row => clean(row.game.providerGameId)));
  const rotationInputDiagnostics = {
    version: 'CPBL-ROTATION-INPUT-COVERAGE-v1', eligibleRestWindowDays: CPBL_ROTATION_MAX_REST_DAYS,
    scope: 'KNOWN_OFFICIAL_GAMES_WITHIN_EXISTING_ROTATION_WINDOW;NOT_ALL_ROSTER_PITCHERS',
    completedGamesSelected: selectedGames.length, completedDetailsFetched: starterDetails.length,
    selectedGameIds: selectedGames.map(row => row.providerGameId), loadedGameIds: [...fetchedIds],
    requestedGameIds: selectedGames.map(row => row.providerGameId),
    fullCandidateCoverageVerified: false,
    missingCompletedDetails: completed.failures,
    reserved: reserved.diagnostics, performanceScope: 'COMPLETED_GAMES_ONLY',
  };
  const players = cpblRosterPlayers(rosterPayload);
  const currentUrl = `https://stats.cpbl.com.tw/api/proxy/v1/games/${encodeURIComponent(clean(game.providerGameId))}`;
  const currentPayload = currentPersonnelAcquisitionEligible(game, new Date().toISOString())
    ? await officialFetch(currentUrl, { ...options, cacheMs: 30_000 }).catch(() => null) : null;
  const currentEvent = options.sourceObservations?.filter(row => row.success && row.url === currentUrl).at(-1);
  const currentPersonnel = parseCpblCurrentPersonnel(currentPayload, game, currentEvent);
  const currentStarter = side => currentPersonnel?.starters?.[side] || null;
  const identity = {};
  for (const side of ['away', 'home']) {
    ...
    identity[side] = officialCurrent
      ...
      : findCpblPlayer(players, starterEvidence?.[side], teamCode)
        || projectCpblRotationStarter(rotationDetails, teamId, game.gameDate);
  }
  ...
  for (const [index, side] of ['away', 'home'].entries()) {
    ...
    if (!starterStats[side] && identity[side]) {
      const teamId = side === 'away' ? game.awayTeamId : game.homeTeamId;
      const recent = recentStarterRows(starterDetails, teamId, identity[side]);
  ...
  let batterRates = null;
  const anchor = [...Object.values(currentPersonnel?.lineups || {}).flatMap(row => row?.players || []),
    ...details.flatMap(row => row.detail?.away?.lineup || [])].find(row => /^\d{10}$/.test(String(row.officialPlayerId || row.id || '')));
  if (anchor && cpblRatesEligible(game, new Date().toISOString())) {
    const url = `https://stats.cpbl.com.tw/players/${anchor.officialPlayerId || anchor.id}`;
    try {
      const html = await officialFetch(url, { ...options, format: 'text' });
      const event = options.sourceObservations?.filter(row => row.url === url && row.success).at(-1);
      const parsed = parseCpblBatterRates(html, clean(game.officialDate).slice(0, 4));
      if (parsed && event && cpblRatesEligible(game, event.fetchedAt) && cpblRatesEligible(game, new Date().toISOString())) {
        batterRates = { ...parsed, sourceEventId: event.id, fetchedAt: event.fetchedAt, url };
      }
    } catch { /* Keep missing evidence explicit; never infer AB from PA. */ }
  }
  const featureSourceScopes = Object.fromEntries(['away', 'home'].flatMap(side => {
    const teamId = game[`${side}TeamId`];
    const scopeIds = rows => rows.filter(row => detailSide(row.detail, row.game, teamId))
      .map(row => clean(row.game.providerGameId));
    return [
      [`${side}.starter`, { completedGameIds: scopeIds(starterDetails), identityGameIds: currentPersonnel?.starters?.[side] ? [game.providerGameId] : [], appearanceGameIds: reserved.details
        .filter(row => detailSide(row.detail, row.game, teamId)?.pitchers?.length).map(row => clean(row.game.providerGameId)) }],
      [`${side}.bullpen`, { completedGameIds: scopeIds(details), appearanceGameIds: [] }],
    ];
  }));
  return { details, starterDetails, rotationDetails, rotationInputDiagnostics, featureSourceScopes, identity, starterStats, gamePatch, batterRates,
    currentLineups: currentPersonnel?.lineups || null };
}
...
export async function buildAsianProductionFeatureSnapshot({
  ...
  history = (Array.isArray(history) ? history : []).filter(row => {
    ...
    return Number.isFinite(at) && Number.isFinite(firstPitchAt) && at < firstPitchAt
      && (!row.statusCode || clean(row.statusCode).toUpperCase() === 'F')
  ...
  try {
    source = league === 'NPB'
      ...
      : league === 'KBO'
        ...
        : await buildCpbl(game, history, options, starterEvidence, appearanceHistory);
  } catch (error) {
  ...
  const featureSnapshot = {
    ...
    rules: {},
    ...(source.rotationInputDiagnostics ? { rotationInputDiagnostics: source.rotationInputDiagnostics } : {}),
  ...
  for (const side of ['away', 'home']) {
    ...
    const recentStarts = identity ? recentStarterRows(source.starterDetails || source.details, teamId, identity) : [];
    const measuredStarter = starterSnapshot({ leagueId: league, game: resolvedGame, side, identity,
    ...
    const starter = measuredStarter || officialStarterIdentitySnapshot({ game: resolvedGame, side, identity });
    const starterPrediction = starter ? null : rotationPrediction(source.rotationDetails || source.details, teamId, resolvedGame.gameDate, league);
    featureSnapshot[side] = {
      teamStrength: strengthSnapshot(league, history, teamId),
      starter: starter || starterPrediction,
      lineup: source.currentLineups?.[side] || projectedLineup(source.details, teamId, league, source.battingStats?.[side], source.battingStatsObservedAt?.[side]),
      bullpen: bullpenSnapshot(source.details, teamId, league, referenceEra, resolvedGame.gameDate),
    };
  }
  if (league === 'CPBL' && source.batterRates) {
    for (const side of ['away', 'home']) {
      const lineup = featureSnapshot[side].lineup;
      if (!lineup) continue;
      for (const player of lineup.players) {
        player.reportedBattingRates = matchCpblBatterRates(source.batterRates, player, CPBL_TEAM_ID[resolvedGame[`${side}Code`]]);
      }
      lineup.rateStatsCoverage = lineup.players.filter(row => row.reportedBattingRates).length / 9;
      lineup.batterRateEvidence = { sourceEventId: source.batterRates.sourceEventId, fetchedAt: source.batterRates.fetchedAt,
        url: source.batterRates.url, path: source.batterRates.path, year: clean(game.officialDate).slice(0, 4),
        statisticalCutoff: null, providerUpdatedAt: source.batterRates.providerUpdatedAt,
        usage: 'DIAGNOSTIC_ONLY;HITS_AND_AB_NOT_SUPPLIED' };
    }
  }
  ...
  if (league === 'CPBL') {
    for (const row of featureSnapshot.sourceEvidence.features) {
      row.sourceScope = row.featureName.endsWith('.lineup')
        ? { lineupGameIds: row.parsedInput?.asOfProviderGameId ? [String(row.parsedInput.asOfProviderGameId)] : [] }
        : source.featureSourceScopes?.[row.featureName] || null;
      row.sourceBindings = cpblFeatureBindings({ featureName: row.featureName, parsedInput: row.parsedInput, game: resolvedGame,
        events: featureSnapshot.sourceEvidence.events, contents: sourceContents, sourceScope: row.sourceScope });
