...
export function parseNpbPitchingStatsHtml(html) {
  ...
  const headers = rowCells($, table.find('thead tr').last()).map(value => value.toUpperCase());
  const aliases = {
    G: ['G', '登板'], SV: ['SV', 'セーブ'], HLD: ['HLD', 'ホールド'], BF: ['BF', '打者'],
    IP: ['IP', '投球回'], H: ['H', '安打'], BB: ['BB', '四球'], SO: ['SO', '三振'],
    ER: ['ER', '自責点'], ERA: ['ERA', '防御率'],
  };
  const index = name => headers.findIndex(value => (aliases[name] || [name]).includes(value));
  const at = (cells, name) => cells[index(name)];
  const rows = [];
  table.find('tbody tr').each((_, row) => {
    const node = $(row);
    const cells = rowCells($, row);
    const name = clean(node.children('td').first().text()).replace(/^[＊*]/, '');
    const ipCell = node.children('td').eq(index('IP'));
    const innings = baseballInnings(ipCell.find('.integer').text() || at(cells, 'IP'), ipCell.find('.fraction').text());
    const hits = number(at(cells, 'H'));
    const walks = number(at(cells, 'BB'));
    if (!name) return;
    rows.push({
      name,
      throws: node.children('td').first().hasClass('left-hand') ? 'L' : 'R',
...
export function parseCpblGameDetailPayload(payload) {
  ...
  const parseSide = block => {
    ...
    for (const row of lineup) if (!unique.has(number(row.Lineup))) unique.set(number(row.Lineup), {
      id: clean(row?.HitterAcnt), name: clean(row?.HitterName), order: number(row?.Lineup),
    ...
    const pitchers = (Array.isArray(block?.Pitchers) ? block.Pitchers : []).map(row => ({
      id: clean(row?.PitcherAcnt), name: clean(row?.PitcherName), starter: clean(row?.RoleType) === '先發',
...
function detailSide(detail, game, teamId) {
  return Number(game.awayTeamId) === Number(teamId) ? detail.away : detail.home;
}

function projectedLineup(details, teamId, baseline) {
  ...
  return {
    available: true, official: false, projected: true, credibleScenario: true, players,
...
function bullpenSnapshot(details, teamId, leagueId, referenceEra, firstPitch) {
  const rows = details.flatMap(item => {
    const side = detailSide(item.detail, item.game, teamId);
    return (side?.pitchers || []).filter(pitcher => pitcher.starter !== true).map(pitcher => ({ ...pitcher, date: item.game.gameDate }));
  });
  ...
  return {
    available: true, pureRelief: true, usageAvailable: true, qualityScope: 'PURE_RELIEF',
...
function starterSnapshot({ leagueId, game, side, identity, stats, referenceEra, recentStarts = [] }) {
  ...
  if (!(expectedInnings > 0) || !(battersFaced >= 30) || (era == null && number(stats.qualityFactor) == null)) return null;
  ...
  return {
    id: clean(identity.id), teamId: Number(side === 'away' ? game.awayTeamId : game.homeTeamId), name: clean(identity.name),
...
async function cpblDetails(games, options) {
  const settled = await Promise.allSettled(games.map(async game => ({
    game,
    detail: parseCpblGameDetailPayload(await officialFetch(`https://stats.cpbl.com.tw/api/proxy/v1/games/${encodeURIComponent(clean(game.providerGameId))}`, options)),
  })));
  return settled.filter(row => row.status === 'fulfilled').map(row => row.value);
}

function recentStarterRows(details, teamId, name) {
  return details.flatMap(item => (detailSide(item.detail, item.game, teamId)?.pitchers || [])
    .filter(row => row.starter === true && compactName(row.name) === compactName(name)));
}

async function buildNpb(game, history, options) {
  const year = clean(game.officialDate).slice(0, 4);
  const recentGames = selectedRecentGames(history, game);
  ...
  const findStats = side => statRows[side].find(row => compactName(row.name) === compactName(identity[side].name));
...
async function buildKbo(game, history, options) {
  ...
  const [analysis, details, weatherPayload] = await Promise.all([
    ...
    kboDetails(selectedRecentGames(history, game), options, cookie),
...
async function buildCpbl(game, history, options, starterEvidence) {
  ...
  const gamePatch = identity.away && identity.home ? {
    ...
    probableSource: 'CPBL_OFFICIAL_ROSTER_VALIDATED_STARTER_IDENTITY',
  } : {};
...
export async function buildAsianProductionFeatureSnapshot({
  ...
  for (const side of ['away', 'home']) {
    ...
    const recentStarts = identity ? recentStarterRows(source.details, teamId, identity.name) : [];
    featureSnapshot[side] = {
      teamStrength: strengthSnapshot(league, history, teamId),
      starter: starterSnapshot({ leagueId: league, game: resolvedGame, side, identity, stats: source.starterStats?.[side], referenceEra, recentStarts }),
