...
export const ASIAN_PRODUCTION_FEATURES_V1_VERSION = 'ASIAN-OFFICIAL-PIT-PLAYER-PIPELINES-2026-09-v1.0.5';
...
async function buildCpbl(game, history, options, starterEvidence) {
  ...
  const anchor = details.flatMap(row => row.detail?.away?.lineup || []).find(row => /^\d{10}$/.test(String(row.officialPlayerId || row.id || '')));
  ...
  return { details, identity, starterStats, gamePatch, batterRates };
}
