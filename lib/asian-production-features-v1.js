...
function rotationPrediction(details, teamId, firstPitch, leagueId) {
  ...
  for (const item of details) {
    ...
    for (const pitcher of detailSide(item.detail, item.game, teamId)?.pitchers || []) {
      ...
      const key = clean(pitcher.id) || compactName(pitcher.name);
      const existing = grouped.get(key) || {
        id: clean(pitcher.id), name: clean(pitcher.name), throws: pitcher.throws || null, starts: [],
      };
      existing.starts.push({ ...pitcher, gameDate: item.game.gameDate, gamePk: item.game.gamePk });
  ...
  const candidates = [...grouped.values()].map(candidate => {
    ...
    const battersFaced = candidate.starts.reduce((sum, row) => sum + (number(row.battersFaced) || 0), 0)
      || inningsPitched * 4.25;
    const rotationFit = restDays < 3 ? 0.01 : Math.exp(-Math.abs(restDays - targetRestDays) / 1.6);
    return {
