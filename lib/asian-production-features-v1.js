...
export const ASIAN_PRODUCTION_FEATURES_V1_VERSION = 'ASIAN-OFFICIAL-PIT-PLAYER-PIPELINES-2026-09-v1.0.1';
...
async function officialFetch(url, {
  ...
  if (cached && Date.now() - cached.at <= cacheMs) {
    sourceObservations?.push({ url, observedAt: new Date(cached.at).toISOString(), fromCache: true });
    return cached.value;
  }
  ...
  try {
    ...
    RESPONSE_CACHE.set(key, { at: observedAt, value });
    sourceObservations?.push({ url, observedAt: new Date(observedAt).toISOString(), fromCache: false });
...
export function projectedLineup(details, teamId, baseline, battingStats = [], battingStatsObservedAt = null) {
  ...
  return {
    ...
    asOfGamePk: latest.game.gamePk, historicalLineupGameDate: latest.game.gameDate,
...
export async function buildAsianProductionFeatureSnapshot({
  ...
  const options = { fetchImpl, timeoutMs, sourceObservations };
  ...
  const observedAt = sourceObservations.map(row => row.observedAt).sort().at(-1) || new Date().toISOString();
