...
function snapshotCoverage(snapshot) {
  const openGameCount = Number(snapshot?.matchedGameCount || snapshot?.games?.length || 0);
  const totalGameCount = Number(snapshot?.scheduleGameCount || snapshot?.rawGameCount || openGameCount);
  const unopenedGameCount = Number(snapshot?.unopenedGameCount ?? Math.max(0, totalGameCount - openGameCount));
  const capturedGameCount = Math.min(totalGameCount, Math.max(Number(snapshot?.rawGameCount || 0), openGameCount));
  const identityBlockedGameCount = Number(snapshot?.identityBlockedGameCount || 0);
  const lockedGameCount = identityBlockedGameCount
    ? (snapshot.unopenedGames || []).filter(row => !row.unavailableReason).length
    : Math.max(0, capturedGameCount - openGameCount);
  const notRenderedGameCount = identityBlockedGameCount
    ? (snapshot.unopenedGames || []).filter(row => row.unavailableReason === 'not-rendered-by-reader').length
    : Math.max(0, totalGameCount - capturedGameCount);
  return {
    openGameCount,
    totalGameCount,
    unopenedGameCount,
...
export function readerSnapshotStatus(snapshot, now = Date.now(), expectedLeague = snapshot?.league) {
  ...
  const waitingMessage = [
    snapshot.identityBlockedGameCount ? `場次待核對 ${snapshot.identityBlockedGameCount} 場` : '',
    lockedGameCount ? `鎖盤等待 ${lockedGameCount} 場` : '',
    notRenderedGameCount ? `Reader 未呈現 ${notRenderedGameCount} 場` : '',
  ...
  return {
    ...
    message: fresh
      ? `${league} Tai888 Reader 已讀取 ${capturedGameCount}/${totalGameCount} 場｜已開盤 ${openGameCount} 場｜${waitingMessage}`
...
export function readerSnapshotPublicView(snapshot, {
  ...
  const { lockedGameCount, notRenderedGameCount } = snapshotCoverage(snapshot);
  ...
  const status = executable ? rawStatus : {
    ...
    message: snapshot && rawStatus.available && !complete
      ? `${league} Tai888 Reader 尚未送出完整新盤；請確認 Reader 電腦、Chrome 與 Tai888 分頁保持開啟，重新送盤後網站會自動恢復`
      : rawStatus.message,
  };
  ...
  return {
    ...
    unopenedGameCount: executable ? snapshot.unopenedGameCount : 0,
    lockedGameCount: executable ? lockedGameCount : 0,
    notRenderedGameCount: executable ? notRenderedGameCount : 0,
    ...
    identityBlockedGameCount: executable ? snapshot.identityBlockedGameCount || 0 : 0,
    // Read-only per-game coverage lets waiting cards recover on status polls.
    // Do not expose prices or signed execution authority through this endpoint.
    gameAvailability: executable ? [...(snapshot.games || []), ...(snapshot.unopenedGames || [])].map(row => ({
      game: {
        league,
        gamePk: row.gamePk,
        ...
        gameNumber: row.game?.gameNumber,
      },
      marketCoverage: {
        openMarkets: row.marketCoverage?.openMarkets ?? new Set((row.markets || []).map(market => market.market)).size,
        availableMarkets: row.marketCoverage?.availableMarkets || [...new Set((row.markets || []).map(market => market.market))],
