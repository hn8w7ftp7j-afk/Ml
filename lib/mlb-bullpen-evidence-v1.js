...
export function bullpenUsageCoverage({ recentFeeds = [], expectedRecentGames, teamId, gameDate, playerIds }) {
  ...
  const games = expected.filter(row => { const key = String(row.gamePk); if (seen.has(key)) return false; seen.add(key); return true; }).map(row => {
    ...
    const missingPlayers = valid ? ids.filter(id => !Array.isArray(playerIds) || playerIds.includes(Number(id))).filter(id => count(team.players?.[`ID${id}`]?.stats?.pitching?.numberOfPitches) == null)
