...
const BACKGROUND_ANALYSIS_AUTH_VERSION = 'BACKGROUND-ANALYSIS-HMAC-v1';
const BACKGROUND_ANALYSIS_MAX_AGE_MS = 24 * 60 * 60 * 1000;
...
export async function createBackgroundAnalysisAuthorization(body, issuedAt = Date.now()) {
  const secret = sessionSecret();
  if (!secret) throw new Error('SESSION_SECRET is not configured');
  const timestamp = Math.trunc(Number(issuedAt));
  const bodyHash = base64url(await crypto.subtle.digest('SHA-256', encoder.encode(JSON.stringify(body))));
  const signature = await hmac(secret, `${BACKGROUND_ANALYSIS_AUTH_VERSION}\n${timestamp}\n${bodyHash}`);
  return { timestamp: String(timestamp), signature };
}

export async function verifyBackgroundAnalysisAuthorization(request, body, now = Date.now()) {
  const secret = sessionSecret();
  const timestampText = String(request?.headers?.get('x-background-analysis-time') || '');
  const signature = String(request?.headers?.get('x-background-analysis-signature') || '');
  const timestamp = Number(timestampText);
  const age = Number(now) - timestamp;
  if (!secret || !Number.isInteger(timestamp) || !signature || age < -60_000 || age > BACKGROUND_ANALYSIS_MAX_AGE_MS) return false;
  const bodyHash = base64url(await crypto.subtle.digest('SHA-256', encoder.encode(JSON.stringify(body))));
  const expected = await hmac(secret, `${BACKGROUND_ANALYSIS_AUTH_VERSION}\n${timestamp}\n${bodyHash}`);
...
export function sessionSecretConfigured() {
  return Boolean(sessionSecret().trim());
}

export function siteAuthConfigured() {
  return appPasswordConfigured() && sessionSecretConfigured();
}
...
export async function requestIsAuthenticated(request) {
  if (!siteAuthConfigured()) return false;
...
export async function requireApiAuth(request) {
  if (!siteAuthConfigured()) {
    return new Response(JSON.stringify({ ok: false, error: '網站驗證尚未完整設定：APP_PASSWORD 與 SESSION_SECRET 都是必要設定' }), {
      status: 503,
