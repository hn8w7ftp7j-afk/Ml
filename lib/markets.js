...
function positiveScore(ev) {
  if (ev < 0.02) return 5 + ev * 40;
  if (ev < 0.04) return 5.8 + (ev - 0.02) * 40;
  if (ev < 0.06) return 6.6 + (ev - 0.04) * 30;
  if (ev < 0.08) return 7.2 + (ev - 0.06) * 40;
  if (ev < 0.11) return 8 + (ev - 0.08) * (0.5 / 0.03);
  if (ev < 0.15) return 8.5 + (ev - 0.11) * 12.5;
  if (ev < 0.21) return 9 + (ev - 0.15) * (0.4 / 0.06);
  return 9.4 + Math.min(0.2, (ev - 0.21) * 1.5);
}

function scoreCap(confidence) {
  const c = clamp(Number(confidence) || 0.75, 0.35, 1);
  if (c < 0.55) return 6.5;
  if (c < 0.65) return 6.9;
  if (c < 0.75) return 7.6;
  if (c < 0.85) return 8.3;
  if (c < 0.92) return 8.8;
  if (c < 0.97) return 9.2;
  return 9.6;
}
...
export function scoreFromEV(weightedEV, confidence = 0.75, options = {}) {
  ...
  let cap = scoreCap(confidence);
  if (options?.integrityWarning || options?.distributionInvalid) cap = Math.min(cap, 6.8);
  if (robustEV <= 0) cap = Math.min(cap, 5.9);
  else if (robustEV < 0.02) cap = Math.min(cap, 6.5);
  else if (robustEV < 0.05) cap = Math.min(cap, 7.1);
  else if (robustEV < 0.08) cap = Math.min(cap, 8.4);
  return clamp(Math.min(raw, cap), 1, 9.6);
}
...
export function parseTaiwanLine(pick) {
  ...
  const validDirection = isTotal ? isOver || isUnder : (isGiving || isReceiving) && Boolean(team);

  return {
    raw,
    valid: legs.length > 0 && validDirection,
    isTotal,
    isOver,
    isUnder,
    isGiving,
...
function teamMatches(candidate, official) {
  const left = normalizeTeamName(candidate);
  const right = normalizeTeamName(official);
  if (!left || !right) return false;
  if (left === right) return true;
  const shorter = Math.min(left.length, right.length);
  return shorter >= 2 && (left.includes(right) || right.includes(left));
}
...
export function outcomeFractionForScore(pick, awayRuns, homeRuns, awayName = '', homeName = '') {
  ...
  if (!p.isTotal) {
    const isAway = teamMatches(p.team, awayName);
    const isHome = teamMatches(p.team, homeName);
...
export function validateMarketPair(market, directions) {
  ...
  if (ds.length === 2 && ds[0]?.pick && ds[1]?.pick) {
    ...
    if (market.includes('大小')) {
      ...
      if (a.valid && b.valid && normalizeTeamName(a.team) === normalizeTeamName(b.team)) errors.push('讓分盤兩邊不可是同一隊');
    }
  }
