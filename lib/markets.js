...
export function scoreFromCompositeEV(conservativeEV, options = {}) {
  ...
  let score = 5.25;
  score += 0.90 * smooth(weightedEV, 0.045);
  score += 0.95 * smooth(robustEV, 0.033);
  score += 0.55 * smooth(conservative, 0.028);
  score += 0.45 * edgeStrength;
  score += 0.45 * ((stability - 0.5) * 2);
  score += 0.32 * ((quality - 0.70) / 0.30);
  score -= 0.42 * flipProbability;
  score -= Math.max(0, weightedEV - conservative) * 1.4;
  ...
  if (integrityWarning || waterEstimated) cap = 6.6;
    ...
    if (weightedEV < 0.02 || robustEV < 0.008) cap = Math.min(cap, 7.4);
    if (weightedEV < 0.04 || robustEV < 0.02) cap = Math.min(cap, 7.9);
    if (weightedEV < 0.07 || robustEV < 0.04) cap = Math.min(cap, 8.4);
  ...
  // Unknown non-core inputs are represented in the scenario range and quality.
  // They are not given an automatic fixed deduction or hard ceiling.
  let floor = 3.5;
...
export function parseTaiwanLine(pick) {
  ...
  if (!lineMatch) {
    return { raw, valid: false, isTotal: false, isOver: false, isUnder: false, isGiving: false, isReceiving: false, team: '' };
  }

  const lineText = lineMatch[1];
  const modifier = lineMatch[2] || '';
  const prefix = raw.slice(0, lineMatch.index);
  ...
  return {
    raw,
    valid: legs.length > 0 && (isTotal ? (isOver || isUnder) : (isGiving || isReceiving)) && Boolean(isTotal || team),
    isTotal,
    isOver,
    isUnder,
    isGiving,
...
/**
 * Taiwan hole-code convention used by this project:
 * +50 at the exact line = the giving/over side wins 50%;
 * -30 at the exact line = the giving/over side loses 30%.
 * The receiving/under side is the exact negative branch.
 */
function exactModifierOutcome(modifier, positiveSide) {
  ...
  const givingOrOver = sign === '+' ? fraction : -fraction;
  return positiveSide ? givingOrOver : -givingOrOver;
}
...
export function outcomeFractionForScore(pick, awayRuns, homeRuns, awayName = '', homeName = '') {
  ...
  let chosenMargin = 0;
  if (!parsed.isTotal) {
    const team = normalizeTeamName(parsed.team);
    const awayTeam = normalizeTeamName(awayName);
    const homeTeam = normalizeTeamName(homeName);
    const isAway = team && awayTeam && (awayTeam.includes(team) || team.includes(awayTeam));
    const isHome = team && homeTeam && (homeTeam.includes(team) || team.includes(homeTeam));
    if ((isAway && isHome) || (!isAway && !isHome)) return null;
    chosenMargin = isAway ? away - home : home - away;
  }

  const settleLeg = line => {
    if (parsed.isTotal) {
      const delta = parsed.isOver ? total - line : line - total;
      if (Math.abs(delta) < 1e-9) return exactModifierOutcome(parsed.modifier, parsed.isOver);
      return settleStandard(delta);
    }
    if (parsed.isGiving) {
      const delta = chosenMargin - line;
      if (Math.abs(delta) < 1e-9) return exactModifierOutcome(parsed.modifier, true);
      return settleStandard(delta);
    }
    const delta = chosenMargin + line;
    if (Math.abs(delta) < 1e-9) return exactModifierOutcome(parsed.modifier, false);
    return settleStandard(delta);
  };

  return parsed.legs.reduce((sum, leg) => sum + settleLeg(leg), 0) / parsed.legs.length;
}
...
export function calculateProfit({ stake, water, fraction, rebateRate = 0.015 }) {
  const principal = Math.max(0, Number(stake) || 0);
  const price = normalizeWater(water);
  const resultFraction = clamp(Number(fraction) || 0, -1, 1);
  if (Math.abs(resultFraction) < 1e-9 || principal === 0) return { profit: 0, rebate: 0, settledAmount: 0 };

  const settledAmount = principal * Math.abs(resultFraction);
  const rebate = settledAmount * Math.max(0, Number(rebateRate) || 0);
  const profit = resultFraction > 0 ? settledAmount * price + rebate : -settledAmount + rebate;
...
export function normalizeVisionGame(raw, scheduleGame = null, defaultWater = null) {
  ...
  const markets = marketMap.map(([market, value, standardFallback]) => {
    ...
    if (market.includes('大小')) {
      ...
      if (!line) return { market, directions: [{ pick: '', water: null, confidence: 0 }, { pick: '', water: null, confidence: 0 }] };
    ...
    if (!line || !favorite || !underdog) return { market, directions: [{ pick: '', water: null, confidence: 0 }, { pick: '', water: null, confidence: 0 }] };
    const favoriteActual = hasActualWater(value?.favoriteWater);
    const underdogActual = hasActualWater(value?.underdogWater);
    const estimateBoth = !favoriteActual && !underdogActual;
    return {
      market,
      directions: [
        { pick: `${favorite}讓${line}`, ...visionWater(value?.favoriteWater, fallback, estimateBoth), confidence: clamp(Number(value?.confidence || 0), 0, 1) },
        { pick: `${underdog}受讓${line}`, ...visionWater(value?.underdogWater, fallback, estimateBoth), confidence: clamp(Number(value?.confidence || 0), 0, 1) },
      ],
    };
  });
