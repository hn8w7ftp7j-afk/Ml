...
export function validateMarketPair(market, directions) {
  ...
  for (const d of ds) {
    ...
    if (d?.water == null || String(d.water).trim() === '' || !Number.isFinite(Number(d.water))) errors.push('水位不可空白');
    else if (Number(d.water) < 0.5 || Number(d.water) > 1.5) errors.push('水位範圍應為 0.500～1.500');
  }
