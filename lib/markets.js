...
export function normalizeVisionGame(raw, scheduleGame = null, defaultWater = null) {
  ...
  const markets = marketMap.map(([market, value, standardFallback]) => {
    ...
    if (!line || !favorite || !underdog || !plausibleMarketLine(market, `${favorite}讓${line}`)) return { market, directions: [{ pick: '', water: null, confidence: 0, integrityError: line ? '盤口數值疑似辨識錯欄' : '' }, { pick: '', water: null, confidence: 0, integrityError: line ? '盤口數值疑似辨識錯欄' : '' }] };
