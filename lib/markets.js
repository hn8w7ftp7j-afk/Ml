...
export function scoreFromEV(weightedEV, confidence = 0.75) {
  ...
  const raw = ev >= 0 ? 5 + ev * 50 * c : 5 + ev * 30;
  return clamp(raw, 1, 9.6);
}
...
export function parseTaiwanLine(pick) {
  const raw = String(pick || '').replace(/\s+/g, '');
...
export function outcomeFractionForScore(pick, awayRuns, homeRuns, awayName = '', homeName = '') {
  ...
  const total = Number(awayRuns) + Number(homeRuns);
  let chosenMargin = 0;
  if (!p.isTotal) {
    ...
    const isAway = away && (away.includes(normalizedTeam) || normalizedTeam.includes(away));
    const isHome = home && (home.includes(normalizedTeam) || normalizedTeam.includes(home));
    if (!isAway && !isHome) return null;
    chosenMargin = isAway ? Number(awayRuns) - Number(homeRuns) : Number(homeRuns) - Number(awayRuns);
  }
...
export function priceCLV(openWater, closeWater) {
  const openBE = breakEvenProbability(openWater);
  const closeBE = breakEvenProbability(closeWater);
  return closeBE - openBE;
}
...
export function validateMarketPair(market, directions) {
  ...
  const errors = [];
  if (ds.length !== 2) errors.push('必須有兩個方向');
  for (const d of ds) {
    if (!d?.pick) errors.push('方向＋盤口不可空白');
    if (!parseTaiwanLine(d?.pick).valid) errors.push(`盤口格式無法辨識：${d?.pick || '空白'}`);
    if (!Number.isFinite(Number(d?.water))) errors.push('水位不可空白');
  }
...
export function normalizeVisionGame(raw, scheduleGame = null, defaultWater = 0.95) {
  const away = scheduleGame?.away || String(raw?.away || '');
  const home = scheduleGame?.home || String(raw?.home || '');
  const marketMap = [
  ...
  const markets = marketMap.map(([market, value]) => {
    if (market.includes('大小')) {
      ...
      return {
        market,
        directions: [
          { pick: line ? `大${line}` : '', water: normalizeWater(value?.overWater, defaultWater), confidence: Number(value?.confidence || 0) },
          { pick: line ? `小${line}` : '', water: normalizeWater(value?.underWater, defaultWater), confidence: Number(value?.confidence || 0) },
        ],
      };
    }
    const line = String(value?.line || '');
    ...
    return {
      market,
      directions: [
        { pick: line && favorite ? `${favorite}讓${line}` : '', water: normalizeWater(value?.favoriteWater, defaultWater), confidence: Number(value?.confidence || 0) },
        { pick: line && underdog ? `${underdog}受讓${line}` : '', water: normalizeWater(value?.underdogWater, defaultWater), confidence: Number(value?.confidence || 0) },
      ],
    };
  });
