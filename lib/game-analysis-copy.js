...
export function buildGameAnalysisCopy(root, { game, analysis, persistence, appVersion, retained = false, copiedAt = new Date().toISOString() }) {
  ...
  return [
    ...
    '以下 currentUsagePolicy 是複製時網站的使用狀態，不改寫當時分析、QA、S／W／R 或原下注紀錄。全場大分正常保留並持續觀察，仍依原有門檻與 QA 判定；不代表已驗證為正 EV。',
    ...
    JSON.stringify((analysis.results || []).map(row => ({
      ...
      currentUsagePolicy: (() => { const policy = marketResearchPolicy(row, game); return policy ? { id: policy.id, label: policy.label, candidateEligible: policy.candidateEligible } : null; })(),
    })), null, 2),
