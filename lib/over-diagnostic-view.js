...
export function signedPercentText(value, digits = 2) {
  return typeof value === 'number' && Number.isFinite(value) ? `${value > 0 ? '+' : ''}${percentText(value, digits)}` : '未提供';
}
...
export function frozenCohortSummary(raw = {}) {
  // Identify the frozen final stage by its stable ID, never by array position or live eligibility.
  const selected = raw.funnel?.stages?.find(row => row.label === 'SIMULATED_SELECTED');
  const selectedN = count(selected?.n), settlementN = count(selected?.nSettlementEvaluated);
  const stakeUnits = finite(selected?.stakeUnits), archivedNet = finite(selected?.netUnits);
  const hasSettlement = settlementN !== null && settlementN > 0;
  const netUnits = hasSettlement ? archivedNet : null;
  const wins = hasSettlement ? count(selected?.winsBeforeRebate) : null;
  const losses = hasSettlement ? count(selected?.lossesBeforeRebate) : null;
  const pushes = hasSettlement ? count(selected?.pushesBeforeRebate) : null;
  const winLossN = wins !== null && losses !== null ? wins + losses : null;
  const dates = [...new Set((raw.games || []).map(row => row.gameDate).filter(value => typeof value === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(value)))].sort();
  return {
    available: Boolean(selected), selectedN, selectedDateCount: count(selected?.nDates),
    modelW: selectedN > 0 ? finite(selected?.meanW) : null,
    modelR: selectedN > 0 ? finite(selected?.meanR) : null,
    settlementN, missingOutcomeN: count(selected?.nMissingOutcome),
    stakeUnits, netUnits,
    // A settled result's denominator is its recorded stake, not all computable games.
    roi: hasSettlement && netUnits !== null && stakeUnits > 0 ? netUnits / stakeUnits : null,
    wins, losses, pushes, winLossN,
    winRateExcludingPush: winLossN > 0 ? wins / winLossN : null,
    bias: finite(selected?.bias),
    fromDate: dates.at(0) ?? null, toDate: dates.at(-1) ?? null, dateCount: dates.length,
    modelVersion: raw.manifest?.modelVersion ?? null,
    diagnosticVersion: raw.manifest?.diagnosticVersion ?? null,
    createdAt: raw.manifest?.createdAt ?? null,
  };
}
export function diagnosticView(raw) {
  ...
  return { ...raw, games, frozenSummary: frozenCohortSummary(raw), calculationChain, pitchingAllocation, funnel: { ...raw.funnel, stages, transitions }, coverage: { groups } };
}
