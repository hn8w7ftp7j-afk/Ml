...
import { readerGameMarketContentHash } from './reader-market-revision-v110.js';
...
export async function verifyCloudBetEvidenceV110(candidate, {
  ...
  const currentGameMarketHash = readerGameMarketContentHash(readerGame.markets);
  ...
  if (snapshotId && inputHash) {
    try {
      const replay = await loadPitReplay({ league, snapshotId, expected: { leagueId: league, gamePk, inputHash } });
      ...
      const latestPit = await loadLatestPitIdentity({ league, gamePk });
