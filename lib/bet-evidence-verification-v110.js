...
import { verifyArchivedMarketEvidence } from './market-integrity-v1.js';
...
export const BET_EVIDENCE_VERIFICATION_V110_VERSION = 'BASEBALL-BET-EVIDENCE-SERVER-VERIFICATION-v11.8.35';
...
export async function verifyCloudBetEvidenceV110(candidate, {
  ...
  let pitErrorCode = 'PIT_SNAPSHOT_NOT_PROVIDED';
  if (snapshotId && inputHash) {
    try {
      ...
      // The browser may hold either the immutable PIT capture or the latest
      // whole-board capture installed after an unrelated game moved. Both are
      // server-authenticated evidence roots. The per-game content hash above
      // still has to match the current Reader game exactly, so a move in this
      // game remains fail-closed while a harmless full-board revision does not
      // disable recording.
      const captureMatchesCurrentBoard = capturedPayloadHash === clean(snapshot.payloadHash).toLowerCase()
        && capturedRawBoardHash === clean(snapshot.rawBoardHash).toLowerCase();
      const captureMatchesPitBoard = capturedPayloadHash === clean(supplied.readerPayloadHash).toLowerCase()
        && capturedRawBoardHash === clean(supplied.readerRawBoardHash).toLowerCase();
      if ((!captureMatchesCurrentBoard && !captureMatchesPitBoard)
        || clean(supplied?.readerBoardDate) !== date) {
        throw new Error('下注畫面Reader證據與不可變PIT擷取版本不一致');
      }
      const authenticatedRows = replay?.marketAnalysis?.authenticatedSuppliedMarkets;
      const authenticated = exactDirection(authenticatedRows, candidate);
      if (authenticatedRows !== undefined && (!Array.isArray(authenticatedRows) || !authenticated)) {
        throw Object.assign(new Error('PIT缺少對應的原始簽章盤口'), { code: 'PIT_AUTHENTICATED_MARKET_MISSING' });
      }
      const signatureCheck = await verifyArchivedMarketEvidence(league, officialGame, supplied, authenticated);
      if (!signatureCheck.verified) {
        throw Object.assign(new Error(`PIT成交盤缺少有效伺服器簽章（${signatureCheck.code}）`), { code: signatureCheck.code });
      }
      ...
      pit = {
        ...
        dataVersion: clean(replay?.versions?.dataVersion),
        dataQualityReceipt: replay?.marketAnalysis?.dataQualityReceipt || null,
      ...
      pitErrorCode = clean(error?.code) || 'PIT_REPLAY_FAILED';
    }
  }
