import { inspectFeatureMapping } from './cpbl-feature-evidence-v1.js';
...
export function auditAsianSourceEvidence(context = {}) {
  ...
  const rows = required.map(featureName => {
    ...
    const mapping = inspectFeatureMapping(row, id => { const event = events.get(id); const content = contents[event?.contentHash]; return gunzipSync(Buffer.from(content.data, 'base64'), { maxOutputLength: 16 * 1024 * 1024 }).toString(); });
    const input = row?.parsedInput;
    const availability = input?.source === 'UNAVAILABLE' ? 'NOT_CONNECTED' : input?.available === false ? 'MISSING' : input?.projected === true || input?.projectionBased === true ? 'PROJECTED' : input == null ? 'UNKNOWN' : 'REPORTED';
    return { featureName, availability, mapping, sourceBindings: row?.sourceBindings || [], derivationVersion: row?.derivationVersion || null, requiredFeatures: row?.requiredFeatures || [], verificationObject: availability === 'NOT_CONNECTED' ? 'CONTEXT_DEFAULT_ONLY;NO_SOURCE_QUERY_EVIDENCE' : 'BOUND_SOURCE_CONTENT_AND_ACQUISITION_TIMING', status: contradictions.length ? 'FAILED' : missing.length ? 'PENDING' : 'VERIFIED',
      reasons: [...new Set([...contradictions, ...missing])], sourceEventIds: row?.sourceEventIds || [], fieldTraceabilityStatus: 'PENDING', fieldTraceabilityReason: 'RAW_FIELD_TO_TRANSFORM_REPLAY_NOT_VALIDATED', dependencyScope: row?.dependencyScope || null, sourcePaths: row?.sourcePaths || [] };
