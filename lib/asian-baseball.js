...
export function parseNpbGameDetailHtml(html, expectedGame) {
  ...
  $('#gmdivresult tr').each((_, element) => {
    ...
    if (!team) return;
    const cells = row.find('td.gmscore').map((__, cell) => cleanText($(cell).text())).get();
    const separator = cells.findIndex(value => value === '-');
    if (separator < 5 || cells.length < separator + 4) return;
    const inningScores = cells.slice(0, separator).map(npbInningScore);
    const runs = numericScore(cells[separator + 1]);
    if (inningScores.some(value => value == null) || runs == null) return;
    rows.push({ team, inningScores, runs });
  });
  ...
  if (!finalClockConfirmed || rows.length !== 2) return game;
  const away = rows.find(row => row.team.code === game.awayCode);
  const home = rows.find(row => row.team.code === game.homeCode);
  if (!away || !home) {
