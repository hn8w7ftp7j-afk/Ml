...
function normalizeStarter(base, supplied, probable, teamId) {
  ...
  const performanceReady = supplied?.performanceAvailable === true
    ...
    && (traditionalPerformance || componentPerformance)
...
export async function buildAsianGameContext(value, game, {
  ...
  const away = teamContext(history, resolvedGame.awayTeamId, baseline, resolvedGame.gameDate, config.modelConfig, {
  ...
  const home = teamContext(history, resolvedGame.homeTeamId, baseline, resolvedGame.gameDate, config.modelConfig, {
  ...
  const kboRules = leagueId === 'KBO' ? kboRuleState(resolvedGame, features, parkState) : null;
  ...
  const rows = [
    ...
    { name: 'officialStarterHandedness', ready: both('starterHandedness'), status: both('starterHandedness') ? 'CONFIRMED' : 'MISSING', core: true },
  ...
  return {
    ...
    featureProvenance: [
      ...
      { feature: '先發身分', status: starterIdentityConfirmed ? '已確認' : '缺失', source: starterIdentityConfirmed ? (resolvedGame.probableSource || away.starter.source || home.starter.source) : '官方或經驗證先發來源' },
