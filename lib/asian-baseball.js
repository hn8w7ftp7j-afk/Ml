...
export const ASIAN_BASEBALL_VERSION = 'ASIAN-BASEBALL-OFFICIAL-PROVIDERS-2026-08-v2.1.1';
export const ASIAN_SCHEDULE_VERSION = 'ASIAN-OFFICIAL-SCHEDULE-2026-08-v1.1.0';
export const ASIAN_CONTEXT_VERSION = 'ASIAN-SHADOW-CONTEXT-2026-08-v2.1.2';
...
export function parseNpbScheduleHtml(html, date) {
  ...
  $('.unit').each((index, element) => {
    ...
    const game = normalizedGame({ league: 'NPB', sourceId, date, time, awayTeam, homeTeam, venue, status, awayScore, homeScore, gameNumber });
...
export async function buildAsianGameContext(value, game, {
  ...
  const warnings = [
    ...
    ...(productionFeatureError || features?.pipelineError
      ...
      : []),
    ...(projectedCount ? [`OFFICIAL_NOT_YET_PUBLISHED｜${projectedCount}項使用賽前預測或中性先驗；模型實際使用與不確定性處理見逐項資料稽核`] : []),
