...
export async function buildAsianGameContext(value, game, {
  ...
  return {
    ...
    sourceStatuses: {
      starterAssignments: ['away', 'home'].map(side => `${side === 'away' ? '客' : '主'}：${/PROJECTED|FORECAST/i.test(String(({ away, home })[side].starter.assignmentStatus || '') + String(({ away, home })[side].starter.identitySource || '')) ? '輪值推估' : ({ away, home })[side].starter.identitySource === 'CPBL_OFFICIAL_CURRENT_GAME_STARTER' ? '官方當場先發' : '尚未確認官方指派'}`).join('／'),
