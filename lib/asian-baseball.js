...
export const ASIAN_CONTEXT_VERSION = 'ASIAN-SHADOW-CONTEXT-2026-08-v1.1.0';
...
const SOURCE_CONFIG = Object.freeze({
  NPB: Object.freeze({
    ...
    label: 'NPB 官方 BIS',
    zoneOffset: '+09:00',
    modelVersion: 'NPB-SHADOW-JOINT-SCORE-2026-08-v1.1.0',
    rulesVersion: 'NPB-TW-SHADOW-RULES-2026-08-v1.1.0',
    baselineRuns: 3.75,
  ...
  CPBL: Object.freeze({
    ...
    zoneOffset: '+08:00',
    modelVersion: 'CPBL-SHADOW-JOINT-SCORE-2026-08-v1.1.0',
    rulesVersion: 'CPBL-TW-SHADOW-RULES-2026-08-v1.1.0',
    baselineRuns: 4.45,
    modelConfig: Object.freeze({
...
function teamContext(history, teamId, baseline, gameDate, modelConfig) {
  ...
  return {
    ...
    starter: {
      ...
      confirmed: false,
      handedness: null,
      expectedInnings: Number(modelConfig?.neutralStarterInnings) || 5,
      projectionMode: 'NEUTRAL_UNKNOWN_STARTER',
    },
    lineup: { official: false, projected: false, handednessAdjusted: false, offensiveIndex: 1, catcher: null, status: 'MISSING' },
    bullpen: {
...
export async function buildAsianGameContext(value, game, {
  ...
  // Unknown information may be scenario-modelled only after there is a real
  // projected input. The current Asian adapter has no structured starter,
  // projected-lineup, or bullpen source at all; treating its fixed neutral
  // defaults as a scenario would make every game look artificially alike.
  ...
  const missingStateInputs = [
    'probableStartersAndHandedness',
    'officialOrProjectedLineups',
    'pureBullpenUsage',
  ];
  const dataGateV10 = {
    version: 'ASIAN-SHADOW-STATE-GATE-2026-08-v1.2.0',
    rows: [
      ...
      { name: 'probableStartersAndHandedness', status: 'MISSING', core: false },
      { name: 'officialOrProjectedLineups', status: 'MISSING', core: false },
      { name: 'pureBullpenUsage', status: 'MISSING', core: false },
      { name: 'ninthInningWalkoffAndDrawRules', status: 'CONFIRMED', core: true },
    ],
    missing: missingStateInputs,
    projected: ['neutralStarterInnings', 'neutralLineup', 'bullpenUncertainty'],
    ...
    quality: coreReady ? 0.64 : 0.50,
    modelErrorMarginEV: 0.035,
  };
  const warnings = [
    'EXPERIMENTAL_SHADOW｜亞洲聯盟尚未完成跨球季正式校準，所有分數不可下注',
    '核心先發、預估打線與逐投手牛棚來源尚未接入；禁止以固定中性值產生W/R或影子分數',
  ];
  return {
