...
export const ASIAN_BASEBALL_VERSION = 'ASIAN-BASEBALL-OFFICIAL-PROVIDERS-2026-09-v2.1.2';
export const ASIAN_SCHEDULE_VERSION = 'ASIAN-OFFICIAL-SCHEDULE-2026-09-v1.1.1';
export const ASIAN_CONTEXT_VERSION = 'ASIAN-SHADOW-CONTEXT-2026-09-v2.1.3';
...
function predictedTeamContext(team, supplied, modelConfig, leagueId, allowPrediction) {
  ...
  const candidateExpectedInnings = candidates.reduce((sum, candidate) => (
    sum + Number(candidate.probability) * (finiteNumber(candidate.expectedInnings) > 0 ? Number(candidate.expectedInnings) : neutralStarterInnings)
  ), 0);
  const candidateQuality = candidates.reduce((sum, candidate) => sum + candidate.probability * (candidate.qualityFactor ?? 1), 0);
