...
function npbExpectedTeamCodes(date, options = {}) {
  ...
  const providerHint = providerParts.length >= 3 && providerParts[0] === date
    ...
    : null;
  const awayTeam = resolveTeam('NPB', options.expectedAway);
  const homeTeam = resolveTeam('NPB', options.expectedHome);
  if (providerHint && awayTeam && providerHint.awayCode !== awayTeam.code) {
    throw providerError('NPB 保存的客隊身分互相衝突', 409, 'OFFICIAL_IDENTITY_MISMATCH');
  }
  if (providerHint && homeTeam && providerHint.homeCode !== homeTeam.code) {
    throw providerError('NPB 保存的主隊身分互相衝突', 409, 'OFFICIAL_IDENTITY_MISMATCH');
  }
...
function resolveLegacyNpbGame(games, requestedGamePk, date, options = {}) {
  const expected = npbExpectedTeamCodes(date, options);
  if (!expected) return null;
  const candidates = games.filter(game => game.officialDate === date
    && game.awayCode === expected.awayCode
    && game.homeCode === expected.homeCode);
  if (candidates.length > 1) {
    throw providerError('NPB 同日同隊存在多場賽事，無法唯一核對舊場次識別', 409, 'OFFICIAL_IDENTITY_AMBIGUOUS');
  }
...
export async function fetchAsianFinalResult(value, gamePk, date, options = {}) {
  ...
  if (!game && league === 'NPB') game = resolveLegacyNpbGame(games, id, date, options);
