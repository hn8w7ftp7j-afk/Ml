import { externalizeAsianSourceEvidence, hydrateAsianSourceEvidence } from './asian-source-store-v1.js';
import { auditAsianSourceEvidence, captureAsianSource } from './asian-source-evidence-v1.js';
...
import { parseKboResultLinescore } from './kbo-result-linescore.js';

export const ASIAN_BASEBALL_VERSION = 'ASIAN-BASEBALL-OFFICIAL-PROVIDERS-2026-09-v2.2.1';
export const ASIAN_SCHEDULE_VERSION = 'ASIAN-OFFICIAL-SCHEDULE-2026-09-v1.1.2';
export const ASIAN_CONTEXT_VERSION = 'ASIAN-SHADOW-CONTEXT-2026-09-v2.2.1';
...
function statusFields(status) {
  ...
  if (value === 'RESERVED') {
    return { status: '保留比賽', statusEnglish: value, statusCode: 'D' };
  }
...
function npbSourceId(href, date) {
  const id = String(href || '').match(/\/(s\d+)\.html/i)?.[1] || '';
  const sourceDate = id.match(/^s(\d{8})\d+$/i)?.[1];
  if (sourceDate && sourceDate !== date.replaceAll('-', '')) {
    throw providerError('NPB 官方場次識別碼與賽程日期不一致', 409, 'OFFICIAL_IDENTITY_MISMATCH');
  }
  return id;
}

function normalizeNpbGameNumbers(rows) {
  const identities = new Map();
  for (const game of rows) {
    const previous = identities.get(game.providerGameId);
    if (previous && (previous.officialDate !== game.officialDate
      || previous.awayTeamId !== game.awayTeamId || previous.homeTeamId !== game.homeTeamId)) {
      throw providerError('NPB 官方場次識別碼對應到衝突的球隊', 409, 'OFFICIAL_IDENTITY_MISMATCH');
    }
    identities.set(game.providerGameId, game);
  }
  const games = uniqueGames(rows);
  const matchups = new Map();
  for (const game of games) {
    const key = `${game.officialDate}|${game.awayCode}|${game.homeCode}`;
    if (!matchups.has(key)) matchups.set(key, []);
    matchups.get(key).push(game);
  }
  for (const matchup of matchups.values()) {
    // "Game 20" on an NPB card is the season-series meeting, not the
    // twentieth game of a doubleheader. Only distinct same-day source IDs
    // count as separate games; responsive duplicates were removed above.
    matchup.sort((left, right) => Date.parse(left.gameDate) - Date.parse(right.gameDate)
      || left.providerGameId.localeCompare(right.providerGameId));
    matchup.forEach((game, index) => {
      game.gameNumber = index + 1;
      game.doubleHeader = matchup.length > 1 ? 'Y' : 'N';
    });
  }
...
export function parseNpbScheduleHtml(html, date) {
  ...
  $('.unit').each((_, element) => {
    ...
    const hasSideContainers = unit.find('.team_left, .team_right').length > 0;
    const homeTeam = resolveTeam('NPB', hasSideContainers ? unit.find('.team_left .team_name').first().text() : teams[0]);
    const awayTeam = resolveTeam('NPB', hasSideContainers ? unit.find('.team_right .team_name').first().text() : teams[1]);
    ...
    const status = middleParts.find(value => /POSTPON|CANCEL|SUSPEND|NO GAME/i.test(value)) || '';
    const venue = middleParts.find(value => value !== status && !/^\d{1,2}:\d{2}$/.test(value) && !/^Game\s+\d+/i.test(value)) || '';
    const seriesGameNumber = Number(middleParts.join(' ').match(/Game\s+(\d+)/i)?.[1]) || null;
    ...
    const sourceId = npbSourceId(href, date) || `${date}|${awayTeam.code}|${homeTeam.code}|${time}|1`;
    const game = normalizedGame({ league: 'NPB', sourceId, date, time, awayTeam, homeTeam, venue, status, awayScore, homeScore });
    if (game) rows.push({ ...game, seriesGameNumber });
  });
  return normalizeNpbGameNumbers(rows);
}

function npbInningScore(value) {
  const normalized = cleanText(value).toUpperCase();
  // NPB uses a bare "X" when the home club does not bat in the bottom of
  // the final inning. It is a complete, unplayed inning for result validation
  // and contributes zero runs to first-five/full-game arithmetic.
  if (normalized === 'X') return 0;
  const match = normalized.match(/^(\d+)(?:X)?$/);
  return match ? numericScore(match[1]) : null;
}
...
export function parseNpbGameDetailHtml(html, expectedGame) {
  ...
  // A live page may contain incomplete inning cells. Its end-time must be
  // confirmed before either validating a final line or exposing F5 scores.
  ...
  $('#gmdivresult tr').each((_, element) => {
    ...
    // last three columns are R/H/E. Keep every cell in place: filtering blank
    // or nonnumeric cells would silently shift a missing inning into F5.
    const cells = row.find('th, td').filter((__, cell) => {
      const node = $(cell);
      // The official table inserts empty layout spacers after innings 3/6/9.
      // Only that explicit class is ignorable; an empty score cell is missing
      // evidence, and a nonempty spacer is an unexpected provider layout.
      if (!node.hasClass('gmscspan') || node.hasClass('gmscore')) return true;
      if (cleanText(node.text())) {
        throw providerError('NPB 官方比分間隔欄出現非預期內容', 502, 'OFFICIAL_FINAL_RESULT_INVALID');
      }
      return false;
    }).map((__, cell) => cleanText($(cell).text())).get();
    const teamCell = cells.findIndex(value => resolveTeam('NPB', value)?.code === team.code);
    const values = cells.slice(Math.max(0, teamCell + 1));
    const totals = values.slice(-3).map(numericScore);
    const inningCells = values.slice(0, -3);
    if (inningCells.at(-1) === '-') inningCells.pop();
    const inningScores = inningCells.map(npbInningScore);
    const runs = totals[0];
    if (totals.length !== 3 || totals.some(value => value == null)
      || inningScores.length < 5 || inningScores.some(value => value == null)
      || inningScores.reduce((sum, value) => sum + value, 0) !== runs) {
      throw providerError('NPB 官方逐局比分缺失或與終場總分不一致', 502, 'OFFICIAL_FINAL_RESULT_INVALID');
    }
    const parsed = { team, inningCells, inningScores, runs };
    const previous = rowsByTeam.get(team.code);
    if (previous && (previous.runs !== parsed.runs
      || previous.inningCells.join(',').toUpperCase() !== parsed.inningCells.join(',').toUpperCase())) {
  ...
  const innings = away.inningScores.length;
  const homeInnings = home.inningScores.length;
  if (innings < 9 || homeInnings > innings || homeInnings < innings - 1
    || (homeInnings === innings - 1 && home.runs <= away.runs)) {
    throw providerError('NPB 官方完賽明細局數不完整', 502, 'OFFICIAL_FINAL_RESULT_INVALID');
  }
  for (const [side, line] of [['away', away], ['home', home]]) {
    if (line.inningCells.some((value, index) => /X/i.test(value)
      && (side !== 'home' || index !== innings - 1 || home.runs <= away.runs))) {
      throw providerError('NPB 官方逐局比分的 X 位置不合法', 502, 'OFFICIAL_FINAL_RESULT_INVALID');
    }
  }
  const headlineScores = $('.gmboxrun').map((_, cell) => numericScore($(cell).text())).get();
  if (headlineScores.length !== 2
    || headlineScores.some(value => value == null)
    || headlineScores.sort((left, right) => left - right).join(',') !== [away.runs, home.runs].sort((left, right) => left - right).join(',')) {
...
export function parseNpbMonthHtml(html, year, month) {
  ...
  $('.stschedule').each((_, cell) => {
    ...
    $(cell).find('.stvsteam > div').each((_, element) => {
      ...
      let status = '';
      if (match) {
        // The official all-teams calendar uses HOME score - score AWAY,
        // matching the daily card, not the away-first inning/box-score table.
        // Preserve both the team and its score when normalizing to away/home.
        [, homeAlias, , , awayAlias] = match;
        homeScore = numericScore(match[2]);
        awayScore = numericScore(match[3]);
        status = awayScore == null || homeScore == null ? 'POSTPONED' : 'FINAL';
      } else {
        match = text.match(/^([A-Z]+)\s*-\s*([A-Z]+)\s+(\d{1,2}:\d{2})$/i);
        if (!match) return;
        [, homeAlias, awayAlias, time] = match;
      }
...
function cpblScheduleObservation(game) {
  return {
...
function reconcileCpblScheduleRows(rows) {
  if (rows.length === 1) return rows[0];
  const observations = [...new Map(rows.flatMap(row => [
    cpblScheduleObservation(row), ...(row.scheduleReconciliation?.observations || []),
  ]).map(row => [JSON.stringify(row), row])).values()]
    .sort((left, right) => JSON.stringify(left).localeCompare(JSON.stringify(right)));
  const conflict = (code, message) => {
    const error = providerError(`中職官方賽程同場資料衝突（${rows[0].providerGameId}）：${message}`, 502, code);
    error.gamePk = rows[0].gamePk;
    error.scheduleObservations = observations;
    throw error;
  };
  for (const field of ['providerGameId', 'awayTeamId', 'homeTeamId']) {
    if (new Set(rows.map(row => String(row[field]))).size !== 1) {
      conflict('CPBL_SCHEDULE_IDENTITY_CONFLICT', `不一致的${field}`);
    }
  }
  if (rows.some(row => !Number.isFinite(Date.parse(row.gameDate)))) {
    conflict('CPBL_SCHEDULE_STATE_CONFLICT', '缺少有效比賽日期');
  }
  const finals = rows.filter(row => row.statusCode === 'F');
  let candidates;
  let reason;
  if (finals.length) {
    // Neither month ordering nor a later request can decide which contradictory
    // final result is correct. Missing fields are not contradictory observations.
    for (const field of ['gameDate', 'officialDate', 'awayScore', 'homeScore', 'innings']) {
      const known = finals.map(row => row[field]).filter(value => value != null && value !== '');
      if (new Set(known.map(String)).size > 1) conflict('CPBL_SCHEDULE_FINAL_CONFLICT', `完賽${field}不一致`);
    }
    candidates = finals;
    reason = 'FINISHED_OBSERVATION_PRESERVED';
  } else {
    // Across monthly calendars, the newer event date is the rescheduled game;
    // an old postponed row must not overwrite it just because its month was read last.
    const latestAt = Math.max(...rows.map(row => Date.parse(row.gameDate)));
    candidates = rows.filter(row => Date.parse(row.gameDate) === latestAt);
    const active = candidates.filter(row => row.statusCode !== 'S');
    if (new Set(active.map(row => row.statusCode)).size > 1) {
      conflict('CPBL_SCHEDULE_STATE_CONFLICT', '同時點比賽狀態不一致');
    }
    if (active.length) candidates = active;
    reason = 'LATEST_RESCHEDULED_EVENT_DATE';
  }
  const completeness = row => ['awayScore', 'homeScore', 'innings'].filter(field => row[field] != null).length;
  const selected = [...candidates].sort((left, right) => completeness(right) - completeness(left)
    || JSON.stringify(cpblScheduleObservation(left)).localeCompare(JSON.stringify(cpblScheduleObservation(right)))
    || String(left.venue).localeCompare(String(right.venue)))[0];
  return { ...selected, scheduleReconciliation: {
    version: 'CPBL-SCHEDULE-RECONCILIATION-v1', reason, observations,
  } };
}

export function uniqueGames(rows) {
  const byPk = new Map();
  for (const game of rows) {
    const key = `${game.leagueId || game.league}:${game.gamePk}`;
    if (!byPk.has(key)) byPk.set(key, []);
    byPk.get(key).push(game);
  }
  return [...byPk.values()].map(group => (group[0].leagueId || group[0].league) === 'CPBL'
    ? reconcileCpblScheduleRows(group) : group.at(-1)).sort((left, right) => (
...
async function fetchResponse(url, { fetchImpl, timeoutMs, format, method = 'GET', body, headers = {}, sourceEvidence }) {
  ...
  try {
    ...
    const value = format === 'json' ? JSON.parse(raw) : raw;
    if (sourceEvidence) {
      const capture = captureAsianSource({ url, method, requestBody: body, raw,
        representation: rawAvailable ? 'HTTP_RESPONSE_TEXT' : 'DECODED_JSON_TEST_ADAPTER',
        fetchedAt: new Date().toISOString(), httpStatus: response.status });
      sourceEvidence.events.push(capture.event);
      sourceEvidence.contents[capture.content.contentHash] = capture.content;
    }
    return value;
  } catch (error) {
    sourceEvidence?.events.push({ id: randomUUID(), url, method, success: false, httpStatus, fetchedAt: null,
      failedAt: new Date().toISOString(), contentHash: null });
...
async function fetchNpbDay(date, options) {
  ...
  const detailStarters = parseNpbScheduleDetailStartersHtml(scheduleDetailHtml, date);
  const games = parseNpbScheduleHtml(scheduleHtml, date);
  return games.map(game => {
    let row = null;
    for (const starters of [announcementStarters, detailStarters]) {
      const candidates = starters.filter(item => item.awayCode === game.awayCode && item.homeCode === game.homeCode);
      const timed = candidates.filter(item => /^\d{1,2}:\d{2}$/.test(item.time)
        && localIso(date, item.time, SOURCE_CONFIG.NPB.zoneOffset) === game.gameDate);
      if (timed.length === 1) row = timed[0];
      else if (candidates.length === 1 && game.doubleHeader !== 'Y') row = candidates[0];
      if (row) break;
    }
    if (!row) return game;
    return {
...
const KBO_OFFICIAL_TEAM_CODES = Object.freeze({
  KIA: 'HT', SAM: 'SS', LGT: 'LG', DOO: 'OB', KTW: 'KT',
  SSG: 'SK', LOG: 'LT', HAN: 'HH', NCD: 'NC', KIW: 'WO',
});

// The monthly feed sometimes removes its preview links before first pitch.
// Its layout-dependent fallback hash is NOT another game. Resolve missing IDs
// from the independent official daily feed; never infer an official game ID.
export function reconcileKboDailyIdentity(games, payload, date) {
  const compactDate = date.replaceAll('-', '');
  const used = new Set(games.filter(game => /^\d{8}[A-Z]{4}\d$/.test(game.providerGameId)).map(game => game.providerGameId));
  return games.map(game => {
    if (/^\d{8}[A-Z]{4}\d$/.test(game.providerGameId)) return game;
    const away = KBO_OFFICIAL_TEAM_CODES[game.awayCode];
    const home = KBO_OFFICIAL_TEAM_CODES[game.homeCode];
    const candidates = (Array.isArray(payload?.game) ? payload.game : []).filter(row => {
      const id = cleanText(row?.G_ID);
      const header = String(row?.HEADER_NO ?? '');
      return game.league === 'KBO' && game.officialDate === date
        && Number(row?.LE_ID) === 1 && Number(row?.SEASON_ID) === Number(date.slice(0, 4))
        && cleanText(row?.G_DT) === compactDate
        && cleanText(row?.AWAY_ID) === away && cleanText(row?.HOME_ID) === home
        && /^\d$/.test(header) && id === `${compactDate}${away}${home}${header}`
        && localIso(date, cleanText(row?.G_TM), SOURCE_CONFIG.KBO.zoneOffset) === game.gameDate;
    });
    if (candidates.length !== 1 || used.has(candidates[0].G_ID)) {
      throw providerError('KBO 官方場次識別尚未唯一核對，暫停使用替代場次編號', 503, 'KBO_OFFICIAL_IDENTITY_UNAVAILABLE');
    }
    const officialId = cleanText(candidates[0].G_ID);
    used.add(officialId);
    return { ...game, providerGameId: officialId, gamePk: stableAsianGamePk('KBO', officialId),
      identityResolution: { source: 'KBO_OFFICIAL_DAILY_GAME_LIST', previousGamePk: game.gamePk,
        previousProviderGameId: game.providerGameId, officialGameId: officialId } };
  });
}

async function resolveKboMissingIds(games, date, options) {
  if (!games.some(game => !/^\d{8}[A-Z]{4}\d$/.test(game.providerGameId))) return games;
  const payload = await fetchResponse('https://www.koreabaseball.com/ws/Main.asmx/GetKboGameList', {
    ...options, format: 'json', method: 'POST',
    body: new URLSearchParams({ leId: '1', srId: '0,1,3,4,5,6,7,8,9', date: date.replaceAll('-', '') }),
    headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
      'X-Requested-With': 'XMLHttpRequest', Referer: 'https://www.koreabaseball.com/Schedule/GameCenter/Main.aspx' },
  });
  return reconcileKboDailyIdentity(games, payload, date);
}
...
export async function fetchAsianTaipeiSlate(value, date, {
  ...
  const league = leagueId(value);
  if (!validDate(date)) throw providerError('日期格式必須為 YYYY-MM-DD', 400, 'INVALID_BOARD_DATE');
  const evidence = { events: [], contents: {} };
  const options = { fetchImpl, timeoutMs, sourceEvidence: evidence };
  let games;
  if (league === 'NPB') games = await fetchNpbDay(date, options);
  else if (league === 'KBO') {
    const { year, month } = monthCoordinates(date);
    games = await resolveKboMissingIds((await fetchKboMonth(year, month, options)).filter(game => game.officialDate === date), date, options);
  } else games = await fetchCpblDate(date, options);
  let retained = evidence;
  try { retained = (await externalizeAsianSourceEvidence({ sourceEvidence: evidence })).sourceEvidence; }
  catch { /* Preserve inline evidence when durable source storage is unavailable. */ }
  return games.map(game => ({ ...game, identitySourceEvidence: { ...retained,
    complete: evidence.events.some(row => row.success), sourceEventIds: evidence.events.filter(row => row.success).map(row => row.id),
    sourcePaths: ['$'], derivationVersion: ASIAN_SCHEDULE_VERSION,
  } }));
}
...
async function fetchHistory(league, date, options) {
  if (Array.isArray(options.historyGames)) return league === 'CPBL' ? uniqueGames(options.historyGames) : options.historyGames;
  ...
  if (failedMonths.length) {
    try { await externalizeAsianSourceEvidence({ sourceEvidence: options.sourceEvidence }); } catch { /* Preserve the original provider error. */ }
...
function completedBefore(games, gameDate) {
  ...
  return (Array.isArray(games) ? games : []).filter(game => (
    ...
    && finiteNumber(game?.awayScore) != null
    && finiteNumber(game?.awayScore) >= 0
    && finiteNumber(game?.homeScore) != null
    && finiteNumber(game?.homeScore) >= 0
...
function projectedStarterAssignment(supplied, probable = {}) {
  if (supplied?.projected === true || supplied?.confirmed === false
    || (supplied?.playerIdentityVerified === true && supplied?.identityConfirmed !== true)) return true;
  // The two clubs can announce starters at different times. A game-level
  // fallback source must not override this side's explicit assignment evidence.
  const sideSource = [supplied?.assignmentStatus, supplied?.identitySource].filter(Boolean).join(' ');
  return /PROJECTED|PROJECTION|FORECAST|ROTATION_SCENARIO|ROSTER_VALIDATED_REPORTED_STARTER/i.test(sideSource || probable?.source || '');
}

function identityMatchesProbable(probable, supplied, teamId) {
  // Knowing which roster player owns the statistics does not establish that
  // the player has been announced as this game's starter.
  if (!supplied?.identityConfirmed && !supplied?.playerIdentityVerified) return false;
...
function normalizeStarter(base, supplied, probable, teamId) {
  const assignmentProjected = projectedStarterAssignment(supplied, probable);
  ...
  if (!ready) return {
    ...
    starter: {
      ...base,
      ...(identityReady && !assignmentProjected ? {
        teamId: Number(teamId), officialPlayerId: supplied?.officialPlayerId || supplied?.id || null,
        confirmed: true, assignmentStatus: 'OFFICIAL_CONFIRMED',
        identitySource: supplied?.identitySource || probable?.source || null,
        sourceEvidence: supplied?.sourceEvidence || null,
        identityResolutionEvidence: supplied?.identityResolutionEvidence || null,
        scheduleIdentitySourceEvidence: supplied?.scheduleIdentitySourceEvidence || null,
        performanceMissingReason: supplied?.performanceMissingReason || 'OFFICIAL_STARTER_PERFORMANCE_UNAVAILABLE_OR_INSUFFICIENT',
        candidates: [],
      } : {}),
      id: supplied?.id || base.id,
      name: supplied?.name || base.name,
      identityConfirmed: !assignmentProjected && (identityReady || base.identityConfirmed),
      projected: assignmentProjected,
      playerIdentityVerified: identityReady,
      identityMismatch: Boolean(supplied?.identityConfirmed || supplied?.playerIdentityVerified) && !identityReady,
      ...(assignmentProjected ? {
        confirmed: false,
        projected: true,
        assignmentStatus: supplied?.assignmentStatus || 'PROJECTED_ROTATION_SCENARIO',
        identitySource: supplied?.identitySource || probable?.source || null,
      } : {}),
  ...
  const starter = {
    ...
    ...supplied,
    available: true,
    confirmed: !assignmentProjected,
    identityConfirmed: !assignmentProjected,
    projected: assignmentProjected,
    playerIdentityVerified: true,
    ...(assignmentProjected ? { projected: true, assignmentStatus: supplied?.assignmentStatus || 'PROJECTED_ROTATION_SCENARIO' } : {}),
    identityMismatch: false,
    performanceAvailable: true,
    performanceStatus: assignmentProjected ? 'OBSERVED_INDIVIDUAL_PROJECTED_ASSIGNMENT' : 'CONFIRMED_INDIVIDUAL_STARTER',
    performanceScope: 'INDIVIDUAL_STARTER',
    independentOfTeamResults: true,
    projectedFromTeamPitching: false,
    projectionMode: assignmentProjected ? 'PROJECTED_ASSIGNMENT_WITH_OBSERVED_INDIVIDUAL_PERFORMANCE' : 'VERIFIED_INDIVIDUAL_STARTER_PERFORMANCE',
    throws,
    handedness: throws,
...
function kboRuleState(game, features, park) {
  ...
  return {
    ...
    doubleheader: {
      ...
      usageEvidenceVerified: !secondGame || features?.rules?.doubleheader?.firstGameUsageVerified === true,
      usageEvidenceStatus: secondGame && features?.rules?.doubleheader?.firstGameUsageVerified !== true ? 'DIAGNOSTIC_ONLY' : 'NOT_APPLICABLE_OR_VERIFIED',
      projected: doubleheaderProjected,
      uncertaintyExpanded: false,
      modelUsage: doubleheaderProjected ? 'DIAGNOSTIC_ONLY_NO_CALIBRATED_DOUBLEHEADER_EFFECT' : 'OBSERVED',
    },
    weather: {
      ready: weatherReady,
      domeBypass: dome,
...
function cpblRuleState(features) {
  ...
  const projected = !confirmedReady && (!status || (status === 'DIAGNOSTIC_ONLY' && sourced)) && features?.asOf;
  const ready = confirmedReady || Boolean(projected);
  const transitionDiagnostic = ready && status !== 'NOT_APPLICABLE';
  return {
    ready,
    projected: Boolean(projected) || transitionDiagnostic,
    foreignPlayerConstraint: {
      status: ready ? (status === 'NOT_APPLICABLE' ? status : 'DIAGNOSTIC_FOREIGN_STATUS_TRANSITION_UNMODELED') : 'UNRESOLVED',
      ...
      modelUsage: 'DIAGNOSTIC_ONLY_NO_FOREIGN_SUBSTITUTION_TRANSITION_MODEL',
...
function teamContext(history, teamId, baseline, gameDate, modelConfig, probable = {}, supplied = {}) {
  ...
  const period = rows => ({ startDate: rows.map(r => r.date).sort()[0] || null, endDate: rows.map(r => r.date).sort().at(-1) || null, teamId });
  const seasonHitting = { ...hittingBlock(seasonRows, baseline), ...period(seasonRows) };
  const seasonPitching = { ...pitchingBlock(seasonRows, baseline), ...period(seasonRows) };
  const recentHitting = { ...hittingBlock(recentRows, baseline), ...period(recentRows) };
  const recentPitching = { ...pitchingBlock(recentRows, baseline), ...period(recentRows) };
  ...
  const base = {
    ...
    starter: {
      available: false,
      confirmed: false,
      identityConfirmed: Boolean(probable?.name) && !projectedStarterAssignment(null, probable),
      playerIdentityVerified: false,
      ...(projectedStarterAssignment(null, probable) ? { projected: true, assignmentStatus: 'PROJECTED_ROTATION_SCENARIO' } : {}),
...
function predictedTeamContext(team, supplied, modelConfig, leagueId, allowPrediction) {
  ...
  const officialAssignmentKnown = team.upstreamReadiness.starterIdentity === true
    && team.starter.identityConfirmed === true && team.starter.confirmed === true
    && !projectedStarterAssignment(team.starter);
  const rawCandidates = !officialAssignmentKnown && Array.isArray(supplied?.starter?.candidates)
    ? supplied.starter.candidates.filter(candidate => candidate?.name && finiteNumber(candidate?.probability) > 0)
    : [];
  const totalCandidateWeight = rawCandidates.reduce((sum, candidate) => sum + Number(candidate.probability), 0);
  const candidates = rawCandidates.map(candidate => {
    const innings = finiteNumber(candidate.inningsPitched);
    const era = finiteNumber(candidate.era);
    const suppliedQuality = finiteNumber(candidate.qualityFactor);
    // Retain the existing sample shrink and reference. Apply them per actual
    // candidate so the distribution can retain assignment uncertainty; a
    // missing ERA is never interpreted as an observed zero-run performance.
    const reliability = innings > 0 ? innings / (innings + 45) : 0;
    const qualityFactor = suppliedQuality > 0 ? suppliedQuality : era != null && era >= 0 && innings > 0
      ? Math.exp(Math.log(Math.max(0.65, Math.min(1.55, era / Math.max(2.6, neutralStarterInnings * 0.82)))) * reliability)
      : null;
    return {
      ...candidate,
      probability: Number(candidate.probability) / totalCandidateWeight,
      qualityFactor,
      qualityFactorMethod: suppliedQuality > 0 ? 'SUPPLIED_INDIVIDUAL_QUALITY' : qualityFactor != null ? 'EXISTING_ERA_SAMPLE_SHRINK' : 'MISSING',
    };
  });
  const candidateMixtureReady = candidates.length > 0 && candidates.every(candidate => candidate.qualityFactor > 0 && finiteNumber(candidate.expectedInnings) > 0);
  const candidateExpectedInnings = candidateMixtureReady
    ? candidates.reduce((sum, candidate) => sum + candidate.probability * candidate.expectedInnings, 0) : null;
  const candidateQuality = candidateMixtureReady
    ? candidates.reduce((sum, candidate) => sum + candidate.probability * candidate.qualityFactor, 0) : null;
  const primaryPerformanceReady = team.upstreamReadiness.starterPerformance === true
    && team.starter.performanceAvailable === true
    && finiteNumber(team.starter.qualityFactor) > 0
    && finiteNumber(team.starter.expectedInnings) > 0;
  const primaryInnings = primaryPerformanceReady ? finiteNumber(team.starter.expectedInnings) : null;
  const primaryQuality = primaryPerformanceReady ? finiteNumber(team.starter.qualityFactor) : null;
  const projectedInnings = Math.max(2.5, Math.min(7.2, candidateExpectedInnings ?? primaryInnings ?? neutralStarterInnings));
  const projectedQuality = candidateMixtureReady ? Math.max(0.82, Math.min(1.22, candidateQuality)) : primaryQuality ?? 1;
  ...
  const starterObservedReady = team.upstreamReadiness.starterIdentity === true
    ...
    && (!projectedStarterAssignment(team.starter) || candidates.length === 0);
  const starterForecastAvailable = starterObservedReady || (forecastSafe && (stableTeamBaseline || primaryPerformanceReady));
  ...
  return {
    ...
    starter: starterObservedReady || !starterForecastAvailable ? team.starter : {
      ...team.starter,
      available: true,
      confirmed: officialAssignmentKnown,
      identityConfirmed: officialAssignmentKnown,
      projected: !officialAssignmentKnown,
      status: officialAssignmentKnown ? 'CONFIRMED_IDENTITY_MISSING_PERFORMANCE' : 'PROJECTED',
      ...(officialAssignmentKnown ? { assignmentStatus: 'OFFICIAL_CONFIRMED', performanceProjected: true } : {}),
      performanceAvailable: primaryPerformanceReady && !candidateMixtureReady,
      expectedInnings: projectedInnings,
      qualityFactor: projectedQuality,
      projectionMode: officialAssignmentKnown ? 'OFFICIAL_ASSIGNMENT_NEUTRAL_PERFORMANCE'
        : candidateMixtureReady ? 'OFFICIAL_PRIOR_START_ROTATION_MIXTURE'
        : primaryPerformanceReady ? 'PROJECTED_ASSIGNMENT_WITH_OBSERVED_INDIVIDUAL_PERFORMANCE' : 'LEAGUE_NEUTRAL_ROTATION_SCENARIO',
      source: officialAssignmentKnown ? `${leagueId}_OFFICIAL_STARTER_IDENTITY_WITH_NEUTRAL_PERFORMANCE`
        : candidates.length || primaryPerformanceReady
        ? supplied.starter.source || supplied.starter.identitySource || supplied.starter.performanceSource
        : `${leagueId}_LEAGUE_NEUTRAL_STARTER_SCENARIO`,
      candidates,
      candidateMixtureReady,
      candidateAbilityStatus: candidateMixtureReady ? 'ALL_CANDIDATE_ABILITIES_AVAILABLE'
        : candidates.length ? 'INCOMPLETE_CANDIDATE_ABILITIES_NOT_AGGREGATED' : 'NO_CANDIDATE_ABILITIES',
      candidateSetComplete: supplied?.starter?.candidateSetComplete === true,
      probabilityScope: supplied?.starter?.probabilityScope || 'CONDITIONAL_ON_OBSERVED_CANDIDATE_SET',
      candidateNormalization: {
        version: 'ASIAN-EXISTING-CANDIDATE-NORMALIZATION-v1',
        method: 'EXISTING_ERA_SAMPLE_SHRINK_AND_CONDITIONAL_WEIGHTED_MEAN',
        applied: candidateMixtureReady,
        sourcePrimary: { qualityFactor: finiteNumber(supplied?.starter?.qualityFactor), expectedInnings: finiteNumber(supplied?.starter?.expectedInnings) },
        weightedCandidateValues: { qualityFactor: candidateQuality, expectedInnings: candidateExpectedInnings },
        output: { qualityFactor: projectedQuality, expectedInnings: projectedInnings },
        bounds: { qualityFactor: candidateMixtureReady ? [0.82, 1.22] : null, expectedInnings: [2.5, 7.2] },
        candidateProbabilitiesReestimated: false,
        missingAbilitiesReplaced: false,
      },
      uncertaintyExpanded: false,
      uncertaintyMethod: candidateMixtureReady ? 'CANDIDATE_DISTRIBUTION_MIXTURE_PENDING_VALIDATION' : 'NOT_EXPANDED_MISSING_CANDIDATE_ABILITIES',
    },
    lineup: team.upstreamReadiness.lineup === true || !lineupForecastAvailable ? team.lineup : {
      ...
      uncertaintyMethod: 'EXISTING_PROJECTED_LINEUP_SIGMA',
    },
    bullpen: team.upstreamReadiness.bullpen === true || !bullpenForecastAvailable ? team.bullpen : {
      ...
      uncertaintyExpanded: false,
      modelUsage: 'NEUTRAL_QUALITY_ONLY_WORKLOAD_DIAGNOSTIC',
    },
    forecastReadiness: {
...
export async function buildAsianGameContext(value, game, {
  ...
  const historyEvidence = { events: [], contents: {} };
  const appearanceHistory = await fetchHistory(leagueId, game?.officialDate || game?.taipeiDate || game?.gameDate?.slice(0, 10), {
    fetchImpl, timeoutMs, historyGames, sourceEvidence: historyEvidence,
  });
  // Team scoring and performance inputs remain completed-only. The personnel
  // pipeline separately validates actual appearances in suspended games.
  const history = completedBefore(appearanceHistory, game?.gameDate);
  ...
  if (shouldBuildProduction) {
    try {
      const production = await buildAsianProductionFeatureSnapshot({
        leagueId, game: resolvedGame, history, appearanceHistory, fetchImpl, timeoutMs, starterEvidence,
      });
  ...
  const effectiveFeatures = leagueId === 'KBO' && !(features?.weather?.available || features?.weather?.scenarioAvailable)
    ? {
      ...
      weather: {
        ...
        projectionBased: true,
        uncertaintyExpanded: false,
        modelUsage: 'NEUTRAL_MEAN_ONLY_WEATHER_UNCERTAINTY_NOT_CALIBRATED',
      },
    }
    : features;
  ...
  const projectedStarterScenario = projectedStarterAssignment(away.starter)
    || projectedStarterAssignment(home.starter);
  const rows = [
  ...
  const warnings = [
    ...
    ...(productionFeatureError || features?.pipelineError
      ...
      : []),
    ...(projectedCount ? [`OFFICIAL_DATA_NOT_CONFIRMED｜${projectedCount}項未取得已確認資料，使用賽前預測或中性先驗；不代表官方尚未公布，模型使用見逐項資料稽核`] : []),
  ...
  const inputCutoffAt = new Date().toISOString();
  let sourceEvidence = {
    contentHashes: [...new Set([...(game?.identitySourceEvidence?.contentHashes || []), ...(features.sourceEvidence?.contentHashes || [])])],
    version: 'ASIAN-SOURCE-EVIDENCE-v1',
    events: [...historyEvidence.events, ...(game?.identitySourceEvidence?.events || []), ...(features.sourceEvidence?.events || [])],
    contents: { ...historyEvidence.contents, ...game?.identitySourceEvidence?.contents, ...features.sourceEvidence?.contents },
    features: [
      { featureName: 'gameIdentity', complete: game?.identitySourceEvidence?.complete === true,
        sourceEventIds: game?.identitySourceEvidence?.sourceEventIds || [],
        sourcePaths: game?.identitySourceEvidence?.sourcePaths || [], parsedInput: {
          gamePk: resolvedGame.gamePk, awayTeamId: resolvedGame.awayTeamId, homeTeamId: resolvedGame.homeTeamId, gameDate: resolvedGame.gameDate },
      },
      ...[['park', effectiveParkState.park], ['weather', effectiveFeatures.weather], ['rules', leagueRuleState]].map(([featureName, parsedInput]) => ({
        featureName, complete: true, requiredFeatures: ['gameIdentity', 'history'],
        sourceEventIds: featureName === 'weather' && parsedInput?.source === 'UNAVAILABLE' ? [] : (features.sourceEvidence?.events || []).filter(row => row.success).map(row => row.id),
        dependencyScope: 'CONSERVATIVE_CONTEXT_DERIVATION_INPUT_GROUP',
        derivationVersion: ASIAN_CONTEXT_VERSION, sourcePaths: ['$'], parsedInput,
      })),
      { featureName: 'history', complete: historyEvidence.events.length === 3,
        sourceEventIds: historyEvidence.events.map(row => row.id), sourcePaths: ['$'],
        derivationVersion: ASIAN_CONTEXT_VERSION, parsedInput: history },
      ...(features.sourceEvidence?.features || []),
    ],
  };
  // Resolve only stored source references; this does not re-fetch any provider.
  try { sourceEvidence = (await externalizeAsianSourceEvidence({ sourceEvidence })).sourceEvidence; }
  catch { sourceEvidence = { ...sourceEvidence, persistenceStatus: 'SOURCE_STORE_UNAVAILABLE_INLINE_EVIDENCE_RETAINED' }; }
  let auditEvidence = sourceEvidence;
  if (sourceEvidence.contentHashes.length) {
    try {
      const hydrated = await hydrateAsianSourceEvidence({ sourceEvidence: { ...sourceEvidence, contentStorage: 'IMMUTABLE_SOURCE_STORE_V1' } });
      auditEvidence = { ...sourceEvidence, contents: { ...sourceEvidence.contents, ...hydrated.sourceEvidence.contents } };
    } catch { /* Missing raw evidence stays PENDING; the model input is unchanged. */ }
  }
  const sourceTimeAudit = auditAsianSourceEvidence({ inputCutoffAt, game: resolvedGame, sourceEvidence: auditEvidence });
  return {
    inputCutoffAt, sourceEvidence, sourceTimeAudit,
    game: { ...resolvedGame, identitySourceEvidence: undefined, league: leagueId, leagueId, analysisMode: ASIAN_ANALYSIS_MODE, betEligible: false },
    leagueId,
    analysisMode: ASIAN_ANALYSIS_MODE,
    executable: false,
    betEligible: false,
    ...
    featureProvenance: [
      ...
      { feature: '先發能力', status: starterPerformanceConfirmed ? '已確認' : bothForecast('starter') ? '候選加權平均／聯盟中性預測' : '缺失', source: starterPerformanceConfirmed ? `${away.starter.source}／${home.starter.source}` : '官方過往先發候選；缺少能力採中性先驗；候選分布混合尚待驗證，未宣稱擴大誤差' },
      ...
      const mapping = { '聯盟得分基準': ['history'], '球隊近期比分代理': ['history'], '球隊能力基準': ['history'],
        '先發身分': ['gameIdentity', 'away.starter', 'home.starter'], '先發能力': ['away.starter', 'home.starter'],
        '打線': ['away.lineup', 'home.lineup'], '牛棚': ['away.bullpen', 'home.bullpen'], '球場': ['park'], '天氣／主審': ['weather'] };
      const names = mapping[row.feature] || [];
      const checks = sourceTimeAudit.rows.filter(item => names.includes(item.featureName));
      const dependencies = new Set(names.flatMap(name => [name, ...(sourceEvidence.features.find(item => item.featureName === name)?.requiredFeatures || [])]));
      const sourceEventIds = [...new Set(sourceEvidence.features.filter(item => dependencies.has(item.featureName)).flatMap(item => item.sourceEventIds || []))];
      const times = sourceEvidence.events.filter(event => sourceEventIds.includes(event.id) && event.success)
        .map(event => Date.parse(event.fetchedAt)).filter(Number.isFinite);
      return { ...row, featureName: row.feature, sourceEventIds,
        fetchedAt: times.length ? new Date(Math.max(...times)).toISOString() : null,
        publishedAt: null, dataCutoff: null,
        timeEvidenceStatus: checks.some(item => item.status === 'FAILED') ? 'FAILED'
          : checks.length === names.length && checks.every(item => item.status === 'VERIFIED') ? 'VERIFIED' : 'PENDING' };
    }),
    ...
    starterModelingMode: starterPerformanceConfirmed
      ? (projectedStarterScenario ? 'PROJECTED_ASSIGNMENT_WITH_OBSERVED_INDIVIDUAL_PERFORMANCE' : 'VERIFIED_INDIVIDUAL_STARTER_PERFORMANCE')
    ...
    sourceStatuses: {
      starterAssignments: ['away', 'home'].map(side => {
        const starter = ({ away, home })[side].starter;
        const projected = /PROJECTED|FORECAST/i.test(`${starter.assignmentStatus || ''} ${starter.identitySource || ''}`);
        const confirmed = starter.identityConfirmed === true && starter.confirmed === true;
        return `${side === 'away' ? '客' : '主'}：${projected ? '輪值推估' : confirmed ? '官方當場先發' : '尚未確認官方指派'}`;
      }).join('／'),
      starterIdentity: starterIdentityConfirmed ? 'CONFIRMED' : bothForecast('starter') ? 'PROJECTED_ROTATION_MIXTURE' : 'MISSING',
      starters: starterPerformanceConfirmed ? (projectedStarterScenario ? 'PROJECTED_ASSIGNMENT_OBSERVED_INDIVIDUAL' : 'CONFIRMED_INDIVIDUAL') : bothForecast('starter') ? 'PROJECTED_CANDIDATE_OR_NEUTRAL' : 'MISSING_INDIVIDUAL_PERFORMANCE',
    ...
    ...(features.rotationInputDiagnostics ? { rotationInputDiagnostics: features.rotationInputDiagnostics } : {}),
...
function npbExpectedTeamCodes(date, options = {}, league = 'NPB') {
  const providerParts = cleanText(options.expectedProviderGameId).split('|');
  const providerHint = providerParts.length >= 3 && providerParts[0] === date
    ? { awayCode: providerParts[1], homeCode: providerParts[2] }
    : null;
  const awayTeam = resolveTeam(league, options.expectedAway);
  const homeTeam = resolveTeam(league, options.expectedHome);
  if (providerHint && awayTeam && providerHint.awayCode !== awayTeam.code) {
    throw providerError(`${league} 保存的客隊身分互相衝突`, 409, 'OFFICIAL_IDENTITY_MISMATCH');
  }
  if (providerHint && homeTeam && providerHint.homeCode !== homeTeam.code) {
    throw providerError(`${league} 保存的主隊身分互相衝突`, 409, 'OFFICIAL_IDENTITY_MISMATCH');
  }
  const awayCode = awayTeam?.code || providerHint?.awayCode || '';
  const homeCode = homeTeam?.code || providerHint?.homeCode || '';
  return awayCode && homeCode ? { awayCode, homeCode } : null;
}

export function resolveLegacyAsianResultGame(league, games, requestedGamePk, date, options = {}) {
  league = leagueId(league);
  const expected = npbExpectedTeamCodes(date, options, league);
  if (!expected) return null;
  const candidates = games.filter(game => (game.leagueId || game.league) === league && game.officialDate === date
    && game.awayCode === expected.awayCode
    && game.homeCode === expected.homeCode);
  if (candidates.length > 1) {
    throw providerError(`${league} 同日同隊存在多場賽事，無法唯一核對舊場次識別`, 409, 'OFFICIAL_IDENTITY_AMBIGUOUS');
  }
  if (candidates.length !== 1) return null;
  const official = candidates[0];
  return {
    ...official,
    // The ticket gamePk is the permanent ledger identity. Keep it unchanged,
    // while retaining the newly published official id for detail retrieval and
    // audit provenance.
    gamePk: requestedGamePk,
    officialGamePk: official.gamePk,
    gameNumber: Number.isSafeInteger(Number(options.expectedGameNumber))
      && Number(options.expectedGameNumber) > 0
      ? Number(options.expectedGameNumber)
      : Number(official.gameNumber || 1),
  };
}
...
export async function fetchAsianFinalResult(value, gamePk, date, options = {}) {
  ...
  if (!game && ['NPB', 'KBO'].includes(league)) game = resolveLegacyAsianResultGame(league, games, id, date, options);
  if (!game) throw providerError('官方賽程找不到指定場次', 404, 'OFFICIAL_GAME_NOT_FOUND');
  if (league === 'KBO' && game.statusCode === 'F' && /^\d{8}[A-Z]{4}\d$/.test(game.providerGameId)) {
    const payload = await fetchResponse('https://www.koreabaseball.com/ws/Schedule.asmx/GetScoreBoardScroll', {
      fetchImpl: options.fetchImpl || globalThis.fetch,
      timeoutMs: options.timeoutMs || 12_000,
      format: 'json', method: 'POST',
      body: new URLSearchParams({ leId: '1', srId: '0', seasonId: date.slice(0, 4), gameId: game.providerGameId }),
      headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'X-Requested-With': 'XMLHttpRequest', Referer: 'https://www.koreabaseball.com/Schedule/GameCenter/Main.aspx' },
    });
    game = parseKboResultLinescore(payload, game);
  }
