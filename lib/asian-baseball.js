...
export async function buildAsianGameContext(value, game, {
  ...
  const coreReady = Boolean(game?.gamePk && game?.awayTeamId && game?.homeTeamId && history.length > 0);
  const dataGateV10 = {
    version: 'ASIAN-SHADOW-STATE-GATE-2026-08-v1.1.0',
    rows: [
    ...
    missing: ['probableStartersAndHandedness', 'officialOrProjectedLineups', 'pureBullpenUsage'],
    projected: ['neutralStarterInnings', 'neutralLineup', 'bullpenUncertainty'],
    blocking: coreReady ? [] : ['officialScheduleIdentityOrRecentLeagueResults'],
