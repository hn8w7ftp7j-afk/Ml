...
export const ASIAN_BASEBALL_VERSION = 'ASIAN-BASEBALL-OFFICIAL-PROVIDERS-2026-08-v1.4.0';
export const ASIAN_SCHEDULE_VERSION = 'ASIAN-OFFICIAL-SCHEDULE-2026-08-v1.0.0';
export const ASIAN_CONTEXT_VERSION = 'ASIAN-SHADOW-CONTEXT-2026-08-v1.5.0';
...
const SOURCE_CONFIG = Object.freeze({
  NPB: Object.freeze({
    ...
    label: 'NPB 官方 BIS',
    zoneOffset: '+09:00',
    modelVersion: 'NPB-SHADOW-JOINT-SCORE-2026-08-v1.4.0',
    rulesVersion: 'NPB-TW-SHADOW-RULES-2026-08-v1.4.0',
    baselineRuns: 3.75,
  ...
  KBO: Object.freeze({
    ...
    label: 'KBO 官方賽程',
    zoneOffset: '+09:00',
    modelVersion: 'KBO-SHADOW-JOINT-SCORE-2026-08-v1.4.0',
    rulesVersion: 'KBO-TW-SHADOW-RULES-2026-08-v1.4.0',
    baselineRuns: 4.65,
    modelConfig: Object.freeze({
  ...
  CPBL: Object.freeze({
    ...
    zoneOffset: '+08:00',
    modelVersion: 'CPBL-SHADOW-JOINT-SCORE-2026-08-v1.4.0',
    rulesVersion: 'CPBL-TW-SHADOW-RULES-2026-08-v1.4.0',
    baselineRuns: 4.45,
    modelConfig: Object.freeze({
...
function hittingBlock(rows, baseline) {
  ...
  return {
    ...
    gamesPlayed: count,
    runsPerGame,
    // Official Asian adapters currently have team runs, not independent
    // component batting stats. Keep unavailable components neutral so the
    // same runs-per-game signal is not counted again as synthetic OPS/ISO.
    ops: 0.72,
    avg: 0.255,
    obp: 0.325,
    slg: 0.395,
...
function teamContext(history, teamId, baseline, gameDate, modelConfig, probable = {}) {
  ...
  return {
    ...
    starter: {
      ...
      // The schedule can confirm who is starting, but the current adapters do
      // not yet provide an independent pitcher stat line.  Never upgrade a
      // team-rate prior to CONFIRMED merely because a name is available.
      confirmed: false,
      identityConfirmed: Boolean(probable?.name),
      performanceStatus: 'PROJECTED_TEAM_RATE_PRIOR',
      id: probable?.id || null,
      name: probable?.name || '',
      handedness: null,
      throws: null,
    ...
    lineup: { available: projectionReady, official: false, projected: projectionReady, handednessAdjusted: false, offensiveIndex: 1, catcher: null, players: [], sampleGames: seasonRows.length, status: projectionReady ? 'PROJECTED' : 'MISSING', source: 'OFFICIAL_TEAM_RESULTS_WEIGHTED_LINEUP_SCENARIO_NO_DOUBLE_COUNT' },
    bullpen,
...
export async function buildAsianGameContext(value, game, {
  ...
  const stateRows = [
    { name: 'probableOrProjectedStarters', ready: away.starter.available && home.starter.available, status: 'PROJECTED' },
  ...
  return {
    ...
    featureProvenance: [
      ...
      { feature: '先發', status: away.starter.identityConfirmed && home.starter.identityConfirmed ? '身分已確認／能力預估' : '預估', source: away.starter.identityConfirmed && home.starter.identityConfirmed ? `${game.probableSource || config.label}；投手能力仍用球隊投球率先驗` : '官方近期賽果輪值情境' },
    ...
    starterModelingMode: away.starter.identityConfirmed && home.starter.identityConfirmed ? 'OFFICIAL_STARTER_IDENTITY_WITH_PROJECTED_TEAM_RATE_PRIOR' : 'ROTATION_SCENARIO_TEAM_RATE_PRIOR',
    sourceStatuses: {
