export const MLB_RUN_MODEL_V103_VERSION = 'MLB-CORRELATED-RISK-RUN-PROFILE-2026-08-v11.0.3';
...
function bullpenProfile(team = {}, league = {}) {
  ...
  const pureReliefAvailable = relief?.pureRelief === true
    && Number.isFinite(Number(relief?.qualityFactor))
...
function expectedStarterInnings(block = {}) {
  ...
  const supplied = Number(block?.expectedInnings);
  if (Number.isFinite(supplied)) return clamp(supplied, 1.0, 7.2);
  const innings = Math.max(0, finite(block?.inningsPitched, 0));
