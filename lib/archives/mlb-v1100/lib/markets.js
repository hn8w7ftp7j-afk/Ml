...
export function validateMarketPair(market, directions) {
  // Never truncate an over-complete market. A third row is a coverage error,
  // not a direction that may be silently discarded.
  const rows = Array.isArray(directions) ? directions : [];
  const errors = [];
  // Zero rows means a genuinely unopened market. Once a row is supplied, even
  // an empty/partially parsed row is coverage evidence and must be BLOCKED
  // explicitly instead of being relabelled as unopened.
  if (!rows.length) return errors;
  if (rows.length !== 2) errors.push('已開盤市場必須有兩個方向');
  if (rows.length > 2) errors.push(`同市場出現${rows.length}筆方向，禁止靜默截斷`);
  ...
  for (const row of rows) {
    ...
    if (row?.water == null || String(row.water).trim() === '') errors.push('已開盤方向缺少水位');
    else if (!hasActualWater(row.water)) errors.push('水位範圍應為 0.010～3.000');
  }
