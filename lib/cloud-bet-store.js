...
async function persistBetUpdates(values, previousValues = []) {
...
export async function settleOpenCloudBets({ league = '', limit = 500 } = {}) {
  ...
  const candidates = current
    .filter(bet => (bet.status === 'OPEN' || isRetryableResultGap(bet)) && (!id || bet.league === id))
  ...
  return persistBetUpdates(updates, candidates);
}
