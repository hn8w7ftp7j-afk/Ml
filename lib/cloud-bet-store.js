...
export async function upsertCloudBet(value, { verification = null } = {}) {
  ...
  bet = sanitizeCloudBet({
    ...
    readerEvidenceStatus: verification?.reader?.captureFreshAtRecord === true
    ...
    formulaDiagnosticScore: pit?.formulaDiagnosticScore ?? null,
    shadowDiagnosticScore: pit?.shadowDiagnosticScore ?? null,
...
export async function cancelOpenCloudBet(value) {
  ...
  const [row] = await sql()`
    ...
    LIMIT 1
  `;
  const current = sanitizeCloudBet(row?.payload);
  if (!current) {
  ...
  if (current.status !== 'OPEN') {
    const error = new Error(current.status === 'CANCELLED' ? '這筆下注已取消' : '只有尚未結算的下注可以取消');
    error.code = 'BET_NOT_OPEN';
    error.status = 409;
    throw error;
  }
  const gameStartsAt = Date.parse(String(current.gameDate || ''));
  if (!Number.isFinite(gameStartsAt) || Date.now() >= gameStartsAt) {
  ...
  const cancelled = sanitizeCloudBet({
    ...current,
    status: 'CANCELLED',
    settlement: null,
    cancelledAt,
    cancellation: { type: 'USER_CANCELLED_PRESTART', cancelledAt },
  });
  const updated = await sql()`
    UPDATE baseball_private_bets_v2
    SET status = 'CANCELLED', payload = ${JSON.stringify(cancelled)}::jsonb, updated_at = NOW()
    WHERE id = ${id}
      AND status = 'OPEN'
  ...
  `;
  if (!updated.length) {
    const error = new Error('下注狀態已更新或比賽已開打，請重新整理');
    error.code = 'BET_CANCEL_CONFLICT';
    error.status = 409;
    throw error;
  }
  return listCloudBets();
