...
import { isRetryableResultGap } from './bet-settlement-service.js';
...
export function buildServerVerifiedCloudBet(value, verification, {
  ...
  const candidate = {
    ...
    dataVersion: cleanText(pit?.dataVersion, 180) || null,
    dataQualityReceipt: pit?.dataQualityReceipt || null,
...
export async function insertVerifiedCloudBetAtomically(bet, { database = null } = {}) {
  ...
  if (outcome.created !== true) {
    // ON CONFLICT may wait for another transaction that committed this exact
    // request. Read in a new statement so its committed row is visible. This
    // also recovers a retry whose first insert succeeded just before first pitch.
    const existing = await findMatchingPersistedCloudBet(bet, { database: query });
    if (existing) return { created: false, idempotent: true, betId: existing.id };
  ...
  if (outcome.id !== bet.id) {
    throw cloudBetMutationError('永久下注帳本回傳的紀錄識別不一致，請重新核對帳本', 'BET_INSERT_ID_MISMATCH', 503);
  }
  return { created: true, betId: outcome.id };
}
...
// Price identity rounds water for indexing; retry matching deliberately checks
// the actual water and stake as well so two different requests never collapse.
export function cloudBetIntentMatches(candidate, persisted) {
  if (!cloudBetCandidateCanWrite(candidate) || !persisted) return false;
  const league = cleanText(candidate.league, 8).toUpperCase();
  const date = cleanText(candidate.date, 10);
  const gamePk = Number(candidate.gamePk);
  const water = finite(candidate.water);
  const stake = finite(candidate.stake);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !Number.isSafeInteger(gamePk) || gamePk <= 0
    || water == null || water <= 0 || water > 5 || stake == null || stake <= 0 || stake > MAX_STAKE
    || !cleanText(candidate.market, 30) || !cleanText(candidate.pick, 160)) return false;
  return persisted.league === league
    && persisted.positionIdentity === betPositionIdentity(date, gamePk, candidate, league)
    && persisted.priceIdentity === betPriceIdentity(date, gamePk, candidate, league)
    && canonicalBetPick(persisted.pick) === canonicalBetPick(candidate.pick)
    && persisted.water === water
    && persisted.stake === stake;
}

// This is read-only recovery of a previously verified durable record. Current
// Reader freshness/PIT validation still gates every NEW insert; a lost HTTP
// response must not require a fresh market or a still-pregame clock to read it.
export async function findMatchingPersistedCloudBet(value, { database = null } = {}) {
  if (!cloudBetCandidateCanWrite(value)) return null;
  const positionKey = betPositionIdentity(value.date, value.gamePk, value, value.league);
  if (!database) {
  ...
  const rows = await query`
    ...
    LIMIT 1
  `;
  const existing = sanitizeCloudBet(rows[0]?.payload);
  return existing && existing.status !== 'CANCELLED' && trustedLedgerEvidence(existing)
    && cloudBetIntentMatches(value, existing) ? existing : null;
}

export async function confirmCloudBetPersistence(value, mutation, {
  listBets = listCloudBets,
  readByIds = listCloudBetsByIds,
} = {}) {
  const betId = cleanText(mutation?.betId, 120);
  if (!betId || (mutation?.created !== true && mutation?.idempotent !== true)) {
    throw cloudBetMutationError('下注紀錄未取得有效寫入回執，請重新核對帳本', 'BET_PERSISTENCE_NOT_CONFIRMED', 503);
  }
  const bets = await listBets();
  // Targeted readback is last and is not limited by the 5,000-row ledger page.
  const rows = await readByIds([betId]);
  const persisted = rows.find(bet => bet?.id === betId);
  if (!persisted || persisted.status === 'CANCELLED' || !trustedLedgerEvidence(persisted)
    || !cloudBetIntentMatches(value, persisted)) {
    throw cloudBetMutationError('下注寫入結果尚未通過永久帳本回讀確認，請重新核對後重試', 'BET_PERSISTENCE_NOT_CONFIRMED', 503);
  }
  return {
    bets: [persisted, ...bets.filter(bet => bet.id !== betId)],
    created: mutation.created === true,
    idempotent: mutation.idempotent === true,
    betId,
    persistence: { status: 'CONFIRMED', betId, readBack: true },
  };
}

export async function recoverPersistedCloudBet(value) {
  const existing = await findMatchingPersistedCloudBet(value);
  return existing ? confirmCloudBetPersistence(value, {
    created: false, idempotent: true, betId: existing.id,
  }) : null;
}
...
export async function upsertCloudBet(value, { verification = null } = {}) {
  ...
  return confirmCloudBetPersistence(bet, insertion);
}
...
export async function listCloudBetSettlementCandidates({ league = '', limit = 500, database = sql() } = {}) {
  const id = cleanText(league, 8).toUpperCase();
  if (id && !isLeagueId(id)) throw new Error('聯盟格式不正確');
  const cap = Math.max(1, Math.min(500, Math.trunc(Number(limit) || 500)));
  const rows = await database`
    SELECT payload
    FROM baseball_private_bets_v2
    WHERE (${id} = '' OR league = ${id})
      AND payload->>'readerEvidenceStatus' IN ('SERVER_VERIFIED_CURRENT_READER', 'SERVER_VERIFIED_CAPTURED_READER')
      AND payload->'pitEvidenceVerified' = 'true'::jsonb
      AND payload->>'pitPredictionStatus' = 'IMMUTABLE_PIT_VERIFIED'
      AND (
        status = 'OPEN'
        OR (
          status = 'MANUAL_REVIEW'
          AND payload->>'settlementError' IN ('缺少可驗證的前五局正式賽果', '缺少可驗證的全場正式賽果')
          AND (payload->'settlement' IS NULL OR payload->'settlement' = 'null'::jsonb)
          AND payload->'resultSnapshot'->>'final' = 'true'
        )
      )
    ORDER BY COALESCE(
      NULLIF(payload->>'lastResultCheckAt', ''),
      NULLIF(payload->'resultSnapshot'->>'checkedAt', ''),
      ''
    ) ASC, placed_at ASC, id ASC
    LIMIT ${cap}
  `;
  // The display ledger's most-recent-5,000 cap must never hide old OPEN bets
  // from settlement. A checked unresolved game yields to older unchecked work.
  return rows.map(row => sanitizeCloudBet(row.payload)).filter(bet => trustedLedgerEvidence(bet)
    && (bet.status === 'OPEN' || isRetryableResultGap(bet)));
}

export async function persistBetUpdates(values, previousValues = [], { database = sql() } = {}) {
  const previousById = new Map(previousValues.map(bet => [bet.id, bet]));
  const bets = values.map(sanitizeCloudBet).filter(Boolean);
  let updated = 0;
  for (const bet of bets) {
    const previous = previousById.get(bet.id);
    const previousStatus = isRetryableResultGap(previous) ? 'MANUAL_REVIEW' : 'OPEN';
    // Settlement owns only these fields. Preserve the placed contract, fixed
    // model evidence, and any Reader closing snapshot updated during the fetch.
    const patch = Object.fromEntries([
      'status', 'resultSnapshot', 'settlement', 'settlementError',
      'lastResultCheckAt', 'lastResultError', 'updatedAt',
    ].filter(key => Object.hasOwn(bet, key)).map(key => [key, bet[key]]));
    const rows = await database`
      UPDATE baseball_private_bets_v2
      SET status = ${bet.status}, payload = payload || ${JSON.stringify(patch)}::jsonb, updated_at = NOW()
      WHERE id = ${bet.id} AND status = ${previousStatus}
        AND payload->>'readerEvidenceStatus' IN ('SERVER_VERIFIED_CURRENT_READER', 'SERVER_VERIFIED_CAPTURED_READER')
        AND payload->'pitEvidenceVerified' = 'true'::jsonb
        AND payload->>'pitPredictionStatus' = 'IMMUTABLE_PIT_VERIFIED'
        AND (${previousStatus} = 'OPEN' OR (
          payload->>'settlementError' = ${previous?.settlementError || ''}
          AND (payload->'settlement' IS NULL OR payload->'settlement' = 'null'::jsonb)
          AND payload->'resultSnapshot'->>'final' = 'true'
        ))
      RETURNING id
  ...
  return { updated, skipped: bets.length - updated };
}

export async function settleOpenCloudBets({ league = '', limit = 500, timeBudgetMs = 240_000 } = {}) {
  const startedAt = Date.now();
  const id = cleanText(league, 8).toUpperCase();
  if (id && !isLeagueId(id)) throw new Error('聯盟格式不正確');
  const budget = Math.max(1, Math.min(240_000, Number(timeBudgetMs) || 240_000));
  requireDurableDatabase();
  await ensureSchema();
  const candidates = await listCloudBetSettlementCandidates({ league: id, limit });
  if (!candidates.length) return listCloudBets();
  const { settleBetTickets } = await import('./bet-settlement-service.js');
  let persistedTickets = 0;
  let skippedWrites = 0;
  const updates = await settleBetTickets(candidates, {
    concurrency: 4,
    timeBudgetMs: Math.max(0, budget - (Date.now() - startedAt)),
    onGroupSettled: async (groupUpdates, previous) => {
      const persisted = await persistBetUpdates(groupUpdates, previous);
      persistedTickets += persisted.updated;
      skippedWrites += persisted.skipped;
    },
  });
  const runSummary = updates.reduce((summary, bet) => {
    ...
    checkedTickets: updates.length,
    deferredTickets: candidates.length - updates.length,
    timeBudgetExhausted: updates.length < candidates.length,
    persistedTickets,
    skippedWrites,
