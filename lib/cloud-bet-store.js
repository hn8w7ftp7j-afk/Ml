...
export async function upsertCloudBet(value) {
  ...
  bet = {
    ...
    pitPredictionStatus: prediction.ok ? 'IMMUTABLE_PIT_CAPTURED' : 'PIT_CAPTURE_REJECTED',
    pitPredictionErrors: prediction.errors,
  };
  if (!cloudBetLeagueCanWrite(bet.league)) throw new Error(`${bet.league} 目前不可寫入實際下注紀錄`);
  await ensureSchema();
  const duplicate = await sql()`SELECT id FROM baseball_private_bets_v2 WHERE position_key = ${bet.positionIdentity} LIMIT 1`;
...
export async function mergeCloudBets(values) {
  ...
  for (const source of bets) {
    const prediction = source.pitPrediction ? { ok: true, prediction: source.pitPrediction, errors: [] } : buildPitPredictionFromBetV109(source);
    const bet = {
      ...source,
      pitPrediction: prediction.prediction,
      pitPredictionStatus: prediction.ok ? 'IMMUTABLE_PIT_CAPTURED' : 'LEGACY_PIT_UNAVAILABLE',
      pitPredictionErrors: prediction.errors,
    };
    const duplicate = await sql()`SELECT id FROM baseball_private_bets_v2 WHERE id = ${bet.id} OR position_key = ${bet.positionIdentity} LIMIT 1`;
