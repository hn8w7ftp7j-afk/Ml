...
export async function settleBetTicket(value) {
  ...
  // New tickets carry the immutable officialDate and are checked strictly
  // against it. Legacy tickets without officialDate are still safe to resolve
  // by the globally unique official gamePk instead of incorrectly comparing
  // the Taipei board date to the league-local official date.
  const officialResultDate = clean(bet.officialDate);
  const resultOptions = officialResultDate ? { date: officialResultDate } : {};
