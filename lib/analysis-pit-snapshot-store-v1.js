...
import { persistAnalysisDirectionHistoryBestEffort } from './analysis-direction-history-v1.js';
...
export const ANALYSIS_PIT_PAYLOAD_ENCODING_VERSION = 'BASEBALL-PIT-JSON-PAYLOAD-v1.0.0';
...
export function encodeAnalysisPitPayload(value, {
  ...
  if (rawBytes <= Number(inlineLimitBytes)) {
    return {
      ...
      payloadHash,
      // Store the exact serialized bytes. PostgreSQL JSONB reorders object
      // keys, so persisting the parsed object makes an unchanged payload fail
      // its original byte-length/hash check when replayed from the database.
      data: text,
    };
  }
  ...
  if (compressed.byteLength <= Number(compressedLimitBytes)) {
    return {
      ...
      base64Bytes: Math.ceil(compressed.byteLength / 3) * 4,
      payloadHash,
...
export async function encodeAnalysisPitPayloadAsync(value, options = {}) {
  ...
  const rawBytes = raw.byteLength;
  const payloadHash = exactSha256(raw);
  if (rawBytes <= Number(inlineLimitBytes)) return {
    version: ANALYSIS_PIT_PAYLOAD_ENCODING_VERSION,
    encoding: 'JSON',
    rawBytes,
    compressedBytes: null,
...
export function decodeAnalysisPitPayload(envelope, { allowLegacyJsonbReordered = false } = {}) {
  if (!envelope || envelope.version !== ANALYSIS_PIT_PAYLOAD_ENCODING_VERSION) {
  ...
  let text;
  const legacyJsonObject = envelope.encoding === 'JSON'
    && typeof envelope.data !== 'string'
    && Object.prototype.hasOwnProperty.call(envelope, 'value');
  if (envelope.encoding === 'JSON') {
    text = typeof envelope.data === 'string'
      ? envelope.data
      : jsonText(envelope.value, '內嵌內容');
  }
  ...
  if (raw.byteLength !== Number(envelope.rawBytes) || exactSha256(raw) !== envelope.payloadHash) {
    // v1.0 inline payloads were stored as parsed JSON objects. JSONB preserves
    // their semantic value but not insertion order, so the original exact
    // bytes cannot be reconstructed. Only immutable rows loaded by the
    // database replay path may use this compatibility branch. Direct callers
    // remain strict and new rows keep their exact JSON text above.
    if (allowLegacyJsonbReordered === true
      && legacyJsonObject
      && HASH_PATTERN.test(cleanText(envelope.payloadHash, 64).toLowerCase())) {
...
export function buildAnalysisPitSnapshotRecord({
  ...
  const record = {
    ...
    distributionId,
    distributionHash,
    distributionStorage: distributionPayload.encoding === 'JSON' ? 'FULL_JSON'
...
export function buildAnalysisPitReplayBundle(record, {
  parentRecords = [],
  expected = {},
  allowLegacyJsonbReorderedPayloads = false,
} = {}) {
  ...
  const decodeOptions = { allowLegacyJsonbReordered: allowLegacyJsonbReorderedPayloads };
  const distributionSnapshot = decodeAnalysisPitPayload(cursor.distributionPayload, decodeOptions);
  ...
  return {
    ...
    frozenContext: decodeAnalysisPitPayload(record.frozenContextPayload, decodeOptions),
    marketAnalysis: decodeAnalysisPitPayload(record.marketAnalysisPayload, decodeOptions),
...
async function selectAnalysisPitSnapshot(snapshotId, league) {
  const rows = await sql()`
    ...
    LIMIT 1
  `;
  return rows.length ? analysisPitRecordFromDatabaseRow(rows[0]) : null;
}
...
export async function loadAnalysisPitReplay({ league: leagueValue, snapshotId, expected = {} }) {
  ...
  return buildAnalysisPitReplayBundle(record, {
    parentRecords: parents,
    expected: { ...expected, leagueId: league },
    allowLegacyJsonbReorderedPayloads: true,
  });
}
...
export async function persistAnalysisPitSnapshot(record) {
  ...
  `;
  const rows = await sql()`
    SELECT snapshot_id, league_id, external_game_id, analysis_type, input_hash, core_fingerprint,
           distribution_id, distribution_hash, parent_snapshot_id, parent_analysis_type, parent_input_hash, parent_distribution_id, replay_identity_hash
    FROM baseball_analysis_pit_snapshots
    WHERE league_id = ${record.leagueId} AND analysis_type = ${record.analysisType} AND input_hash = ${record.inputHash}
    LIMIT 1
  `;
  if (!rows.length) return { stored: false, reason: 'POSTGAME_WRITE_REJECTED', snapshotId: record.snapshotId };
  assertStoredAnalysisPitIdentity(record, rows[0]);
  return { stored: true, inserted: inserted.length > 0, idempotent: inserted.length === 0, snapshotId: record.snapshotId };
}
...
export async function persistAnalysisPitSnapshotForResponse(input, { requiredWhenConfigured = true } = {}) {
  ...
  try {
    ...
    if (result.stored === true) {
      ...
      // public W/R values.
      const directionHistory = await persistAnalysisDirectionHistoryBestEffort({
        snapshotRecord: record,
        analysis: input?.analysis,
        readerSnapshot: input?.readerSnapshot || null,
      });
