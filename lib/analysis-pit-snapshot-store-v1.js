...
import { qualityReceiptFromAudit } from './performance-evidence-v1.js';
...
function analysisVersions(versions, analysis, context) {
  ...
  return {
    ...
    ...(source.marketEvidenceVersion ? {
      marketEvidenceVersion: requiredText(source.marketEvidenceVersion, '已驗證市場輸入版本', 180),
    } : {}),
  };
}

function compactMarketAnalysis(analysis, markets, previousMarkets, authenticatedSuppliedMarkets) {
  return {
    ...
    replayEnvironment: analysis?.replayEnvironment || null,
    distributionQA: analysis?.distributionQA || null,
    runExplanation: analysis?.runExplanation || null,
    dataAudit: analysis?.dataAudit || null,
    dataQuality: analysis?.dataQuality ?? null,
    dataQualityReceipt: qualityReceiptFromAudit(analysis?.dataAudit),
    ...
    ...(Array.isArray(authenticatedSuppliedMarkets) ? { authenticatedSuppliedMarkets } : {}),
...
function replayIdentity(record) {
  ...
  const orderedVersions = {
    ...
    ...(record.versions?.marketEvidenceVersion ? { marketEvidenceVersion: record.versions.marketEvidenceVersion } : {}),
  };
  return {
...
function semanticVersions(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return value;
  const {
    pitPayloadEncodingVersion: omittedPitPayloadEncodingVersion,
    ...versions
  } = value;
  return versions;
}
...
function semanticContract(value) {
  ...
  return {
    ...contract,
    ...(contract.versions ? { versions: semanticVersions(contract.versions) } : {}),
  };
}

function analysisPitSemanticIdentity(record) {
  validateAnalysisPitSnapshotRecord(record);
  // Payload encoding is a storage implementation detail. A retry may read an
  // immutable row written by an older encoder while carrying the exact same
  // decoded evidence and model result. Keeping the encoder version in the
  // semantic identity falsely turns that safe migration retry into a PIT
  // conflict, even though replayIdentityHash still preserves the exact bytes.
  return {
    schemaVersion: record.schemaVersion,
    ...
    versions: semanticVersions(record.versions),
  };
}

export function analysisPitSemanticIdentityHash(record) {
  return stableSha256(analysisPitSemanticIdentity(record));
}

export function analysisPitSemanticIdentityDiff(leftRecord, rightRecord) {
  const left = analysisPitSemanticIdentity(leftRecord);
  const right = analysisPitSemanticIdentity(rightRecord);
  return [...new Set([...Object.keys(left), ...Object.keys(right)])]
    .filter(key => stableSha256(left[key] ?? null) !== stableSha256(right[key] ?? null));
}

function semanticNestedDiff(left, right, prefix = '', depth = 3, output = []) {
  if (output.length >= 32 || stableSha256(left ?? null) === stableSha256(right ?? null)) return output;
  const leftObject = left != null && typeof left === 'object';
  const rightObject = right != null && typeof right === 'object';
  if (depth <= 0 || !leftObject || !rightObject
    || Array.isArray(left) !== Array.isArray(right)) {
    output.push(prefix || 'UNKNOWN');
    return output;
  }
  const keys = Array.isArray(left) && Array.isArray(right)
    ? [...new Set([...left.keys(), ...right.keys()])].map(String)
    : [...new Set([...Object.keys(left), ...Object.keys(right)])].sort();
  for (const key of keys) {
    semanticNestedDiff(left[key], right[key], prefix ? `${prefix}.${key}` : key, depth - 1, output);
    if (output.length >= 32) break;
  }
  return output;
}

export function analysisPitSemanticIdentityNestedDiff(leftRecord, rightRecord) {
  return semanticNestedDiff(
    analysisPitSemanticIdentity(leftRecord),
    analysisPitSemanticIdentity(rightRecord),
  );
}
...
export function buildAnalysisPitSnapshotRecord({
  ...
  const marketAnalysisPayload = encodedPayloads?.marketAnalysisPayload || encodeAnalysisPitPayload(compactMarketAnalysis(analysis, markets, previousMarkets, authenticatedSuppliedMarkets), {
    label: '市場分析',
...
export async function buildAnalysisPitSnapshotRecordAsync(input) {
  ...
  try { input = { ...input, frozenContext: await externalizeAsianSourceEvidence(input?.frozenContext) }; }
  catch {
    // Source-store availability is separate from the existing prediction/QA gates.
    // Retain exact inline evidence in the new snapshot when external storage fails.
    input = { ...input, frozenContext: { ...input.frozenContext,
      sourceEvidence: { ...input.frozenContext?.sourceEvidence, persistenceStatus: 'SOURCE_STORE_UNAVAILABLE_INLINE_EVIDENCE_RETAINED' } } };
  }
  const [frozenContextPayload, marketAnalysisPayload, distributionPayload] = await Promise.all([
    ...
    encodeAnalysisPitPayloadAsync(compactMarketAnalysis(input?.analysis, input?.markets, input?.previousMarkets, input?.authenticatedSuppliedMarkets), {
...
export function analysisPitRecordFromDatabaseRow(row) {
  ...
  const checked = validateAnalysisPitSnapshotRecord(record);
  // Trusted database insertion time is independent of user/model timestamps.
  // Do not add it to, or rewrite, the original signed payload.
  return { ...checked, databasePersistedAt: row.created_at == null ? null : isoInstant(row.created_at, '資料庫入庫') };
}
...
export function buildAnalysisPitReplayBundle(record, { parentRecords = [], expected = {} } = {}) {
  ...
  return {
    snapshotId: record.snapshotId,
    databasePersistedAt: record.databasePersistedAt || null,
    distributionDatabasePersistedAt: cursor.databasePersistedAt || null,
    leagueId: record.leagueId,
    gameIdentity: record.gameIdentity,
    inputHash: record.inputHash,
    coreFingerprint: record.coreFingerprint,
...
async function selectAnalysisPitSnapshot(snapshotId, league) {
  ...
  try {
    rows = await sql()`
      SELECT created_at, snapshot_id, schema_version, league_id, external_game_id, game_number, game_identity,
...
// Diagnostic reads deliberately do not call ensureAnalysisPitSchema(): opening
// a historical comparison must not create tables, settle bets, or write history.
export async function listAnalysisPitComparisonSnapshots({ league: leagueValue, gamePk, order = 'latest' }) {
  const league = leagueId(leagueValue);
  if (!Number.isSafeInteger(gamePk) || gamePk <= 0 || !['latest', 'oldest'].includes(order)) throw new Error('PIT診斷查詢範圍無效');
  if (!analysisPitDatabaseConfigured()) throw new Error('PIT診斷需要DATABASE_URL');
  let rows;
  try {
    rows = order === 'oldest' ? await sql()`
      SELECT snapshot_id, game_start, data_as_of, analysis_as_of, line_as_of,
             versions, evidence_status, quarantine_status, calibration_eligibility
      FROM baseball_analysis_pit_snapshots
      WHERE league_id = ${league} AND external_game_id = ${gamePk} AND analysis_type = 'FULL'
      ORDER BY analysis_as_of ASC, snapshot_id ASC LIMIT 21
    ` : await sql()`
      SELECT snapshot_id, game_start, data_as_of, analysis_as_of, line_as_of,
             versions, evidence_status, quarantine_status, calibration_eligibility
      FROM baseball_analysis_pit_snapshots
      WHERE league_id = ${league} AND external_game_id = ${gamePk} AND analysis_type = 'FULL'
      ORDER BY analysis_as_of DESC, snapshot_id DESC LIMIT 21
    `;
  } catch (error) { throw markDatabaseError(error, 'ANALYSIS_PIT_DIAGNOSTIC_LIST_FAILED'); }
  return { hasMore: rows.length > 20, snapshots: rows.slice(0, 20).map(row => ({
    snapshotId: row.snapshot_id, gameStart: row.game_start, dataAsOf: row.data_as_of,
    analysisAsOf: row.analysis_as_of, lineAsOf: row.line_as_of, versions: jsonColumn(row.versions),
    evidenceStatus: row.evidence_status, quarantineStatus: row.quarantine_status,
    calibrationEligibility: row.calibration_eligibility, payloadIntegrity: 'NOT_CHECKED_IN_METADATA_LIST',
  })) };
}

export async function loadAnalysisPitComparisonSnapshot({ league: leagueValue, gamePk, snapshotId }) {
  const league = leagueId(leagueValue);
  if (!Number.isSafeInteger(gamePk) || gamePk <= 0
    || typeof snapshotId !== 'string' || !new RegExp(`^${league}:${gamePk}:FULL:[a-f0-9]{64}$`).test(snapshotId)) throw new Error('PIT診斷快照必須屬於指定聯盟與場次的FULL分析');
  if (!analysisPitDatabaseConfigured()) throw new Error('PIT診斷需要DATABASE_URL');
  const record = await selectAnalysisPitSnapshot(snapshotId, league);
  if (!record) return null;
  // Verify payload hashes and the stored distribution as well as row metadata.
  try { buildAnalysisPitReplayBundle(record, { expected: { leagueId: league, gamePk } }); }
  catch (error) { throw analysisPitIntegrityError(error); }
  return record;
}
...
export async function persistAnalysisPitSnapshot(record) {
  ...
  if (storedRecord.replayIdentityHash !== record.replayIdentityHash
    ...
    const differingFields = analysisPitSemanticIdentityNestedDiff(storedRecord, record);
    throw new Error(`PIT資料庫重播識別衝突：semanticIdentityHash[${differingFields.join(',') || 'UNKNOWN'}]`);
  }
  return {
    stored: true,
