...
export function assertCanonicalAnalysisPitRetryIdentity(record, storedRecord) {
  validateAnalysisPitSnapshotRecord(record);
  validateAnalysisPitSnapshotRecord(storedRecord);
  const actual = {
    snapshotId: storedRecord.snapshotId,
    leagueId: storedRecord.leagueId,
    gamePk: storedRecord.gameIdentity?.gamePk,
    gameNumber: storedRecord.gameIdentity?.gameNumber,
    analysisType: storedRecord.analysisType,
    inputHash: storedRecord.inputHash,
    coreFingerprint: storedRecord.coreFingerprint,
    distributionId: storedRecord.distributionId,
    distributionHash: storedRecord.distributionHash,
    parentInputHash: storedRecord.parentInputHash || null,
    parentSnapshotId: storedRecord.parentSnapshotId || null,
    parentAnalysisType: storedRecord.parentAnalysisType || null,
    parentDistributionId: storedRecord.parentDistributionId || null,
    parentDistributionHash: storedRecord.parentDistributionHash || null,
  };
  const expected = {
    snapshotId: record.snapshotId,
    leagueId: record.leagueId,
    ...
    distributionId: record.distributionId,
    distributionHash: record.distributionHash,
    parentInputHash: record.parentInputHash || null,
    parentSnapshotId: record.parentSnapshotId || null,
    parentAnalysisType: record.parentAnalysisType || null,
    parentDistributionId: record.parentDistributionId || null,
    parentDistributionHash: record.parentDistributionHash || null,
  };
  for (const key of Object.keys(expected)) {
    if (String(actual[key] ?? '') !== String(expected[key] ?? '')) {
      throw new Error(`PIT資料庫 canonical 重試識別衝突：${key}`);
    }
  }
  return true;
}
...
export async function persistAnalysisPitSnapshot(record) {
  ...
  if (storedRecord.replayIdentityHash !== record.replayIdentityHash
    ...
    // The snapshot id is derived from the immutable model input. A deployment
    // may add diagnostic/provenance fields to the encoded analysis payload
    // without changing that model input. In that case the already persisted
    // row remains the canonical PIT record and a retry must be idempotent.
    // Still fail closed whenever any game, core-input, distribution or parent
    // identity differs; those differences can change the calculated result.
    assertCanonicalAnalysisPitRetryIdentity(record, storedRecord);
  }
  return {
    stored: true,
