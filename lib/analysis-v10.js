...
export const ANALYSIS_V10_VERSION = 'BASEBALL-RAW-WEIGHTED-ROBUST-EV-2026-08-v10.2.0';
...
function minimumWaterThresholds(weightedSummary, scenarioRows, rebateRate, crossMarketVerified) {
  ...
  return {
    ...
    score8_5: { ...required(0.070, 0.040), crossMarketVerified, marketQualificationRequired: false },
  };
}
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      const asianReliability = clamp(finite(context?.asianCalibration?.sampleReliability, 1), 0.05, 1);
      const reliabilityAdjustedWeightedEV = rawWeightedEV * asianReliability;
      const reliabilityAdjustedRobustEV = Math.min(reliabilityAdjustedWeightedEV, rawRobustEV * asianReliability);
      const reliabilityAdjustedProbability = 0.5
        + (weightedSummary.modelProbability - 0.5) * asianReliability;
      const evCalibration = qualifyEvV103({
        row,
        rawWeightedEV: reliabilityAdjustedWeightedEV,
        rawRobustEV: reliabilityAdjustedRobustEV,
        modelProbability: reliabilityAdjustedProbability,
        rebateRate,
        gate,
      });
      ...
      results.push({
        ...
        rawWeightedEV,
        rawRobustEV,
        uncalibratedWeightedEV: rawWeightedEV,
        uncalibratedRobustEV: rawRobustEV,
        asianReliabilityAdjustment: context?.asianCalibration ? {
          version: context.asianCalibration.version,
          reliability: asianReliability,
          adjustedWeightedEV: reliabilityAdjustedWeightedEV,
          adjustedRobustEV: reliabilityAdjustedRobustEV,
          adjustedModelProbability: reliabilityAdjustedProbability,
