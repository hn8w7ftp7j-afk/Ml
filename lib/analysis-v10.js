...
export const ANALYSIS_V10_VERSION = 'BASEBALL-RAW-WEIGHTED-ROBUST-EV-2026-08-v10.0.0';
...
export function evaluateMarketsFromDistribution({ context, markets, previousMarkets = [], settings = {}, distributionSnapshot }) {
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      const weightedEV = weightedSummary.ev;
      const robust = robustEvaluation({ scenarioRows, snapshot: distributionSnapshot, gate, weightedEV });
      const integrity = integrityAudit({ summary: weightedSummary, scenarioRows, weightedEV, scenarioWeightedEV, robust, gate });
      ...
      const movement = movementComparison({ previous, row, cells: combinedDistribution, context, rebateRate, weightedEV });
      ...
      results.push({
        ...
        score: 0,
        scoreAudit: { ok: integrity.passed, evidenceOnly: true, errors: integrity.failures, engineVersion: ANALYSIS_V10_VERSION },
        scoreBreakdown: null,
        tag: integrity.passed ? 'V10影子雙EV待固定公式評分' : '⛔ QA未通過｜不評分｜不下注',
        betEligible: false,
        unitSuggestion: null,
        primaryRisks: primaryRisks({ gate, robust, marketName, rawGap, context }),
        movement,
        distributionId: distributionSnapshot.distributionId,
        sourceStatuses: context?.sourceStatuses || context?.dataGateV10?.rows || {},
        dataGateV10: gate,
        numericalQA: {
          passed: weightedEV <= 0 || robust.numericLower95 > 0,
          standardError: robust.numericSE,
          lower95: robust.numericLower95,
          signStable: weightedEV <= 0 || robust.numericLower95 > 0,
          simulationsPerScenario: distributionSnapshot.simulationsPerScenario,
        },
        evDoubleCheck: {
          passed: weightedSummary.evDoubleCheckError <= 1e-9 && Math.abs(weightedEV - scenarioWeightedEV) <= 1e-6,
          directEV: weightedSummary.ev,
          bucketEV: weightedSummary.evFromBuckets,
          scenarioWeightedEV,
          combinedDistributionEV: weightedEV,
          maximumBucketError: weightedSummary.evDoubleCheckError,
          aggregationError: Math.abs(weightedEV - scenarioWeightedEV),
          tolerance: 1e-6,
