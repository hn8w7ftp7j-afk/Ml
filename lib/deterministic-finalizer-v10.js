...
export const FINAL_ENGINE_VERSION = 'BASEBALL-S-FIRST-DIAGNOSTIC-WARNINGS-2026-08-v11.2.0';
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const formulaVisible = leagueScoreValidated && qa.passed && boundary.ok && third.passed;
    const qaQualified = formulaVisible;
