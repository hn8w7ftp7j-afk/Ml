...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const calibrationReasons = Array.isArray(row.evCalibration?.reasons)
      ...
      : [];
    const dataQualityWarningOnly = row.evCalibration?.qualified === false
      && calibrationReasons.length > 0
      && calibrationReasons.every(reason => /^核心棒球資料品質\d+(?:\.\d+)?低於\d+(?:\.\d+)?安全門檻$/.test(reason))
    ...
    const formulaVisible = leagueScoreValidated && boundary.ok && (qa.passed || dataQualityWarningOnly);
