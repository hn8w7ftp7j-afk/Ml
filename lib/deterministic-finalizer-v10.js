...
export const FINAL_ENGINE_VERSION = 'BASEBALL-DETERMINISTIC-SHADOW-2026-08-v10.1.0';
export const UNCERTAINTY_SET_VERSION = 'BASEBALL-GH27-Q10-DATA-MARGIN-EXACT-v1.0.0';
export const FORMAL_SCORING_ENABLED = false;
export const SCORE_RELEASE_STATUS = 'SHADOW_EXACT_MODEL_NOT_FORMAL';
...
function shadowTag(score, label) {
  if (score == null) return '⛔ QA未通過｜不評分｜不下注';
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const diagnosticScore = qa.passed && boundary.ok && third.passed ? scoreResult.score : null;

    row.shadowDiagnosticScore = diagnosticScore;
    row.score = null;
    row.scoreStatus = diagnosticScore == null ? 'BLOCKED' : 'SHADOW_VALIDATED';
    ...
    row.tag = shadowTag(diagnosticScore, scoreResult.label);
    row.evFlipProbability = null;
    row.evFlipStatus = '情境權重診斷，不作頻率機率或額外扣分';
    row.modelErrorStatus = '已納入數值下界與資料誤差margin；正式門檻仍待locked OOS／forward驗證';
    row.riskMetricOverlapAudit = {
      robustEV: '情境Q10、數值下界與資料誤差的保守短板；影子使用',
  ...
  return {
    ...
    riskCalibrationStatus: {
      robustEV: 'shadow-q10-numerical-lower-data-margin',
