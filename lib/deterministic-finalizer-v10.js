...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const formulaDisplayWarningOnly = ((row.evCalibration?.qualified === false
      && calibrationReasons.length > 0
      && calibrationReasons.every(formulaDisplayReason))
      || staleReaderDisplayOnly)
    ...
    const formulaVisible = leagueScoreValidated && boundary.ok && (qa.passed || formulaDisplayWarningOnly);
    ...
    row.scoreBreakdown = {
      ...
      dataQualityWarningOnly: formulaDisplayWarningOnly,
