import { wrGapExceedsReference, currentWrWarnings } from './wr-gap-warning.js';
...
export const FINAL_ENGINE_VERSION = 'BASEBALL-S-FIRST-DIAGNOSTIC-WARNINGS-2026-08-v11.2.1';
...
// These thresholds only add diagnostic warnings. Tai888 remains the execution
// payoff and never rewrites model W/R, S, or the shared score distribution.
export const TARGET_MARKET_PROBABILITY_GAP_WARNING_THRESHOLD = 0.10;
// Backward-compatible export for audit consumers. It is no longer a hard gate.
export const MAX_TARGET_MARKET_PROBABILITY_GAP_FOR_QA = TARGET_MARKET_PROBABILITY_GAP_WARNING_THRESHOLD;
...
function shadowTag(score, label, { qaPassed = true, leagueValidated = true } = {}) {
  if (!qaPassed && score == null) return '⛔ QA BLOCK｜模型W/R照實顯示｜S分停用｜不列排名';
  if (score == null) return '⛔ 無法建立固定 S 分數';
  if (!leagueValidated) return `⚠️ ${score.toFixed(1)}｜${label}｜聯盟模型未驗證｜固定 S 分數僅供檢查｜不列排名`;
  if (!qaPassed) return `⛔ ${score.toFixed(1)}｜${label}｜QA BLOCK｜固定 S 分數僅供檢查｜不列排名`;
  if (score >= 8.5) return `🔥 ${score.toFixed(1)}｜${label}｜模型影子分析`;
  if (score >= 7.2) return `🟢 ${score.toFixed(1)}｜${label}｜模型影子分析`;
  return `⚪ ${score.toFixed(1)}｜${label}｜模型影子分析`;
}
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const calibrationContractAllowed = analysis?.alignmentAudit?.targetMarketCalibration === 'DISABLED_EXECUTION_PRICE_ONLY';
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const qa = baseQa(row);
    const calibrationReasons = Array.isArray(row.evCalibration?.reasons)
      ? row.evCalibration.reasons.map(reason => clean(reason)).filter(Boolean)
      : [];
    const dataQualityDisplayReason = reason => (
      /^核心棒球資料品質\d+(?:\.\d+)?低於\d+(?:\.\d+)?安全門檻$/.test(reason)
    );
    const readerFreshnessDisplayReason = reason => (
      /^Tai888 Reader 實際盤已過期或尚未完成最新版本驗證；可追溯快照的原始模型W\/R保留，但不得排名或下注$/.test(reason)
    );
    const formulaDisplayReason = reason => (
      dataQualityDisplayReason(reason) || readerFreshnessDisplayReason(reason)
    );
    const staleReaderDisplayOnly = row.evCalibration?.qualified === true
    ...
    const softCalibrationWarningsOnly = row.evCalibration?.qualified === false
      && calibrationReasons.length > 0
      && calibrationReasons.every(formulaDisplayReason);
    const dataQualityWarningOnly = softCalibrationWarningsOnly
      && calibrationReasons.some(dataQualityDisplayReason);
    const readerFreshnessWarningOnly = staleReaderDisplayOnly
      || (softCalibrationWarningsOnly && calibrationReasons.some(readerFreshnessDisplayReason));
    const dataQualityRankingWarningOnly = dataQualityWarningOnly && !readerFreshnessWarningOnly;
    // A soft display exception may bypass only the named quality/freshness
    // calibration gate. Contract parsing, W/R ordering, numerical confidence,
    // core data and integrity remain hard formula boundaries.
    const formulaInputIntegrityPassed = parseTaiwanLine(row.pick)?.valid === true
      && Number(row.robustEV) <= Number(row.weightedEV) + 1e-12
      && (Number(row.weightedEV) <= 0 || row.numericalQA?.passed === true)
      && row.dataGateV10?.passedForShadowScore === true
      && row.integrityWarning !== true;
    const formulaDisplayWarningOnly = (softCalibrationWarningsOnly || staleReaderDisplayOnly)
      && formulaInputIntegrityPassed;
    ...
    const scenarioSpread = row.evCalibration?.rawScenarioSpread;
    const scenarioStable = typeof scenarioSpread === 'number' && Number.isFinite(scenarioSpread)
      ? !wrGapExceedsReference(scenarioSpread) : row.evCalibration?.scenarioStable !== false;
    const distributionPlausible = targetMarketProbabilityGap == null
      || targetMarketProbabilityGap <= TARGET_MARKET_PROBABILITY_GAP_WARNING_THRESHOLD;
    const plausibilityHardGate = false;
    const plausibilityWarning = distributionPlausible
      ? null
      : `模型／Tai888去水機率高度分歧 ${(targetMarketProbabilityGap * 100).toFixed(2)}pp`;
    // Market disagreement and unusually high EV are diagnostic signals, not
    // proof of a data, distribution, contract, or settlement defect.
    ...
    const extremeEvWarning = extremeWeightedEvReview ? '極高模型EV，建議複核' : null;
    // Estimated-data quality and scenario spread are warning-only. Contract,
    // freshness, integrity, identity, and mathematical failures still suppress
    // ranking (and hard formula defects suppress the displayed score).
    const formulaThirdAuditPassed = third.failures.every(failure => failure === 'baseQa');
    const formulaVisible = leagueScoreValidated
      && boundary.ok
      && formulaInputIntegrityPassed
      && formulaThirdAuditPassed
      && (qa.passed || formulaDisplayWarningOnly);
    // Low estimated-data quality is warning-only. This also upgrades cached
    // rows produced by older releases where the same condition was stored as
    // the sole calibration failure. Reader freshness and every real contract,
    // identity, numerical, distribution or integrity error remain blocking.
    const qaQualified = leagueScoreValidated
      && boundary.ok
      && formulaInputIntegrityPassed
      && formulaThirdAuditPassed
      && (qa.passed || dataQualityRankingWarningOnly);
    const formulaDiagnosticScore = formulaVisible ? calculatedScore : null;
    const diagnosticScore = qaQualified ? calculatedScore : null;
    const calibrationWarnings = currentWrWarnings(row.evCalibration?.auditWarnings || [], scenarioSpread).filter(warning => !(
      extremeWeightedEvReview && /極高模型EV|未校準模型W達/.test(String(warning || ''))
    ));
    const cachedDataQualityWarnings = dataQualityWarningOnly
      ? calibrationReasons.filter(dataQualityDisplayReason)
        .map(reason => `${reason.replace('安全門檻', '參考線')}；保留評分與排名，並標示預估資料風險`)
      : [];
    const diagnosticWarnings = unique([
      plausibilityWarning,
      extremeEvWarning,
      ...cachedDataQualityWarnings,
      ...calibrationWarnings,
    ]);

    row.formulaDiagnosticScore = formulaDiagnosticScore;
    row.shadowDiagnosticScore = diagnosticScore;
    row.formulaDiagnosticBand = formulaVisible ? scoreResult.band : null;
    row.formulaDiagnosticLabel = formulaVisible ? scoreResult.label : null;
        ...
        : Number(row.weightedEV) <= 0 ? '加權EV未大於0'
          : Number(row.robustEV) <= 0 ? '穩健EV未大於0'
            : formulaDiagnosticScore < candidateThreshold ? `公式分數未達${candidateThreshold.toFixed(1)}`
              : scenarioStable
                ? '雙EV為正、分數達門檻且資料QA通過'
                : '雙EV為正、分數達門檻且資料QA通過；W/R差距超過5個百分點列警告';
    row.score = null;
    ...
    row.scoreBreakdown = {
      ...
      rawScore: formulaVisible ? scoreResult.rawScore : null,
      rawUnqualifiedScore: !formulaVisible ? calculatedScore : null,
      formulaDiagnosticScore,
      qaQualifiedDiagnosticScore: diagnosticScore,
      diagnosticScore,
      finalScore: null,
      progress: scoreResult.progress,
      caps: [...scoreResult.caps, ...(!scenarioStable ? ['SCENARIO_SPREAD_OVER_5_PERCENT'] : []), ...(extremeWeightedEvReview ? ['WEIGHTED_EV_20_PERCENT_WARNING'] : []), ...(!distributionPlausible ? ['TARGET_MARKET_PROBABILITY_GAP_WARNING'] : [])],
      diagnosticWarnings,
      noGptScoring: true,
      ...
      dataQualityWarningOnly,
      readerFreshnessWarningOnly,
      formulaDisplayWarningOnly,
      formalScoringEnabled: FORMAL_SCORING_ENABLED,
      scoreReleaseStatus: SCORE_RELEASE_STATUS,
    };
    row.scoreAudit = {
      ok: qaQualified && diagnosticScore != null,
      ...
      boundary,
      thirdAudit: third,
      plausibility: { passed: true, observedWithinLimit: distributionPlausible, hardGate: plausibilityHardGate, failures: [], auditWarnings: plausibilityWarning ? [plausibilityWarning] : [], targetMarketProbabilityGap, source: explicitTai888Gap != null ? 'tai888MarketProbabilityGap' : legacyRawGap != null ? 'rawMarketProbabilityGap' : null, maximumGap: TARGET_MARKET_PROBABILITY_GAP_WARNING_THRESHOLD },
      extremeEvReview: { required: extremeWeightedEvReview, passed: true, hardGate: false, threshold: EXTREME_WEIGHTED_EV_REVIEW_THRESHOLD, preservesModelValues: true, auditWarnings: extremeEvWarning ? [extremeEvWarning] : [] },
      diagnosticWarnings,
      corrections: [],
      noGptScoring: true,
      formalScoringEnabled: FORMAL_SCORING_ENABLED,
    ...
    row.diagnosticWarnings = diagnosticWarnings;
    row.betEligible = false;
    row.unitSuggestion = null;
    row.unitStatus = '模型影子分析｜Unit未設定';
    row.tag = shadowTag(formulaDiagnosticScore, row.formulaDiagnosticLabel || scoreResult.label, {
      qaPassed: qaQualified,
      leagueValidated: leagueScoreValidated,
    });
    row.evFlipProbability = null;
    row.evFlipStatus = '情境權重診斷，不作頻率機率或額外扣分';
    row.modelErrorStatus = 'W/R只來自棒球聯合比分分布；Tai888只作成交payoff與分歧警示，獨立市場只作可選外部稽核；市場警示不回灌模型、不取消S或排名';
    row.riskMetricOverlapAudit = {
      robustEV: '模型情境Q10與資料誤差下界；尚未完成locked OOS，只作影子模型診斷',
      dataQuality: '低於0.85只作警告；核心資料缺失或身分衝突仍BLOCK，不直接改寫分數',
      marketVerification: '獨立同合約共識只作外部稽核，不取代模型EV',
      targetMarketCalibration: '停用；Tai888只作成交payoff與分歧警示，不改寫比分分布、EV、S或排名',
  ...
  for (const market of [...new Set(results.map(row => row.market))]) {
    ...
    for (const row of pair) {
      row.pairAudit = audit;
      if (!audit.passed) {
        const rawUnqualifiedScore = optionalFinite(row.scoreBreakdown?.rawUnqualifiedScore)
          ?? optionalFinite(row.formulaDiagnosticScore);
        row.formulaDiagnosticScore = null;
        row.shadowDiagnosticScore = null;
        row.formulaDiagnosticBand = null;
        row.formulaDiagnosticLabel = null;
        row.score = null;
        ...
        row.scoreBreakdown = {
          ...
          qaQualifiedDiagnosticScore: null,
