...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    // Scenario instability and plausibility remain ranking gates. They do not
    // alter or hide the inspectable fixed W/R formula output: Tai888 is an
    // execution price and audit reference, never model truth that may rewrite
    // the score distribution or erase a calculable S value.
