...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    // A low-confidence data estimate is a ranking block, not a formula
    // failure. Keep the fixed S visible for inspection while QA/ranking remain
    // fail-closed. Contract, freshness, integrity, and mathematical failures
    // still suppress the score entirely.
