...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    // A hard MLB plausibility failure means the upstream distribution is not
    // qualified to publish a number as EV/S. Keep the raw calculation only in
    // the audit payload; never tilt the baseball distribution toward Tai888.
