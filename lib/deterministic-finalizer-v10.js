...
export const FINAL_ENGINE_VERSION = 'BASEBALL-DETERMINISTIC-SHADOW-2026-08-v10.6.1';
...
function thirdAudit(row, scoreResult, qaPassed) {
  ...
  const checks = {
    ...
    targetMarketNotUsedForCalibration: row.marketCalibrationApplied === false || row.marketBaselineApplied === true,
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const leagueScoreValidated = leagueId === 'MLB'
    && ['DISABLED_EXECUTION_PRICE_ONLY', 'PROVISIONAL_EXTREME_GAP_MARKET_BASELINE'].includes(analysis?.alignmentAudit?.targetMarketCalibration)
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const qaQualified = leagueScoreValidated && qa.passed && boundary.ok && third.passed && distributionPlausible;
    ...
    row.rankingQualified = diagnosticScore != null
      ...
      && scenarioStable
      && distributionPlausible;
    row.rankingQualificationReason = !leagueScoreValidated ? '聯盟模型未驗證'
      : !distributionPlausible ? plausibilityFailure
    ...
    row.scoreBreakdown = {
      ...
      caps: [...scoreResult.caps, ...(!scenarioStable ? ['SCENARIO_SPREAD_OVER_5_PERCENT'] : []), ...(!distributionPlausible ? ['TARGET_MARKET_PROBABILITY_GAP_QA_BLOCK'] : [])],
      noGptScoring: true,
      targetMarketCalibrationDisabled: row.marketCalibrationApplied === false,
      qaOnlyAsGateOrBlock: true,
      qaControlsQualificationNotFormulaVisibility: distributionPlausible,
      formalScoringEnabled: FORMAL_SCORING_ENABLED,
      scoreReleaseStatus: SCORE_RELEASE_STATUS,
    };
    row.scoreAudit = {
      ok: diagnosticScore != null && distributionPlausible,
      ...
      boundary,
      thirdAudit: third,
      plausibility: { passed: distributionPlausible, failures: plausibilityFailure ? [plausibilityFailure] : [], targetMarketProbabilityGap, source: explicitTai888Gap != null ? 'tai888MarketProbabilityGap' : legacyRawGap != null ? 'rawMarketProbabilityGap' : null, maximumGap: MAX_TARGET_MARKET_PROBABILITY_GAP_FOR_QA },
      corrections: [],
      noGptScoring: true,
      formalScoringEnabled: FORMAL_SCORING_ENABLED,
    };
    ...
    row.tag = shadowTag(formulaDiagnosticScore, row.formulaDiagnosticLabel || scoreResult.label, {
      qaPassed: qa.passed && boundary.ok && third.passed && distributionPlausible,
      leagueValidated: leagueScoreValidated,
    });
    row.evFlipProbability = null;
    row.evFlipStatus = '情境權重診斷，不作頻率機率或額外扣分';
    row.modelErrorStatus = row.marketBaselineApplied
      ? '市場基準暫行W/R：原始模型與Tai888雙邊去水差距超過10pp；完成真實locked OOS前不視為獨立模型優勢'
      : 'W/R只來自棒球聯合比分分布；獨立市場只作可選外部稽核；尚未locked OOS校準，仍只作Shadow診斷';
    row.riskMetricOverlapAudit = {
      ...
      targetMarketCalibration: row.marketBaselineApplied ? '極端背離時啟用市場基準暫行分布；非獨立模型優勢' : '停用；Tai888只作成交payoff',
    ...
    } else if (diagnosticScore == null || !distributionPlausible) {
      failures.push(`${row.market}｜${row.pick}：${[...qa.failures, ...boundary.errors, ...third.failures, ...(plausibilityFailure ? [plausibilityFailure] : [])].join('；')}`);
    }
    if (scoreResult.caps.includes('INDEPENDENT_MARKET_AUDIT_UNAVAILABLE')) {
      corrections.push(`${row.market}｜${row.pick}：缺少獨立同合約市場，仍列分析排名但標示外部稽核未完成`);
    }
    return row;
  });
