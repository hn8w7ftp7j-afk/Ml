...
export const FINAL_ENGINE_VERSION = 'BASEBALL-DETERMINISTIC-SHADOW-2026-08-v10.8.1';
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    row.scoreBreakdown = {
      ...
      caps: [...scoreResult.caps, ...(!scenarioStable ? ['SCENARIO_SPREAD_OVER_5_PERCENT'] : []), ...(!plausibilityQualified ? ['TARGET_MARKET_PROBABILITY_GAP_QA_BLOCK'] : !distributionPlausible ? ['ASIAN_TARGET_MARKET_GAP_AUDIT_ONLY'] : [])],
      noGptScoring: true,
