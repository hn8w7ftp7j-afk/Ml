...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    // Scenario instability and an uncalibrated 15%+ W remain ranking gates.
    // A failed distribution-plausibility check is different: the upstream
    ...
    row.scoreBreakdown = {
      ...
      caps: [...scoreResult.caps, ...(!scenarioStable ? ['SCENARIO_SPREAD_OVER_5_PERCENT'] : []), ...(!distributionPlausible ? ['TARGET_MARKET_PROBABILITY_GAP_QA_BLOCK'] : []), ...(extremeModelReview ? ['UNCALIBRATED_W_OVER_15_PERCENT'] : [])],
      noGptScoring: true,
