...
export const FINAL_ENGINE_VERSION = 'BASEBALL-DETERMINISTIC-SHADOW-2026-08-v10.5.0';
export const UNCERTAINTY_SET_VERSION = 'BASEBALL-MODEL-SCENARIO-LOWER-v2.1.0';
...
function thirdAudit(row, scoreResult, qaPassed) {
  ...
  const checks = {
    ...
    probabilityCoverage: row.evCalibration?.referencePriorEligible === true,
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    if (row.water == null || !hasActualWater(row.water) || !weightedCalculable || !robustCalculable || !readerExecutionQualified) {
      ...
      row.scoreSource = 'V10.4.3三莊逐腿payoff共識雙EV短板公式｜影子模式';
    ...
    const calculatedScore = Number.isFinite(Number(scoreResult.score)) ? Number(scoreResult.score) : null;
    ...
    row.scoreSource = 'V10.5聯合比分分布模型 W/R 固定雙EV短板公式｜影子模式';
    row.scoreFormulaVersion = SCORE_FORMULA_VERSION;
    row.scoreContractVersion = SCORE_FORMULA_VERSION;
    ...
    row.modelErrorStatus = 'W/R只來自棒球聯合比分分布；獨立市場只作外部稽核；尚未locked OOS校準，仍只作Shadow診斷';
