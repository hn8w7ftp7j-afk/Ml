...
export const FINAL_ENGINE_VERSION = 'BASEBALL-W-FIRST-DETERMINISTIC-SHADOW-2026-08-v11.1.0';
...
function shadowTag(score, label, { qaPassed = true, leagueValidated = true } = {}) {
  if (!qaPassed && score == null) return '⚠️ QA BLOCK｜模型W/R照實顯示｜S分停用｜不列排名｜不可下注';
...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    // A hard MLB plausibility failure blocks S/ranking/execution only. The raw
    // model W/R remains public and the distribution is never tilted to Tai888.
    ...
    const qaQualified = formulaVisible && plausibilityQualified;
    const formulaDiagnosticScore = formulaVisible && plausibilityQualified ? calculatedScore : null;
    ...
    row.scoreBreakdown = {
      ...
      rawUnqualifiedScore: formulaVisible && !plausibilityQualified ? calculatedScore : null,
