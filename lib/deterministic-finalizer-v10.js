...
export function finalizeDeterministicAnalysis({ analysis, game, settings = {} }) {
  ...
  const results = (analysis?.results || []).map(source => {
    ...
    const targetMarketProbabilityGap = Number(row.rawMarketProbabilityGap);
    const distributionPlausible = !Number.isFinite(targetMarketProbabilityGap)
    ...
    row.scoreAudit = {
      ...
      boundary,
      thirdAudit: third,
      plausibility: { passed: distributionPlausible, failures: plausibilityFailure ? [plausibilityFailure] : [], targetMarketProbabilityGap, maximumGap: MAX_TARGET_MARKET_PROBABILITY_GAP_FOR_QA },
      corrections: [],
      noGptScoring: true,
      formalScoringEnabled: FORMAL_SCORING_ENABLED,
    };
