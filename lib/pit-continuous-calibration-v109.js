...
export const CONTINUOUS_CALIBRATION_V109_VERSION = 'baseball-continuous-hierarchical-isotonic-v3-result-cutoff';
...
const latest = (rows, field) => rows.reduce((time, row) => row[field] > time ? row[field] : time, '') || null;
...
export function fitWeightedIsotonicV109(rows, { maximumBins = 20, minimumBinSize = 20, through = null, halfLifeDays = 730 } = {}) {
  ...
  const trainedThrough = through || latest(rows, 'settledAt') || latest(rows, 'gameStart') || new Date().toISOString();
  const bins = [];
...
export function predictWeightedIsotonicV109(model, rawWeightedEv) {
  const raw = finite(rawWeightedEv);
  if (!Array.isArray(model) || !model.length || raw == null) return null;
  const hit = model.find(block => raw <= block.maxRawEv);
...
export function buildContinuousOosCalibrationV109(inputs, options = {}) {
  ...
  for (const month of months) {
    ...
    // A previous game's date does not prove its result was available. Freeze
    // the fold before both its calendar boundary and every validation model.
    const trainingCutoff = validation.reduce((cutoff, row) => row.modelAsOf < cutoff ? row.modelAsOf : cutoff, `${month}-01T00:00:00.000Z`);
    const train = rows.filter(row => monthOf(row.gameStart) < month && row.settledAt < trainingCutoff);
    if (train.length < minimumTrainRows || validation.length < (options.minimumRowsPerFold || 10)) continue;
    const hierarchy = fitHierarchy(train, { ...options, through: trainingCutoff });
    const knownResiduals = predictions.filter(row => row.settledAt < trainingCutoff);
    const monthly = residualBlocks(knownResiduals, monthOf);
    const daily = residualBlocks(knownResiduals, dayOf);
    const foldRobustAdjustment = monthly.length && daily.length ? Math.min(
      quantile(monthly.map(row => row.residual), options.robustQuantile || 0.10),
      quantile(daily.map(row => row.residual), options.robustQuantile || 0.10),
    ) : null;
    const foldRows = validation.map(row => {
      const calibratedW = predictHierarchy(hierarchy, row);
      return {
        ...row, calibratedW, validationMonth: month, trainingCutoff,
        robustR: foldRobustAdjustment == null ? null : calibratedW + foldRobustAdjustment,
        robustAdjustment: foldRobustAdjustment,
        robustResidualRows: knownResiduals.length,
        robustTrainedThrough: latest(knownResiduals, 'settledAt'),
      };
    });
    predictions.push(...foldRows);
    folds.push({ validationMonth: month, trainingCutoff, trainedThrough: latest(train, 'settledAt'), trainingGameThrough: latest(train, 'gameStart'), trainRows: train.length, validationRows: validation.length, robustResidualRows: knownResiduals.length, robustTrainedThrough: latest(knownResiduals, 'settledAt') });
  }
  ...
  // The final artifact may use all known OOS residuals. Historical predictions
  // retain the adjustment that was knowable at their fold, never this hindsight fit.
  const withRobust = predictions;
  const positive = withRobust.filter(row => row.calibratedW > 0 && row.robustR != null && row.robustR > 0);
  ...
  const trainedThrough = latest(rows, 'settledAt');
  const hierarchy = fitHierarchy(rows, { ...options, through: trainedThrough });
  const artifactCore = {
    schemaVersion: PIT_OBSERVATION_SCHEMA_V109,
    calibrationVersion: CONTINUOUS_CALIBRATION_V109_VERSION,
    trainedThrough,
    trainingGameThrough: latest(rows, 'gameStart'),
    trainedThroughBasis: 'LATEST_RESULT_SETTLED_AT',
    ...
    diagnostics: {
      ...
      oosMeanR: mean(withRobust.map(row => row.robustR).filter(Number.isFinite)),
      oosRobustSampleSize: withRobust.filter(row => Number.isFinite(row.robustR)).length,
