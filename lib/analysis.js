...
export const MODEL_VERSION = 'GPT研究整合聯合情境模型-2026-08-v7.0.1';
export const RULES_VERSION = 'MLB-TW-EXECUTION-2026-08-v7.0.1';
...
export function analyzeMarkets({ context, markets, previousMarkets = [], settings = {} }) {
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      if (calibration.rawProbabilityGap > 0.18 || calibration.divergenceRisk > 0.18) score = Math.min(score, 7.4);
      ...
      if (calibration.rawProbabilityGap > 0.12 || calibration.divergenceRisk > 0.12) units = Math.min(units, 0.5);
