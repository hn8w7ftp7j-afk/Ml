...
export const MODEL_VERSION = 'GPT完整指令聯合情境模型-2026-08-v8.0.0';
export const RULES_VERSION = 'MLB-TW-EXECUTION-2026-08-v8.0.0';
...
export function analyzeMarkets({ context, markets, previousMarkets = [], settings = {} }) {
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    for (const row of rows) {
      ...
      const scenarioEvaluations = scenarios.map(scenario => {
        ...
        const calibratedProbability = rawSummary.modelProbability;
        return {
          id: scenario.id,
      ...
      results.push({
        ...
        calibratedMarketProbabilityGap: null,
        ...
        outcomeProbabilitiesSource: 'GPT完整資料聯合情境原始比分分布（市場不回灌）',
        fairWater,
  ...
  return {
    ...
    alignmentAudit: {
      ...
      modules: [
        { name: '實際開盤市場與單邊水位', status: '已實作；僅作價格/EV，不回灌過盤率' },
