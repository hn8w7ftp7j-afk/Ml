...
export function normalizeModelConfig(input) {
  ...
  const automaticRunnerValue = valueAt(input, [['automaticRunner']]);
  if (automaticRunnerValue !== undefined && typeof automaticRunnerValue !== 'boolean') {
    throw new Error('模型設定 automaticRunner 必須是布林值');
  }
  ...
  return {
    ...
    automaticRunner: automaticRunnerValue ?? DEFAULT_MODEL_CONFIG.automaticRunner,
  };
}
...
function offenseProfile(team, opposingStarter, adjustment = {}, leagueRunsBaseline = 4.35) {
  ...
  const runs = blend(season.runsPerGame, recent.runsPerGame, recentWeight, leagueRunsBaseline);
  ...
  let factor = geometricBlend([
    [ratio(runs, leagueRunsBaseline, 0.76, 1.27), 0.32],
...
function starterProfile(starter, adjustment = {}, leagueRunsBaseline = 4.2) {
  ...
  const era = blend(season.era, recent.era, recentWeight, leagueRunsBaseline);
  ...
  let factor = geometricBlend([
    [ratio(era, leagueRunsBaseline, 0.70, 1.42), 0.29],
    [ratio(fip, leagueRunsBaseline, 0.72, 1.38), 0.25],
...
function bullpenProfile(team, adjustment = {}, leagueRunsBaseline = 4.2) {
  const recent = team?.recentPitching || {};
  const recentWeight = sampleWeight(recent.inningsPitched, 55, 0.08, 0.24);
  const recentEra = blend(leagueRunsBaseline, recent.era, recentWeight, leagueRunsBaseline);
  ...
  const qualityFactor = Number.isFinite(suppliedQuality)
    ...
    : geometricBlend([[ratio(recentEra, leagueRunsBaseline, 0.82, 1.22), 0.58], [ratio(recentWhip, 1.3, 0.86, 1.18), 0.42]]);
  let factor = geometricBlend([
    [qualityFactor, 0.56],
    [ratio(recentEra, leagueRunsBaseline, 0.84, 1.20), 0.12],
    [1 + fatigue * 0.17, 0.21],
...
function gameContextProfile(context, modelConfig = DEFAULT_MODEL_CONFIG) {
  ...
  const leagueId = contextLeagueId(context);
  const asianLeague = ASIAN_LEAGUES.has(leagueId);
  const asianBaseline = clamp(safe(context?.league?.runsPerTeamGame, 4.35), modelConfig.baselineBounds.full.min, modelConfig.baselineBounds.full.max);
  const offenseBaseline = asianLeague ? asianBaseline : 4.35;
  const pitchingBaseline = asianLeague ? asianBaseline : 4.2;
  const awayStarter = starterProfile(context?.away?.starter, expertResidual(expert, 'awayStarter'), pitchingBaseline);
  const homeStarter = starterProfile(context?.home?.starter, expertResidual(expert, 'homeStarter'), pitchingBaseline);
  const awayOffense = offenseProfile(context?.away, context?.home?.starter, expertResidual(expert, 'awayOffense'), offenseBaseline);
  const homeOffense = offenseProfile(context?.home, context?.away?.starter, expertResidual(expert, 'homeOffense'), offenseBaseline);
  const awayBullpen = bullpenProfile(context?.away, expertResidual(expert, 'awayBullpen'), pitchingBaseline);
  const homeBullpen = bullpenProfile(context?.home, expertResidual(expert, 'homeBullpen'), pitchingBaseline);
...
function simulateScenario(scenario, simulations, seed, modelConfig = DEFAULT_MODEL_CONFIG) {
  ...
  for (let simulation = 0; simulation < simulations; simulation += 1) {
    ...
    while (awayRuns === homeRuns && extraInning < modelConfig.extraInningsLimit) {
      extraInning += 1;
      const automaticRunnerBoost = modelConfig.automaticRunner === true ? 0.16 : 0;
      awayRuns += poissonSample((awayLateRate * 1.32 + automaticRunnerBoost) * sharedGameFactor * awayGameFactor, random);
      const homeRunsThisInning = poissonSample((homeLateRate * 1.32 + automaticRunnerBoost) * sharedGameFactor * homeGameFactor, random);
      const needed = awayRuns - homeRuns + 1;
...
export function buildDistributionSnapshot({ context, settings = {} }) {
  ...
  const gamePk = Number(context?.game?.gamePk);
  const snapshotCore = {
    version: 'MLB-FROZEN-JOINT-DISTRIBUTION-2026-08-v1.0.0',
    gamePk: Number.isSafeInteger(gamePk) && gamePk > 0 ? gamePk : null,
  ...
  return {
    ...snapshotCore,
    distributionHash,
    distributionId: `${snapshotCore.gamePk || 'game'}-${distributionHash.slice(0, 20)}`,
    drawProb,
  };
}
...
function minimumWaterThresholds(scenarioEvaluations, robustSets, currentWater, rebateRate, crossMarketVerified) {
  ...
  return {
    ...
    score8_5: {
      ...
      marketQualificationRequired: false,
    },
