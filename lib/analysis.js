...
export const MODEL_VERSION = 'GPT完整指令聯合情境模型-2026-08-v8.1.0';
export const RULES_VERSION = 'MLB-TW-EXECUTION-2026-08-v8.1.0';
...
export function analyzeMarkets({ context, markets, previousMarkets = [], settings = {} }) {
  ...
  for (const marketName of MARKET_ORDER) {
    ...
    if (pair.length === 2) {
      ...
      if (pair.every(result => Number.isFinite(result.score) && result.score > 5.000001)) {
        for (const result of pair) {
          result.integrityWarning = true;
