import { analysisIsUnopenedOnly } from './analysis-display-state-v116.js';

// v7 invalidates current-board views produced before personnel source ownership,
// lineup slot, official starter identity and assignment corrections. Historical PIT/ledger rows
// remain immutable; reopening the live board must fetch current evidence.
...
function compactAnalysisData(data) {
  ...
  // An all-UNOPENED response is live Reader waiting state, not a completed
  // model result. Persisting it for 72 hours made a transient partial board
  // reappear as a giant blank eight-direction card after reopening the PWA.
  if (analysisIsUnopenedOnly(data)) return null;
  return {
    game: data.game || null,
...
export function compactAnalysisBoard(board, { storage = false } = {}) {
  return (Array.isArray(board) ? board : []).flatMap(item => {
    ...
    const referenceData = compactAnalysisData(item.referenceData) || customData;
    // Completed actual-board responses commonly use the exact same immutable
    // payload twice. Store it once; the existing null fallback restores the
    // complete value, including QA, hashes and all distribution evidence.
    const referenceEqualsCustom = storage && JSON.stringify(referenceData) === JSON.stringify(customData);
    return [{
      game: item.game,
      ...
      referenceData: referenceEqualsCustom ? null : referenceData,
      customData,
      identityConflict: item.identityConflict || null,
      status: 'done',
...
export function createAnalysisBoardCacheEntry({ league, date, board, savedAt = Date.now() } = {}) {
  const compactBoard = compactAnalysisBoard(board, { storage: true });
...
export function restoreAnalysisBoardCache(entry, { league, date, now = Date.now(), maxAgeMs = DEFAULT_MAX_AGE_MS } = {}) {
  const requestedLeague = clean(league).toUpperCase();
  const version = Number(entry?.version);
  if (!entry || version !== CACHE_VERSION) return [];
  if (entry.league !== requestedLeague || entry.date !== clean(date)) return [];
