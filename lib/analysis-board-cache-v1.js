// v2 invalidates browser snapshots whose MLB W/R may have been produced by
// the removed per-direction Tai888 market-baseline tilt.
...
export function restoreAnalysisBoardCache(entry, { league, date, now = Date.now(), maxAgeMs = DEFAULT_MAX_AGE_MS } = {}) {
  ...
  if (!entry || ![1, CACHE_VERSION].includes(version)) return [];
  // The removed probability feedback only affected MLB analysis-v11. Keep
  // valid Asian shadow-board recovery intact while forcing MLB to recompute.
  if (version === 1 && requestedLeague === 'MLB') return [];
