...
export const NHL_PERSONNEL_FEED_VERSION = 'NHL-OFFICIAL-EDITORIAL-PERSONNEL-v1';
  ...
  const operation = (async () => {
    ...
    try {
      ...
      if (response?.redirected || response?.type === 'opaqueredirect' || (response?.status >= 300 && response.status < 400)) {
        value = issue('NHL_PERSONNEL_REDIRECT_NOT_FOLLOWED', { httpStatus: response.status || null, source });
        ...
        value = issue(httpStatus === 403 ? 'NHL_PERSONNEL_FORBIDDEN' : httpStatus === 429 ? 'NHL_PERSONNEL_RATE_LIMITED'
...
function explicitConfirmedGoalies(texts, listedGoalies, roster) {
  ...
  for (const player of listedGoalies) {
    ...
    for (const text of texts) {
      for (const sentence of normalized(text).split(/[.!?…]/)) {
        ...
        if (new RegExp(`(?:^|\\s)${name} (?:will start|starts|will get the start|will make the start)(?: in goal| in net)?(?: tonight| today)?$`).test(sentence.trim())) matches.push(player);
      }
    }
  }
