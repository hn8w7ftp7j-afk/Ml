...
import { validateNhlPersonnelObservation } from './personnel-observation.js';
...
export function validateNhlObservation(kind, key, payload, { now = Date.now() } = {}) {
  ...
  if (kind === 'PERSONNEL') errors.push(...validateNhlPersonnelObservation(payload, key, { now }).errors);
