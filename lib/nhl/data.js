...
import { normalizeNhlGameReport } from './game-report.js';
import { nhlTeamSummaryUrl, normalizeNhlTeamSummary, validNhlTeamSummaryScope } from './team-summary.js';
import { nhlSourceCooldown, recordNhlSourceRateLimit } from './source-retry-policy.js';

export const NHL_DATA_VERSION = 'NHL-OFFICIAL-DATA-v1.3.0';
export const NHL_SOURCES = Object.freeze({
  ...
  lineupNews: { provider: 'NHL', url: 'https://www.nhl.com/news/nhl-lineup-projections-2025-26-season', role: 'dated_editorial_lineup_injury_evidence', structuredFeed: false, adapter: 'NHL-OFFICIAL-EDITORIAL-PERSONNEL-v1', productionEnabled: true },
...
// Bounded, deduplicated official reads. 403/429 never retry automatically.
// A 429 blocks new requests to that host until its stated Retry-After expires.
// Other transient failures retain the existing at-most-one-retry policy.
export async function fetchNhlJson(url, { fetchImpl = globalThis.fetch, timeoutMs = 6500, ttlMs = 60_000, negativeTtlMs = 30_000, now = Date.now, retry = true } = {}) {
  ...
  if (parsed.protocol !== 'https:' || !['api-web.nhle.com', 'api.nhle.com'].includes(parsed.hostname)
    || parsed.port || String(url).includes('#') || parsed.username || parsed.password || /^https:\/\/[^/?#]*@/i.test(String(url))) return fail('NHL_SOURCE_URL_NOT_ALLOWED');
  if (typeof fetchImpl !== 'function') return fail('NHL_FETCH_UNAVAILABLE');
  if (typeof now !== 'function' || !Number.isFinite(now()) || !Number.isFinite(new Date(now()).getTime())
    || !Number.isFinite(timeoutMs) || timeoutMs <= 0 || ![ttlMs, negativeTtlMs].every(value => Number.isFinite(value) && value >= 0)) return fail('NHL_SOURCE_FETCH_OPTIONS_INVALID');
  ...
  if (cached && cached.expiresAt > now()) return { ...structuredClone(cached.value),
    ...(cached.value?.code === 'NHL_SOURCE_RATE_LIMITED' ? nhlSourceCooldown(fetchImpl, url, now()) : {}), cached: true };
  const cooling = nhlSourceCooldown(fetchImpl, url, now());
  if (cooling) return fail('NHL_SOURCE_RATE_LIMITED', { httpStatus: 429, attempts: 0, ...cooling,
    source: { provider: 'NHL', url, rateLimitObservedAt: cooling.rateLimitObservedAt } });
  const operation = (async () => {
    let result;
    for (let attempt = 0; attempt < (retry ? 2 : 1); attempt += 1) {
      const cooldown = nhlSourceCooldown(fetchImpl, url, now());
      if (cooldown) { result = fail('NHL_SOURCE_RATE_LIMITED', { httpStatus: 429, attempts: attempt, ...cooldown,
        source: { provider: 'NHL', url, rateLimitObservedAt: cooldown.rateLimitObservedAt } }); break; }
      ...
      try {
        ...
        const response = await Promise.race([fetchImpl(url, { headers: { Accept: 'application/json' }, signal: controller.signal, cache: 'no-store', redirect: 'manual' }), timeout]);
        if (response?.redirected || response?.type === 'opaqueredirect' || (response?.status >= 300 && response.status < 400)) {
          result = fail('NHL_SOURCE_REDIRECT_NOT_FOLLOWED', { httpStatus: response.status || null, source: { provider: 'NHL', url, fetchedAt }, attempts: attempt + 1 }); break;
        }
        if (response?.url && response.url !== parsed.href) {
          result = fail('NHL_SOURCE_RESPONSE_URL_MISMATCH', { source: { provider: 'NHL', url, fetchedAt }, attempts: attempt + 1 }); break;
        }
        if (!response?.ok) {
          const status = Number(response?.status) || null;
          const policy = status === 429 ? recordNhlSourceRateLimit(fetchImpl, url, { retryAfter: response.headers?.get?.('retry-after'), now: now() }) : {};
          result = fail(status === 403 ? 'NHL_SOURCE_FORBIDDEN' : status === 429 ? 'NHL_SOURCE_RATE_LIMITED' : 'NHL_SOURCE_HTTP_ERROR', { httpStatus: status,
            source: { provider: 'NHL', url, requestedAt: fetchedAt, fetchedAt: new Date(now()).toISOString() }, attempts: attempt + 1, ...policy });
          ...
          // A request can straddle puck drop. Acquisition means the complete
          // validated body is available, not when the HTTP request began.
          result = { ok: true, status: 'OK', data, source: { provider: 'NHL', url, requestedAt: fetchedAt, fetchedAt: new Date(now()).toISOString(), sourceUpdatedAt: Number.isFinite(modified) && modified <= now() ? new Date(modified).toISOString() : null, contentHash: hash(data) }, attempts: attempt + 1 };
          break;
        }
    ...
    store.set(url, { value, expiresAt: value.code === 'NHL_SOURCE_RATE_LIMITED' && value.retryAt
...
export async function fetchNhlGame(gameId, options = {}) {
  ...
  const additional = options.includeDetails === true ? await Promise.all(['boxscore', 'play-by-play', 'right-rail'].map(path => fetchNhlJson(`${NHL_SOURCES.gamecenter.url}${gameId}/${path}`, options))) : [];
  const [box, pbp, report] = additional;
  ...
  if (report?.ok) game.officialReport = normalizeNhlGameReport(report.data, game, report.source);
  const issues = [...game.qa.issues, ...additional.filter(row => !row.ok).map(row => row.code)];
  if (game.officialReport?.ok === false) issues.push(...game.officialReport.issues);
  if (game.observedSituationStatistics?.ok === false) issues.push(...game.observedSituationStatistics.issues);
...
export async function fetchNhlTeamSummary(teamId, season, gameType, options = {}) {
  if (!validNhlTeamSummaryScope(teamId, season, gameType)) return fail('NHL_TEAM_SUMMARY_SCOPE_INVALID');
  // The official season-report query took 8–13 seconds in real acquisition.
  // One 20-second attempt fits the existing 28-second browser budget; do not
  // restart the same slow query at the generic Gamecenter 6.5-second cutoff.
  const result = await fetchNhlJson(nhlTeamSummaryUrl(teamId, season, gameType), { ttlMs: 5 * 60_000, timeoutMs: 20_000, retry: false, ...options });
  return result.ok ? normalizeNhlTeamSummary(result.data, { teamId, season, gameType }, result.source) : result;
}

export function deriveNhlEventStatistics(payload, source = null) {
  const validId = value => Number.isSafeInteger(value) && value > 0;
  if (!Array.isArray(payload?.plays) || !validId(payload?.awayTeam?.id) || !validId(payload?.homeTeam?.id)
    || payload.awayTeam.id === payload.homeTeam.id) return { ok: false, status: 'BLOCK', issues: ['NHL_PBP_SCHEMA_INVALID'], source };
  const events = new Map();
  const issues = new Set();
  const shotTypes = new Set(['goal', 'shot-on-goal', 'missed-shot', 'blocked-shot']);
  for (const event of payload.plays) {
    if (!event || !validId(event.eventId)) { issues.add('NHL_PBP_EVENT_ID_INVALID'); continue; }
    // Identical repeated events can be deduplicated. Conflicting revisions have
    // no ordering evidence: never choose whichever row happened to arrive first.
    const prior = events.get(event.eventId);
    const evidence = row => JSON.stringify([row.typeDescKey, row.periodDescriptor, row.timeInPeriod, row.situationCode, row.details]);
    if (prior && evidence(prior) !== evidence(event)) issues.add('NHL_PBP_EVENT_REVISION_CONFLICT');
    else events.set(event.eventId, event);
    if (shotTypes.has(event.typeDescKey) && event.periodDescriptor?.periodType !== 'SO') {
      if (![payload.awayTeam.id, payload.homeTeam.id].includes(event.details?.eventOwnerTeamId)) issues.add('NHL_PBP_EVENT_TEAM_CONFLICT');
      if (!['REG', 'OT'].includes(event.periodDescriptor?.periodType)) issues.add('NHL_PBP_PERIOD_UNKNOWN');
    }
  }
  if (issues.size) return { ok: false, status: 'BLOCK', issues: [...issues], source };
  ...
  for (const player of Array.isArray(payload.rosterSpots) ? payload.rosterSpots : []) {
    if (validId(player?.playerId) && [rows.away.teamId, rows.home.teamId].includes(player.teamId)) {
      if (rosterTeams.has(player.playerId) && rosterTeams.get(player.playerId) !== player.teamId) rosterTeams.set(player.playerId, null);
      else if (!rosterTeams.has(player.playerId)) rosterTeams.set(player.playerId, player.teamId);
    }
  }
  for (const side of ['away', 'home']) { rows[side].fiveOnFive.blockedShotAttempts = 0; rows[side].fiveOnFive.teammateBlockedAttempts = 0; }
  for (const event of events.values()) {
    ...
    if (!shotTypes.has(type)) continue;
    ...
    const bothGoalies = situation[0] === '1' && situation[3] === '1' && awaySkaters >= 3 && homeSkaters >= 3;
    ...
    if (group === 'fiveOnFive' && type === 'blocked-shot') {
      // Resolve both actors against this game's roster. The live official
      // feed attributes the event to the shooter, not the blocking defender.
      const other = key === 'away' ? 'home' : 'away';
      const shooterTeam = rosterTeams.get(event.details?.shootingPlayerId);
      const blockerTeam = rosterTeams.get(event.details?.blockingPlayerId);
      if (shooterTeam === team.teamId && blockerTeam === rows[other].teamId) {
        counts.blockedShotAttempts += 1;
        rows[other].fiveOnFive.blockedShots += 1;
      } else if (shooterTeam === team.teamId && blockerTeam === team.teamId && event.details?.reason === 'teammate-blocked') {
        counts.blockedShotAttempts += 1;
        counts.teammateBlockedAttempts += 1;
      } else unresolvedBlocks += 1;
    }
  }
  const pairedGroups = { fiveOnFive: 'fiveOnFive', powerPlay: 'shortHanded', shortHanded: 'powerPlay' };
  for (const side of ['away', 'home']) {
    const opponent = rows[side === 'away' ? 'home' : 'away'];
    for (const [group, opposite] of Object.entries(pairedGroups)) {
      const own = rows[side][group]; const against = opponent[opposite];
      own.shotsOnGoalAgainst = against.shotsOnGoal;
      own.goalsAgainst = against.goals;
      own.saves = against.shotsOnGoal - against.goals;
      const ratio = (a, b) => unknownSituationEvents === 0 && b > 0 ? a / b : null;
      own.shotShare = ratio(own.shotsOnGoal, own.shotsOnGoal + against.shotsOnGoal);
      own.shootingPercent = ratio(own.goals, own.shotsOnGoal);
      own.savePercent = ratio(own.saves, against.shotsOnGoal);
      if (group === 'fiveOnFive') {
        if (unresolvedBlocks) { own.blockedShots = null; own.blockedShotAttempts = null; own.teammateBlockedAttempts = null; }
        own.unblockedAttempts = own.shotsOnGoal + own.missedShots;
        own.unblockedAttemptsAgainst = against.shotsOnGoal + against.missedShots;
        own.shotAttempts = unresolvedBlocks ? null : own.unblockedAttempts + own.blockedShotAttempts;
      }
    }
  }
  return { ok: true, status: unknownSituationEvents || unresolvedBlocks ? 'WARNING' : 'OK', ...rows, unknownSituationEvents, unresolvedBlocks, source,
    metricScope: 'OBSERVED_EVENTS_NOT_PREGAME_FEATURES', ratioUnit: 'FRACTION',
    powerPlayOpportunities: null, timeOnIce5v5: null, xGF: null, xGA: null, rateMetrics: null,
    issues: [...(unresolvedBlocks ? ['NHL_BLOCK_ACTOR_IDENTITY_UNRESOLVED'] : []), ...(unknownSituationEvents ? ['NHL_SITUATION_RATIOS_WITHHELD_INCOMPLETE_CLASSIFICATION'] : []), 'NHL_EVENT_COUNTS_ONLY_NOT_TIME_EXPOSURE_RATES', 'NHL_XG_NOT_AVAILABLE_FROM_RAW_PBP', 'NHL_PP_OPPORTUNITIES_REQUIRE_PENALTY_TIMELINE'] };
}
