...
export const NHL_DATA_VERSION = 'NHL-OFFICIAL-DATA-v1.0.0';
...
export async function fetchNhlRoster(team, season = 'current', options = {}) {
  ...
  const players = ['forwards', 'defensemen', 'goalies'].flatMap(key => result.data[key].map(player => normalizeNhlPlayer(player, { teamId, source: result.source }))).filter(Boolean);
