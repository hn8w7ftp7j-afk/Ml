# Tai888 Reader v2.1.21 FRAME-CONFLICT-GUARD

## 2.1.21 盤口衝突防護

同一分頁的內嵌盤面若出現不同盤口或水位，停止同步該聯盟並顯示衝突提示。包含較少場次的盤面也會比較共同盤口；不以回應先後猜測新舊。這項修正阻擋已重現的來源選擇漏洞，尚未取得使用者實際 DOM 證實個案根因。

## 保留 2.1.20 連線修復

- 開啟視窗後，自動同步開啟時立即偵測，並顯示完成或錯誤原因。
- 「修復讀取」只重新整理無回應的 Tai888 分頁，完成後按「立即同步」。
- 找不到分頁時清除舊的正常狀態，不再留在「尚未偵測」。
- 此修正不代表使用者電腦或 Production 已實測通過。
...
8. 在擴充功能頁確認顯示 `2.1.21 FRAME-CONFLICT-GUARD`。
