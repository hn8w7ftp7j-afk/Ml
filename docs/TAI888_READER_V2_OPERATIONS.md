...
`READER_PAIR_SECRET` must be independent. Reader pairing never uses `TAI888_PASSWORD`, and the website session never uses either Tai888 credential. `JBOT_API_TOKEN` is optional when a separate reference market is desired. `THE_ODDS_API_KEY` enables MLB-only, three-book, five-minute same-contract verification. External prices remain audit-only: they never change the score distribution, model EV (W), or robust EV (R). Secret values must remain server-side and are never committed.
