...
- Add append-only source content/event tables. New async snapshot writes externalize raw contents transactionally before storing references; old snapshots are not rewritten. Price-only transport omits raw bodies only after durable storage acknowledgement; otherwise it preserves inline fallback evidence. New writes verify content and acquisition records by reading them back before acknowledging storage.
...
Follow-up hardening: persist assembled history/identity/personnel evidence before mobile compaction; propagate transitive dependency failures and mark cycles PENDING; do not let missing raw content hide an independently proven late acquisition. External source read failures preserve the snapshot-integrity result while source availability stays unverified.
