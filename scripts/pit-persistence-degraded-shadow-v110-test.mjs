...
assert.equal(degraded.analysis.results[0].rankingQualified, true, 'PIT保存狀態不得改寫已完成的模型排名資格');
assert.equal(degraded.analysis.results[0].betEligible, false, 'PIT未確認時實際下注紀錄必須暫停');
...
console.log('PIT quota degradation preserves model ranking while disabling durable bet recording PASS');
