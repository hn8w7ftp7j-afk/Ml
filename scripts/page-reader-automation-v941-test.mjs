...
assert.equal(packageJson.version, '11.8.15', 'package/release identity must match the V11.8.15 exact direction-history binding diagnostic');
assert.equal(packageLock.version, '11.8.15', 'package-lock release identity must match V11.8.15');
assert.equal(packageLock.packages?.['']?.version, '11.8.15', 'root lockfile package must match V11.8.15');
assert.equal(APP_VERSION, '11.8.15');
