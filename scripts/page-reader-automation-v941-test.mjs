...
assert.match(page, /const rankingEnabled = activeLeague\.capabilities\.ranking === true/);
...
assert.match(page, /Reader讀取 \$\{coverage\.captured\}\/\$\{coverage\.total\} 場｜完成 \$\{tasks\.length\} 場Tai888信用盤分析/);
assert.match(page, /Reader已讀取\$\{readerCoverage\.captured\}\/\$\{readerCoverage\.total\}場｜已開盤\$\{readerCoverage\.open\}場/);
...
assert.match(page, /actualSource: null, readerPayloadHash: null, customMarkets: \[\], customData: null, referenceData: null, status: 'unopened', statusLabel: 'Tai888實際盤已下架'/);
...
assert.match(page, /const recordable = betRecordable\(row\.item, row, clockNow, bettingEnabled\)/);
assert.match(page, /recordable=\{betRecordable\(item, row, now, betsEnabled\)\}/);
...
assert.match(page, /import \{ betIdentity, betMatches, betPositionIdentity, canonicalBetPick \} from '\.\.\/lib\/bet-ledger\.js'/);
...
assert.match(page, /mergeBetCollections\(data\.bets, current\)/, '空或延遲的雲端結果不得覆蓋本機下注紀錄');
assert.match(page, /action: 'merge'/);
assert.match(page, /action: 'upsert'/);
assert.match(page, /action: 'delete'/);
assert.match(page, /雲端下注紀錄/);
...
assert.match(page, /identity, positionIdentity, league, date, gamePk/);
...
assert.match(recordBetSource, /if \(!bettingEnabled \|\| shadowMode\)/);
...
assert.match(page, /betsEnabled=\{bettingEnabled\} shadowMode=\{shadowMode\}/);
assert.match(page, /analysisEnabled && shadowMode && <LeagueShadowPanel/);
assert.match(page, /rankingEnabled && ranked\.length/);
assert.match(page, /影子評分｜不可下注/);
assert.match(page, /SHADOW｜固定公式與安全鎖已驗證｜不可下注/);
assert.match(page, /const executable = row\.executable === true && actualLineFreshNow\(row, now\)/);
assert.match(page, /const eligibility = executable \? formalBetEligibility\(row, 7\.2, now\) : \{ passed: false \}/);
...
console.log('Baseball EV v9.6.0 multi-league shell, MLB Reader automation, bet ledger and stale-gating audit PASS');
