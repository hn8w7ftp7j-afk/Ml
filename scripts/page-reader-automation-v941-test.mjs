...
assert.equal(packageJson.version, '11.8.45', 'package/release identity must match the V11.8.45 NBA data workspace release');
assert.equal(packageLock.version, '11.8.45', 'lockfile release metadata must match the package release');
assert.equal(packageLock.packages?.['']?.version, '11.8.45', 'root lockfile release metadata must match the package release');
assert.equal(APP_VERSION, '11.8.45');
