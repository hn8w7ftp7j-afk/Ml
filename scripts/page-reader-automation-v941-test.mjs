...
const betActionPolicy = fs.readFileSync('lib/bet-action-state-v118.js', 'utf8');
...
assert.equal(APP_VERSION, '11.9.41', 'Personnel source integrity release retains one shared release version');
...
mustMatch(/const READER_DOWNLOAD_PATH = '\/downloads\/Tai888-Reader-v2\.1\.22-CONFLICT-SAFE\.zip'/, 'Reader download must point at the packaged production artifact');
mustMatch(/href=\{READER_DOWNLOAD_PATH\}/, 'Reader download link must use the packaged production path');
assert.ok(fs.existsSync('public/downloads/Tai888-Reader-v2.1.22-CONFLICT-SAFE.zip'), 'Reader production zip must exist');
assert.ok(fs.existsSync('public/downloads/Tai888-Reader-v2.1.22-CONFLICT-SAFE.zip.sha256'), 'Reader checksum must exist');
...
mustMatch(/requestJSONWithTransientRetry\('\/api\/analysis-jobs'/, 'full-board analysis must start one durable server workflow with mobile transport recovery');
...
assert.match(betOrderUi, /evaluateBetAction\(\{ item: entry\.item, row: entry\.row, now: clockNow, betsEnabled: bettingEnabled, cloudLedgerState: cloudLedgerActionState/, 'bet order must use the canonical durable action evaluator');
...
assert.match(betOrderUi, /evaluateBetAction/, 'bet order must expose a canonical disabled reason when a record cannot be persisted');
...
mustMatch(/bindVerifiedReaderContractsForItem\(item, analysisDirectionRows\(directionAnalysis\)\)/, 'ranking must bind the exact same current Reader contracts as the game card');
mustMatch(/const currentReaderPrice = capturedReaderContractReady/, 'ranking must calculate captured Reader execution eligibility separately from display');
...
mustMatch(/const action = evaluateBetAction\(\{ item, row, now, betsEnabled, cloudLedgerState/, 'game cards must use the canonical action evaluator');
...
mustMatch(/import \{ capturedReaderContractReady, evaluateBetAction \} from '\.\.\/lib\/bet-action-state-v118\.js'/, 'page must import one shared fail-closed Reader and bet-action gate');
