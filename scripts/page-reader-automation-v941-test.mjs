...
assert.equal(packageJson.version, '11.3.0', 'package/release identity must match the V11.3.0 Asian independent-model release');
assert.equal(packageLock.version, '11.3.0', 'package-lock release identity must match V11.3.0');
assert.equal(packageLock.packages?.['']?.version, '11.3.0', 'root lockfile package must match V11.3.0');
assert.equal(APP_VERSION, '11.3.0');
