...
assert.match(analyze, /assessEightDirectionMarketCoverage\(markets, game\)/, '市場必須先獨立建立coverage，不得整場一起BLOCK');
assert.match(analyze, /analysisContractSignature\(league, game, markets\)/, '快取契約必須包含BLOCKED原始盤，避免不同錯誤輸入共用快取');
assert.match(analyze, /attachEightDirectionContract\(deterministicCore, marketCoverage, game, readerProvenance\)/);
