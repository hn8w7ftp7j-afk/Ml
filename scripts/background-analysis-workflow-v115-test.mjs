...
assert.match(page, /pollBackgroundJob\(saved\.runId, generation, date, saved\.gamePks, \{ completedReceipt: saved\.completedReceipt === true \? saved : null, displayOnly: saved\.batchMode === 'all-leagues' \}\)/, 'manual recovery preserves game scope and distinguishes immutable and all-league result display');
