...
assert.match(page, /function directRepriceAuthorityMatches\(item, task, now = Date\.now\(\)\)[\s\S]*!gameIsPrestartNow\(item\.game, now\)[\s\S]*pendingReaderEvidenceHash === expectedEvidenceHash[\s\S]*readerEvidenceIsOlder/, 'a late direct reprice response must require the same existing prestart game and per-game Reader evidence');
