...
assert.equal(manifest.version, '2.1.20');
assert.equal(manifest.version_name, '2.1.20 CONNECTION-RECOVERY');
assert.deepEqual(
...
assert.match(background, /const VERSION = '2\.1\.20'/);
...
console.log('Reader 2.1.20 static security audit: minimal permissions, verified live rescan, self-heal, single-frame board selection, no credential storage, signed device token and strict origins PASS');
