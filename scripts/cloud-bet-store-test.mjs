...
assert.doesNotMatch(page, /async function deleteBet|async function clearLeagueBets|action: 'delete'|action: 'clearLeague'/, '不可變帳本不得顯示或呼叫未支援的刪除／清空操作');
assert.match(page, /不可變帳本/, '帳本UI必須揭露不可變證據政策');
assert.match(page, /下注證據保留，不提供刪除/, '帳本UI不得留下會失敗的刪除按鈕');
