...
const r=await fetch(`${BASE}/api/vision`,{method:'POST',headers:{'Content-Type':'application/json',Origin:BASE,'Sec-Fetch-Site':'same-origin'},body:JSON.stringify({images:[`data:image/jpeg;base64,${fixture}`],schedule,boardPass:true,defaultWater:{全場讓分:0.95,全場大小:0.94,上半讓分:0.94,上半大小:0.93}})});
const text=await r.text(); console.log('STATUS',r.status); console.log(text); if(!r.ok) process.exit(1);
