...
assert.match(page, /const saved = loadBackgroundJob\(league, date, boardRef\.current\)[\s\S]*\}, \[date, league, storageReady, busy, backgroundJobRevision\]\);/, 'a new league/date or newly queued all-league job must retry saved-job reconnect after the old operation releases busy state');
