...
const noRebateAnalysis = evaluateMarketsFromDistribution({ context, markets: input.markets, settings: { rebateRate: 0 }, distributionSnapshot: distribution });
const noRebateAudit = auditSavedPitModel({ ...bundle, marketAnalysis: { ...noRebateAnalysis, suppliedMarkets: input.markets } });
assert.equal(noRebateAudit.settingsRecorded, true);
assert.equal(noRebateAudit.replayRebateRate, 0);
assert.equal(noRebateAudit.evMatchesUnderStatedSettings, true);
const legacySettings = structuredClone(bundle); delete legacySettings.marketAnalysis.calculationSettings;
assert.equal(auditSavedPitModel(legacySettings).settingsRecorded, false);
