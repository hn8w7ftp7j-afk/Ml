...
assert.equal(SCORE_FORMULA_VERSION, 'DUAL-EV-BOTTLENECK-2026-09-v1.9.2');
...
assert.ok(!score(0.115, 0.076, { crossMarketVerified: false }).caps.includes('TWO_INDEPENDENT_MARKETS_NOT_VERIFIED'));
...
assert.equal(score(0.067, 0.038).eligible, false, '底層公式不得直接洩漏正式下注資格');
assert.equal(score(0.067, 0.038).formulaEligible, true, '公式門檻只保留為影子診斷欄位');
assert.equal(score(0.067, 0.038).scoreType, 'SHADOW_DIAGNOSTIC');
...
for (const [w, r] of [[.07,.04],[.08,.048],[.095,.06],[.12,.08],[.3,.2]]) {
  for (const crossMarketVerified of [false, undefined, null]) {
    assert.deepEqual(score(w, r, {crossMarketVerified}), score(w, r, {crossMarketVerified:true}));
  }
}
