...
assert.match(page, /async function probeCloudLedgerRecovery\(\)/, '帳本失敗後排名頁也必須有唯讀恢復探測');
assert.match(page, /cloudLedgerStatus\.state !== 'unavailable'/, '只有帳本停機時才啟動恢復探測');
assert.match(page, /cloudLedgerStatus\.retryAt/, '恢復探測必須遵守Retry-After/backoff，不得狂打資料庫');
