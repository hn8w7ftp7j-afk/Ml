...
assert.equal(LEAGUE_REGISTRY_VERSION, 'SPORTS-LEAGUE-REGISTRY-2026-08-v4.0.0');
...
assert.equal(leagueCanAnalyze('NPB'), true);
assert.equal(leagueCanAnalyze('KBO'), true);
assert.equal(leagueCanAnalyze('CPBL'), true);
for (const id of ['NPB', 'KBO', 'CPBL']) {
  ...
  assert.equal(item.analysisReadiness.status, 'ENABLED_RUNTIME_PIT_FAIL_CLOSED');
  assert.equal(item.analysisReadiness.canBuildDistribution, true);
...
for (const id of ['NPB', 'KBO', 'CPBL']) {
  assert.equal(publicRows.find(row => row.id === id).capabilities.ranking, true);
...
console.log('multi-league registry: four independent Shadow models enabled, ledgers available and formal recommendations off PASS');
