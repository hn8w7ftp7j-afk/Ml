# 多聯盟長期正期望值分析｜9.6.0 / Tai888 Reader 2.1.2

目前整合基線為網站 `9.6.0`、Next.js `15.5.23` 與 Reader `2.1.2 FRAME-SAFE FOUR TABS`。同一網站已建立 MLB、NPB、KBO、CPBL 四個彼此隔離的聯盟模組；MLB 維持正式使用，NPB／KBO／CPBL 接收各自官方賽程與 Tai888 實盤後執行完整診斷分析，但固定為 `EXPERIMENTAL_SHADOW`：可顯示四市場／八方向、EV、風險與排名，不可下注、不產生 portfolio，也不能回落到 MLB 的球隊、賽程或模型快照。
...
目前唯一發布版本是 `2.1.2 FRAME-SAFE FOUR TABS`；同一聯盟出現多個分頁時，Reader 會自動選擇優先且完整的分頁並忽略其他重複頁；單一 Tai888 分頁同時暴露主頁與隱藏 iframe 盤面時，也會依網址與活動時間選擇一份權威盤面，不再因正常盤口跳動停止整個聯盟。安裝前應移除所有舊版，完整解壓後再從 Chrome 擴充功能頁載入 `Tai888-Reader` 資料夾。iPhone 可保存或轉傳 ZIP，但不能安裝未封裝 Chrome 擴充功能。
