...
- MLB 先發投手異動時必須選擇本場球隊 split，不得直接使用 StatsAPI 第一筆。
- 零先發紀錄的預定投手改為 opener/bulk 風險與預估局數，不再自動給 5.2 局。
- MLB、NPB、KBO、CPBL 的核心資料過期時，即使 Reader 盤口沒變也必須重建比分分布。
- NPB、KBO、CPBL 若只有官方先發姓名、沒有獨立投手能力資料，只標示「身分確認／能力預估」，不再誤報為能力已確認。
- 每場卡片顯示全場與前五局上游得分中心，並明示 Tai888 水位回灌停用。
...
## 10.9.0 Production PIT 與連續校準

- 歷史回放改用與正式站相同的預期得分、聯合比分分布、Tai888 逐腿結算與固定評分鏈；重建資料不足時會明確標記，不以賽後資料補成賽前資料。
- 每筆新下注由伺服器保存不可變的盤口、模型輸出、核心資料與分布雜湊；結算後才形成 OOS observation。
- 校準跨球季連續累積，以時間衰減與部分池化處理環境漂移，不會每年開季歸零，也不會拿 Tai888 盤口反向訓練棒球實力模型。
- 牛棚、打線、先發、屋頂與天氣依距離開賽時間動態刷新；核心資料過期時禁止只換水位，必須重新跑完整分析。
- 六項 MLB 進階特徵各自驗證、各自簽章放行，且總影響限制在每隊預期得分 ±0.30；未通過所有年度與 2026 制度驗證者保持中性。
...
## 10.9.4｜MLB 共用比分分布一致性修正

- 移除模型與 Tai888 去水市場差距 5～10 個百分點時，針對正反方向各自傾斜比分分布的暫行機制。
- MLB 同一市場正反方向現在固定使用同一份凍結比分分布；Tai888 只作成交 payoff 與合理性 QA，不再改寫模型機率、Weighted EV 或 Robust EV。
- 模型與 Tai888 去水市場差距超過 10 個百分點時，保留原始 W／R 供稽核並 QA BLOCK，不以市場回灌製造看似合理的數字或 S 分數。
- 瀏覽器分析快取升級，舊市場回灌結果不再混入新版排名；加入極端總分反例、正反互補與逐腿鏡像回歸測試。
