...
function sanitizeTargets(rows, schedule) {
  ...
  for (const row of (Array.isArray(rows) ? rows : []).slice(0, 20)) {
    ...
    if (current.markets.length && current.marketKeys.size) targets.set(gamePk, current);
  }
...
async function loadOddsApi(date, schedule, targets = []) {
  ...
  // Targeted verification is fail-closed. A failed event-detail request must
  // never fall back to a featured main line and masquerade as the requested
  // Tai888 period/alternate contract.
...
export async function POST(request) {
  try {
    ...
    const payload = {
      ...
      failures,
      message: status.consensusReady
        ? (signedGames.length ? '' : '已連線外部市場，但沒有找到可與目前 Tai888 同賽事、同期間、相鄰盤安全配對的價格。')
        : '外部市場驗證金鑰未設定；不改比分分布或W/R，缺少外部同約驗證時8.5級最高封頂8.4。',
      cache: 'MISS',
    };
