...
export async function GET(request) {
  ...
  const match = /^MLB:([1-9]\d{0,9}):(FULL|PRICE_ONLY_REPRICE):[a-f0-9]{64}$/.exec(snapshotId);
  if (!match) return json({ ok: false, code: 'INVALID_MLB_SNAPSHOT_ID' }, 400);
  const rate = checkRateLimit(request, { id: 'pit-model-audit', limit: 6, windowMs: 60_000 });
  if (!rate.allowed) return rateLimitResponse(rate);
  try {
    const bundle = await loadAnalysisPitReplay({ league: 'MLB', snapshotId, expected: { gamePk: Number(match[1]) } });
    return json({ ok: true, audit: auditSavedPitModel(bundle) });
  } catch (error) {
