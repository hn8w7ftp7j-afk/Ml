...
const VIEWS = new Set(['schedule', 'teams', 'game', 'team', 'player', 'injuries']);
...
export async function GET(request) {
  ...
  if (!VIEWS.has(view) || !validDate || !validId || !validSeason || !['regular', 'preseason', 'postseason'].includes(seasonType)
    || (['game', 'team', 'player'].includes(view) && !id)
  ...
  try {
    ...
    return NextResponse.json(result, { headers: { 'Cache-Control': 'no-store', 'X-NBA-Module': NBA_MODULE_VERSION } });
  } catch (error) {
