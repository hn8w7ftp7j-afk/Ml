...
import { splitEvidenceNote, bullpenUsageText, inningsEvidenceView, featureTimeText, savedLeagueLimitations, assignmentEvidenceView, measurementEvidenceView, personnelAssignmentSummary, personnelNames, lineupOrderText, bullpenRosterView, sourceLineageView, umpireEvidenceView } from '../lib/personnel-evidence-display.js';
...
import { marketResearchRestriction, isResearchOnlyMarket } from '../lib/market-research-policy.js';
import {
...
const READER_DOWNLOAD_PATH = '/downloads/Tai888-Reader-v2.1.22-CONFLICT-SAFE.zip';
...
function BetLedgerDashboard({ bets, cloudLedgerStatus, cloudLedgerBusy, reportCloudLedgerFailure, period, setPeriod, selectedLeague, setSelectedLeague, selectedMarket, setSelectedMarket, refreshSettlements, onCancel }) {
  ...
  const marketLabel = betStatMarketLabel(selectedMarket);
  ...
  return <section className="panel ledgerPanel">
    ...
    <div className="breakdownGrid marketBreakdown" aria-label="上半場大分小分獨立統計">
      {FIRST5_TOTAL_STAT_FILTERS.map(({ id, label }) => {
        const rows = leagueBets.filter(bet => matchesBetStatMarket(bet, id));
        return <BreakdownButton key={id} label={label} active={selectedMarket === id} summary={summarizeBetLedger(rows).overall} onClick={() => setSelectedMarket(selectedMarket === id ? 'ALL' : id)}/>;
      })}
    </div>
...
function diagnosticVerdict(row, formulaScore, qaPassed, leagueValidated, game) {
  const researchPolicy = marketResearchRestriction(row, game);
  if (researchPolicy) return { icon: '🧪', label: researchPolicy.label, ranking: false, reason: researchPolicy.reason };
  const weightedEV = modelEvValue(row);
  const robustEV = robustEvValue(row);
...
function researchLedgerActionText(action, policy) {
  return policy && action?.recordable && !action?.disabled && action?.kind !== 'cancel'
    ? '已自行下注？記帳'
    : action?.text;
}
...
function ResultRow({ row, game, limitations = [], onBet, onCancel, onRecheck, recoveryBusy = false, betState = null, action, now, inactiveNotice = '' }) {
  ...
  const verdict = diagnosticVerdict(row, formulaScore, qaPassed, leagueValidated, game);
  const scoreClass = researchPolicy ? 'researchOnlyScore' : formulaScore == null ? (qaPassed ? 'pass' : 'warning') : formulaScore >= 8.5 ? 'strongest' : formulaScore >= 7.2 ? 'candidate' : 'pass';
  const scoreMark = researchPolicy ? '🧪' : scoreIcon(formulaScore, qaPassed && !calibrationBlocked && leagueValidated);
  ...
  return <div className={`scoreRow ${researchPolicy ? 'researchOnlyRow' : ''}`}>
    ...
    <div className="rowActions">
      {actualLine && <div>
        <button className={`mini ${action.kind === 'cancel' ? 'cancel' : latest ? 'recorded' : action.recordable ? researchPolicy ? 'researchLedger' : 'green' : 'unavailable'}`} disabled={action.disabled} title={action.title} onClick={() => action.kind === 'cancel' ? onCancel(latest) : onBet(row)}>{researchLedgerActionText(action, researchPolicy)}</button>
        <ReaderRecovery action={action} onRecheck={onRecheck} busy={recoveryBusy}/>
...
function AnalysisDataAudit({ audit, persistence }) {
  const snapshotLink = persistence?.snapshotId && <p>快照：{persistence.confirmed ? '已保存' : '保存待確認'}｜<a href={`/api/pit-model-audit?snapshotId=${encodeURIComponent(persistence.snapshotId)}`} target="_blank" rel="noreferrer">查看查核</a></p>;
  if (!audit?.rows?.length) return <details className="details dataAudit"><summary>人員與資料｜未保存查核明細</summary><p>這份分析未保存逐項取得與使用紀錄。</p>{snapshotLink}</details>;
  ...
  const umpire = umpireEvidenceView(audit);
  const supportingData = audit.supportingData?.filter(row => row.key !== 'umpire') || [];
  return <details className="details dataAudit">
    <summary>先發與打線｜{personnelAssignmentSummary(audit)}</summary>
    <p className="muted">以這次分析保存內容為準；當場人選與能力數據分開顯示。</p>
    <div className="dataAuditRows">{audit.rows.map(row => {
      const assignment = assignmentEvidenceView(row);
      const measurement = measurementEvidenceView(row);
      const isAssignment = ['starter', 'lineup'].includes(row.category);
      const badge = isAssignment ? assignment : { ...measurement, tone: measurement.status };
      const names = personnelNames(row);
      const innings = row.category === 'starter' ? inningsEvidenceView(row) : null;
      const usage = row.usageStatus === 'UNVERIFIED' ? '未保存逐欄聲明' : [row.usedInMean && '得分中心', row.usedInUncertainty && '不確定性'].filter(Boolean).join('、') || (row.usedInMean === false && row.usedInUncertainty === false ? '未使用／僅供診斷' : '未記錄');
      return <div className="dataAuditRow" key={row.id || row.key}>
        <div className="dataAuditHeading"><strong>{row.label}</strong><span className={`dataAuditStatus ${badge.tone}`}>{badge.label}</span></div>
        {isAssignment && <p><b>{row.category === 'starter' ? '當場先發' : '當場打線'}：</b>{assignment.label}｜<b>能力數據：</b>{measurement.label}</p>}
        {row.category === 'starter' && <p><b>保存人選：</b>{names.join('、') || '未保存'}{Number.isFinite(innings.value) ? `｜預估 ${innings.value.toFixed(2)} 局` : ''}</p>}
        {row.category === 'lineup' && <p>保存名單 {row.coverage?.identityCount ?? names.length}/{row.coverage?.expectedCount ?? '未保存'} 人</p>}
        {row.category === 'lineup' && <details><summary>查看打線與棒次（{names.length} 人）</summary>{row.modelBattingInputs?.length ? row.modelBattingInputs.map((player, index) => <p key={`${player.id || player.name}-${index}`}>{lineupOrderText(player, audit.league)}｜{player.name || player.id || '姓名未保存'}</p>) : <p>{names.join('、') || '未保存逐人打線'}</p>}</details>}
        {row.category === 'bullpen' && <BullpenEvidence row={row} league={audit.league}/>}
        <p><b>模型使用紀錄：</b>{usage}</p>
        <details><summary>數據、來源與使用明細</summary>
          <p><b>來源：</b>{row.source || '未提供'}｜取得 {row.observedAt ? localTime(row.observedAt) : '時間未保存'}｜統計截至 {row.asOf || '未保存'}</p>
          <p>{sourceLineageView(row)}；限保存的解析欄位，原始來源與賽前可得性另行核對。</p>
          {splitEvidenceNote(row) && <p>{splitEvidenceNote(row)}</p>}
          {innings && <p>{innings.historyStatus}｜方法：{innings.method || '未保存'}｜局數樣本：{innings.sampleGames ?? '未保存'} 場。{innings.note}</p>}
          {row.category === 'lineup' && <p>{lineupCoverageDisplay(row)} 打線指數：{row.metrics?.offensiveIndex ?? '未保存'}。數值為 1 不單獨證明實測能力中性。</p>}
          {row.category === 'bullpen' && <p>近期後援用量與覆蓋：{bullpenUsageText(row)}。缺值不代表 0 球或充分休息。</p>}
          <pre>{evidenceJSON({ ...row, displayInningsEvidence: innings || undefined })}</pre>
        </details>
      </div>;
    })}</div>
    <div className="dataAuditRow">
      <div className="dataAuditHeading"><strong>主審</strong><span className={`dataAuditStatus ${umpire.tone}`}>{umpire.label}</span></div>
      <p>{umpire.name || '姓名未保存'}｜{umpire.effect}</p>
      {umpire.row && <details><summary>主審來源與使用明細</summary><pre>{evidenceJSON(umpire.row)}</pre></details>}
    </div>
    {supportingData.length > 0 && <details className="dataAuditRow"><summary>本季、近期與環境資料</summary><pre>{evidenceJSON(supportingData)}</pre></details>}
    {audit.otherUsage?.length > 0 && <details className="dataAuditRow"><summary>球場、天氣與進階欄位使用明細</summary><pre>{evidenceJSON(audit.otherUsage)}</pre></details>}
    <details className="dataAuditRow"><summary>保存與時點查核</summary>
      {snapshotLink}
      <p>{audit.temporal?.limitation || '這是本次取得紀錄，不等於已驗證的歷史賽前快照。'}</p>
      <p>保存、來源核驗、模型重播與預測成效分別查核；保存使用聲明不代表已重跑模型。</p>
      {sourceTimeAudit && <><p>來源內容與取得時點：{({ VERIFIED: '通過', PENDING: '證據不足', FAILED: '核對失敗' })[sourceTimeAudit.status] || '未核對'}｜輸入鎖定：{sourceTimeAudit.inputCutoffAt || '未保存'}</p>{sourceTimeAudit.rows?.map(row => <p key={row.featureName}>{row.featureName}｜{({ NOT_CONNECTED: '未接入來源', MISSING: '資料缺失', PROJECTED: '含推估資料', REPORTED: '有輸入紀錄', UNKNOWN: '未記錄' })[row.availability] || '舊快照未記錄可用性'}｜{featureTimeText(row)}</p>)}<pre>{evidenceJSON(sourceTimeAudit.rows)}</pre></>}
...
function BullpenEvidence({ row, league }) {
  ...
  const roster = bullpenRosterView(row);
  return <div>
    <p>{roster.candidateLabel}｜{roster.historyLabel}</p>
    <details><summary>投手名單與歷史登板</summary>
      <p><b>保存投手名單：</b>{roster.candidates.join('、') || '未保存逐人名單'}｜{roster.completeness}</p>
      <p><b>歷史登板：</b>{roster.historical.join('、') || '未保存逐人歷史登板'}</p>
      <p>{roster.note}</p>
      <p>名單基準日：{evidence.rosterDates}｜能力統計截至：{evidence.metricDates}{evidence.metricDatesIncomplete ? '（部分未保存）' : ''}｜最近登板日：{evidence.latestUsageDate}</p>
      <p>可出賽程度依近期用量估計；可用性 1 不是官方確認，也不是 100% 出賽機率。</p>
      {league === 'MLB' && <p>用量天數依比賽時間差換算，逐場 daysAgo 與權重請見來源明細。</p>}
      {evidence.samples.length > 0 && <details><summary>後援能力樣本局數（十進位）</summary>{evidence.samples.map((player, index) => <p key={index}>{player.name}：{player.innings == null ? '未保存' : player.innings.toFixed(2)} 局</p>)}</details>}
    </details>
  </div>;
}
...
export default function Home() {
  const notificationControlRef = useRef(null);
  const notificationPreparingRef = useRef(false);
  const [notificationPreparing, setNotificationPreparing] = useState(false);
  async function startWithCompletionNotification(start) {
    if (notificationPreparingRef.current) return;
    notificationPreparingRef.current = true;
    setNotificationPreparing(true);
    try {
      await notificationControlRef.current?.prepare();
    } finally {
      notificationPreparingRef.current = false;
    ...
    return bindVerifiedReaderContractsForItem(item, analysisDirectionRows(directionAnalysis))
      ...
      const inactiveNotice = !gamePrestart
        ...
        : !currentReaderPrice
          ? '尚無已驗證的Reader盤口｜保留分析數值｜排名依QA判定｜新增下注紀錄暫停'
          : '';
      const researchPolicy = marketResearchRestriction(row, item.game);
      const rankingEligible = !researchPolicy && currentAnalysisExecutable
      ...
      return { item, row, stableKey, gamePk: item.game.gamePk, matchup: matchup(item.game), market: row.market, pick: row.pick,
        ...
        currentReaderPrice, inactiveNotice, rankingEligible, currentAnalysisExecutable, researchPolicy };
      });
  })
          ...
          for (const id of LEAGUE_IDS) {
            ...
            if (Number(batch.total) > 0 && submittedAllLeagueRunRef.current === expectedRunId) {
              try {
                ...
                saveAnalysisBoardCache(id, batchDate, next);
                ...
                resultLoaded = true;
                if (!recovery.completed || recovery.stored) clearBackgroundJob(id, batchDate, expectedRunId);
              } catch (cause) {
                if (!stillCurrentRun()) return;
                loadError = `結果尚未載入：${String(cause?.message || cause)}。請按「載入先前分析（不重算）」重試。`;
              }
            }
  ...
  useEffect(() => {
    ...
    const restoredBoard = storageReady ? (allLeagueBoardsRef.current.get(`${league}:${date}`) || loadAnalysisBoardCache(league, date)) : [];
    ...
    pollBackgroundJob(saved.runId, generation, date, saved.gamePks, { completedReceipt: saved.completedReceipt === true ? saved : null, displayOnly: saved.batchMode === 'all-leagues' }).then(result => {
      ...
      if (saved.batchMode === 'all-leagues' && resultActuallyLoaded && result?.recoveryPersisted === true) {
        ...
        if (completedRun?.runId === saved.runId) {
          publishAllLeagueRun(updateAllLeagueAnalysisLeague(completedRun, league, { resultLoaded: true, status: completedRun.leagues?.[league]?.serverStatus || summarizeAllLeagueBatchResult(result).status, message: '' }));
        }
      }
  ...
  useEffect(() => {
    ...
    allLeagueBoardsRef.current.set(`${league}:${date}`, board.filter(item => analysisItemMatchesScope(item, { league, date })));
        ...
        try {
          ...
          if (state.status === 'completed') {
            ...
            if (displayOnly && !completedReceipt) {
              ...
              if (!recovery.completed || recovery.stored) clearBackgroundJob(league, targetDate, runId);
              setProgress(value => ({ ...value, active: false, running: 0 }));
              return { ...result, recoveryPersisted: true };
            }
            if (completedReceipt) {
  ...
  async function oneClickAnalyzeAll() {
    ...
    try {
      ...
      submittedAllLeagueRunRef.current = job.runId;
  ...
  function recheckReaderItem(item) {
    const gamePk = Number(item?.game?.gamePk);
    if (!Number.isSafeInteger(gamePk) || gamePk <= 0) {
      setError('此場缺少有效賽事識別；未啟動任何場次分析。');
      return;
    }
  ...
  async function oneClickAnalyze(automaticKey = '', selectedGamePk = null) {
    ...
    if (savedBatchJob?.runId && savedBatchJob?.batchMode === 'all-leagues' && allLeagueRunning) {
      setBackgroundJobRevision(value => value + 1);
