'use client';
import { runExplanationDisplay, externalVerificationExplanation } from '../lib/mlb-diagnostic-display-v1.js';
...
function AnalysisDataAudit({ audit, persistence }) {
  ...
  return <details className="details dataAudit">
    ...
    <div className="dataAuditRows">{audit.rows.map(row => <div className="dataAuditRow" key={row.id || row.key}>
      ...
      {row.inningsEstimate && <p><b>預估投球局數：</b>{row.inningsEstimate.value.toFixed(2)} 局（推估）｜{row.inningsEstimate.note}</p>}
