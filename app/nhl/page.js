...
export default function NhlWorkspace() {
  ...
  const [personnelVersions, setPersonnelVersions] = useState({});
  const [shotResearch, setShotResearch] = useState({});
  ...
  useEffect(() => {
    try {
      const saved = readNhlWorkspace(localStorage);
      if (saved?.league === 'NHL') {
        if (saved.date && saved.boards?.[saved.date]?.league === 'NHL') setDate(saved.date);
        ...
        setBoards(saved.boards); setDetails(saved.details);
      }
  ...
  useEffect(() => {
    if (!ready) return;
    try {
      saveNhlWorkspace({ date, selectedGame, boards, details, personnel, contexts, rosters, teamSummaries, shotResearch }, localStorage);
    } catch { setError('這台裝置無法保存 NHL 顯示快照；目前結果仍保留在畫面，可重新載入官方資料。'); }
  }, [boards, details, personnel, contexts, rosters, teamSummaries, shotResearch, date, selectedGame, ready]);
  useEffect(() => {
    const refresh = () => {
      const saved = readNhlWorkspace(localStorage);
      const restore = (setter, section) => setter(value => mergeNhlWorkspace({ league: 'NHL', [section]: value }, { [section]: saved[section] })[section]);
      restore(setBoards, 'boards'); restore(setDetails, 'details'); restore(setPersonnel, 'personnel');
      restore(setContexts, 'contexts'); restore(setRosters, 'rosters'); restore(setTeamSummaries, 'teamSummaries');
      restore(setShotResearch, 'shotResearch');
    };
    refresh(); window.addEventListener(NHL_WORKSPACE_EVENT, refresh);
    return () => window.removeEventListener(NHL_WORKSPACE_EVENT, refresh);
  }, []);
  ...
  function loadPlayer(id) {
    ...
    return run(`player:${id}`, async () => {
      ...
      if (result.player?.playerId !== id || (result.player?.leagueId || result.player?.league) !== 'NHL' || (result.player?.leagueId && result.player.leagueId !== 'NHL') || (result.player?.league && result.player.league !== 'NHL')) throw new Error('球員識別不符，已保留先前資料。');
  ...
  function loadTeamSummary(team, gameType) {
    ...
    return run(`team-summary:${key}`, async () => {
      ...
      persistNhlWorkspaceResult('teamSummaries', key, result);
    });
  }
  function loadContext(id) { return run(`context:${id}`, async () => { const result = await request('context', { gameId: id }); if (result.gameId !== id) throw new Error('賽程身分不符'); setContexts(value => ({ ...value, [id]: result })); persistNhlWorkspaceResult('contexts', id, result); }); }
  function loadShotResearch(id) { return run(`shots:${id}`, async () => { const result = await request('observed-shot-research', { gameId: id }); if (result.gameId !== id || result.research?.game?.gameId !== id || result.research?.leagueId !== 'NHL') throw new Error('射門研究的賽事身分不符。'); setShotResearch(value => ({ ...value, [id]: preferNhlWorkspaceRecord(value[id], result) })); persistNhlWorkspaceResult('shotResearch', id, result); }); }
  function loadVersions(id) { return run(`versions:${id}`, async () => { const result = await request('versions', { gameId: id }); if (result.gameId !== id || !Array.isArray(result.versions) || result.versions.some(row => row.league !== 'NHL' || (row.game?.gameId || row.gameId) !== id)) throw new Error('來源版本的賽事身分不符。'); setVersions(value => ({ ...value, [id]: result.versions })); }); }
  function loadPersonnel(id) { return run(`personnel:${id}`, async () => { const result = await request('personnel', { gameId: id }); if (result.gameId !== id || result.personnel?.gameId !== id || result.personnel?.leagueId !== 'NHL') throw new Error('人員資料的賽事身分不符。'); setPersonnel(value => ({ ...value, [id]: preferNhlWorkspaceRecord(value[id], result) })); persistNhlWorkspaceResult('personnel', id, result); }); }
  function loadPersonnelVersions(id) { return run(`personnel-versions:${id}`, async () => { const result = await request('personnel-versions', { gameId: id }); if (result.gameId !== id || !Array.isArray(result.versions) || result.versions.some(row => row.league !== 'NHL' || row.gameId !== id || row.personnel?.gameId !== id)) throw new Error('人員版本的賽事身分不符。'); setPersonnelVersions(value => ({ ...value, [id]: result.versions })); }); }
  ...
  return <main className="appShell nhlApp">
    ...
    <nav className="leagueTabs" aria-label="聯盟切換">{LEAGUE_IDS.map(id => <Link prefetch={false} className="sportModuleLink" key={id} href={`/?league=${id}`}><b>{id}</b><small>{leagueConfig(id).shortLabel}</small></Link>)}<NbaEntry/><Link prefetch={false} className="sportModuleLink active" href="/nhl" aria-current="page"><b>NHL</b><small>冰球</small></Link></nav>
    ...
    {tab === 'schedule' && <>
      ...
      {selectedGame === row.gameId && game && <ObservedShotPanel game={game} value={shotResearch[row.gameId]} busy={busy[`shots:${row.gameId}`]} onLoad={() => loadShotResearch(row.gameId)}/>}
