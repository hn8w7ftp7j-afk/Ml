...
import { cancelNbaShadow, canResumeNbaShadow, nbaShadowIsBusy, nbaShadowKey, readNbaShadow, runNbaShadow, subscribeNbaShadow } from '../../lib/nba/shadow-client.js';
...
const labels = { running: '讀取與驗證中', paused: '讀取已中斷・可續跑', completed: '研究已完成', partial: '部分資料未取得', cancelled: '已停止', blocked: 'QA 阻擋', failed: '執行失敗' };
...
export default function ShadowPanel({ games, teamId, seasonType }) {
  ...
  return <section className={styles.panel} aria-label="籃球 Shadow 模型研究">
    ...
    <p className={styles.muted}>一個球季需逐場讀取，可能花數分鐘。切換 NBA 頁籤或返回原聯盟不會中斷；重新整理會停止讀取，但每場成功資料會保存於此分頁，24 小時內可手動續跑。關閉分頁可能清除瀏覽器保存資料，不會自動重新分析。</p>
    {canResumeNbaShadow(job) && <button type="button" disabled={nbaShadowIsBusy()} onClick={() => { void runNbaShadow(games, teamId, seasonType, { resume: true }); }}>繼續未完成驗證（已保存 {job.checkpoint.length} 場）</button>}
    ...
    {job?.resumed > 0 && <p>本次已核對並沿用 {job.resumed} 場成功快照；其餘場次重新讀取。</p>}
    {job?.persistence === 'memory_only' && <p role="alert" className={styles.error}>瀏覽器儲存空間不足或禁止儲存；結果目前只保留在這個頁面的記憶體，重新整理會遺失。其他聯盟資料未刪除。</p>}
    {report && <>
      <p className={styles.muted}>報告完成時間：{job.reportBuiltAt || '未完成'}{running || ['cancelled', 'paused', 'failed'].includes(job.status) ? '（目前保留上次報告，並非本次尚未完成的結果）' : ''}</p>
